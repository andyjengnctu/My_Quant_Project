...
BREAKOUT_QUALITY_LABEL_HORIZON_BARS = 40  # 進場後用來判定 PASS、REJECT 或 IGNORE 的未來交易日數。
BREAKOUT_QUALITY_LABEL_ATR_LEN = 14  # 建立標籤與 R 倍數基準時使用的 ATR 計算期數。
BREAKOUT_QUALITY_LABEL_ATR_BUY_TOL = 1.5  # 判定突破後是否可成交時允許的買價偏離 ATR 倍數。
BREAKOUT_QUALITY_LABEL_ATR_TIMES_INIT = 2.0  #  標籤模擬的初始風險距離，以 ATR 倍數表示。
BREAKOUT_QUALITY_POSITIVE_MFE_R = 1.5  # 將事件標記為 PASS 所需達到的最大有利變動 R 倍數。
BREAKOUT_QUALITY_NEGATIVE_MAE_R = -1.0  # 將事件視為不利失敗時使用的最大不利變動 R 倍數界線。
BREAKOUT_QUALITY_REJECT_CONFIRM_MFE_R = 1.0  # REJECT 標籤判定中，用來確認後續反彈仍不足的 MFE R 上限。
BREAKOUT_QUALITY_DEAD_MFE_R = 0.5  # 區分無效／低動能事件時使用的最低 MFE R 界線。
BREAKOUT_QUALITY_EVALUATE_FROM_BARS_AFTER_ENTRY = 1  # 自進場後第幾根交易 bar 起開始計算標籤路徑。
