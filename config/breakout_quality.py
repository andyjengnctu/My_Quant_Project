...
BREAKOUT_QUALITY_MODEL_RESEARCH_EXPERIMENT_PROFILE = "daily_universal_predicted_safety_weighted_pure_mfe_full_list_ndcg_pairwise"
...
def get_high_safety_weighted_pure_mfe_contract() -> dict[str, Any]:
    """MR-13AF: canonical Pure-MFE ranking weighted only by PIT-safe predicted Safety."""
        ...
        if self.training_objective == TRAINING_OBJECTIVE_BINARY_CLASSIFICATION:
            ...
            if self.training_objective == TRAINING_OBJECTIVE_DAILY_DUAL_COMPONENT_R_REGRESSION:
                ...
                expected_epoch_metric = (
                    ...
                    if self.training_objective == TRAINING_OBJECTIVE_DAILY_SAFETY_RAW_MFE_PAIRWISE_RANKING
                    else "raw_mfe_mean_daily_spearman"
...
_CONTINUOUS_RANKER_TARGET_POLICIES: dict[str, ContinuousRankerTargetPolicy] = {
...
_CONTINUOUS_RANKER_TRAINING_POLICIES: dict[str, ContinuousRankerTrainingPolicy] = {
...
def get_continuous_ranker_training_policy(training_objective: str) -> ContinuousRankerTrainingPolicy:
    try:
...
def _resolve_continuous_ranker_target_policy(
