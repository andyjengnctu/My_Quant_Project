...
# 開發期先固定單一路徑為 pre-deploy 選參 + 單一連續 OOS 驗證。
# 目前不啟用 legacy 全資料模式，也不啟用 promotion / champion 流程。
OPTIMIZER_ARCHITECTURE_MODE = 'fixed_predeploy_oos'
LEGACY_OPTIMIZER_ENABLED = False
PROMOTION_FLOW_ENABLED = False

# 固定驗證口徑：2010~2019 選參，2020~latest 做 OOS 驗證。
PREDEPLOY_SELECTION_START_YEAR = 2010
OOS_EVALUATION_START_YEAR = 2020
...
TRAINING_SPLIT_POLICY = {
    "architecture_mode": OPTIMIZER_ARCHITECTURE_MODE,
    "legacy_model_enabled": LEGACY_OPTIMIZER_ENABLED,
    "promotion_flow_enabled": PROMOTION_FLOW_ENABLED,
