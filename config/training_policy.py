...
# inner validation 相對排名上限。0.50 代表只接受 finalists validation 排名前半段。
OPTIMIZER_INNER_VALIDATE_MAX_RANK_PERCENTILE = 0.50
...
# base_lm>0 版本的額外 retention 門檻。
# 選取邏輯：local_min_score > 0 且 local_retention > 此值後，再依 base_rank 取第一名。
OPTIMIZER_BASE_LOCAL_MIN_GT0_RETENTION_MIN = 0.5
...
def build_training_score_policy_snapshot():
    return {
        ...
        "OPTIMIZER_BASE_LOCAL_MIN_GT0_RETENTION_MIN": OPTIMIZER_BASE_LOCAL_MIN_GT0_RETENTION_MIN,
