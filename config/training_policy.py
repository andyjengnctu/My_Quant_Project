...
# Step 0 架構收斂：先固定單一路徑為 pre-deploy 選參 + OOS 驗證。
# 目前開發期先停用 legacy 全資料模式、final holdout 與 champion/challenger 升版主流程。
STEP0_OPTIMIZER_ARCHITECTURE_MODE = 'fixed_predeploy_oos'
STEP0_DISABLE_LEGACY_OPTIMIZER_MODE = True
STEP0_DISABLE_PROMOTION_AND_CHAMPION_FLOW = True

# Step 1 固定驗證口徑：2010~2019 選參，2020~latest 做 OOS 驗證。
STEP1_PREDEPLOY_TRAIN_START_YEAR = 2010
STEP1_OOS_START_YEAR = 2020
OPTIMIZER_TRAIN_START_YEAR = STEP1_PREDEPLOY_TRAIN_START_YEAR
OPTIMIZER_MIN_TRAIN_YEARS = STEP1_OOS_START_YEAR - STEP1_PREDEPLOY_TRAIN_START_YEAR
TRAINING_SPLIT_POLICY = {
    "architecture_mode": STEP0_OPTIMIZER_ARCHITECTURE_MODE,
    "disable_legacy_model": STEP0_DISABLE_LEGACY_OPTIMIZER_MODE,
    "disable_promotion_flow": STEP0_DISABLE_PROMOTION_AND_CHAMPION_FLOW,
    "train_start_year": OPTIMIZER_TRAIN_START_YEAR,
    "min_train_years": OPTIMIZER_MIN_TRAIN_YEARS,
    "search_train_end_year": STEP1_OOS_START_YEAR - 1,
    "oos_start_year": STEP1_OOS_START_YEAR,
}
