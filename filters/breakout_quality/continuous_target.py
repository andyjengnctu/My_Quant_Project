...
def daily_full_horizon_low_adverse_target_from_cached_path(
    high_prices: np.ndarray,
    low_prices: np.ndarray,
    *,
    anchor_price: float,
    ...
    """Build the MR-13M path-safety target as negative adverse-to-peak R.

    The selected peak is still the earliest maximum high across the complete fixed
    horizon.  MFE is retained only to identify that peak; it contributes no reward
    to the target.  Higher target values mean smaller adverse excursion on the path
    required to reach the selected peak.
    """
    ...
    if not source.valid:
        return source
    risk_budget = float(spec.risk_budget_return)
    target_raw_r = -float(source.adverse_return_to_peak) / risk_budget
    if not math.isfinite(target_raw_r):
        return _invalid_result("non_finite_target")
    return StrategyAlignedContinuousTargetResult(
        valid=True,
...
def build_daily_full_horizon_low_adverse_contract(
    policy: BreakoutQualityLabelPolicy,
) -> dict[str, object]:
    """Return the MR-13M adverse-only path-safety target contract."""

    spec = StrategyAlignedContinuousTargetSpec.from_label_policy(policy)
    return {
        "schema_version": CONTINUOUS_TARGET_SCHEMA_VERSION,
        "target_id": DAILY_FULL_HORIZON_LOW_ADVERSE_TARGET_ID,
        "objective_family": "daily_cross_sectional_full_horizon_low_adverse",
        "information_source": "fixed_future_high_low_path",
        "sample_scope": "daily_eligible_stock_days",
        "horizon_bars": int(spec.horizon_bars),
        "risk_budget_return": float(spec.risk_budget_return),
        "formula": "- adverse_return_required_to_reach_full_horizon_max_mfe_peak / risk_budget_return",
        "peak_rule": "earliest maximum high across the complete fixed horizon; MFE only selects the peak",
        "risk_rule": "risk-barrier touch is diagnostic only and never truncates the future path",
        "adverse_scope": "worst low from horizon start through selected peak bar",
        "higher_is_better": True,
        "mfe_reward_included": False,
