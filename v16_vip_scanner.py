...
def process_single_stock(file_path, ticker, params):
    try:
        ...
        if stats['is_setup_today']:
            reference_order = build_reference_order_estimate(stats['buy_limit'], stats['stop_loss'], params)
        ...
        if extended_candidate is not None:
            ...
            reference_order = build_reference_order_estimate(limit_price, init_sl, params)
            proj_qty = reference_order['qty']
            if proj_qty == 0:
                return ('candidate', None, None, None, None, ticker, sanitize_issue)
