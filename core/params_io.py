...
PARAM_COMPAT_DEFAULT_FIELDS = {"use_breakout_ema_filter", "breakout_ema_len", "vol_breakout_mult"}
...
def load_params_from_json(json_file):
    ...
    try:
        ...
        if is_rolling_oos_param_set_payload(data):
            raise ValueError(
                "此檔案是 rolling OOS active-param replay 參數組，不是實盤單一 param.json；"
                "請在支援 rolling OOS / portfolio active-param replay 的畫面讀取，或改選最新單一參數檔。"
            )
