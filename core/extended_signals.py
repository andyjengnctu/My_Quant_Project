...
def ensure_extended_signal_counterfactual_anchor(signal_state, *, t_open, t_low, params):
    ...
    entry_ref_price = adjust_long_buy_fill_price(min(t_open, original_limit))
    invalidation_barrier = calc_initial_stop_from_reference(entry_ref_price, original_atr, params)
    completion_barrier = calc_frozen_target_price(entry_ref_price, invalidation_barrier)
...
def build_extended_candidate_plan_from_signal(signal_state, sizing_capital, params):
    ...
    sizing_stop_ref = calc_initial_stop_from_reference(limit_reference, entry_atr, params)
    sizing_trail_ref = calc_initial_trailing_stop_from_reference(limit_reference, entry_atr, params)
    target_price = calc_frozen_target_price(limit_reference, sizing_stop_ref)
    base_plan = {
