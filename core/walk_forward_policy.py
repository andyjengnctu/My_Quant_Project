...
VALID_OBJECTIVE_MODES = {"legacy_base_score", "split_test_romd"}
...
def _load_policy_payload(path: str) -> dict:
    ...
    if ext == ".py":
        ...
        if not isinstance(loaded, dict):
            raise ValueError("walk-forward policy Python 設定檔必須定義 TRAINING_SPLIT_POLICY 或 WALK_FORWARD_POLICY dict")
        return dict(loaded)
