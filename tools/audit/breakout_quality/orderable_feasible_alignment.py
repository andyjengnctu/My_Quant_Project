...
def _load_common_daily_target(
    *,
    ...
    # Calendar must include target-incomplete dates as well.  Otherwise the last
    # ~horizon bars would silently backfill to an older target-valid date, which
    # would turn a post-replay missing target into lookback substitution.
...
def _information_date_map(trade_dates: pd.Series, calendar: np.ndarray) -> pd.Series:
...
def _annotate_orderable(
    ...
    *,
    ...
    rows["common_information_date"] = _information_date_map(rows["trade_date"], target_calendar)
