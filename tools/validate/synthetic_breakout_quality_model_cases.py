...
def validate_breakout_quality_daily_pit_batch_gate_contract_case(_base_params):
    """Pin the MR-13C/13D/13E Selection PIT batch to one shared scientific contract."""

    case_id = "BREAKOUT_QUALITY_DAILY_PIT_BATCH_GATE"
    results = []
    summary = {"ticker": case_id, "synthetic": True}
    ...
    expected_profiles = (
        ("MR-13C", DAILY_UNIVERSAL_NO_TIME_PERCENTILE_MSE_PROFILE),
        ("MR-13D", DAILY_UNIVERSAL_NO_TIME_UPPER_TAIL_PAIRWISE_PROFILE),
        ("MR-13E", DAILY_UNIVERSAL_NO_TIME_FULL_LIST_NDCG_PAIRWISE_PROFILE),
    )
    add_check(
        results,
        "synthetic_breakout_quality",
        case_id,
        "daily_pit_batch_contains_exactly_forward_gate_authorized_13c_13d_13e",
        expected_profiles,
        gate.model_profiles,
    )

    workflows = tuple(
        get_breakout_quality_workflow_settings(experiment_profile=profile)
        for _model_id, profile in gate.model_profiles
    )
    shared_contracts = {
        (
            item.filter_id,
            item.model_architecture,
            item.continuous_target_id,
            item.training_sample_scope,
            item.seed,
            item.point_in_time_score_start_date,
            item.point_in_time_score_end_date,
            item.point_in_time_fold_months,
            item.point_in_time_inner_validation_months,
        )
    ...
    add_check(
        ...
        "synthetic_breakout_quality",
        case_id,
        "daily_pit_batch_profiles_share_dataset_target_seed_period_fold_contract",
        1,
        len(shared_contracts),
    )
    add_check(
        results,
        "synthetic_breakout_quality",
        case_id,
        "daily_pit_batch_profiles_are_all_pit_capable_without_runtime_dl_sources",
        True,
        all(item.supports_point_in_time_scores for item in workflows),
    )
    ...
    with (
        ...
        patch(
            "tools.filters.breakout_quality.application._interactive_continuous_pit_gate_batch",
            return_value=73,
        ) as batch_route,
    ):
        rc = app._interactive_model_research("apps/research.py model")
    add_check(
        results,
        "synthetic_breakout_quality",
        case_id,
        "active_mr13e_menu_option_2_routes_to_shared_pit_batch",
        (73, 1),
        (rc, batch_route.call_count),
    )

    with patch(
        "config.breakout_quality.BREAKOUT_QUALITY_CONTINUOUS_RANKER_PIT_GATE_PROFILES",
        (("MR-13C", DAILY_UNIVERSAL_NO_TIME_PERCENTILE_MSE_PROFILE),),
    ):
        try:
            get_breakout_quality_continuous_ranker_pit_gate_settings()
    ...
    add_check(
        ...
        "synthetic_breakout_quality",
        case_id,
        "daily_pit_batch_rejects_single_profile_or_incomplete_gate",
        True,
        fail_fast,
    )
