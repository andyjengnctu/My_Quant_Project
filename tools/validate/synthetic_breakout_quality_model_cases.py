...
def validate_breakout_quality_direct_r_huber_contract_case(_base_params):
    ...
    from filters.breakout_quality.torch_runtime import TorchExecutionPlan
    from services.breakout_quality import train_continuous_ranker as ranker_train
    from tools.filters.breakout_quality.train_continuous_ranker import (
        parse_args as parse_continuous_ranker_args,
    )
    ...
    fake_logits = np.asarray([[3.0, 1.5], [-2.0, 0.5]], dtype=np.float32)
    with patch.object(ranker_train, "strict_parallel_batched_logits", return_value=fake_logits):
        predicted_r = ranker_train.predict_scores(
            None,
            None,
        ...
        rank_score = ranker_train.predict_scores(
            None,
            None,
            np.zeros((2, 1, 1), dtype=np.float32),
            np.zeros((2, 0), dtype=np.float32),
    ...
    shifted = fake_logits.astype(np.float64) - fake_logits.max(axis=1, keepdims=True)
    expected_rank = (np.exp(shifted)[:, LABEL_PASS] / np.exp(shifted).sum(axis=1)).astype(np.float32)
    add_check(
        results,
        "synthetic_breakout_quality",
        case_id,
        ...
        bool(
            ...
            and np.allclose(rank_score.astype(float), expected_rank.astype(float), atol=1e-7)
        ),
    )
    ...
    class _TinyDirectRModel(nn.Module):
        def __init__(self):
            super().__init__()
            self.linear = torch.nn.Linear(1, 2, bias=False)

        def forward(self, xb, cb):
            del cb
            return self.linear(xb[:, 0, 0].reshape(-1, 1))

    tiny_model = _TinyDirectRModel()
    tiny_optimizer = torch.optim.SGD(tiny_model.parameters(), lr=0.05)
    cpu_plan = TorchExecutionPlan(
    ...
    before_weight = tiny_model.linear.weight.detach().clone()
    train_loss = ranker_train._train_epoch(
        torch,
        tiny_model,
        tiny_optimizer,
        ...
        np.asarray(["2020-01-02", "2020-01-03", "2020-01-06"], dtype=object),
        training_objective=TRAINING_OBJECTIVE_DAILY_RAW_R_REGRESSION,
        batch_size=3,
        seed=42,
    ...
    with (
        patch.object(ranker_train, "_new_model_and_optimizer", return_value=(object(), object())),
        patch.object(ranker_train, "build_grad_scaler", return_value=None),
        patch.object(ranker_train, "_train_epoch", return_value=0.5),
        patch.object(ranker_train, "predict_scores", return_value=np.asarray([0.0, 0.0], dtype=np.float32)),
        patch.object(ranker_train, "split_metrics", side_effect=validation_rows),
    ):
        epoch_choice = ranker_train.select_epoch(
            torch,
            ...
            np.asarray([-1.0, 0.5, 1.0, 2.0], dtype=np.float32),
            np.asarray([0.1, 0.4, 0.6, 0.9], dtype=np.float32),
            np.asarray([0, 1], dtype=np.int64),
