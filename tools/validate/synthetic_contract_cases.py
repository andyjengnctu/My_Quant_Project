...
def validate_gui_trade_box_capital_and_round_trip_contract_case(_base_params):
    ...
    add_check(results, "output_contract", case_id, "single_stock_sidebar_trade_info_includes_reserved_and_actual_spend_fields", True, 'text="交易資訊"' in inspector_source and '_selected_tp_var' in inspector_source and '_selected_limit_var' in inspector_source and '_selected_entry_var' in inspector_source and '_selected_stop_var' in inspector_source and '_selected_capital_var' in inspector_source and 'set_workbench_capital_display_text(' in inspector_source)
    add_check(results, "output_contract", case_id, "single_stock_sidebar_sets_reserved_and_actual_spend_from_hover_snapshot", True, 'reserved_text=self._format_sidebar_amount_value("預留", snapshot.get("reserved_capital"))' in inspector_source and 'actual_text=self._format_sidebar_amount_value("實支", snapshot.get("buy_capital"))' in inspector_source and 'display_mode=resolve_workbench_capital_display_mode_for_snapshot(snapshot)' in inspector_source and 'reserved_capital = buy_signal_meta.get("reserved_capital")' in charting_source and 'reserved_capital = buy_trade_meta.get("reserved_capital")' in charting_source and 'buy_capital = buy_trade_meta.get("buy_capital")' in charting_source and '"buy_capital": buy_capital' in charting_source)
...
def validate_gui_trade_count_and_sidebar_sync_contract_case(base_params):
    ...
    exit_flow_source = build_project_absolute_path("tools", "trade_analysis", "exit_flow.py").read_text(encoding="utf-8")
    marker_price_block = portfolio_inspector_source.split("def _resolve_marker_price", 1)[1].split("def _is_buy_trade_row", 1)[0]
    add_check(results, "output_contract", case_id, "single_stock_sidebar_declares_trade_info_block", True, 'text="交易資訊"' in inspector_source and '_selected_tp_var' in inspector_source and '_selected_entry_var' in inspector_source and '_selected_stop_var' in inspector_source and '_selected_capital_var' in inspector_source and 'set_workbench_capital_display_text(' in inspector_source)
    ...
    add_check(results, "output_contract", case_id, "portfolio_sidebar_declares_reserved_and_actual_spend_fields", True, 'text="交易資訊"' in portfolio_inspector_source and '_selected_capital_var' in portfolio_inspector_source and 'set_workbench_capital_display_text(' in portfolio_inspector_source and 'resolve_workbench_capital_display_mode_for_snapshot(snapshot)' in portfolio_inspector_source)
