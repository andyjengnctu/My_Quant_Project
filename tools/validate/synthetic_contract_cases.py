...
def validate_gui_trade_count_and_sidebar_sync_contract_case(base_params):
    ...
    exit_flow_source = build_project_absolute_path("tools", "trade_analysis", "exit_flow.py").read_text(encoding="utf-8")
    marker_price_block = portfolio_inspector_source.split("def _resolve_marker_price", 1)[1].split("def _is_buy_trade_row", 1)[0]
    add_check(results, "output_contract", case_id, "single_stock_sidebar_declares_trade_info_block", True, 'text="交易資訊"' in inspector_source and '_selected_tp_var' in inspector_source and '_selected_entry_var' in inspector_source and '_selected_stop_var' in inspector_source and '_selected_reserved_capital_var' in inspector_source)
    ...
    add_check(results, "output_contract", case_id, "portfolio_sidebar_declares_reserved_and_actual_spend_fields", True, 'text="交易資訊"' in portfolio_inspector_source and '_selected_reserved_capital_var' in portfolio_inspector_source and '_selected_actual_spend_var' in portfolio_inspector_source)
    ...
    add_check(results, "output_contract", case_id, "portfolio_single_ticker_summary_is_compact", True, "投報度: {invested_return_text}" in portfolio_inspector_source and "總損益: {total_pnl:+,.0f}" in portfolio_inspector_source and "交易次數: {exit_count} (正常: {normal_trade_count} | 延續: {extended_trade_count})" in portfolio_inspector_source and "錯失買進: {missed_buy_count} | 錯失賣出: {missed_sell_count}" in portfolio_inspector_source and "勝率: {win_rate_text}" in portfolio_inspector_source and "累計預留" not in portfolio_inspector_source and "累計實支" not in portfolio_inspector_source and "買進成交" not in portfolio_inspector_source)
