...
def validate_gui_single_stock_refined_visual_contract_case(_base_params):
    ...
    add_check(results, "output_contract", case_id, "single_stock_sidebar_fonts_scaled_10_percent", True, "WORKBENCH_RIGHT_SIDEBAR_FONT_SCALE = 0.81" in workbench_source and "WORKBENCH_RIGHT_SIDEBAR_CHIP_FONT" in workbench_source and "WORKBENCH_RIGHT_SIDEBAR_BODY_FONT" in workbench_source)
    return results, summary
...
def validate_gui_signal_annotation_and_forced_close_visual_contract_case(_base_params):
    ...
    add_check(results, "output_contract", case_id, "forced_close_marker_uses_yellow_visual", True, '"強制結算": {"plotly_symbol": "square", "mpl_marker": "s", "color": "#facc15"}' in charting_source)
...
def validate_gui_trade_box_capital_and_round_trip_contract_case(_base_params):
    ...
    add_check(results, "output_contract", case_id, "buy_trade_label_formats_reentry_as_heavy_reentry", True, "進場類型: 重進" in reentry_label_text and "結果: 成交" in reentry_label_text)
    ...
    add_check(results, "output_contract", case_id, "buy_trade_label_overlay_includes_buy_and_missed_traces", True, "CHART_TRADE_LABEL_TRACE_NAMES" in charting_source and '"買進(延續候選)"' in charting_source and '"買進(重進)"' in charting_source and '"錯失買進(重進)"' in charting_source and "supported_traces = set(CHART_TRADE_LABEL_TRACE_NAMES)" in charting_source)
    ...
    add_check(results, "output_contract", case_id, "chart_limit_buy_legend_is_unified", True, 'trace_name = "限價買進"' in charting_source and '"限價買進(延續候選)"' not in charting_source and '("限價線", MATPLOTLIB_LIMIT_COLOR' in charting_source and '進場類型: {entry_type_label}' in charting_source)
...
def validate_gui_trade_count_and_sidebar_sync_contract_case(base_params):
    ...
    add_check(results, "output_contract", case_id, "portfolio_sidebar_declares_reserved_and_actual_spend_fields", True, 'text="交易資訊"' in portfolio_inspector_source and '_selected_capital_var' in portfolio_inspector_source and 'set_workbench_capital_display_text(' in workbench_source and 'resolve_workbench_capital_display_mode_for_snapshot(snapshot)' in workbench_source)
    add_check(results, "output_contract", case_id, "portfolio_history_summary_uses_runtime_params_and_fixed_risk_cache_key", True, 'params = result_payload.get("params")' in portfolio_inspector_source and 'loaded_params.fixed_risk = float(options["fixed_risk"])' in portfolio_inspector_source and 'fixed_risk = getattr(params, "fixed_risk", options.get("fixed_risk", None))' in portfolio_inspector_source and 'cache_key = (resolved_ticker, str(options.get("params_path") or "").strip(), None if fixed_risk is None else float(fixed_risk))' in portfolio_inspector_source)
    ...
    add_check(results, "output_contract", case_id, "portfolio_exit_day_active_level_rows_are_engine_output", True, "active_level_rows=active_level_rows" in portfolio_engine_source and "append_portfolio_position_active_level_row(" in portfolio_exits_source and "require_open_qty=False" in portfolio_exits_source)
    add_check(results, "output_contract", case_id, "portfolio_reserved_fill_rate_uses_filled_buy_count", True, "entry_stats['filled_buy_count']" in portfolio_entries_source and "filled_buy_count / (filled_buy_count + total_missed_buys)" in portfolio_engine_source and "profile_stats['filled_buy_count']" in portfolio_engine_source)
    add_check(results, "output_contract", case_id, "portfolio_buy_rows_keep_reserved_and_actual_capital_separate", True, "'預留總金額': milli_to_money(reserved_cost_milli)" in portfolio_entries_source and '"reserved_capital": None if pd.isna(reserved_capital) else float(reserved_capital)' in portfolio_inspector_source and '"reserved_capital": reserved_capital' in charting_source and '"預留總金額", "投入總金額"' in portfolio_reporting_source)
    add_check(results, "output_contract", case_id, "portfolio_missed_buy_rows_keep_qty_reserved_and_zero_actual_spend", True, "'股數': chosen_entry_plan['qty']" in portfolio_entries_source and "'預留總金額': milli_to_money(reserved_cost_milli)" in portfolio_entries_source and "'投入總金額': 0.0" in portfolio_entries_source)
    add_check(results, "output_contract", case_id, "portfolio_missed_sell_rows_keep_reference_fields", True, "'停損價': pos.get('trailing_stop', pos.get('initial_stop'))" in portfolio_exits_source and "'參考收盤價': get_fast_close(fast_df, pos=t_pos)" in portfolio_exits_source and "'股數': pos.get('qty')" in portfolio_exits_source)
    add_check(results, "output_contract", case_id, "portfolio_dropdown_includes_compact_ticker_stats", True, "event_rows = df_tr[df_tr.apply(_is_portfolio_kline_event_row, axis=1)].copy()" in portfolio_inspector_source and "總損益 {stats.get('total_pnl_text', '-')}" in portfolio_inspector_source and "勝率 {stats.get('win_rate_text', '-')}" in portfolio_inspector_source and "交易次數 {stats.get('trade_count_text', '-')}" in portfolio_inspector_source and "錯買" not in portfolio_inspector_source)
    add_check(results, "output_contract", case_id, "portfolio_ticker_chart_uses_all_kline_events_not_actual_trades_only", True, "df_tr.apply(_is_portfolio_kline_event_row, axis=1)" in portfolio_inspector_source and 'elif str(action).startswith("錯失買進")' in portfolio_inspector_source and 'elif action == "錯失賣出"' in portfolio_inspector_source)
    add_check(results, "output_contract", case_id, "portfolio_kline_distinguishes_normal_extended_reentry_buy_actions", True, 'BUY_TRADE_ACTIONS = ("買進", "買進(延續候選)", "買進(重進)")' in portfolio_inspector_source and 'return "買進(重進)"' in portfolio_inspector_source and 'return "買進(延續候選)"' in portfolio_inspector_source and 'CHART_BUY_TRACE_NAMES = ("買進", "買進(延續候選)", "買進(重進)")' in charting_source and '"買進(延續候選)": "買進"' not in charting_source)
    add_check(results, "output_contract", case_id, "portfolio_kline_labels_use_stock_trade_sequence_not_total_count", True, "next_trade_sequence += 1" in portfolio_inspector_source and 'enriched["trade_sequence"] = int(trade_sequence)' in portfolio_inspector_source and 'enriched["trade_count"]' not in portfolio_inspector_source and "交易次數: 第 {int(trade_sequence)} 次" in charting_source)
    add_check(results, "output_contract", case_id, "portfolio_single_ticker_summary_is_compact", True, "總損益: {total_pnl:+,.0f} ({invested_return_text})" in portfolio_inspector_source and "投報度: {invested_return_text}" not in portfolio_inspector_source and "交易次數: {exit_count} (正常: {normal_trade_count} | 延續: {extended_trade_count} | 重進: {reentry_trade_count})" in portfolio_inspector_source and "錯失買進: {missed_buy_count} | 錯失賣出: {missed_sell_count}" in portfolio_inspector_source and "勝率: {win_rate_text}" in portfolio_inspector_source and "累計預留" not in portfolio_inspector_source and "累計實支" not in portfolio_inspector_source and "買進成交" not in portfolio_inspector_source)
    add_check(results, "output_contract", case_id, "single_stock_workbench_has_fixed_risk_ui_like_portfolio", True, "FIXED_RISK_LABELS" in inspector_source and "self._fixed_risk_display_var" in inspector_source and "params.fixed_risk = float(fixed_risk)" in inspector_source and 'parse_float_strict(raw_value, "固定風險比例"' in inspector_source)
    add_check(results, "output_contract", case_id, "single_stock_pending_analysis_request_keeps_fixed_risk", True, "self._analysis_pending_request = (ticker, params_path, fixed_risk)" in inspector_source and "def _consume_pending_analysis_request(self, completed_ticker, completed_params_path, completed_fixed_risk):" in inspector_source and "float(pending_fixed_risk) == float(completed_fixed_risk)" in inspector_source)
...
def validate_gui_workbench_contract_case(base_params):
    ...
    if len(panel_specs) > 3:
        ...
        add_check(results, "output_contract", case_id, "gui_workbench_data_ops_tab_label", "資料中心", data_ops_panel_spec.get("tab_label"))
        add_check(results, "output_contract", case_id, "gui_workbench_data_ops_backend", "services.trading.market_data_ops.build_market_data_ops_read_model", data_ops_panel_spec.get("backend_runner"))
        add_check(results, "output_contract", case_id, "gui_workbench_data_ops_has_no_artifact_outputs", [], data_ops_panel_spec.get("artifact_keys"))
    ...
    add_check(
        ...
        and "from services.workbench_ui.data_ops_panel import" not in workbench_source,
    )
    ...
    add_check(results, "output_contract", case_id, "gui_data_ops_panel_consumes_canonical_read_model", True, "build_market_data_ops_read_model" in data_ops_panel_source)
    add_check(results, "output_contract", case_id, "gui_data_ops_panel_uses_canonical_due_worker", True, "run_trading_market_data_auto_update" in data_ops_panel_source)
    add_check(results, "output_contract", case_id, "gui_data_ops_panel_uses_canonical_full_update", True, "run_trading_market_data_update" in data_ops_panel_source)
    add_check(results, "output_contract", case_id, "gui_data_ops_panel_delegates_scheduler_mutation_to_service", True, "install_or_update_market_data_scheduler" in data_ops_panel_source and "set_market_data_scheduler_enabled" in data_ops_panel_source and "remove_market_data_scheduler" in data_ops_panel_source)
    add_check(results, "output_contract", case_id, "gui_data_ops_panel_has_no_raw_scheduler_subprocess", True, "subprocess" not in data_ops_panel_source and "schtasks" not in data_ops_panel_source and "Register-ScheduledTask" not in data_ops_panel_source)
    add_check(results, "output_contract", case_id, "gui_data_ops_panel_does_not_write_raw_state", True, "atomic_write_json" not in data_ops_panel_source and "load_json_strict" not in data_ops_panel_source)
    ...
    add_check(results, "output_contract", case_id, "gui_portfolio_performance_tab_close_button_is_chart_overlay", True, "close_host = record.get(\"close_host\")" in close_button_factory_block and "PERFORMANCE_CHART_CLOSE_BUTTON_SIZE_PX" in portfolio_tab_close_block and "PERFORMANCE_CHART_CLOSE_BUTTON_MARGIN_PX" in portfolio_tab_close_block)
    ...
    add_check(results, "output_contract", case_id, "gui_portfolio_performance_hover_crosshair_enabled", True, "def _bind_performance_chart_hover(" in portfolio_inspector_source and 'canvas.mpl_connect("motion_notify_event", _on_motion)' in portfolio_inspector_source)
    add_check(results, "output_contract", case_id, "gui_portfolio_performance_hover_has_time_axis_and_return_lines", True, "mouse_time_axis_enabled" in portfolio_inspector_source and "strategy_return_line_enabled" in portfolio_inspector_source and "benchmark_return_line_enabled" in portfolio_inspector_source and "axis.axvline(" in portfolio_inspector_source and "axis.axhline(" in portfolio_inspector_source)
    add_check(results, "output_contract", case_id, "gui_portfolio_performance_hover_uses_strategy_and_benchmark_values", True, "strategy_return_values" in portfolio_inspector_source and "benchmark_return_values" in portfolio_inspector_source and "V16：" in portfolio_inspector_source and "benchmark_label" in portfolio_inspector_source)
    add_check(results, "output_contract", case_id, "gui_portfolio_performance_hover_disconnects_on_close", True, "hover_connection_ids" in portfolio_inspector_source and "canvas.mpl_disconnect(connection_id)" in portfolio_inspector_source)
    ...
    add_check(results, "output_contract", case_id, "gui_single_prepare_console_task_does_not_delete_history", False, '.delete("1.0", "end")' in shared_prepare_console_block)
    ...
    add_check(results, "output_contract", case_id, "gui_portfolio_prepare_console_task_does_not_delete_history", False, '.delete("1.0", "end")' in shared_prepare_console_block)

    add_check(
        results, "output_contract", case_id, "gui_single_scanner_console_preserves_scanner_ansi_colors", True,
        "SCANNER_CONSOLE_COLORS" in inspector_source
        and "WorkbenchInspectorSharedMixin" in inspector_source
        and "def _insert_ansi_text" in workbench_source
        and "self._console_colors" in workbench_source
        and "ANSI_ESCAPE_SEQUENCE_PATTERN" not in inspector_source,
    )
    ...
    add_check(results, "output_contract", case_id, "gui_portfolio_ui_years_override_optimizer_artifact_range", True, "start_year = ui_start_year" in portfolio_inspector_source and "end_year = ui_end_year" in portfolio_inspector_source and "optimizer_aligned = False" in portfolio_inspector_source)
    add_check(results, "output_contract", case_id, "gui_portfolio_does_not_force_optimizer_replay_date_range", True, 'replay_start_date = str(artifact_meta.get("replay_start_date") or "") if use_optimizer_range else ""' not in portfolio_inspector_source and 'start_date=options.get("replay_start_date") or representative_date' not in portfolio_inspector_source and 'end_date=options.get("replay_end_date") or rolling_last_date' not in portfolio_inspector_source and 'end_date=options.get("replay_end_date") or ensemble_last_date' not in portfolio_inspector_source)
