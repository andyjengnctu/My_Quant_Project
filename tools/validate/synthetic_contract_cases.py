...
def validate_gui_trade_box_capital_and_round_trip_contract_case(_base_params):
    ...
    add_check(results, "output_contract", case_id, "buy_trade_label_overlay_includes_buy_and_missed_traces", True, 'supported_traces = {"買進", "買進(延續候選)", "錯失買進", "停利", "停損賣出", "指標賣出", "強制結算", "錯失賣出"}' in charting_source)
...
def validate_gui_trade_count_and_sidebar_sync_contract_case(base_params):
    ...
    add_check(results, "output_contract", case_id, "portfolio_single_ticker_summary_is_compact", True, "總損益: {total_pnl:+,.0f} ({invested_return_text})" in portfolio_inspector_source and "投報度: {invested_return_text}" not in portfolio_inspector_source and "交易次數: {exit_count} (正常: {normal_trade_count} | 延續: {extended_trade_count})" in portfolio_inspector_source and "錯失買進: {missed_buy_count} | 錯失賣出: {missed_sell_count}" in portfolio_inspector_source and "勝率: {win_rate_text}" in portfolio_inspector_source and "累計預留" not in portfolio_inspector_source and "累計實支" not in portfolio_inspector_source and "買進成交" not in portfolio_inspector_source)
