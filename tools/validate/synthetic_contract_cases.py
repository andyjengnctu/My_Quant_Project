...
def validate_gui_portfolio_summary_platform_renderer_contract_case(_base_params):
    ...
    add_check(results, "output_contract", case_id, "portfolio_summary_uses_rasterized_renderer_independent_of_tk_text_palette", True, 'from matplotlib.backends.backend_agg import FigureCanvasAgg' in portfolio_panel_source and 'canvas.print_png(png_buffer)' in portfolio_panel_source and 'tk.PhotoImage(data=base64.b64encode(' in portfolio_panel_source and 'self._summary_canvas.create_image(' in portfolio_panel_source)
    add_check(results, "output_contract", case_id, "portfolio_summary_avoids_tk_text_and_embedded_widget_foreground_paths", True, 'self._summary_canvas.create_text(' not in portfolio_panel_source and 'self._summary_text = tk.Text(' not in portfolio_panel_source and 'create_window((0, 0), window=' not in portfolio_panel_source)
    add_check(results, "output_contract", case_id, "portfolio_summary_measures_segments_before_rasterizing", True, 'probe_renderer.get_text_width_height_descent(' in portfolio_panel_source and '_summary_matplotlib_font_properties' in portfolio_panel_source and 'self._summary_rows' in portfolio_panel_source)
    add_check(results, "output_contract", case_id, "portfolio_summary_syncs_canvas_scrollregion_to_raster_content", True, 'def _sync_summary_canvas_geometry(self):' in portfolio_panel_source and 'self._summary_canvas.configure(scrollregion=(' in portfolio_panel_source and 'self._summary_content_width' in portfolio_panel_source and 'self._summary_content_height' in portfolio_panel_source)
    return results, summary
