...
def validate_gui_trade_count_and_sidebar_sync_contract_case(base_params):
    ...
    add_check(results, "output_contract", case_id, "portfolio_history_summary_labels_independent_period_not_portfolio", True, "單股獨立歷史績效（非投組）" in portfolio_inspector_source and "口徑: 單股獨立回測，非投組實際成交" in portfolio_inspector_source and "全資料區間" in portfolio_inspector_source and "固定風險:" in portfolio_inspector_source)
    ...
    add_check(results, "output_contract", case_id, "portfolio_dropdown_includes_missed_only_tickers", True, "event_rows = df_tr[df_tr.apply(_is_portfolio_kline_event_row, axis=1)].copy()" in portfolio_inspector_source and "成交 {stats.get('buy_count', 0)} / 錯買 {stats.get('missed_buy_count', 0)} / 錯賣 {stats.get('missed_sell_count', 0)}" in portfolio_inspector_source)
    ...
    add_check(results, "output_contract", case_id, "portfolio_single_ticker_summary_shows_missed_and_capital_efficiency", True, "錯失買進 {missed_buy_count} / 錯失賣出 {missed_sell_count}" in portfolio_inspector_source and "累計預留 {total_reserved_capital:,.0f}" in portfolio_inspector_source and "總損益 {total_pnl:+,.0f} / 投報 {invested_return_text}" in portfolio_inspector_source)
