...
from .checks import bind_synthetic_case, bind_checks, add_check, run_bound_checks, make_synthetic_validation_params, run_scanner_reference_check, run_scanner_reference_check_on_clean_df
...
def validate_output_contract_case(_base_params):
    ...
    with tempfile.TemporaryDirectory(prefix="output_contract_") as temp_dir:
        ...
        check("validate_summary_exists", True, validate_summary_path.exists())
        validate_payload = json.loads(validate_summary_path.read_text(encoding="utf-8"))
        check("validate_summary_required_keys_present", [], sorted(REQUIRED_VALIDATE_SUMMARY_KEYS - set(validate_payload.keys())))
        check("validate_summary_status", "FAIL", validate_payload.get("status"))
        check("validate_summary_total_checks", 3, validate_payload.get("total_checks"))
        check("validate_summary_pass_count", 1, validate_payload.get("pass_count"))
        check("validate_summary_skip_count", 1, validate_payload.get("skip_count"))
        check("validate_summary_fail_count", 1, validate_payload.get("fail_count"))
        check("validate_summary_real_ticker_count", 3, validate_payload.get("real_ticker_count"))
        ...
        check("issue_report_xlsx_exists", True, bool(xlsx_path) and Path(xlsx_path).exists())
        workbook = load_workbook(xlsx_path)
        expected_sheets = ["failed_only", "failed_tickers", "failed_modules"]
        check("issue_report_sheet_names", expected_sheets, workbook.sheetnames)
        failed_only_headers = [cell.value for cell in workbook["failed_only"][1]]
        check("issue_report_failed_only_headers", ["ticker", "module", "metric", "passed", "expected", "actual", "note"], failed_only_headers)
        failed_tickers_headers = [cell.value for cell in workbook["failed_tickers"][1]]
        check("issue_report_failed_tickers_headers", ["ticker", "failed_checks"], failed_tickers_headers)
        failed_only_ticker = workbook["failed_only"]["A2"].value
        check("issue_report_ticker_normalized_text", "1102", str(failed_only_ticker))
        ...
        check("optimizer_profile_csv_exists", True, profile_csv_path.exists())
        check("optimizer_profile_summary_exists", True, profile_summary_path.exists())
        fieldnames, profile_rows = _read_csv_rows(profile_csv_path)
        check("optimizer_profile_csv_header", PROFILE_FIELDS, fieldnames)
        check("optimizer_profile_csv_row_count", 1, len(profile_rows))
        profile_summary = json.loads(profile_summary_path.read_text(encoding="utf-8"))
        check("optimizer_profile_summary_trial_count", 1, profile_summary.get("trial_count"))
        check("optimizer_profile_summary_avg_keys_present", [], sorted(REQUIRED_PROFILE_SUMMARY_AVG_KEYS - set(profile_summary.get("avg", {}).keys())))
        check("optimizer_profile_summary_avg_objective_wall_sec", 1.2, profile_summary.get("avg", {}).get("objective_wall_sec"))
...
def validate_quick_gate_bare_except_guard_contract_case(_base_params):
    ...
    with tempfile.TemporaryDirectory(prefix="quick_gate_bare_except_contract_", dir=str(staging_root)) as temp_dir:
        ...
        check("run_static_checks_declares_bare_except_scan", True, bare_step is not None)
        check("bare_except_scan_fails_when_bare_except_exists", "FAIL", None if bare_step is None else bare_step.get("status"))
        check("bare_except_scan_reports_bare_except_file", True, any("bare_except_probe.py" in hit for hit in bare_hits))
        check("bare_except_scan_does_not_flag_typed_except", False, any("typed_except_probe.py" in hit for hit in bare_hits))
...
def validate_quick_gate_output_path_guard_contract_case(_base_params):
    ...
    run_bound_checks(
        check,
        (
            ('portfolio_replay_runtime_project_root_matches_repository_root', PROJECT_ROOT.resolve(), runtime_project_root,),
            ('portfolio_replay_runtime_output_dir_is_project_scoped', (PROJECT_ROOT / 'outputs' / 'portfolio_sim').resolve(), runtime_output_dir,),
        ),
    )
    ...
    with tempfile.TemporaryDirectory(prefix="quick_gate_output_path_contract_", dir=str(staging_root)) as temp_dir:
        ...
        check("quick_gate_main_wires_output_path_contract_guard", True, "output_path_contract::valid_category_resolves_under_outputs" in pass_step_names)
        check("quick_gate_main_wires_outputs_root_layout_guard", True, "outputs_root_layout::root_has_only_category_dirs" in pass_step_names)
        check("quick_gate_main_wires_log_path_contract_guard", True, "log_path_contract::append_issue_log_outputs_root_file_rejected" in pass_step_names)
        check("quick_gate_main_wires_log_dir_outputs_root_guard", True, "log_path_contract::resolve_log_dir_outputs_root_rejected" in pass_step_names)
        check("quick_gate_output_path_probe_pass_return_code", 0, pass_rc)
        check("quick_gate_output_path_probe_pass_status", "PASS", pass_payload.get("status"))
        check("quick_gate_output_path_probe_fail_return_code", 1, fail_rc)
        check("quick_gate_output_path_probe_fail_status", "FAIL", fail_payload.get("status"))
        check("quick_gate_output_path_probe_fail_propagates_failed_step", ["outputs_root_layout::root_has_only_category_dirs"], fail_payload.get("failed_steps"))
...
def validate_local_regression_summary_contract_case(_base_params):
    ...
    with tempfile.TemporaryDirectory(prefix="local_regression_summary_contract_") as temp_dir:
        ...
        check("preflight_summary_contract_lines", True, "status" in preflight_text and "failed_packages : coverage, optuna" in preflight_text)
        ...
        check("dataset_prepare_pass_required_keys", [], sorted(REQUIRED_DATASET_PREP_SUMMARY_KEYS - set(dataset_pass_json.keys())))
        check("dataset_prepare_pass_text_contract", True, "dataset_dir : data/tw_stock_data_vip_reduced" in dataset_pass_text and f"csv_count   : {dataset_pass_json.get("csv_count")}" in dataset_pass_text and "members_sha : " in dataset_pass_text and "content_sha : " in dataset_pass_text)
        ...
        check("dataset_prepare_fail_status", "FAIL", dataset_fail.get("status"))
        check("dataset_prepare_fail_text_contract", True, "error_type  : RuntimeError" in dataset_fail_text and "error_msg   : zip missing" in dataset_fail_text)
        ...
        check("chain_summary_required_keys", [], sorted(REQUIRED_CHAIN_SUMMARY_KEYS - set(chain_json.keys())))
        check("chain_summary_csv_header", CHAIN_SUMMARY_CSV_FIELDS, chain_csv_fields)
        ...
        check("ml_smoke_summary_required_keys", [], sorted(REQUIRED_ML_SMOKE_SUMMARY_KEYS - set(ml_smoke_json.keys())))
        ...
        check("meta_quality_summary_required_keys", [], sorted(REQUIRED_META_QUALITY_SUMMARY_KEYS - set(meta_json.keys())))
        check("meta_quality_summary_coverage_threshold_keys", [], sorted([key for key in ["status", "line_percent_covered", "branch_percent_covered", "line_min_percent", "branch_min_percent", "missing_targets", "zero_covered_targets"] if key not in meta_json.get("coverage", {})]))
        check("meta_quality_summary_checklist_status_key", True, meta_json.get("checklist", {}).get("status") == "DONE")
        check("meta_quality_summary_formal_entry_required_keys", [], sorted(REQUIRED_META_QUALITY_FORMAL_ENTRY_KEYS - set(meta_json.get("formal_entry", {}).keys())))
        check("meta_quality_summary_formal_entry_has_no_retired_steps_key", False, "steps" in meta_json.get("formal_entry", {}))
        ...
        check("quick_gate_summary_required_keys", [], sorted(REQUIRED_QUICK_GATE_SUMMARY_KEYS - set(quick_gate_json.keys())))
        ...
        check("quick_gate_in_memory_py_compile_passes_valid_source", "PASS", py_compile_result.get("status"))
        check("quick_gate_in_memory_compileall_equivalent_passes_valid_source", "PASS", compileall_result.get("status"))
        ...
        check("quick_gate_in_memory_py_compile_rejects_invalid_source", "FAIL", invalid_py_compile.get("status"))
        check("quick_gate_in_memory_compileall_equivalent_rejects_invalid_source", "FAIL", invalid_compileall.get("status"))
        ...
        check("quick_gate_runtime_failure_return_code", 1, runtime_rc)
        check("quick_gate_runtime_failure_status", "FAIL", runtime_json.get("status"))
        check("quick_gate_runtime_failure_marks_runtime_step", ["__runtime__"], runtime_json.get("failed_steps"))
        check("quick_gate_runtime_failure_includes_error_fields", True, runtime_json.get("error_type") == "RuntimeError" and "synthetic quick gate crash" in runtime_json.get("runtime_error", ""))
        check("quick_gate_runtime_failure_console_has_runtime_error", True, "synthetic quick gate crash" in runtime_console)
        check("quick_gate_runtime_failure_does_not_leak_runtime_payload_to_parent_stdout", "", runtime_stdout_text)
        ...
        check("master_summary_required_keys", [], sorted(REQUIRED_MASTER_SUMMARY_KEYS - set(master_json.keys())))
        check("master_summary_not_run_steps_contract", ["chain_checks", "ml_smoke", "meta_quality"], master_json.get("not_run_step_names"))
        check("master_summary_bundle_entries_posix", True, all("\\" not in item for item in master_json.get("bundle_entries", [])))
...
def validate_run_all_manifest_failure_master_summary_contract_case(_base_params):
    ...
    with tempfile.TemporaryDirectory(prefix="run_all_manifest_fail_contract_") as temp_dir:
        ...
        check("manifest_failed_master_summary_required_keys", [], sorted(REQUIRED_MASTER_SUMMARY_KEYS - set(master_json.keys())))
        check("manifest_failed_master_summary_not_run_steps", expected_not_run, master_json.get("not_run_step_names"))
        check("manifest_failed_master_summary_preflight_status", "NOT_RUN", master_json.get("preflight", {}).get("status"))
        check("manifest_failed_master_summary_preflight_blocked_by", "manifest", master_json.get("preflight", {}).get("blocked_by"))
        check("manifest_failed_master_summary_dataset_prepare_status", "NOT_RUN", master_json.get("dataset_prepare", {}).get("status"))
        check("manifest_failed_master_summary_dataset_prepare_blocked_by", "manifest", master_json.get("dataset_prepare", {}).get("blocked_by"))
        check("manifest_failed_master_summary_payload_failures_empty", [], master_json.get("payload_failures"))
...
def validate_artifact_lifecycle_contract_case(_base_params):
    ...
    results, summary, check, check_true = bind_synthetic_case(case_id, 'artifact_contract')
    ...
    with tempfile.TemporaryDirectory(prefix="artifact_lifecycle_") as temp_dir:
        ...
        try:
            ...
            check("select_bundle_paths_pass_minimum_set",
                [
                    "master_summary.json",
            ...
            check("select_bundle_paths_fail_includes_nested_debug_files", True, "nested/debug.log" in preferred_fail_rel)

            manifest_payload = local_common.build_artifacts_manifest(run_dir)
            check("artifacts_manifest_counts_all_files", 12, manifest_payload.get("artifact_count"))
            manifest_paths = [item.get("relative_path") for item in manifest_payload.get("artifacts", [])]
            check("artifacts_manifest_uses_posix_relative_paths", True, all(isinstance(item, str) and "\\" not in item and not item.startswith("/") for item in manifest_paths))
            check("artifacts_manifest_includes_nested_debug_file", True, "nested/debug.log" in manifest_paths)
            ...
            check("artifacts_manifest_excludes_self_reference", False, local_common.ARTIFACTS_MANIFEST_FILENAME in written_manifest_paths)
            check("artifacts_manifest_written_hashes_are_verifiable", True, written_manifest_hashes_valid)

            bundle_path = local_common.build_bundle_zip(run_dir, "to_chatgpt_bundle.zip", include_paths=[probe_master, probe_console])
            check("bundle_zip_exists", True, bundle_path.exists())
            with zipfile.ZipFile(bundle_path, "r") as zf:
                zip_members = sorted(zf.namelist())
            check("bundle_zip_member_list", ["console_tail.txt", "master_summary.json"], zip_members)

            archived_bundle = local_common.archive_bundle_history(bundle_path)
            check("archive_bundle_exists", True, archived_bundle.exists())
            check("archive_bundle_removed_from_run_dir", False, bundle_path.exists())
            check("archive_bundle_stays_under_output_root", str(output_root.resolve()), str(archived_bundle.parent.resolve()))
            ...
            check("root_bundle_copy_exists", True, root_copy_1.exists())
            root_bundles_after_first = sorted(path.name for path in project_root.glob("to_chatgpt_bundle*.zip"))
            check("root_bundle_old_copy_removed_on_publish", [root_copy_1.name], root_bundles_after_first)
            ...
            check("rerun_bundle_overwrites_run_dir_zip", ["console_tail.txt", "master_summary.json"], rebundle_members)
            check("rerun_bundle_contains_latest_console_tail", "tail updated\n", rebundle_console_normalized)
            ...
            check("root_bundle_only_latest_copy_kept", [root_copy_2.name], root_bundles_after_second)
            ...
            check("root_bundle_copy_matches_latest_archive_contents", "tail updated\n", root_console_normalized)
            ...
            check("retention_removed_old_archive_count", 1, retention.get("removed_count"))
            check("retention_removed_old_archive_missing", False, old_archive.exists())
            remaining_archives = sorted(path.name for path in output_root.glob("to_chatgpt_bundle*.zip"))
            check("retention_keeps_recent_archives", sorted([archived_bundle.name, archived_bundle_2.name]), remaining_archives)
        finally:
...
def validate_dataset_fingerprint_contract_case(_base_params):
    ...
    check("dataset_fingerprint_has_required_keys", [], sorted([
    ...
    check("dataset_fingerprint_algorithm_is_sha256", "sha256", dataset_info.get("fingerprint_algorithm"))
    check("dataset_member_hash_length_is_64", 64, len(str(dataset_info.get("csv_members_sha256", ""))))
    check("dataset_content_hash_length_is_64", 64, len(str(dataset_info.get("csv_content_sha256", ""))))
    check("dataset_total_bytes_positive", True, int(dataset_info.get("csv_total_bytes", 0)) > 0)
    ...
    check("dataset_prepare_summary_preserves_member_hash", dataset_info.get("csv_members_sha256"), summary_json.get("csv_members_sha256"))
    check("dataset_prepare_summary_preserves_content_hash", dataset_info.get("csv_content_sha256"), summary_json.get("csv_content_sha256"))
    check("dataset_prepare_summary_preserves_total_bytes", dataset_info.get("csv_total_bytes"), summary_json.get("csv_total_bytes"))
    check("dataset_prepare_summary_text_displays_hashes", True, "members_sha : " in summary_txt and "content_sha : " in summary_txt)
...
def validate_reduced_dataset_dynamic_contract_case(_base_params):
    ...
    check("reduced_dataset_live_csv_count_matches_members", len(actual_members), int(dataset_info.get("csv_count", 0)))
    check("reduced_dataset_live_total_bytes_positive", True, int(dataset_info.get("csv_total_bytes", 0)) > 0)
    ...
    run_bound_checks(
        check,
        (
            ('reduced_dataset_contract_accepts_different_member_set', sorted(synthetic_members.keys()), dynamic_members,),
            ('reduced_dataset_contract_uses_runtime_member_count', len(synthetic_members), int(dynamic_info.get('csv_count', 0)),),
            ('reduced_dataset_contract_uses_runtime_fingerprint', expected_dynamic_fingerprint, {'csv_total_bytes': dynamic_info.get('csv_total_bytes'), 'csv_members_sha256': dynamic_info.get('csv_members_sha256'), 'csv_content_sha256': dynamic_info.get('csv_content_sha256'), 'fingerprint_algorithm': dynamic_info.get('fingerprint_algorithm')},),
        ),
    )
...
def validate_run_all_dataset_prepare_pass_main_contract_case(_base_params):
    ...
    check("run_all_main_dataset_prepare_pass_return_code", 0, rc)
    check("run_all_main_dataset_prepare_pass_overall_status", "PASS", master_summary.get("overall_status"))
    check("run_all_main_dataset_prepare_payload_status", "PASS", master_summary.get("dataset_prepare", {}).get("status"))
    check("run_all_main_dataset_prepare_not_run_steps_empty", [], master_summary.get("not_run_step_names", []))
    check("run_all_main_dataset_prepare_selected_steps", list(run_all_module.STEP_NAMES), master_summary.get("selected_steps"))
    check("run_all_parent_wall_time_source_is_persisted", formal_wall_time_module.FORMAL_WALL_TIME_SOURCE, master_summary.get("formal_wall_time", {}).get("source"))
    check("run_all_parent_wall_time_critical_path_uses_outer_durations", 0.04, master_summary.get("formal_wall_time", {}).get("critical_path_duration_sec"))
    check("meta_quality_receives_parent_wall_time_source", formal_wall_time_module.FORMAL_WALL_TIME_SOURCE, meta_quality_summary.get("performance", {}).get("formal_wall_time_source"))
    check("meta_quality_receives_parent_wall_time_critical_path", 0.04, meta_quality_summary.get("performance", {}).get("critical_path_duration_sec"))
    check("run_all_formal_primary_param_file_exists", True, formal_param_exists)
    check("run_all_formal_primary_param_uses_current_config_defaults", params_to_json_dict(V16StrategyParams()), formal_param_payload)
    check("run_all_all_child_steps_receive_isolated_param_override", [str(formal_param_path)] * len(child_param_paths), child_param_paths)
    check("run_all_dataset_prepare_records_formal_param_source", str(formal_param_path), dataset_prepare_summary.get("formal_primary_param_source"))
    check("run_all_dataset_prepare_records_formal_param_source_kind", "generated_from_current_config_defaults", dataset_prepare_summary.get("formal_primary_param_source_kind"))
    check("consistency_startup_failure_writes_summary", "FAIL", startup_failure_summary.get("status"))
    check("consistency_startup_failure_summary_has_required_keys", True, REQUIRED_VALIDATE_SUMMARY_KEYS.issubset(startup_failure_summary))
    check("consistency_startup_failure_summary_records_phase", "primary_param_source_load", startup_failure_summary.get("startup_failure_phase"))
    check("consistency_startup_failure_summary_records_error_type", "FileNotFoundError", startup_failure_summary.get("error_type"))
    for key in local_common.DATASET_INFO_KEYS:
        if key in dataset_info:
            check(f"run_all_main_dataset_prepare_preserves_{key}", dataset_info[key], dataset_prepare_summary.get(key))
            check(f"run_all_main_master_summary_dataset_info_preserves_{key}", dataset_info[key], master_summary.get("dataset_info", {}).get(key))
...
def validate_atomic_write_contract_case(_base_params):
    ...
    with tempfile.TemporaryDirectory(prefix="atomic_write_contract_") as temp_dir:
        ...
        check("atomic_write_json_preserves_previous_contents_on_replace_failure", original_json, json_path.read_text(encoding="utf-8"))
        check("atomic_write_text_preserves_previous_contents_on_replace_failure", original_text, text_path.read_text(encoding="utf-8"))
        check("atomic_write_cleanup_removes_temp_files_after_failure", [], remaining_temp_files)
    ...
    check("atomic_write_json_success_after_failure", "final", final_json.get("status"))
    check("atomic_write_text_success_after_failure", "final-text\n", final_text)
...
def validate_atomic_write_cleanup_error_preserves_root_exception_case(_base_params):
    ...
    check("atomic_write_cleanup_failure_preserves_root_exception_type", "OSError", type(raised_exc).__name__)
    check("atomic_write_cleanup_failure_preserves_root_exception_message", True, "simulated atomic replace failure" in str(raised_exc))
    check("atomic_write_cleanup_failure_records_cleanup_note", True, any("simulated temp cleanup failure" in note for note in notes))
    check("atomic_write_cleanup_failure_leaves_temp_file_visible_for_manual_cleanup", 1, len(remaining_temp_files))
...
def validate_meta_quality_performance_memory_contract_case(_base_params):
    ...
    with tempfile.TemporaryDirectory(prefix="meta_quality_performance_memory_contract_") as temp_dir:
        ...
        check("performance_memory_contract_ok", True, perf.get("ok"))
        check("performance_step_peak_memory_keys", ["chain_checks", "consistency", "ml_smoke", "quick_gate"], sorted(perf.get("step_peak_process_memory_mb", {}).keys()))
        check("performance_max_peak_memory_value", 64.0, perf.get("max_step_peak_process_memory_mb"))
        check("performance_meta_quality_peak_memory_value", 40.0, perf.get("meta_quality_peak_process_memory_mb"))
        check("performance_child_does_not_claim_formal_wall_time", None, perf.get("formal_wall_time_source"))
        ...
        check("performance_parallel_critical_path_uses_parent_wall_max_not_sum", 5.25, parent_wall.get("critical_path_duration_sec"))
        check("performance_parent_aggregate_duration_retained_for_diagnostics", 11.25, parent_wall.get("aggregate_step_duration_sec"))
        check("performance_parent_wall_time_is_canonical_source", formal_wall_time_module.FORMAL_WALL_TIME_SOURCE, parent_wall.get("source"))
        ...
        check("performance_total_budget_is_actually_enforced_by_parent_wall_time", "FAIL", total_budget_result.get("status"))
        ...
        check("performance_consistency_budget_uses_parent_wall_time", "FAIL", consistency_budget_result.get("status"))
        ...
        check("performance_parent_wall_merge_updates_meta_status", "FAIL", merged_meta.get("status"))
        check("performance_parent_wall_merge_updates_meta_source", formal_wall_time_module.FORMAL_WALL_TIME_SOURCE, merged_meta.get("performance", {}).get("formal_wall_time_source"))
        check("performance_parent_wall_merge_preserves_failed_step_reason", True, "performance_consistency_within_budget" in merged_meta.get("failures", []))
        ...
        check("performance_synthetic_case_budget_is_actually_enforced",
            "FAIL",
        ...
        run_bound_checks(
            check,
            (
                ('performance_process_memory_is_telemetry_not_absolute_blocker', False, 'performance_process_peak_memory_within_budget' in performance_result_names,),
                ('performance_manifest_has_no_platform_specific_absolute_memory_budget', False, 'performance_peak_process_memory_mb' in local_common.MANIFEST_DEFAULTS,),
            ),
        )
        ...
        run_bound_checks(
            check,
            (
                ('missing_upstream_performance_summary_is_blocked_not_secondary_fail', 'BLOCKED', required_summary_result.get('status'),),
                ('missing_upstream_performance_summary_still_blocks_meta_completion', False, blocked_perf.get('ok'),),
            ),
        )
...
def validate_scanner_live_capital_contract_case(base_params):
    ...
    check("scanner_live_capital_qty", 2000, projected_qty)
    check("scanner_live_capital_proj_cost", 200000.0, projected_cost)
    check("scanner_result_status", "buy", None if scanner_result is None else scanner_result[0])
    check("scanner_result_proj_cost_uses_scanner_live_capital", 200000.0, None if scanner_result is None else float(scanner_result[1]))
    check("scanner_result_message_mentions_scanner_live_capital_cost", True, False if scanner_result is None else "參考投入:200,000" in scanner_result[4])
...
def validate_debug_chart_payload_without_html_export_contract_case(base_params):
    ...
    check("debug_chart_payload_available_without_html_export", True, chart_payload is not None)
    check("debug_chart_path_omitted_without_html_export", None, analysis_result.get("chart_path"))
    check("debug_chart_payload_total_bars_matches_clean_df", len(clean_df), 0 if chart_payload is None else len(chart_payload.get("x", [])))

    if chart_payload is not None:
        check("debug_chart_payload_default_view_nonempty_without_html_export", True, int(chart_payload["default_view"]["end_idx"]) >= int(chart_payload["default_view"]["start_idx"]))
...
def validate_debug_empty_price_df_chart_payload_contract_case(base_params):
    ...
    chart_payload = analysis_result.get("chart_payload")
    check("debug_empty_price_df_chart_payload_exists", True, chart_payload is not None)
    check("debug_empty_price_df_chart_payload_is_placeholder_single_bar", 1, 0 if chart_payload is None else len(chart_payload.get("x", [])))
    if chart_payload is not None:
        check("debug_empty_price_df_chart_payload_volume_zero", 0.0, float(chart_payload["volume"][0]))
        check("debug_empty_price_df_chart_payload_open_zero", 0.0, float(chart_payload["open"][0]))
...
def validate_gui_embedded_chart_contract_case(base_params):
    ...
    check("gui_embedded_chart_backend", "services.trade_analysis.charting.create_matplotlib_trade_chart_figure", panel_spec.get("inline_chart_backend"))
    check("gui_embedded_chart_default_show_volume", False, panel_spec.get("default_show_volume"))
    ...
    chart_payload = analysis_result.get("chart_payload")
    check("gui_embedded_chart_payload_exists", True, chart_payload is not None)
    ...
    check("gui_embedded_chart_default_view_nonempty", True, end_idx >= start_idx)
    check("gui_embedded_chart_focus_positions_in_view", True, all(start_idx <= pos <= end_idx for pos in focus_positions))

    visible_ranges = compute_visible_value_ranges(chart_payload, start_idx=start_idx, end_idx=end_idx)
    check("gui_embedded_chart_visible_price_range_valid", True, visible_ranges["price_max"] > visible_ranges["price_min"])
    check("gui_embedded_chart_visible_volume_range_valid", True, visible_ranges["volume_max"] > visible_ranges["volume_min"])
    ...
    check("gui_embedded_chart_contract_volume_visible", True, contract_with_volume.get("volume_visible"))
    check("gui_embedded_chart_volume_overlay_mode", "inset", contract_with_volume.get("volume_overlay_mode"))
    check("gui_embedded_chart_volume_overlay_axis_present", True, contract_with_volume.get("volume_overlay_axis_present"))
    ...
    check("gui_embedded_chart_full_history_render_window", large_contract.get("total_bar_count", 0), large_contract.get("render_bar_count", 0))
    check("gui_embedded_chart_full_history_navigation_enabled", True, large_contract.get("full_history_navigation_enabled"))
    check("gui_embedded_chart_cjk_font_candidates_include_jhenghei", True, "Microsoft JhengHei" in get_matplotlib_cjk_font_candidates())
    check("gui_embedded_chart_cjk_font_candidates_include_noto_tc", True, "Noto Sans CJK TC" in get_matplotlib_cjk_font_candidates())
    large_figure.clear()
...
def validate_chart_payload_optional_overlay_keys_contract_case(_base_params):
    ...
    check("chart_payload_optional_limit_line_filled", 8, int(len(normalized_payload.get("limit_line", []))))
    check("chart_payload_optional_entry_line_filled", 8, int(len(normalized_payload.get("entry_line", []))))
    check("chart_payload_optional_tp_line_filled", 8, int(len(normalized_payload.get("tp_line", []))))
    check("chart_payload_optional_stop_line_filled", 8, int(len(normalized_payload.get("stop_line", []))))
    check("chart_payload_optional_summary_box_defaulted", [], normalized_payload.get("summary_box"))
    check("chart_payload_optional_status_box_defaulted", {}, normalized_payload.get("status_box"))

    figure = create_matplotlib_trade_chart_figure(chart_payload=minimal_payload, ticker="2330", show_volume=False)
    contract = getattr(figure, "_stock_chart_contract", {})
    check("chart_payload_optional_overlay_keys_figure_builds_successfully", True, contract.get("total_bar_count", 0) == len(minimal_payload["x"]))
    figure.clear()
    return results, summary
...
def validate_gui_mouse_navigation_contract_case(_base_params):
    ...
    check("gui_chart_does_not_require_navigation_toolbar", True, "NavigationToolbar2Tk" not in inspector_source)
    check("gui_chart_binds_mouse_navigation", True, "bind_matplotlib_chart_navigation(figure, canvas)" in inspector_source)
    check("charting_declares_mouse_navigation_binder", True, "def bind_matplotlib_chart_navigation(figure, canvas):" in charting_source)
    check("charting_supports_wheel_zoom_event", True, '"scroll_event"' in charting_source)
    check("charting_supports_left_drag_pan_event", True, '"motion_notify_event"' in charting_source and 'event.button != 1' in charting_source)
    ...
    figure = create_matplotlib_trade_chart_figure(chart_payload=minimal_payload, ticker="2330", show_volume=False)
    contract = getattr(figure, "_stock_chart_contract", {})
    check("figure_contract_toolbar_not_required", False, contract.get("toolbar_required"))
    check("figure_contract_mouse_zoom_flag_default_false_until_canvas_bind", False, contract.get("mouse_wheel_zoom_enabled"))
    check("chart_navigation_binder_callable", True, callable(bind_matplotlib_chart_navigation))
    return results, summary
...
def validate_gui_navigation_canvas_stub_cleanup_contract_case(_base_params):
    ...
    check("canvas_stub_declares_grab_mouse", True, callable(getattr(canvas, "grab_mouse", None)))
    check("canvas_stub_declares_release_mouse", True, callable(getattr(canvas, "release_mouse", None)))

    sentinel_axis = object()
    canvas.grab_mouse(sentinel_axis)
    canvas.release_mouse(sentinel_axis)
    check("canvas_stub_release_mouse_clears_grabbed_axis", None, canvas._grabbed_axis)
    check("canvas_stub_release_mouse_records_axis", 1, len(canvas.release_calls))
    return results, summary
...
def validate_gui_dark_theme_and_keyboard_pan_contract_case(_base_params):
    ...
    check("gui_workbench_declares_deep_dark_theme", True, workbench_spec.get("ui_theme") == "deep_dark")
    check("gui_workbench_configures_dark_ttk_theme", True, "def configure_workbench_theme(root):" in workbench_source and 'style.theme_use("clam")' in workbench_source)
    check("charting_supports_keyboard_pan_event", True, '"key_press_event"' in charting_source)
    check("charting_supports_status_chip_rendering", True, "def _render_status_chips(axis_price, status_box, label_font):" in charting_source)
    check("charting_supports_dynamic_candle_widths", True, "def _compute_dynamic_linewidths(axis_price, figure):" in charting_source)
    ...
    contract = getattr(figure, "_stock_chart_contract", {})
    check("figure_contract_keyboard_pan_enabled", True, contract.get("keyboard_pan_enabled"))
    check("figure_contract_dynamic_candle_width_enabled", True, contract.get("dynamic_candle_width_enabled"))
    check("figure_contract_status_chip_layout", "right_bottom", contract.get("status_chip_layout"))
    figure.clear()
    return results, summary
...
def validate_gui_chart_workspace_contract_case(_base_params):
    ...
    inspector_source = build_project_absolute_path("services", "workbench_ui", "single_stock_inspector.py").read_text(encoding="utf-8")
    check("gui_chart_workspace_uses_notebook_tabs", True, 'ttk.Notebook(self, style="Workbench.TNotebook")' in inspector_source)
    check("gui_chart_workspace_omits_summary_tab", False, 'text="執行摘要"' in inspector_source)
    check("gui_chart_workspace_has_trade_detail_tab", True, 'text="交易明細"' in inspector_source)
    check("gui_chart_workspace_default_volume_hidden", True, 'self._show_volume_var = tk.BooleanVar(value=False)' in inspector_source)
    check("gui_chart_workspace_volume_toggle_rerenders_chart", True, 'show_volume=bool(self._show_volume_var.get())' in inspector_source)

    workbench_spec = importlib.import_module("services.workbench_ui").build_workbench_spec()
    panel_spec = workbench_spec.get("panels", [])[0]
    check("gui_chart_workspace_panel_default_show_volume", False, panel_spec.get("default_show_volume"))
    check("gui_chart_workspace_startup_window_mode", "maximized", workbench_spec.get("startup_window_mode"))
    ...
    contract = getattr(figure, "_stock_chart_contract", {})
    check("gui_chart_workspace_full_history_bar_count", len(chart_payload["x"]), contract.get("render_bar_count"))
    check("gui_chart_workspace_volume_overlay_ratio_leq_quarter", True, contract.get("volume_overlay_ratio", 1.0) <= 0.25)
    check("gui_chart_workspace_volume_overlay_mode", "inset", contract.get("volume_overlay_mode"))
    check("gui_chart_workspace_volume_overlay_axis_present", True, contract.get("volume_overlay_axis_present"))
    figure.clear()
...
def validate_record_signal_annotation_meta_contract_case(_base_params):
    ...
    check("record_signal_annotation_accepts_meta_keyword", 1, len(chart_context["signal_annotations"]))
    check("signal_annotation_meta_retained_in_chart_context", meta_payload, chart_context["signal_annotations"][0].get("meta") if chart_context["signal_annotations"] else None)
    check("signal_annotation_meta_retained_in_chart_payload", meta_payload, first_annotation.get("meta"))
    check("signal_annotation_profit_pct_available_for_sell_face_resolution", 3.25, (first_annotation.get("meta") or {}).get("profit_pct"))
    return results, summary
...
def validate_gui_scanner_console_and_latest_contract_case(_base_params):
    ...
    check("gui_panel_uses_full_dataset_only", False, 'text="資料集"' in inspector_source)
    check("gui_panel_exposes_scanner_button", True, 'text="計算候選股"' in inspector_source)
    check("gui_panel_exposes_candidate_dropdown", True, '_candidate_combo' in inspector_source)
    check("gui_panel_exposes_history_button", True, 'text="計算歷史績效股"' in inspector_source)
    check("gui_panel_exposes_history_dropdown", True, '_history_combo' in inspector_source)
    check("gui_history_dropdown_uses_sort_probe_text", True, 'build_scanner_sort_probe_text(' in inspector_source)
    check("gui_panel_exposes_console_tab", True, 'text="Console"' in inspector_source)
    check("gui_panel_exposes_latest_button", True, 'text="回到最新K線"' in inspector_source)
    check("gui_panel_calls_scanner_runner", True, 'run_daily_scanner(' in inspector_source)
    check("gui_panel_calls_history_scanner_runner", True, 'run_history_qualified_scanner(' in inspector_source)
    check("charting_declares_scroll_to_latest_helper", True, 'def scroll_chart_to_latest(' in charting_source)
    check("charting_declares_right_padding_bars", True, 'CHART_RIGHT_PADDING_BARS' in charting_source)
    check("scanner_runner_returns_candidate_rows_payload", True, '"candidate_rows": list(candidate_rows)' in scanner_source or "'candidate_rows': list(candidate_rows)" in scanner_source)
    check("scanner_runner_returns_history_rows_payload", True, "\"history_qualified_rows\": list(scan_state['rows'])" in scanner_source or "'history_qualified_rows': list(scan_state['rows'])" in scanner_source)
    check("scanner_display_declares_asset_growth_probe", True, '資產成長' in scanner_display_source and 'build_scanner_sort_probe_text' in scanner_display_source)
    check("scanner_row_text_uses_sort_probe_text", True, 'build_scanner_sort_probe_text(' in stock_processor_source)
    check("scanner_history_rows_preserve_asset_growth_metric", True, 'asset_growth' in stock_processor_source and "'asset_growth': asset_growth_pct" in stock_processor_source)
    return results, summary
...
def validate_gui_sidebar_latest_preview_contract_case(_base_params):
    ...
    check("gui_panel_removes_execute_button", False, 'text="執行回測"' in inspector_source)
    check("gui_panel_runs_backtest_on_enter", True, 'ticker_entry.bind("<Return>"' in inspector_source)
    check("gui_panel_runs_backtest_on_candidate_select", True, 'self.after_idle(self._run_analysis)' in inspector_source)
    check("gui_panel_declares_right_sidebar_signal_chip", True, '_signal_chip' in inspector_source and '歷史績效表' in inspector_source)
    check("gui_panel_omits_html_button", False, '開啟 HTML K 線圖' in inspector_source)
    check("gui_panel_declares_selected_date_value_block", True, '選取日線值' in inspector_source and '_selected_open_var' in inspector_source and '_selected_high_var' in inspector_source and '_selected_low_var' in inspector_source and '_selected_close_var' in inspector_source and '_selected_volume_var' in inspector_source)
    check("charting_declares_future_preview_contract", True, 'future_preview' in charting_source and '_render_future_preview_lines' in charting_source)
    check("charting_declares_dynamic_right_padding", True, 'extra_right_padding = max(' in charting_source and 'CHART_RIGHT_PADDING_RATIO' in charting_source)
    check("backtest_uses_future_preview_for_latest_signal", True, 'set_chart_future_preview(' in backtest_source)
    check("charting_reduces_latest_blank_space_to_about_one_sixth", True, "CHART_RIGHT_PADDING_RATIO = 0.1666666667" in charting_source)
    return results, summary
...
def validate_gui_single_stock_refined_visual_contract_case(_base_params):
    ...
    check("single_stock_chart_uses_pure_black_bg", True, 'MATPLOTLIB_DARK_BG = "#000000"' in charting_source)
    check("single_stock_sidebar_signal_chip_uses_fixed_text", True, 'self._sidebar_signal_var = tk.StringVar(value=SIDEBAR_SIGNAL_CHIP_TEXT)' in inspector_source or 'self._sidebar_signal_var = tk.StringVar(value="出現買入訊號")' in inspector_source)
    check("single_stock_sidebar_gate_chip_uses_fixed_text", True, 'self._sidebar_history_var = tk.StringVar(value=SIDEBAR_HISTORY_CHIP_TEXT)' in inspector_source or 'self._sidebar_history_var = tk.StringVar(value="符合歷史績效")' in inspector_source)
    check("single_stock_gui_removes_chart_hint_footer_label", False, 'textvariable=self._chart_hint_var' in inspector_source)
...
def validate_gui_extended_preview_continuity_contract_case(base_params):
    ...
    check("single_stock_signal_chip_runtime_text_stays_fixed", True, 'self._sidebar_signal_var.set(SIDEBAR_SIGNAL_CHIP_TEXT)' in inspector_source and 'self._sidebar_signal_var.set(signal_text)' not in inspector_source)
    check("single_stock_history_chip_runtime_text_stays_fixed", True, 'self._sidebar_history_var.set(SIDEBAR_HISTORY_CHIP_TEXT)' in inspector_source and 'self._sidebar_history_var.set(history_text)' not in inspector_source)
    check("entry_preview_uses_candidate_layer_for_new_signals", True, 'preview_candidate_plan = build_normal_candidate_plan' in entry_flow_source)
    check("entry_preview_uses_candidate_layer_for_extended_days", True, 'preview_candidate_plan = build_extended_candidate_plan_from_signal' in entry_flow_source)
    check("entry_preview_extended_candidate_plan_stays_counterfactual_signature", True, 'build_extended_candidate_plan_from_signal(active_extended_signal, sizing_cap, params, ticker=ticker, security_profile=security_profile, trade_date=effective_trade_date)' in entry_flow_source)
...
def validate_gui_buy_signal_annotation_anchor_price_contract_case(base_params):
    ...
    backtest_source = build_project_absolute_path("services", "trade_analysis", "backtest.py").read_text(encoding="utf-8")
    entry_flow_source = build_project_absolute_path("services", "trade_analysis", "entry_flow.py").read_text(encoding="utf-8")
    check("buy_signal_annotation_anchor_keeps_signal_low_even_when_entry_plan_exists", True, "anchor_price = float(signal_low)" in backtest_source and "anchor_price = float(entry_plan['limit_price'])" not in backtest_source)
    check("buy_trade_marker_uses_fill_price", True, 'action="買進"' in entry_flow_source and "price=entry_result['buy_price']" in entry_flow_source)

    case = build_synthetic_competing_candidates_case(base_params, make_synthetic_validation_params)
    ticker = case["primary_ticker"]
    params = case["params"]
    raw_frame = case["frames"][ticker].copy()
    frame, _sanitize_stats = sanitize_ohlcv_dataframe(
        raw_frame,
        ticker,
    ...
    check("buy_signal_annotation_anchor_matches_signal_low", expected_signal_low, None if annotation is None else float(annotation.get("anchor_price")))
    check("buy_signal_annotation_meta_keeps_entry_price_preview", float(entry_plan["limit_price"]), None if annotation is None else float((annotation.get("meta") or {}).get("entry_price", np.nan)))
    ...
    check("buy_trade_marker_price_matches_fill_price", fill_price, None if trade_marker is None else float(trade_marker.get("price")))
    ...
    check("buy_signal_annotation_without_entry_plan_falls_back_to_signal_low", expected_fallback_signal_low, None if fallback_annotation is None else float(fallback_annotation.get("anchor_price")))
...
def validate_gui_trade_marker_and_tp_visual_contract_case(_base_params):
    ...
    check("indicator_sell_marker_uses_green_horizontal_line", True, '"指標賣出": {"plotly_symbol": "line-ew-open", "mpl_marker": "_", "color": MATPLOTLIB_INDICATOR_SELL_COLOR}' in charting_source)
    check("tp_visual_uses_yellow_color", True, 'MATPLOTLIB_TP_COLOR = "#facc15"' in charting_source)
    check("exit_trade_labels_include_trade_count", True, '"trade_sequence": {"label": "交易次數"' in charting_source and "'trade_count': _resolve_completed_trade_count(completed_trade_snapshot, include_current_round_trip=False)" in exit_flow_source)
    check("buy_signal_annotation_exports_limit_and_reserved_for_autoscale", True, "'limit_price': float(entry_plan['limit_price'])" in backtest_source and "'reserved_capital': float(reserved_capital)" in backtest_source and "'tp_price': float(tp_line)" not in backtest_source)
    ...
    check("buy_signal_limit_preview_participates_in_visible_price_range", True, float(visible_ranges.get("price_max", 0.0)) > 130.0)
...
def validate_gui_trade_box_capital_and_round_trip_contract_case(_base_params):
    ...
    check("debug_view_initial_capital_uses_scanner_live_capital_basis", True, 'cloned_params.initial_capital = resolve_scanner_live_capital(cloned_params)' in trade_log_source)
    check("buy_signal_annotation_includes_current_capital_and_reserved_capital", True, '資金:' in backtest_source and '預留:' in backtest_source)
    check("final_exit_marker_uses_completed_snapshot_after_same_day_exits", True, 'include_current_date_exits=True' in exit_flow_source)
    ...
    check("trade_info_boxes_use_actual_bbox_collision_layout", True, '_layout_chart_annotation_boxes' in charting_source and '_annotation_base_bbox' in charting_source and '_bbox_overlaps_with_padding' in charting_source and 'draw_event' in charting_source and 'annotation_layout_axis": "vertical_then_minimal_horizontal_fallback"' in charting_source)
    ...
    check("trade_info_boxes_treat_visible_candles_and_trade_lines_as_collision_obstacles", True, '_build_visible_candle_obstacle_bboxes' in charting_source and '_build_visible_trade_line_obstacle_bboxes' in charting_source and 'obstacle_bboxes=[*candle_obstacles, *trade_line_obstacles]' in charting_source and 'annotation_avoids_visible_candles": True' in charting_source and 'annotation_avoids_trade_price_lines": True' in charting_source)
    ...
    check("trade_info_boxes_deduplicate_fields_already_in_right_sidebar", True, 'trade_info_sidebar_deduplicated": True' in charting_source and '"買進": ("qty", "entry_type")' in charting_source and '"停損": ("qty", "sell_capital", "pnl", "pnl_pct")' in charting_source)
    ...
    check("tp_trade_label_keeps_sell_amount_and_leg_pnl_without_sidebar_entry_price", True, "成交:" not in tp_label_text and "金額: 32,100" in tp_label_text and "損益: +4,567" in tp_label_text)
    check("exit_trade_label_keeps_per_trade_pnl_without_sidebar_summary_metrics", True, "損益: +3,333" in exit_label_text and "最大回撤:" not in exit_label_text and "交易次數:" not in exit_label_text and "勝率:" not in exit_label_text)
    check("exit_trade_info_box_omits_result_row", False, any(line.startswith("結果:") for line in exit_label_text.split("\n")))
    check("exit_trade_label_omits_sidebar_duplicate_capital_and_keeps_no_total_pnl_label", True, "資金:" not in exit_label_text and "總損益:" not in exit_label_text and "風報比:" not in exit_label_text and "EV:" not in exit_label_text)
    ...
    build_trade_line_obstacles = getattr(charting_module, "_build_visible_trade_line_obstacle_bboxes")
    line_figure = Figure(figsize=(8, 5), dpi=96)
    line_canvas = FigureCanvasAgg(line_figure)
    line_axis = line_figure.add_subplot(111)
    line_axis.set_xlim(0, 10)
    line_axis.set_ylim(0, 10)
    line_payload = {
        "x": np.arange(11, dtype=float),
        "shadow_stop_line": np.full(11, np.nan),
        "shadow_tp_line": np.full(11, np.nan),
        "shadow_entry_line": np.full(11, np.nan),
        "shadow_limit_line": np.full(11, np.nan),
        "stop_line": np.full(11, 5.0),
        "tp_line": np.full(11, np.nan),
        "entry_line": np.full(11, np.nan),
        "limit_line": np.full(11, np.nan),
        "future_preview": {},
    }
    line_artist = line_axis.annotate(
        "停損\n股數: 1,000\n損益: -1,000",
        xy=(5, 5),
        xytext=(0, 0),
        textcoords="offset points",
        ha="center",
        va="center",
        bbox={"boxstyle": "round,pad=0.38", "fc": "#654321"},
    )
    tag_chart_annotation_layout(line_artist, placement="above", base_position=(0, 0), kind="trade")
    line_canvas.draw()
    line_renderer = line_canvas.get_renderer()
    line_obstacles = build_trade_line_obstacles(line_axis, line_payload)
    line_bbox_before = line_artist.get_window_extent(line_renderer)
    check("trade_info_box_test_fixture_initially_overlaps_trade_price_line", True, any(line_bbox_before.overlaps(obstacle) for obstacle in line_obstacles))
    line_changed = layout_chart_annotation_boxes(
        line_axis,
        [line_artist],
        line_renderer,
        obstacle_bboxes=line_obstacles,
    )
    line_canvas.draw()
    line_renderer = line_canvas.get_renderer()
    line_bbox_after = line_artist.get_window_extent(line_renderer)
    check("trade_info_boxes_move_to_reveal_trade_price_line", True, line_changed)
    check("trade_info_boxes_do_not_cover_trade_price_lines_after_layout", False, any(line_bbox_after.overlaps(obstacle) for obstacle in line_obstacles))
...
def validate_gui_trade_count_and_sidebar_sync_contract_case(base_params):
    ...
    exit_flow_source = build_project_absolute_path("services", "trade_analysis", "exit_flow.py").read_text(encoding="utf-8")
    marker_price_block = portfolio_inspector_source.split("def _resolve_marker_price", 1)[1].split("def _is_buy_trade_row", 1)[0]
    check("single_stock_sidebar_declares_trade_info_block", True, 'text="交易資訊"' in inspector_source and '_selected_tp_var' in inspector_source and '_selected_entry_var' in inspector_source and '_selected_stop_var' in inspector_source and '_selected_capital_var' in inspector_source and 'def _update_selected_value_sidebar' in workbench_source and 'set_workbench_capital_display_text(' in workbench_source)
    check("single_stock_sidebar_omits_buy_signal_info_block", True, 'text="買訊資訊"' not in inspector_source and '_selected_reserved_var' not in inspector_source and '_selected_signal_capital_var' not in inspector_source)
    check("chart_hover_snapshot_keeps_trade_line_values_for_sidebar", True, '"tp_price": float(chart_payload["tp_line"][idx])' in charting_source and '"limit_price": float(chart_payload["limit_line"][idx])' in charting_source and '"entry_price": float(chart_payload["entry_line"][idx])' in charting_source and '"stop_price": float(chart_payload["stop_line"][idx])' in charting_source)
    check("portfolio_sidebar_declares_reserved_and_actual_spend_fields", True, 'text="交易資訊"' in portfolio_inspector_source and '_selected_capital_var' in portfolio_inspector_source and 'set_workbench_capital_display_text(' in workbench_source and 'resolve_workbench_capital_display_mode_for_snapshot(snapshot)' in workbench_source)
    check("portfolio_history_summary_uses_runtime_params_and_fixed_risk_cache_key", True, 'params = result_payload.get("params")' in portfolio_inspector_source and 'loaded_params.fixed_risk = float(options["fixed_risk"])' in portfolio_inspector_source and 'fixed_risk = getattr(params, "fixed_risk", options.get("fixed_risk", None))' in portfolio_inspector_source and 'cache_key = (resolved_ticker, str(options.get("params_path") or "").strip(), None if fixed_risk is None else float(fixed_risk))' in portfolio_inspector_source)
    check("portfolio_history_summary_labels_are_compact", True, 'text="單股歷史績效表"' in portfolio_inspector_source and "口徑: 單股獨立回測，非投組實際成交" not in portfolio_inspector_source and "全資料區間" not in portfolio_inspector_source and 'f"固定風險: {fixed_risk_text}"' not in portfolio_inspector_source)
    check("portfolio_sidebar_width_increases_by_one_third", True, "PORTFOLIO_RIGHT_SIDEBAR_WIDTH_SCALE = 4 / 3" in portfolio_inspector_source and "width=PORTFOLIO_RIGHT_SIDEBAR_WIDTH" in portfolio_inspector_source and "minsize=PORTFOLIO_RIGHT_SIDEBAR_WIDTH" in portfolio_inspector_source and "wraplength=PORTFOLIO_RIGHT_SIDEBAR_WRAPLENGTH" in portfolio_inspector_source)
    check("single_sidebar_width_matches_portfolio_expanded_width", True, "SINGLE_STOCK_RIGHT_SIDEBAR_WIDTH_SCALE = 4 / 3" in inspector_source and "width=SINGLE_STOCK_RIGHT_SIDEBAR_WIDTH" in inspector_source and "minsize=SINGLE_STOCK_RIGHT_SIDEBAR_WIDTH" in inspector_source and "wraplength=SINGLE_STOCK_RIGHT_SIDEBAR_WRAPLENGTH" in inspector_source)
    check("single_and_portfolio_trade_nav_has_no_gap_after_latest_button",
        True,
    ...
    check("portfolio_marker_price_requires_recorded_execution_or_missed_reference_price", True, "return np.nan" in marker_price_block and '"Low"' not in marker_price_block and '"High"' not in marker_price_block and '"Close"' not in marker_price_block and '"買入限價"' in marker_price_block and '"停損價", "參考收盤價"' in marker_price_block)
    check("portfolio_active_level_rows_are_engine_output_not_gui_replay", True, "execute_bar_step" not in portfolio_inspector_source and "portfolio_active_level_rows" in portfolio_engine_source and "_record_portfolio_active_level_rows" in portfolio_inspector_source)
    check("single_stock_exit_day_active_levels_are_recorded", True, "_record_exit_day_active_levels(" in exit_flow_source and "fallback_qty=prev_qty" in exit_flow_source and "stop_price=_resolve_exit_day_stop_line" in exit_flow_source)
    check("portfolio_exit_day_active_level_rows_are_engine_output", True, "active_level_rows=active_level_rows" in portfolio_engine_source and "append_portfolio_position_active_level_row(" in portfolio_exits_source and "require_open_qty=False" in portfolio_exits_source)
    check("portfolio_reserved_fill_rate_uses_filled_buy_count", True, "entry_stats['filled_buy_count']" in portfolio_entries_source and "filled_buy_count / (filled_buy_count + total_missed_buys)" in portfolio_engine_source and "profile_stats['filled_buy_count']" in portfolio_engine_source)
    ...
    # AI: The canonical UI contract is ticker | official name | source; ranking statistics
    ...
    check("final_exit_trade_count_uses_completed_round_trip_index", expected_trade_count, forced_close_meta.get("trade_count"))
    check("final_exit_marker_action_is_forced_close", "強制結算", forced_close_marker.get("trace_name"))
...
def validate_gui_chart_margin_and_latest_extended_preview_contract_case(base_params):
    ...
    check("single_stock_chart_tightens_left_bottom_margins_for_more_candle_space", True, 'MATPLOTLIB_SUBPLOT_LEFT = 0.046' in charting_source and 'MATPLOTLIB_SUBPLOT_BOTTOM = 0.058' in charting_source and 'axis_price.tick_params(axis="y", colors=MATPLOTLIB_TEXT_COLOR, labelsize=11, pad=6)' in charting_source)
    check("single_stock_legend_keeps_small_inset_gap_from_top_left_boundary", True, 'bbox_to_anchor=(0.012, 1.012)' in charting_source and 'MATPLOTLIB_SUBPLOT_TOP = 0.986' in charting_source)
    check("single_stock_latest_extended_candidate_uses_future_preview_helper", True, 'build_extended_candidate_plan_from_signal(' in backtest_source and 'latest_extended_preview = build_extended_candidate_plan_from_signal(' in backtest_source and 'security_profile=resolved_security_profile' in backtest_source and 'trade_date=dates[-1]' in backtest_source and '_apply_chart_future_preview_from_plan(chart_context, latest_extended_preview)' in backtest_source)
    ...
    check("extended_preview_shadow_limit_reaches_latest_actual_bar", True, bool(shadow_limit_line.size > 6 and np.isfinite(shadow_limit_line[6])))
    check("extended_preview_shadow_stop_uses_shadow_state_at_latest_actual_bar", True, bool(shadow_stop_line.size > 6 and np.isfinite(shadow_stop_line[6])))
    check("extended_preview_shadow_tp_uses_shadow_state_at_latest_actual_bar", True, bool(shadow_tp_line.size > 6 and np.isfinite(shadow_tp_line[6])))
    check("latest_extended_candidate_future_preview_limit_exists", True, bool(np.isfinite(float(future_preview.get("limit_price", np.nan)))))
    check("latest_extended_candidate_future_preview_stop_uses_shadow_state", True, bool(np.isfinite(float(future_preview.get("stop_price", np.nan)))))
    check("latest_extended_candidate_future_preview_tp_uses_shadow_state", True, bool(np.isfinite(float(future_preview.get("tp_half_price", np.nan)))))
...
def validate_gui_latest_raw_signal_preview_helper_contract_case(_base_params):
    ...
    check("latest_raw_signal_preview_uses_normal_entry_plan_helper", True, "build_normal_entry_plan" in backtest_source and "entry_plan_preview = build_normal_entry_plan(" in backtest_source and "security_profile=resolved_security_profile" in backtest_source)
    check("latest_raw_signal_preview_helper_is_imported_from_core_entry_plans", True, "build_normal_entry_plan" in referenced_names and "build_normal_entry_plan" in imported_entry_plan_symbols)
    check("latest_tail_preview_candidate_helper_is_imported_from_core_entry_plans", True, "build_normal_candidate_plan" in referenced_names and "build_normal_candidate_plan" in imported_entry_plan_symbols)
    check("latest_tail_preview_threads_security_profile", True, "latest_entry_plan_preview = build_normal_candidate_plan(" in backtest_source and "security_profile=resolved_security_profile" in backtest_source)
    return results, summary
...
def validate_gui_chart_overlay_layout_and_pan_contract_case(_base_params):
    ...
    run_bound_checks(
        check,
        (
            ('gui_panel_uses_explicit_workbench_dark_styles', True, 'style="Workbench.TNotebook"' in inspector_source and 'style="Workbench.TFrame"' in inspector_source and ('style="Workbench.TLabel"' in inspector_source),),
            ('gui_workbench_uses_palette_force_for_dark_theme', True, 'root.tk_setPalette(' in workbench_source,),
            ('chart_mouse_pan_uses_pixel_anchor', True, 'anchor_px' in charting_source and 'bars_per_pixel = (float(origin_right) - float(origin_left)) / axis_width_px' in charting_source and ('interaction_flags["dragging"]' in charting_source),),
        ),
    )
    ...
    check("figure_contract_status_chip_layout_right_bottom", "right_bottom", contract.get("status_chip_layout"))
    check("figure_contract_buy_trade_label_boxes_enabled", True, contract.get("buy_trade_label_boxes_enabled"))
    check("figure_contract_mouse_drag_pan_mode_pixel_anchor", "pixel_anchor", contract.get("mouse_drag_pan_mode"))
    check("figure_contract_grid_alpha_subtle", True, float(contract.get("grid_alpha", 1.0)) <= 0.40)
    check("figure_trade_labels_include_buy_and_sell_markers", 2, len(state.get("trade_label_artists") or []))
    figure.clear()
    return results, summary
...
def validate_gui_chart_recent_view_signal_overlay_contract_case(base_params):
    ...
    inspector_source = build_project_absolute_path("services", "workbench_ui", "single_stock_inspector.py").read_text(encoding="utf-8")
    check("gui_inline_chart_skips_html_export_by_default", True, "export_chart=False" in inspector_source and "return_chart_payload=True" in inspector_source)
    ...
    check("gui_chart_default_recent_view_ends_at_last_bar", len(chart_payload["x"]) - 1, int(default_view.get("end_idx", -1)))
    check("gui_chart_default_recent_view_narrower_than_full_history", True, int(default_view.get("start_idx", 0)) > 0)
    check("gui_chart_contract_default_lookback_months", 6, contract.get("default_lookback_months"))
    check("gui_chart_contract_hover_value_display_enabled", True, contract.get("hover_value_display_enabled"))
    check("gui_chart_contract_twse_up_color", "#ff5b6e", contract.get("twse_up_color"))
    check("gui_chart_contract_twse_down_color", "#18b26b", contract.get("twse_down_color"))
    check("gui_chart_contract_summary_box_present", True, contract.get("summary_box_present"))
    check("gui_chart_contract_status_box_present", True, contract.get("status_box_present"))
    check("gui_chart_contract_signal_annotation_present", True, contract.get("signal_annotation_count", 0) >= 1)
    figure.clear()
    ...
    check("gui_chart_buy_signal_annotation_precedes_execution_by_one_bar", signal_dates[1].strftime("%Y-%m-%d"), buy_signal_dates[0] if buy_signal_dates else "")
    check("gui_chart_buy_execution_occurs_on_next_bar", signal_dates[2].strftime("%Y-%m-%d"), buy_trade_dates[0] if buy_trade_dates else "")
    check("gui_chart_sell_signal_annotation_precedes_execution_by_one_bar", signal_dates[3].strftime("%Y-%m-%d"), sell_signal_dates[0] if sell_signal_dates else "")
    check("gui_chart_sell_execution_occurs_on_next_bar", signal_dates[4].strftime("%Y-%m-%d"), sell_trade_dates[0] if sell_trade_dates else "")
...
def validate_gui_workbench_contract_case(base_params):
    ...
    check("gui_app_thin_entry_main", apps_gui.main, tools_gui.main)
    check("gui_workbench_entry_module", "apps.workbench", workbench_spec.get("entry_module"))
    check("gui_workbench_title", "股票工具工作台", workbench_spec.get("title"))
    check("gui_workbench_startup_window_mode", "maximized", workbench_spec.get("startup_window_mode"))
    ...
    if panel_spec:
        ...
        check("gui_workbench_inline_chart_backend", "services.trade_analysis.charting.create_matplotlib_trade_chart_figure", panel_spec.get("inline_chart_backend"))
    ...
    run_bound_checks(
        ...
        (
            ...
            ('gui_data_ops_panel_uses_workbench_notebook_style', True, 'ttk.Notebook(self, style=WORKBENCH_NOTEBOOK_STYLE)' in data_ops_panel_source,),
            ('gui_data_ops_panel_uses_compact_dataset_dates_and_expected_publish', True, 'latest_data_date' in data_ops_panel_source and 'expected_publish_at' in data_ops_panel_source and 'next_check_at' in data_ops_panel_source,),
            ('gui_data_ops_panel_uses_single_tree_per_table_without_sync_resize_chain', True, '_bind_synced_vertical_scroll' not in data_ops_panel_source and '_schedule_trees' not in data_ops_panel_source,),
            ('gui_data_ops_panel_tables_stretch_and_keep_horizontal_overflow_safe', True, 'stretch=True' in data_ops_panel_source and 'xscrollcommand=' in data_ops_panel_source,),
            ('gui_data_ops_panel_headers_support_toggle_sorting', True, '_register_sortable_tree' in data_ops_panel_source and '_sort_tree_by_column' in data_ops_panel_source and '_apply_tree_sort' in data_ops_panel_source,),
            ('gui_data_ops_panel_defaults_expected_publish_ascending', True, 'default_column="expected_publish"' in data_ops_panel_source and 'default_column="publish"' in data_ops_panel_source and 'default_ascending=True' in data_ops_panel_source,),
            ('gui_data_ops_panel_sorting_uses_raw_values_and_missing_last', True, '_set_tree_sort_values' in data_ops_panel_source and 'present.append' in data_ops_panel_source and 'missing.append' in data_ops_panel_source,),
            ('gui_data_ops_panel_uses_true_per_cell_status_color_overlay', True, '_TreeCellColorOverlay' in data_ops_panel_source and 'foreground=color' in data_ops_panel_source and '_cell_overlays' in data_ops_panel_source,),
            ('gui_data_ops_panel_does_not_depend_on_color_emoji_status_markers', True, all(marker not in data_ops_panel_source for marker in ('🟢', '🟡', '🔴', '⚪', '🔵')),),
            ('gui_data_ops_panel_kpi_details_are_single_line', True, '_kpi_detail_vars["trading"].set(f"target {target} | valid scan {valid_scan_date} | {strategy_id}")' in data_ops_panel_source and 'm | Task: {scheduler_status} | next {scheduler_next}' in data_ops_panel_source,),
            ('gui_data_ops_panel_progress_aligns_with_kpi_column_ranges', True, 'summary_grid = ttk.Frame' in data_ops_panel_source and 'box.grid(row=0, column=col' in data_ops_panel_source and 'trading_readiness_box.grid(row=1, column=0, columnspan=1' in data_ops_panel_source and 'readiness_box.grid(row=1, column=1, columnspan=3' in data_ops_panel_source and 'quota_box.grid(row=1, column=4, columnspan=2' in data_ops_panel_source,),
            ('gui_data_ops_panel_trading_progress_uses_required_v2_ssot', True, 'ready_v2 / required_v2' in data_ops_panel_source and 'required V2 {ready_v2}/{required_v2}' in data_ops_panel_source,),
            ('gui_data_ops_panel_delegates_scheduler_mutation_to_service', True, 'install_or_update_market_data_scheduler' in data_ops_panel_source and 'set_market_data_scheduler_enabled' in data_ops_panel_source and ('remove_market_data_scheduler' in data_ops_panel_source),),
            ('gui_data_ops_panel_has_no_raw_scheduler_subprocess', True, 'subprocess' not in data_ops_panel_source and 'schtasks' not in data_ops_panel_source and ('Register-ScheduledTask' not in data_ops_panel_source),),
            ('gui_data_ops_panel_does_not_write_raw_state', True, 'atomic_write_json' not in data_ops_panel_source and 'load_json_strict' not in data_ops_panel_source,),
            ('gui_data_ops_panel_uses_market_data_timezone_ssot', True, 'format_datetime_in_timezone' in data_ops_panel_source and 'MARKET_DATA_V2_PUBLICATION_POLICY' in data_ops_panel_source and '.astimezone()' not in data_ops_panel_source,),
            ('gui_single_stock_open_error_uses_project_relative_display_path', True, 'project_relative_display_path' in inspector_source and '找不到檔案或資料夾：{normalized_path}' not in inspector_source,),
            ('gui_portfolio_param_metadata_uses_strict_json_loader', True, 'load_json_strict' in portfolio_inspector_source and 'json.load(' not in portfolio_inspector_source,),
            ('gui_headless_coverage_omits_runtime_ui_files', True, 'HEADLESS_COVERAGE_OMIT_PATTERNS' in meta_quality_coverage_source and 'services" / "workbench_ui" / "*.py"' in meta_quality_coverage_source,),
            ('gui_validate_coverage_omits_runtime_ui_files', True, 'HEADLESS_COVERAGE_OMIT_PATTERNS' in validate_main_source and 'workbench_ui' in validate_main_source and ('omit=HEADLESS_COVERAGE_OMIT_PATTERNS' in validate_main_source),),
            ('coverage_omits_torch_generated_remote_module_sources', True, 'PROJECT_ROOT / "_remote_module_*"' in meta_quality_coverage_source and 'os.path.join(PROJECT_ROOT, "_remote_module_*")' in validate_main_source,),
            ('gui_trade_navigation_forces_immediate_redraw', True, 'figure.canvas.draw()' in charting_source and 'figure.canvas.flush_events()' in charting_source,),
            ('gui_trade_navigation_uses_compact_center_window', True, 'window_width = max(' in charting_source and 'min(current_width' in charting_source,),
            ('gui_single_trade_navigation_uses_buy_markers', True, 'trace_names=BUY_TRADE_TRACE_NAMES' in inspector_source,),
            ('gui_portfolio_trade_navigation_uses_buy_markers', True, 'trace_names=BUY_TRADE_TRACE_NAMES' in portfolio_inspector_source,),
        ),
    )
    ...
    check("gui_portfolio_performance_tab_close_button_is_positioned", True, "button.place(" in portfolio_tab_close_block and "button.lift()" in portfolio_tab_close_block)
    check("gui_portfolio_performance_tab_close_button_is_real_button", True, "tk.Button(" in close_button_factory_block and "command=lambda" in close_button_factory_block)
    check("gui_portfolio_performance_tab_close_button_is_chart_overlay", True, "close_host = record.get(\"close_host\")" in close_button_factory_block and "PERFORMANCE_CHART_CLOSE_BUTTON_SIZE_PX" in portfolio_tab_close_block and "PERFORMANCE_CHART_CLOSE_BUTTON_MARGIN_PX" in portfolio_tab_close_block)
    check("gui_portfolio_performance_tab_close_refresh_preserves_live_buttons", True, "if str(tab_id) in live_tab_ids:" in portfolio_tab_close_block and "continue" in portfolio_tab_close_block)
    check("gui_portfolio_performance_tab_title_omits_legacy_x", False, "end_year_label}  ×" in portfolio_inspector_source)
    check("gui_portfolio_performance_chart_header_is_compact", True, "PERFORMANCE_CHART_TITLE_FONT_SIZE = 12" in portfolio_inspector_source and "PERFORMANCE_CHART_TITLE_PAD_PX = 6" in portfolio_inspector_source and "PERFORMANCE_CHART_SUBPLOT_TOP = 0.945" in portfolio_inspector_source)
    check("gui_portfolio_performance_hover_crosshair_enabled", True, "def _bind_performance_chart_hover(" in portfolio_inspector_source and 'canvas.mpl_connect("motion_notify_event", _on_motion)' in portfolio_inspector_source)
    check("gui_portfolio_performance_hover_has_time_axis_and_return_lines", True, "mouse_time_axis_enabled" in portfolio_inspector_source and "strategy_return_line_enabled" in portfolio_inspector_source and "benchmark_return_line_enabled" in portfolio_inspector_source and "axis.axvline(" in portfolio_inspector_source and "axis.axhline(" in portfolio_inspector_source)
    check("gui_portfolio_performance_hover_uses_strategy_and_benchmark_values", True, "strategy_return_values" in portfolio_inspector_source and "benchmark_return_values" in portfolio_inspector_source and "V16：" in portfolio_inspector_source and "benchmark_label" in portfolio_inspector_source)
    check("gui_portfolio_performance_hover_disconnects_on_close", True, "hover_connection_ids" in portfolio_inspector_source and "canvas.mpl_disconnect(connection_id)" in portfolio_inspector_source)
    check("gui_portfolio_performance_tab_header_click_close_unbound", False, 'notebook.bind("<Button-1>", self._on_performance_tab_click)' in portfolio_inspector_source or 'notebook.bind("<ButtonRelease-1>", self._on_performance_tab_click)' in portfolio_inspector_source)
    check("gui_single_trade_indexes_survive_chart_clear", True, "trade_indexes = self._resolve_chart_navigation_indexes(chart_payload)" in inspector_source and "self._current_chart_trade_indexes = trade_indexes" in inspector_source)
    check("gui_portfolio_trade_indexes_survive_chart_clear", True, "trade_indexes = self._resolve_kline_navigation_indexes(chart_payload)" in portfolio_inspector_source and "self._current_chart_trade_indexes = trade_indexes" in portfolio_inspector_source)
    check("inspector_prefers_canonical_trade_analysis_runner", True, "run_ticker_analysis" in inspector_source and "run_debug_ticker_analysis" not in inspector_source)
    check("inspector_prefers_canonical_trade_analysis_data_dir_helper", True, "resolve_trade_analysis_data_dir" in inspector_source and "resolve_debug_data_dir" not in inspector_source)
    check("inspector_prefers_canonical_trade_chart_figure_alias", True, "create_matplotlib_trade_chart_figure" in inspector_source and "create_matplotlib_debug_chart_figure" not in inspector_source)
    check("workbench_panel_registry_omits_legacy_debug_runner_alias", False, "run_debug_ticker_analysis" in workbench_source)
    check("workbench_panel_registry_omits_legacy_debug_chart_alias", False, "create_matplotlib_debug_chart_figure" in workbench_source)
    ...
    shared_prepare_console_block = workbench_source.split("    def _prepare_console_for_new_task(", 1)[1].split("    def _clear_console", 1)[0]
    run_bound_checks(
        check,
        (
            ('gui_single_new_scanner_task_preserves_console_history', True, '_prepare_console_for_new_task()' in single_start_scanner_block and '_clear_console()' not in single_start_scanner_block,),
            ('gui_single_prepare_console_task_does_not_delete_history', False, '.delete("1.0", "end")' in shared_prepare_console_block,),
            ('gui_portfolio_new_backtest_task_preserves_console_history', True, '_prepare_console_for_new_task()' in portfolio_run_backtest_block and '_clear_console()' not in portfolio_run_backtest_block,),
            ('gui_portfolio_prepare_console_task_does_not_delete_history', False, '.delete("1.0", "end")' in shared_prepare_console_block,),
            ('gui_single_scanner_console_preserves_scanner_ansi_colors', True, 'SCANNER_CONSOLE_COLORS' in inspector_source and 'WorkbenchInspectorSharedMixin' in inspector_source and ('def _insert_ansi_text' in workbench_source) and ('self._console_colors' in workbench_source) and ('ANSI_ESCAPE_SEQUENCE_PATTERN' not in inspector_source),),
            ('gui_portfolio_has_end_year_dropdown', True, 'END_YEAR_LATEST_LABEL' in portfolio_inspector_source and 'text="結束年"' in portfolio_inspector_source and ('_end_year_combo' in portfolio_inspector_source),),
            ('gui_portfolio_passes_end_year_to_backend', True, 'end_year=options["end_year"]' in portfolio_inspector_source and 'export_portfolio_reports(df_eq, df_tr, df_yearly, options["benchmark_ticker"], options["start_year"], end_year=options["end_year"])' in portfolio_inspector_source,),
            ('gui_portfolio_ui_years_override_optimizer_artifact_range', True, 'start_year = ui_start_year' in portfolio_inspector_source and 'end_year = ui_end_year' in portfolio_inspector_source and ('optimizer_aligned = False' in portfolio_inspector_source),),
            ('gui_portfolio_does_not_force_optimizer_replay_date_range', True, 'replay_start_date = str(artifact_meta.get("replay_start_date") or "") if use_optimizer_range else ""' not in portfolio_inspector_source and 'start_date=options.get("replay_start_date") or representative_date' not in portfolio_inspector_source and ('end_date=options.get("replay_end_date") or rolling_last_date' not in portfolio_inspector_source) and ('end_date=options.get("replay_end_date") or ensemble_last_date' not in portfolio_inspector_source),),
            ('portfolio_runner_filters_by_optional_end_year', True, 'def _filter_market_dates_by_end_year' in portfolio_runner_source and 'end_year=None' in portfolio_runner_source and ('resolved_sorted_dates' in portfolio_runner_source),),
        ),
    )
    ...
    check("gui_help_exit_code", 0, exit_code)
    check("gui_help_mentions_entry", True, "apps/workbench.py" in help_output)
    ...
    debug_module = importlib.import_module("services.trade_analysis.trade_log")
    charting_module = importlib.import_module("services.trade_analysis.charting")
    check("trade_log_exposes_canonical_trade_analysis_data_dir_helper", True, hasattr(debug_module, "resolve_trade_analysis_data_dir"))
    check("charting_exposes_canonical_trade_chart_alias", True, hasattr(charting_module, "create_matplotlib_trade_chart_figure"))
    check("charting_exposes_index_navigation_helper", True, hasattr(charting_module, "scroll_chart_to_index"))
    check("charting_exposes_adjacent_trade_navigation_helper", True, hasattr(charting_module, "scroll_chart_to_adjacent_trade"))
    check("charting_adjacent_trade_prev_from_latest", 12, charting_module.resolve_adjacent_trade_index(19, [4, 12], direction=-1))
    check("charting_adjacent_trade_next_wraps_from_latest", 4, charting_module.resolve_adjacent_trade_index(19, [4, 12], direction=1))
    check("charting_adjacent_trade_next_from_first", 12, charting_module.resolve_adjacent_trade_index(4, [4, 12], direction=1))
    check("charting_adjacent_trade_empty_returns_none", None, charting_module.resolve_adjacent_trade_index(4, [], direction=1))
    ...
    with tempfile.TemporaryDirectory(prefix="gui_workbench_contract_") as temp_dir:
        ...
        check("gui_backend_trade_logs_payload", _normalize_nan_records(legacy_df), _normalize_nan_records(analysis_result["trade_logs_df"]))
        check("gui_backend_chart_artifact_omitted", None, analysis_result.get("chart_path"))
        check("gui_backend_excel_artifact_exists", True, excel_exists)
        check("gui_backend_chart_payload_exists", True, analysis_result.get("chart_payload") is not None)
        check("gui_backend_excel_artifact_name", f"Debug_TradeLog_{ticker}.xlsx", os.path.basename(analysis_result["excel_path"]))
...
def validate_dataset_prepare_fallback_write_traceability_case(_base_params):
    ...
    check("fallback_summary_preserves_primary_write_error", "ValueError: primary summary write failed", fallback_summary.get("summary_write_error"))
    check("fallback_summary_records_json_write_failure", True, any(str(item).startswith("json:OSError: json fallback disk full") for item in fallback_write_errors))
    check("fallback_summary_records_text_write_failure", True, any(str(item).startswith("txt:OSError: text fallback permission denied") for item in fallback_write_errors))
    check("fallback_summary_emits_stderr_when_no_fallback_file_written", True, "dataset prepare fallback summary write failed" in stderr_text and "json fallback disk full" in stderr_text and "text fallback permission denied" in stderr_text)
