...
def validate_gui_trade_count_and_sidebar_sync_contract_case(base_params):
    ...
    add_check(results, "output_contract", case_id, "portfolio_history_summary_labels_are_compact", True, 'text="單股歷史績效"' in portfolio_inspector_source and "口徑: 單股獨立回測，非投組實際成交" not in portfolio_inspector_source and "全資料區間" not in portfolio_inspector_source and 'f"固定風險: {fixed_risk_text}"' not in portfolio_inspector_source)
    ...
    add_check(results, "output_contract", case_id, "portfolio_single_ticker_summary_is_compact", True, "交易次數 {exit_count} 次" in portfolio_inspector_source and "勝率 {win_rate_text}" in portfolio_inspector_source and "錯失買進 {missed_buy_count} / 錯失賣出 {missed_sell_count}" in portfolio_inspector_source and "總損益 {total_pnl:+,.0f}" in portfolio_inspector_source and "投報 {invested_return_text}" in portfolio_inspector_source and "累計預留" not in portfolio_inspector_source and "累計實支" not in portfolio_inspector_source and "買進成交" not in portfolio_inspector_source)
