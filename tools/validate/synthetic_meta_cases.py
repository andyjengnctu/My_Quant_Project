...
def validate_architecture_helper_module_file_tree_sync_case(_base_params):
    case_id = "META_ARCHITECTURE_HELPER_MODULE_FILE_TREE_SYNC"
    results = []
    summary = {"ticker": case_id, "synthetic": True}
    ...
    required_paths = [
        ...
        "tools/trade_analysis/history_snapshot.py",
        "tools/validate/meta_contracts.py",
        "tools/validate/synthetic_display_cases.py",
        "tools/validate/synthetic_reporting_cases.py",
    ]

    for idx, fragment in enumerate(required_tree_fragments, start=1):
        add_check(results, "meta_architecture_contract", case_id, f"architecture_file_tree_lists_required_helper_module_{idx}", True, fragment in architecture_text)
    for path_str in required_paths:
        add_check(results, "meta_architecture_contract", case_id, f"repo_ships_{path_str.replace('/', '_').replace('.', '_')}", True, (PROJECT_ROOT / path_str).exists())

    summary["required_tree_fragments"] = required_tree_fragments
    summary["source_paths"] = ["doc/ARCHITECTURE.md", *required_paths]
    return results, summary


def validate_architecture_support_module_file_tree_sync_case(_base_params):
    case_id = "META_ARCHITECTURE_SUPPORT_MODULE_FILE_TREE_SYNC"
    results = []
    summary = {"ticker": case_id, "synthetic": True}
    ...
    required_paths = [
        "core/output_retention.py",
        "tools/optimizer/runtime.py",
        "tools/optimizer/session.py",
        ...
        "tools/validate/synthetic_portfolio_common.py",
        "tools/validate/synthetic_frame_utils.py",
        "tools/validate/synthetic_case_builders.py",
    ]

    for idx, fragment in enumerate(required_tree_fragments, start=1):
        add_check(results, "meta_architecture_contract", case_id, f"architecture_file_tree_lists_required_support_module_{idx}", True, fragment in architecture_text)
    for path_str in required_paths:
        add_check(results, "meta_architecture_contract", case_id, f"repo_ships_{path_str.replace('/', '_').replace('.', '_')}", True, (PROJECT_ROOT / path_str).exists())
...
def validate_test_suite_help_text_has_no_stale_wording_or_bare_term_case(_base_params):
    ...
    add_check(results, "meta_contract", case_id, "test_suite_help_text_has_no_stale_exact_contract_enumeration", False, "single-backtest public profit/equity consistency contract" in help_line or "治理原則 exhaustive-check / checklist-sort-guard contract" in help_line or "exhaustive-check / checklist-sort-guard contract" in help_line)
