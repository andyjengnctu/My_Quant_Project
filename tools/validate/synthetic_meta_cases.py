...
def validate_price_utils_array_tick_normalization_contract_case(_base_params):
    ...
    add_check(results, "meta_contract", case_id, "scanner_response_threads_latest_trade_date", True, 'trade_date = None if df.empty else df["Date"].iloc[-1]' in scanner_processor_source and 'build_scanner_response_from_stats(ticker=ticker, stats=stats, params=params, sanitize_stats=sanitize_stats, trade_date=trade_date)' in scanner_processor_source)
