...
def validate_market_data_v2_trading_workbench_sidecar_contract_case(_base_params):
    ...
    check("smart_downloader_displays_dynamic_request_window_policy_source", True, "Window start policy" in downloader_source and "request_date_start_sources" in auto_update_source)
...
def validate_market_data_v2_trading_direct_consumer_cutover_contract_case(_base_params):
    ...
    with TemporaryDirectory(prefix="round8_v2_consumer_") as temp_dir:
        ...
        pool_stats = dict(first["current_execution_pool_stats"])
        check("consumer_state_pool_funnel_broad_reference_count", 3, pool_stats["stockinfo_broad_reference_count"])
        check("consumer_state_pool_funnel_pit_member_count", 3, pool_stats["listed_count"])
        check("consumer_state_pool_funnel_exact_price_count", 3, pool_stats["listed_with_exact_price_count"])
