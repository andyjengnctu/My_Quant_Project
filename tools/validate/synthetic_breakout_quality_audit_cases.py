...
def validate_breakout_quality_audit_framework_contract_case(_base_params):
    ...
    add_check(
        ...
        case_id,
        "formal_audit_config_has_no_completed_one_shot_gate",
        ("breakout_quality", 0, 0, "outputs/audit"),
        (
            get_audit_module_ids()[0],
            len(definitions),
            len(enabled),
            AUDIT_OUTPUT_ROOT,
        ),
    ...
    add_check(
        ...
        "synthetic_breakout_quality",
        case_id,
        "audit_runner_reports_disabled_when_no_formal_audit_is_active",
        ("DISABLED", 0),
        (status.get("overall_status"), len(status.get("rows") or [])),
    )
    ...
    add_check(
        ...
        "synthetic_breakout_quality",
        case_id,
        "research_audit_utilities_remain_catalogued_without_formal_gate",
        True,
        bool("regime-audit" in commands and "audit-point-in-time-scores" in commands),
    )

    summary["workflow"] = "minimal_formal_audit_topology"
