"""Four-parameter-set by two-DL-state risk adaptation research gate.
...
from tools.filters.breakout_quality.strategy_adapt import _build_base_policy, _outer_rolling_argv
...
def _runtime_contract(*, root, args, baseline_contract, fold_overrides, runtime_artifact, arm_id, training_dl_enabled, binary_pit):
    payload = {
...
def _run_optimizer_arm(*, root, args, settings, baseline_contract, runtime_artifact, output_dir, arm_id, training_dl_enabled, binary_pit):
...
def run_param_adaptation_gate(*, project_root=PROJECT_ROOT, argv=None):
    ...
    runtime_artifact = load_runtime_artifact_contract(str(root), str(args.filter_id), str(args.model_architecture), str(args.experiment_profile))
