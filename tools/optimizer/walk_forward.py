...
def build_oos_total_performance(rows: list[dict], *, ret_key: str) -> dict:
    ...
    if len(ordered_rows) == 1:
        ...
        annual_return_key = 'annual_return_pct' if ret_key == 'ret_pct' else 'benchmark_annual_return_pct'
        mdd_key = 'mdd' if ret_key == 'ret_pct' else 'benchmark_mdd'
        return {
            'window_count': 1,
            'linked_total_return_pct': float(ret_pct),
            'annualized_return_pct': float(_safe_float(row.get(annual_return_key, 0.0))),
            'max_drawdown_pct': float(_safe_float(row.get(mdd_key, 0.0))),
            'positive_window_rate': 100.0 if ret_pct > 0.0 else 0.0,
            'ending_equity_factor': float(1.0 + (ret_pct / 100.0)),
