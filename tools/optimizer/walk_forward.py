...
def build_upgrade_gate_assessment(*, summary: dict) -> dict:
    ...
    recommendation = '已產生單一連續 OOS 績效，可用於固定區間驗證。' if quality_pass else '無法產生單一連續 OOS 績效，暫不可驗證。'
    return {
        'status': status,
        'oos_ready': bool(quality_pass),
...
def build_test_period_metrics(report: dict | None) -> dict:
    ...
    return {
        ...
        'breakout_trades': int(period.get('breakout_trades', period.get('normal_trades', 0)) or 0),
        'reentry_trades': int(period.get('reentry_trades', 0) or 0),
