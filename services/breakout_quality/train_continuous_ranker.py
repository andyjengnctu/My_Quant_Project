...
def _combine_pairwise_head_supervision(
    torch,
    head_logits,
    head_targets,
    batch_dates,
    *,
    ...
    logits = tuple(head_logits)
    targets = tuple(head_targets)
    policies = tuple(pair_weight_policies)
    if not (len(logits) == len(targets) == len(policies)):
        raise ValueError("pairwise head supervision component count不一致")
    losses_and_counts = []
    for head_logit, head_target, weight_policy in zip(logits, targets, policies):
        margin = head_logit.float()[:, LABEL_PASS] - head_logit.float()[:, LABEL_REJECT]
        losses_and_counts.append(
...
def _train_epoch(
    ...
    *,
        ...
        with autocast_context(torch, plan):
            if loss_handler == CONTINUOUS_RANKER_LOSS_HANDLER_SAFETY_MFE_JOINT_TRI_PAIRWISE:
                if target.ndim != 2 or int(target.shape[1]) != 3:
                    raise ValueError("Safety/Raw-MFE/Joint tri-head target必須為[N,3]")
                if not hasattr(model, "forward_safety_raw_mfe_hmhs_heads"):
                    raise ValueError("Safety/Raw-MFE/Joint objective需要tri-head model architecture")
                batch_dates = group_dates_series.iloc[ids].to_numpy()
                loss, loss_weight = _combine_pairwise_head_supervision(
                    torch,
                    model.forward_safety_raw_mfe_hmhs_heads(xb, cb),
                    tuple(target[:, index] for index in range(3)),
                    batch_dates,
                    training_policy=training_policy,
                    pairwise_reduction=str(pairwise_reduction),
                    pair_weight_policies=(str(pair_weight_policy),) * 3,
                )
                if loss is None:
                    continue
                ...
                batch_dates = group_dates_series.iloc[ids].to_numpy()
                # A0/AH weights use same-date average-rank percentiles of predicted
                # Safety, not raw probability. A1 changes only the Safety model source.
                safety_percentile = _same_date_average_rank_percentile(
                    torch, torch.softmax(safety_logits.float(), dim=1)[:, LABEL_PASS].detach(), batch_dates
                )
                loss, loss_weight = _combine_pairwise_head_supervision(
                    torch,
                    (safety_logits, raw_mfe_logits),
                    (target[:, 0], torch.stack([target[:, 1], safety_percentile], dim=1)),
                    batch_dates,
                    training_policy=training_policy,
                    pairwise_reduction=str(pairwise_reduction),
                    pair_weight_policies=(
                ...
                loss, loss_weight = _combine_pairwise_head_supervision(
                    torch,
                    model.forward_safety_conditional_mfe_heads(xb, cb),
                    (target[:, 0], target[:, 1]),
                    batch_dates,
                    training_policy=training_policy,
                    pairwise_reduction=str(pairwise_reduction),
                    pair_weight_policies=(str(pair_weight_policy),) * 2,
                )
                if loss is None:
                    continue
                ...
                loss, loss_weight = _combine_pairwise_head_supervision(
                    torch,
                    model.forward_conditional_heads(xb, cb),
                    (target[:, 0], target[:, 1]),
                    batch_dates,
                    training_policy=training_policy,
                    pairwise_reduction=str(pairwise_reduction),
                    pair_weight_policies=(CONTINUOUS_RANKER_PAIR_WEIGHT_POLICY_NONE,) * 2,
                )
                if loss is None:
                    continue
            else:
                logits = model(xb, cb)
                if loss_handler == CONTINUOUS_RANKER_LOSS_HANDLER_PERCENTILE_MSE:
                    score = torch.softmax(logits.float(), dim=1)[:, LABEL_PASS]
                    loss = F.mse_loss(score, target, reduction="mean")
                    loss_weight = int(len(ids))
                elif loss_handler == CONTINUOUS_RANKER_LOSS_HANDLER_RAW_R:
                    ...
                    # Existing two output neurons are reinterpreted only by this capability:
                    # LABEL_REJECT -> adverse-to-peak R, LABEL_PASS -> favorable MFE R.
                    loss = F.mse_loss(logits.float(), target, reduction="mean")
                    loss_weight = int(len(ids))
                elif loss_handler == CONTINUOUS_RANKER_LOSS_HANDLER_SINGLE_PAIRWISE:
                ...
                elif loss_handler == CONTINUOUS_RANKER_LOSS_HANDLER_LISTWISE:
                    margins = logits.float()[:, LABEL_PASS] - logits.float()[:, LABEL_REJECT]
                    ...
                    raise ValueError(
                        f"不支援的continuous ranker loss capability: {loss_handler!r}"
                    )
        reuse_loss_scalar = bool(
            training_policy.uses_pairwise_loss
    ...
    if training_policy.epoch_loss_aggregation == CONTINUOUS_RANKER_EPOCH_LOSS_AGGREGATION_MEAN_BATCH:
        # Preserve historical scalar-regression and multi-head reporting exactly.
...
def _predict_logits(
    ...
    *,
    ...
    if output_head is not None:
        kwargs["output_head"] = str(output_head)
    if relational:
        kwargs["same_date_group_labels"] = _same_date_inference_labels(model, group_dates, ids)
        kwargs["same_date_relation_history_indices"] = _same_date_relation_history_plan(
            model, group_dates, ids
        )
    return strict_parallel_batched_logits(
        torch,
        model,
        feature_bank,
        group_context,
...
def _pass_probability(logits: np.ndarray) -> np.ndarray:
...
def predict_scores(
    ...
    *,
    ...
    logits = _predict_logits(
        torch, model, feature_bank, group_context, group_ids,
        batch_size=int(batch_size), plan=plan, group_dates=group_dates, relational=True,
    )
    training_policy = get_continuous_ranker_training_policy(training_objective)
    ...
    return _pass_probability(logits)
...
def predict_safety_conditional_mfe_scores(
    ...
    *,
    ...
    logits = _predict_logits(
        torch, model, feature_bank, group_context, group_ids,
        batch_size=int(batch_size), plan=plan, output_head="both",
        group_dates=group_dates, relational=True,
    )
    if logits.ndim != 2 or int(logits.shape[1]) != 4:
        raise ValueError("Safety→Conditional-MFE model output必須為[N,4]")
    return {
        "raw_safety": _pass_probability(logits[:, :2]),
        "conditional_mfe": _pass_probability(logits[:, 2:]),
    }
...
def hs_priority_mfe_metrics(
    group_ids: np.ndarray,
    group_table: pd.DataFrame,
    targets: HsPriorityMfeTargets,
    scores: dict[str, np.ndarray],
    *,
    include_top_k_quality: bool = False,
) -> dict[str, Any]:
    """Evaluate the non-compensatory all-daily HS-priority final head."""

    ids = np.asarray(group_ids, dtype=np.int64)
    safety_scores = np.asarray(scores["raw_safety"], dtype=np.float32)
    priority_scores = np.asarray(scores["conditional_mfe"], dtype=np.float32)
    if len(safety_scores) != len(ids) or len(priority_scores) != len(ids):
        raise ValueError("HS-Priority MFE score/group長度不一致")

    adverse = pd.to_numeric(group_table["target_adverse_r"], errors="coerce").to_numpy(dtype=np.float32)
    favorable = pd.to_numeric(group_table["target_favorable_r"], errors="coerce").to_numpy(dtype=np.float32)
    true_hs = np.asarray(targets.true_hs_mask[ids], dtype=bool)
    hs_ids = ids[true_hs]
    hs_scores = priority_scores[true_hs]
    hs_only = (
        split_metrics(
            hs_ids,
            group_table,
    ...
    hs_boundary = daily_rank_metrics(
        dates, priority_scores, true_hs.astype(np.float32)
    )
    return {
        "raw_safety": split_metrics(
            ids,
        ...
        "hs_priority_mfe": split_metrics(
            ids,
            group_table,
            targets.hs_priority_mfe_relevance,
            targets.hs_priority_mfe_relevance,
            priority_scores,
            include_top_k_quality=bool(include_top_k_quality),
        ),
        "conditional_mfe_true_hs": hs_only,
        "hs_vs_ls_boundary": hs_boundary,
    }
...
def predict_safety_raw_mfe_scores(
    ...
    *,
    ...
    """Return MR-13S Raw-Safety and absolute Raw-MFE head probabilities."""

    legacy = predict_safety_conditional_mfe_scores(
        torch, model, feature_bank, group_context, group_ids,
        batch_size=int(batch_size), plan=plan,
    )
    return {
        "raw_safety": np.asarray(legacy["raw_safety"], dtype=np.float32),
        "raw_mfe": np.asarray(legacy["conditional_mfe"], dtype=np.float32),
    }


def predict_safety_raw_mfe_hmhs_scores(
    torch,
    model,
    feature_bank: np.ndarray,
    group_context: np.ndarray,
    ...
    *,
    ...
    """Return all MR-13T head probabilities from one shared-encoder pass."""

    logits = _predict_logits(
        torch, model, feature_bank, group_context, group_ids,
        batch_size=int(batch_size), plan=plan, output_head="tri_head",
    ).astype(np.float32)
    if logits.ndim != 2 or int(logits.shape[1]) != 6:
        raise ValueError("Safety/Raw-MFE/HMHS model output必須為[N,6]")

    return {
        "raw_safety": _pass_probability(logits[:, :2]),
        "raw_mfe": _pass_probability(logits[:, 2:4]),
        "joint_hmhs": _pass_probability(logits[:, 4:6]),
    }
...
def predict_safety_raw_mfe_joint_min_scores(
    ...
    *,
    ...
    """Return MR-13W tri-head probabilities with target-correct joint key."""

    legacy = predict_safety_raw_mfe_hmhs_scores(
        torch,
        model,
        feature_bank,
        group_context,
    ...
    return {
        "raw_safety": np.asarray(legacy["raw_safety"], dtype=np.float32),
        "raw_mfe": np.asarray(legacy["raw_mfe"], dtype=np.float32),
        "joint_min": np.asarray(legacy["joint_hmhs"], dtype=np.float32),
    }


def _mean_daily_average_precision(
    dates: np.ndarray,
    labels: np.ndarray,
    scores: np.ndarray,
    ...
    for _date, day in frame.groupby("date", sort=True):
        ap = _average_precision(
            day["label"].to_numpy(dtype=np.int64),
            day["score"].to_numpy(dtype=np.float64),
        )
...
def _joint_top_fraction_metrics(
    labels: np.ndarray,
    safety_percentile: np.ndarray,
    mfe_percentile: np.ndarray,
    scores: np.ndarray,
    *,
    fraction: float,
) -> dict[str, Any]:
    ...
    if len(y) == 0:
        return {
            "n": 0,
            "hmhs_pct": None,
            "hmhs_enrichment": None,
            "mean_actual_safety_percentile": None,
            "mean_actual_mfe_percentile": None,
        }
    count = max(1, int(math.ceil(len(y) * float(fraction))))
    selected = np.argsort(-s, kind="mergesort")[:count]
    population = float(y.mean())
    selected_rate = float(y[selected].mean())
    return {
        "n": int(count),
        "hmhs_pct": selected_rate * 100.0,
        "hmhs_enrichment": None if population <= 0.0 else float(selected_rate / population),
        "mean_actual_safety_percentile": float(np.mean(np.asarray(safety_percentile, dtype=np.float64)[selected])),
        "mean_actual_mfe_percentile": float(np.mean(np.asarray(mfe_percentile, dtype=np.float64)[selected])),
    }


def _joint_min_top_fraction_metrics(
    safety_percentile: np.ndarray,
    mfe_percentile: np.ndarray,
    scores: np.ndarray,
    *,
    ...
    joint_min = np.minimum(safety, mfe)
    score_values = np.asarray(scores, dtype=np.float64)
    ...
    count = max(1, int(math.ceil(len(score_values) * float(fraction))))
    selected = np.argsort(-score_values, kind="mergesort")[:count]
    hmhs = (safety >= HMHS_HIGH_PERCENTILE_CUTOFF) & (mfe >= HMHS_HIGH_PERCENTILE_CUTOFF)
    population_hmhs = float(hmhs.mean())
    selected_hmhs = float(hmhs[selected].mean())
    return {
        "n": int(count),
        "mean_joint_min": float(joint_min[selected].mean()),
        "mean_safety": float(safety[selected].mean()),
        "mean_mfe": float(mfe[selected].mean()),
        "hmhs_pct": selected_hmhs * 100.0,
        "hmhs_enrichment": (
            None if population_hmhs <= 0.0 else float(selected_hmhs / population_hmhs)
        ),
    }


def _predicted_quintiles(dates: np.ndarray, values: np.ndarray) -> np.ndarray:
    score = np.asarray(values, dtype=np.float64)
    if score.ndim != 1 or not bool(np.isfinite(score).all()):
        raise ValueError("MR-13S Model Gate prediction必須為finite 1D")
    percentile = build_same_date_percentile_targets(
        score, np.ones(len(score), dtype=bool), pd.to_datetime(dates, errors="raise")
    )
...
def safety_mfe_truth_geometry(
    ...
    """Return actual same-day Safety×Pure-MFE support without re-ranking a subset.

    The inputs must already be the canonical daily-universal same-day percentiles.  This
    matters for breakout-candidate diagnostics: filtering to candidate rows must not
...
def safety_raw_mfe_metrics(
    ...
    *,
    ...
    """MR-13S marginal-head metrics plus the model-only joint-geometry Gate."""

    ids = np.asarray(group_ids, dtype=np.int64)
    safety_scores = np.asarray(scores["raw_safety"], dtype=np.float32)
    mfe_scores = np.asarray(scores["raw_mfe"], dtype=np.float32)
    if len(safety_scores) != len(ids) or len(mfe_scores) != len(ids):
        raise ValueError("Safety→Raw-MFE score/group長度不一致")
    adverse = pd.to_numeric(group_table["target_adverse_r"], errors="coerce").to_numpy(dtype=np.float32)
    favorable = pd.to_numeric(group_table["target_favorable_r"], errors="coerce").to_numpy(dtype=np.float32)
    head_metrics = {
        "raw_safety": split_metrics(
        ...
        "raw_mfe": split_metrics(
            ids, group_table, favorable, targets.primary_mfe_percentile, mfe_scores,
            include_top_k_quality=bool(include_top_k_quality),
        ),
    }
    dates = pd.to_datetime(group_table.iloc[ids]["date"], errors="raise").to_numpy()
    safety_q = _predicted_quintiles(dates, safety_scores)
    mfe_q = _predicted_quintiles(dates, mfe_scores)
    actual_high_mfe = np.asarray(targets.primary_mfe_percentile[ids] >= HMHS_HIGH_PERCENTILE_CUTOFF, dtype=bool)
    actual_high_safety = np.asarray(targets.low_adverse_safety_percentile[ids] >= HMHS_HIGH_PERCENTILE_CUTOFF, dtype=bool)
    actual_hmhs = actual_high_mfe & actual_high_safety
    cells: list[list[dict[str, Any]]] = []
    for safety_level in range(1, 6):
        row: list[dict[str, Any]] = []
        for mfe_level in range(1, 6):
            mask = (safety_q == safety_level) & (mfe_q == mfe_level)
            n = int(mask.sum())
            row.append({
                "n": n,
                "actual_hmhs_pct": None if n == 0 else float(actual_hmhs[mask].mean() * 100.0),
            })
        cells.append(row)
    cohorts: list[dict[str, Any]] = []
    for safety_level in range(1, 6):
        mask = safety_q == safety_level
        n = int(mask.sum())
        rank = daily_rank_metrics(dates[mask], mfe_scores[mask], favorable[ids][mask]) if n else {}
        cohorts.append({
            "predicted_safety_quintile": int(safety_level),
            "n": n,
            "raw_mfe_to_actual_mfe_mean_daily_spearman": rank.get("mean_daily_spearman"),
            "valid_days": int(rank.get("rankable_date_count", 0) or 0),
            "high_mfe_pct": None if n == 0 else float(actual_high_mfe[mask].mean() * 100.0),
            "hmhs_pct": None if n == 0 else float(actual_hmhs[mask].mean() * 100.0),
        })
    joint_product = safety_scores.astype(np.float64) * mfe_scores.astype(np.float64)
    joint_rank = daily_rank_metrics(dates, joint_product, actual_hmhs.astype(np.float64))
    predicted_relation = daily_rank_metrics(dates, safety_scores, mfe_scores)
    truth_geometry = safety_mfe_truth_geometry(
        dates,
        targets.low_adverse_safety_percentile[ids],
        targets.primary_mfe_percentile[ids],
    )
    upper_right = cells[4][4]
    head_metrics["model_gate"] = {
        "population_n": int(len(ids)),
        "population_hmhs_pct": float(actual_hmhs.mean() * 100.0) if len(ids) else None,
        "predicted_joint_geometry": cells,
        "upper_right_s5_m5": dict(upper_right),
        "safety_cohorts": cohorts,
        "joint_product_to_actual_hmhs_mean_daily_spearman": joint_rank.get("mean_daily_spearman"),
        "joint_product_valid_days": int(joint_rank.get("rankable_date_count", 0) or 0),
        ...
        "actual_truth_geometry": truth_geometry,
        "truth_cutoff": float(HMHS_HIGH_PERCENTILE_CUTOFF),
        "predicted_quintile_method": "same_date_average_zero_based_percentile_then_fixed_quintile",
        "status": "diagnostic_only_no_fit_no_threshold_selection",
    }
    return head_metrics


def safety_raw_mfe_hmhs_metrics(
    group_ids: np.ndarray,
    group_table: pd.DataFrame,
    targets: ConditionalMfeOpportunityTargets,
    scores: dict[str, np.ndarray],
    *,
    ...
    """MR-13T marginal preservation plus direct-HM/HS retrieval diagnostics."""

    ids = np.asarray(group_ids, dtype=np.int64)
    base = safety_raw_mfe_metrics(
        ids,
        group_table,
        targets,
        {"raw_safety": scores["raw_safety"], "raw_mfe": scores["raw_mfe"]},
        include_top_k_quality=bool(include_top_k_quality),
    )
    joint_scores = np.asarray(scores["joint_hmhs"], dtype=np.float32)
    if len(joint_scores) != len(ids):
        raise ValueError("Direct HM/HS score/group長度不一致")
    dates = pd.to_datetime(group_table.iloc[ids]["date"], errors="raise").to_numpy()
    safety_true = np.asarray(targets.low_adverse_safety_percentile[ids], dtype=np.float32)
    mfe_true = np.asarray(targets.primary_mfe_percentile[ids], dtype=np.float32)
    hmhs = np.asarray(
        (safety_true >= HMHS_HIGH_PERCENTILE_CUTOFF)
        & (mfe_true >= HMHS_HIGH_PERCENTILE_CUTOFF),
        dtype=np.int64,
    )
    population_rate = float(hmhs.mean()) if len(hmhs) else 0.0
    direct_rank = daily_rank_metrics(dates, joint_scores, hmhs.astype(np.float64))
    direct_ap = _average_precision(hmhs, joint_scores)
    direct_daily_ap, direct_daily_ap_days = _mean_daily_average_precision(dates, hmhs, joint_scores)
    product_scores = (
        np.asarray(scores["raw_safety"], dtype=np.float64)
        * np.asarray(scores["raw_mfe"], dtype=np.float64)
    )
    product_rank = daily_rank_metrics(dates, product_scores, hmhs.astype(np.float64))
    product_ap = _average_precision(hmhs, product_scores)
    product_daily_ap, product_daily_ap_days = _mean_daily_average_precision(dates, hmhs, product_scores)

    base["joint_hmhs"] = {
        "group_count": int(len(ids)),
        "positive_count": int(hmhs.sum()),
        "population_hmhs_pct": population_rate * 100.0 if len(hmhs) else None,
        "pairwise_concordance": direct_rank.get("pairwise_concordance"),
        "comparable_pair_count": int(direct_rank.get("comparable_pair_count", 0) or 0),
        "global_average_precision": direct_ap,
        "mean_daily_average_precision": direct_daily_ap,
        "daily_average_precision_days": int(direct_daily_ap_days),
        "top_10pct": _joint_top_fraction_metrics(hmhs, safety_true, mfe_true, joint_scores, fraction=0.10),
        "top_20pct": _joint_top_fraction_metrics(hmhs, safety_true, mfe_true, joint_scores, fraction=0.20),
        "target": "direct_HMHS_indicator",
        "truth_cutoff": float(HMHS_HIGH_PERCENTILE_CUTOFF),
    }
    base["joint_product_control"] = {
        "pairwise_concordance": product_rank.get("pairwise_concordance"),
        "global_average_precision": product_ap,
        "mean_daily_average_precision": product_daily_ap,
        "daily_average_precision_days": int(product_daily_ap_days),
        "top_10pct": _joint_top_fraction_metrics(hmhs, safety_true, mfe_true, product_scores, fraction=0.10),
        "top_20pct": _joint_top_fraction_metrics(hmhs, safety_true, mfe_true, product_scores, fraction=0.20),
        "score": "raw_safety_probability_times_raw_mfe_probability",
    }
    return base


def safety_raw_mfe_joint_min_metrics(
    group_ids: np.ndarray,
    group_table: pd.DataFrame,
    targets: ConditionalMfeOpportunityTargets,
    scores: dict[str, np.ndarray],
    *,
    ...
    """MR-13W marginal preservation plus continuous maximin joint-quality Gate."""
    ...
    joint_scores = np.asarray(scores["joint_min"], dtype=np.float32)
    if len(joint_scores) != len(ids):
        raise ValueError("Joint-Min score/group長度不一致")
    dates = pd.to_datetime(group_table.iloc[ids]["date"], errors="raise").to_numpy()
    safety_true = np.asarray(targets.low_adverse_safety_percentile[ids], dtype=np.float32)
    mfe_true = np.asarray(targets.primary_mfe_percentile[ids], dtype=np.float32)
    joint_true = np.minimum(safety_true, mfe_true).astype(np.float32, copy=False)
    rank = daily_rank_metrics(dates, joint_scores, joint_true)
    base["joint_min"] = {
        "group_count": int(len(ids)),
        "population_joint_min_mean": (
            float(joint_true.mean()) if len(joint_true) else None
        ),
        "mean_daily_spearman": rank.get("mean_daily_spearman"),
        "median_daily_spearman": rank.get("median_daily_spearman"),
        "global_spearman": calculate_spearman(joint_scores, joint_true),
        "pairwise_concordance": rank.get("pairwise_concordance"),
        "comparable_pair_count": int(rank.get("comparable_pair_count", 0) or 0),
        "top_10pct": _joint_min_top_fraction_metrics(
            safety_true, mfe_true, joint_scores, fraction=0.10
        ),
        "top_20pct": _joint_min_top_fraction_metrics(
            safety_true, mfe_true, joint_scores, fraction=0.20
        ),
        "target": "min_same_date_safety_and_pure_mfe_percentiles",
        "status": "continuous_joint_rank_no_threshold_no_oos_fit",
    }
    return base


def direct_hmhs_metrics(
    group_ids: np.ndarray,
    group_table: pd.DataFrame,
    targets: ConditionalMfeOpportunityTargets,
    scores: np.ndarray,
    *,
    ...
    """MR-13U direct-HM/HS H-only learnability metrics.

    H is the predeclared intersection of the canonical same-date Safety and Pure-MFE
    percentiles.  This evaluator is frozen/read-only and does not fit a threshold.
    """
    ...
    population_rate = float(hmhs.mean()) if len(hmhs) else 0.0
    rank = daily_rank_metrics(dates, score_values, hmhs.astype(np.float64))
    global_ap = _average_precision(hmhs, score_values)
    daily_ap, daily_ap_days = _mean_daily_average_precision(dates, hmhs, score_values)
    order = np.argsort(score_values, kind="mergesort")
    decile_count = max(1, int(math.ceil(len(ids) * 0.10))) if len(ids) else 0
    bottom = order[:decile_count]
    top = order[-decile_count:] if decile_count else np.asarray([], dtype=np.int64)
    top_k_quality = (
        daily_top_k_metrics(
            dates,
            score_values,
            hmhs.astype(np.float64),
            hmhs.astype(np.float64),
            top_k=BREAKOUT_QUALITY_CONTINUOUS_RANKER_REPORT_TOP_K,
            boundary_width=BREAKOUT_QUALITY_CONTINUOUS_RANKER_REPORT_BOUNDARY_WIDTH,
        )
        if include_top_k_quality and len(ids)
        else None
    )
    return {
        "group_count": int(len(ids)),
        "positive_count": int(hmhs.sum()),
        "population_hmhs_pct": population_rate * 100.0 if len(hmhs) else None,
        "percentile_target_count": int(len(ids)),
        "mse_vs_daily_percentile": (
            float(np.mean((score_values - hmhs.astype(np.float64)) ** 2))
            if len(ids)
            else None
        ),
        "global_spearman_vs_raw_target": calculate_spearman(score_values, hmhs.astype(np.float64)),
        "global_spearman_vs_daily_percentile": calculate_spearman(score_values, hmhs.astype(np.float64)),
        **rank,
        "global_average_precision": global_ap,
        "binary_pr_auc": global_ap,
        "mean_daily_average_precision": daily_ap,
        "daily_average_precision_days": int(daily_ap_days),
        "top_10pct": _joint_top_fraction_metrics(
            hmhs, safety_true, mfe_true, score_values, fraction=0.10
        ),
        "top_20pct": _joint_top_fraction_metrics(
            hmhs, safety_true, mfe_true, score_values, fraction=0.20
        ),
        "top_k_quality": top_k_quality,
        "score_mean": float(score_values.mean()) if len(score_values) else None,
        "score_std": float(score_values.std(ddof=1)) if len(score_values) > 1 else 0.0,
        "top_score_decile_raw_target_mean": (
            float(hmhs[top].mean()) if len(top) else None
        ),
        "bottom_score_decile_raw_target_mean": (
            float(hmhs[bottom].mean()) if len(bottom) else None
        ),
        "target": "direct_HMHS_indicator",
        "truth_cutoff": float(HMHS_HIGH_PERCENTILE_CUTOFF),
    }
...
def predict_conditional_mfe_safety_scores(
    ...
    *,
    ...
    logits = _predict_logits(
        torch, model, feature_bank, group_context, group_ids,
        batch_size=int(batch_size), plan=plan, output_head="conditional_both",
    ).astype(np.float32)
    if logits.ndim != 2 or int(logits.shape[1]) != 4:
        raise ValueError("conditional MFE-safety model output必須為[N,4]")

    return {
        "primary_mfe": _pass_probability(logits[:, :2]),
        "conditional_safety": _pass_probability(logits[:, 2:]),
    }
...
def _training_target_for_profile(
    ...
    target_builder = str(recipe.training_policy.target_builder)
    opportunity_target_attrs = {
        CONTINUOUS_RANKER_TARGET_BUILDER_CONDITIONAL_MFE_SINGLE: "single_head_training_target",
        CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_CONDITIONAL_MFE: "duo_head_training_target",
        CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE: "safety_raw_mfe_training_target",
        CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_HMHS: "safety_raw_mfe_hmhs_training_target",
        CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_JOINT_MIN: "safety_raw_mfe_joint_min_training_target",
        CONTINUOUS_RANKER_TARGET_BUILDER_DIRECT_HMHS: "direct_hmhs_target",
    }
    if target_builder in opportunity_target_attrs:
        targets = build_conditional_mfe_opportunity_targets_for_training(
            group_table, percentile_target
        )
        return np.asarray(getattr(targets, opportunity_target_attrs[target_builder]), dtype=np.float32)
    if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_CONDITIONAL_MFE_SAFETY:
    ...
    if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_HS_PRIORITY_MFE:
        return build_hs_priority_mfe_targets(
            group_table, np.isfinite(np.asarray(raw_target, dtype=np.float32))
        ).training_target
    if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_PRIMARY:
        return build_safety_primary_targets(group_table, percentile_target).training_target
    ...
    if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_RAW_R:
        return np.asarray(raw_target, dtype=np.float32)
    if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_DUAL_COMPONENT_R:
        ...
        targets = np.column_stack([
            pd.to_numeric(group_table[column], errors="coerce").to_numpy(dtype=np.float32)
        ...
        if not np.array_equal(
            ...
            raise ValueError("dual-component target-valid universe與composite raw target不一致")
        return targets
    if (
        ...
        if len(valid_safety) and (
            float(valid_safety.min()) < 0.0 or float(valid_safety.max()) > 1.0
        ):
            raise ValueError("predicted-Safety pair context必須為0～1 same-date percentile")
        return np.column_stack([
            np.asarray(percentile_target, dtype=np.float32), safety,
        ]).astype(np.float32, copy=False)
    return np.asarray(percentile_target, dtype=np.float32)


def _raw_r_loss_options(profile, training_policy) -> tuple[str | None, float | None]:
    if str(training_policy.loss_handler) != CONTINUOUS_RANKER_LOSS_HANDLER_RAW_R:
        return None, None
    return (
        str(profile.loss_name),
        None if profile.raw_r_huber_delta_r is None else float(profile.raw_r_huber_delta_r),
    )


def _adaptive_horizon_target_provider(
    *,
    loss_handler: str,
    profile,
    feature_bank: np.ndarray,
    ...
    provider = AdaptiveHorizonSafetyTargetProvider(
    ...
    provider.prepare_ids(fit_ids)
    return provider


def _evaluate_epoch_split(
    torch,
    model,
    feature_bank: np.ndarray,
    group_context: np.ndarray,
    ...
    *,
    ...
    hs_priority_targets,
    raw_r_loss_name: str | None,
    raw_r_delta: float | None,
    selection_metrics_only: bool = False,
    validation_split: bool = False,
    include_pareto_metrics: bool = False,
) -> dict[str, Any]:
    details = {
        "conditional_metrics": None,
        "reverse_conditional_metrics": None,
        "safety_raw_mfe_metrics": None,
        "hs_conditional_mfe_metrics": None,
        "hs_priority_mfe_metrics": None,
        "safety_raw_mfe_hmhs_metrics": None,
        "safety_raw_mfe_joint_min_metrics": None,
        "direct_hmhs_metrics": None,
        "pareto_metrics": None,
        "selection_only": False,
    }
    predict_common = dict(batch_size=int(args.evaluation_batch_size), plan=plan)
    metric_family = None
    family_keys = None

    if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_DIRECT_HMHS:
        scores = predict_scores(
            torch, model, feature_bank, group_context, ids,
            training_objective=profile.training_objective, **predict_common,
        )
        metrics = direct_hmhs_metrics(ids, group_table, conditional_mfe_targets, scores)
        details["direct_hmhs_metrics"] = metrics
    elif (
        target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_JOINT_MIN
        and bool(selection_metrics_only)
    ):
        scores = predict_scores(
            torch, model, feature_bank, group_context, ids,
            training_objective=profile.training_objective, **predict_common,
        )
        metrics = epoch_selection_metrics(
            ids, group_table, raw_target, percentile_target, scores
        )
        details["selection_only"] = True
    else:
        multihead_spec = {
            CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_JOINT_MIN: (
                predict_safety_raw_mfe_joint_min_scores, safety_raw_mfe_joint_min_metrics,
                conditional_mfe_targets, "raw_mfe", "raw_safety", "raw_mfe",
                "safety_raw_mfe_joint_min_metrics", False, None,
            ),
            CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_HMHS: (
                predict_safety_raw_mfe_hmhs_scores, safety_raw_mfe_hmhs_metrics,
                conditional_mfe_targets, "raw_mfe", "raw_safety", "raw_mfe",
                "safety_raw_mfe_hmhs_metrics", False, None,
            ),
            CONTINUOUS_RANKER_TARGET_BUILDER_HS_CONDITIONAL_MFE: (
                predict_safety_conditional_mfe_scores, hs_conditional_mfe_metrics,
                hs_conditional_targets, "conditional_mfe", "raw_safety",
                "conditional_mfe_true_hs", "hs_conditional_mfe_metrics", True,
                "HS-Conditional MFE target bundle未建立",
            ),
            CONTINUOUS_RANKER_TARGET_BUILDER_HS_PRIORITY_MFE: (
                predict_safety_conditional_mfe_scores, hs_priority_mfe_metrics,
                hs_priority_targets, "conditional_mfe", "raw_safety", "hs_priority_mfe",
                "hs_priority_mfe_metrics", False, "HS-Priority MFE target bundle未建立",
            ),
            CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE: (
                predict_safety_raw_mfe_scores, safety_raw_mfe_metrics,
                conditional_mfe_targets, "raw_mfe", "raw_safety", "raw_mfe",
                "safety_raw_mfe_metrics", False, None,
            ),
            CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_CONDITIONAL_MFE: (
                predict_safety_conditional_mfe_scores, safety_conditional_mfe_metrics,
                conditional_mfe_targets, "conditional_mfe", "raw_safety", "conditional_mfe",
                "reverse_conditional_metrics", False, None,
            ),
        }.get(target_builder)
        if multihead_spec is not None:
            (
                predictor, metric_fn, target_bundle, score_key, primary_key, selection_key,
                detail_key, validation_uses_group_dates, missing_target_message,
            ) = multihead_spec
            if target_bundle is None and missing_target_message is not None:
                raise RuntimeError(missing_target_message)
            extra_predict = (
                {"group_dates": group_table["date"]}
                if validation_split and validation_uses_group_dates
                else {}
            )
            head_scores = predictor(
                torch, model, feature_bank, group_context, ids,
                **predict_common, **extra_predict,
            )
            metric_family = metric_fn(ids, group_table, target_bundle, head_scores)
            scores = head_scores[score_key]
            metrics = metric_family[selection_key]
            details[detail_key] = metric_family
            family_keys = (primary_key, selection_key)
        elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_CONDITIONAL_MFE_SINGLE:
            scores = predict_scores(
                torch, model, feature_bank, group_context, ids,
                training_objective=profile.training_objective, **predict_common,
            )
            metrics = split_metrics(
                ids, group_table,
                conditional_mfe_targets.conditional_mfe_residual,
                conditional_mfe_targets.conditional_mfe_percentile,
                scores,
            )
        elif conditional_targets is not None:
            head_scores = predict_conditional_mfe_safety_scores(
                torch, model, feature_bank, group_context, ids, **predict_common,
            )
            duo_metrics = conditional_mfe_safety_metrics(
                ids, group_table, raw_target, conditional_targets, head_scores
            )
            scores = head_scores["primary_mfe"]
            metrics = duo_metrics["primary_mfe"]
            details["conditional_metrics"] = duo_metrics
        else:
            scores = predict_scores(
                torch, model, feature_bank, group_context, ids,
                training_objective=profile.training_objective, **predict_common,
    ...
    if details["selection_only"]:
        primary_daily_spearman = None
        daily_spearman = metrics.get("mean_daily_spearman")
        validation_mse = float(metrics["mse_vs_daily_percentile"])
    elif details["direct_hmhs_metrics"] is not None:
        primary_daily_spearman = metrics.get("mean_daily_spearman")
        daily_spearman = primary_daily_spearman
        validation_mse = float(metrics["mse_vs_daily_percentile"])
    elif metric_family is not None:
        primary_key, selection_key = family_keys
        primary_daily_spearman = metric_family[primary_key].get("mean_daily_spearman")
        daily_spearman = metric_family[selection_key].get("mean_daily_spearman")
        validation_mse = float(metric_family[selection_key]["mse_vs_daily_percentile"])
    elif details["conditional_metrics"] is not None:
        primary_daily_spearman = metrics.get("mean_daily_spearman")
        daily_spearman = details["conditional_metrics"]["conditional_safety"].get(
            "mean_daily_spearman"
        )
        validation_mse = float(
            details["conditional_metrics"]["conditional_safety"]["mse_vs_daily_percentile"]
        )
    else:
        primary_daily_spearman = metrics.get("mean_daily_spearman")
        daily_spearman = primary_daily_spearman
    ...
    if include_pareto_metrics and target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_PARETO_COMPONENTS:
        details["pareto_metrics"] = pareto_pair_concordance_metrics(
            ids, group_table, raw_target, scores
        )

    return {
        "scores": scores,
        "metrics": metrics,
        "primary_daily_spearman": primary_daily_spearman,
...
def select_epoch(
    ...
    *,
    ...
    loss_handler = str(training_policy.loss_handler)
    auxiliary_target_bundle = str(training_policy.auxiliary_target_bundle)
    conditional_targets = (
        build_conditional_targets(group_table, percentile_target)
        if auxiliary_target_bundle == CONTINUOUS_RANKER_AUX_TARGET_CONDITIONAL_MFE_SAFETY
        else None
    )
    conditional_mfe_targets = (
    ...
    adaptive_horizon_target_provider = _adaptive_horizon_target_provider(
    ...
    raw_r_loss_name, raw_r_delta = _raw_r_loss_options(profile, training_policy)
    pretrained_encoder_state = None
    ...
    best_hmhs_pairwise_concordance = -math.inf
    best_hmhs_global_average_precision = -math.inf
    ...
    if not compact_console:
        label = (
            ...
            else "Validation HS-Priority MFE mean daily Spearman"
            if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_HS_PRIORITY_MFE
            ...
            else "Validation Direct HM/HS Pair concordance"
            if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_DIRECT_HMHS
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        validation_evaluation = _evaluate_epoch_split(
            torch,
            model,
            feature_bank,
            group_context,
            ...
            conditional_mfe_targets=conditional_mfe_targets,
            hs_conditional_targets=hs_conditional_targets,
            hs_priority_targets=hs_priority_targets,
            raw_r_loss_name=raw_r_loss_name,
            raw_r_delta=raw_r_delta,
            selection_metrics_only=bool(selection_metrics_only),
            validation_split=True,
            include_pareto_metrics=True,
        )
        validation_scores = validation_evaluation["scores"]
        validation_metrics = validation_evaluation["metrics"]
        validation_conditional_metrics = validation_evaluation["conditional_metrics"]
        validation_reverse_conditional_metrics = validation_evaluation["reverse_conditional_metrics"]
        validation_safety_raw_mfe_metrics = validation_evaluation["safety_raw_mfe_metrics"]
        validation_hs_conditional_mfe_metrics = validation_evaluation["hs_conditional_mfe_metrics"]
        validation_hs_priority_mfe_metrics = validation_evaluation["hs_priority_mfe_metrics"]
        validation_safety_raw_mfe_hmhs_metrics = validation_evaluation["safety_raw_mfe_hmhs_metrics"]
        validation_safety_raw_mfe_joint_min_metrics = validation_evaluation["safety_raw_mfe_joint_min_metrics"]
        validation_direct_hmhs_metrics = validation_evaluation["direct_hmhs_metrics"]
        validation_pareto_metrics = validation_evaluation["pareto_metrics"]
        validation_used_selection_only_metrics = bool(validation_evaluation["selection_only"])
        primary_daily_spearman = validation_evaluation["primary_daily_spearman"]
        daily_spearman = validation_evaluation["daily_spearman"]
        validation_mse = float(validation_evaluation["validation_mse"])

        if bool(evaluate_train_metrics):
            train_metrics = _evaluate_epoch_split(
                torch,
                model,
                feature_bank,
                group_context,
        ...
        if validation_pareto_metrics is not None:
            pareto_daily = validation_pareto_metrics.get("mean_daily_pareto_pair_concordance")
            pareto_global = validation_pareto_metrics.get("global_pareto_pair_concordance")
        ...
        if loss_handler == CONTINUOUS_RANKER_LOSS_HANDLER_RAW_R:
            ...
            validation_mae = None
            if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_DIRECT_HMHS:
                hmhs_pair = validation_direct_hmhs_metrics.get("pairwise_concordance")
                hmhs_ap = validation_direct_hmhs_metrics.get("global_average_precision")
                if hmhs_pair is None or not math.isfinite(float(hmhs_pair)):
                    raise ValueError("Direct HM/HS validation Pair concordance不可用")
                if hmhs_ap is None or not math.isfinite(float(hmhs_ap)):
                    raise ValueError("Direct HM/HS validation PR-AUC不可用")
                improved = (
                    float(hmhs_pair)
                    > best_hmhs_pairwise_concordance + float(args.early_stopping_min_delta)
                    or (
                        math.isclose(
                            float(hmhs_pair), best_hmhs_pairwise_concordance,
                            rel_tol=0.0, abs_tol=1e-12,
                        )
                        and float(hmhs_ap) > best_hmhs_global_average_precision
                    )
                )
            elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_PARETO_COMPONENTS:
                pareto_daily = float(
        ...
        if improved:
            ...
            if validation_direct_hmhs_metrics is not None:
                best_hmhs_pairwise_concordance = float(
                    validation_direct_hmhs_metrics["pairwise_concordance"]
                )
                best_hmhs_global_average_precision = float(
                    validation_direct_hmhs_metrics["global_average_precision"]
                )
            if validation_pareto_metrics is not None:
        ...
        history.append({
            ...
            "inner_validation_hs_priority_mfe_metrics": validation_hs_priority_mfe_metrics,
        ...
        if not compact_console:
            ...
            elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_CONDITIONAL_MFE_SAFETY:
                print(
            ...
            elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_DIRECT_HMHS:
                print(
                    f"  Epoch {epoch:>3}/{int(args.epochs)} | Train Loss {batch_loss:.6f} "
                    f"| Val HM/HS Pair {float(validation_direct_hmhs_metrics['pairwise_concordance']):.4f} "
                    f"| Val PR-AUC {float(validation_direct_hmhs_metrics['global_average_precision']):.4f} "
                    f"| {elapsed:.1f}s{marker}"
                )
            elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_JOINT_MIN:
                if validation_used_selection_only_metrics:
                    print(
                        f"  Epoch {epoch:>3}/{int(args.epochs)} | Train Loss {batch_loss:.6f} "
            ...
            elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_HMHS:
            ...
            elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_HS_PRIORITY_MFE:
                hs_only = dict(validation_hs_priority_mfe_metrics.get("conditional_mfe_true_hs") or {})
                print(
                    f"  Epoch {epoch:>3}/{int(args.epochs)} | Train Loss {batch_loss:.6f} "
                    f"| Val HS-Priority rho {float(daily_spearman):.4f} "
                    f"| Val HS-only MFE rho {float(hs_only.get('mean_daily_spearman') or 0.0):.4f} "
                    f"| Val Safety rho {float(primary_daily_spearman):.4f} "
                    f"| {elapsed:.1f}s{marker}"
                )
            ...
            elif target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_CONDITIONAL_MFE:
                print(
                    f"  Epoch {epoch:>3}/{int(args.epochs)} | Train Loss {batch_loss:.6f} "
    ...
    result = {
        ...
        "best_validation_hs_priority_mfe_mean_daily_spearman": (
            float(best_daily_spearman)
            if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_HS_PRIORITY_MFE
            else None
        ),
        "best_validation_direct_hmhs_pairwise_concordance": (
            ...
            if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_DIRECT_HMHS
            and math.isfinite(best_hmhs_pairwise_concordance)
            else None
        ),
        "best_validation_direct_hmhs_global_average_precision": (
            float(best_hmhs_global_average_precision)
            if target_builder == CONTINUOUS_RANKER_TARGET_BUILDER_DIRECT_HMHS
            and math.isfinite(best_hmhs_global_average_precision)
            else None
        ),
        "best_validation_raw_mfe_mean_daily_spearman": (
            float(best_daily_spearman)
            if target_builder in {
                CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE,
                CONTINUOUS_RANKER_TARGET_BUILDER_SAFETY_RAW_MFE_HMHS,
