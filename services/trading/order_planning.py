...
def _render_proposed_orders_text(payload: dict[str, Any]) -> str:
    lines = [
        ...
        "說明：新訊號／再進場的停利線是盤前策略參考；延續／延續(TBD) 的停利線欄沿用 inherited shadow completion barrier。",
    ...
    if not orders:
        ...
        lines.append("#  股票      類型          限價      股數       預留資金       Stop       停利線")
        for row in orders:
            lines.append(
...
def build_trading_proposed_order_plan(*, project_root: str | Path) -> dict[str, Any]:
    ...
    assert_trading_new_allocation_allowed(operations)
    ...
    candidate_snapshot_sha = compute_file_sha256(snapshot_path)
    ...
    for rank, (row, plan) in enumerate(zip(reservation["selected_rows"], reservation["selected_plans"]), 1):
        orders.append({
            ...
            "execution_plan_seed": deepcopy(plan),
            "signal_lineage": deepcopy(row.get("signal_lineage")),
            "sort_value": row.get("sort_value"),
            "expected_value": row.get("expected_value", row.get("ev")),
            "params_signature": str(row.get("params_signature") or ""),
            "ensemble_member_key": str(row.get("ensemble_member_key") or ""),
    ...
    if compute_file_sha256(runtime["selected_path"]) != current_param_sha:
        raise RuntimeError("Trading selected strategy params 在建立建議掛單期間已變更；請重跑")
    if compute_file_sha256(snapshot_path) != candidate_snapshot_sha:
        raise RuntimeError("Trading candidate snapshot 在建立建議掛單期間已變更；請重跑")
    latest_operations = build_trading_operations_status(root)
    assert_trading_new_allocation_allowed(latest_operations)
