...
def _parse_optimizer_study_scope_request_raw(raw_value: str, *, source_label: str):
    try:
        ...
        raise ValueError(f"Study Mode 只接受 Enter/Study-Full 或 1/Study-OOS。來源={source_label}，收到: {raw_value!r}") from exc
...
def _attach_study_scope_from_env(request: dict, environ) -> dict:
    ...
    resolved["study_scope"] = _parse_optimizer_study_scope_request_raw(
        raw_scope,
        source_label=f"ENV:{OPTIMIZER_STUDY_SCOPE_ENV_VAR}" if raw_scope else "DEFAULT:Study-Full",
    )
    return resolved
...
def _parse_optimizer_run_request_raw(raw_value: str, *, source_label: str):
    ...
    return {
        ...
        "action": OPTIMIZER_MENU_ACTION_EXPORT_CANDIDATE if int(trial_count) == 0 else OPTIMIZER_MENU_ACTION_TRAIN,
        "source": source_label,
    }
...
def resolve_optimizer_run_request(environ):
    ...
    return {
        ...
        "action": OPTIMIZER_MENU_ACTION_EXPORT_CANDIDATE,
