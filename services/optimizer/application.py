...
def _replay_payload_on_current_trade_window(session, payload: dict, *, selector: str) -> float:
    ...
    from services.optimizer.callbacks import _run_static_ensemble_dashboard_replay
...
def generate_walk_forward_report_from_payload(*, session, params_payload, dataset_label, db_file, best_trial_number=None, walk_forward_policy: dict):
    ...
    from services.optimizer.walk_forward import evaluate_walk_forward, write_walk_forward_report
...
def _run_nonrolling_seed_ensemble_member_process_task(task: dict) -> dict | None:
    ...
    def _execute() -> dict | None:
        ...
        from services.optimizer.study_utils import build_best_params_payload_from_trial, is_qualified_trial_value
...
def _finalize_single_seed_study_base_only_outputs(
    *,
    ...
    if dashboard_session is not None and getattr(dashboard_session, "raw_data_cache_data_dir", None):
        try:
            from services.optimizer.static_ensemble_dashboard import print_optimizer_static_ensemble_console_dashboard
...
def _run_nonrolling_random_seed_ensemble_training(
    *,
    ...
    from services.optimizer.static_ensemble_dashboard import (
...
def main(argv=None, environ=None):
    ...
    if has_help_flag(argv):
        program_name = resolve_cli_program_name(argv, "apps/research.py optimizer")
    ...
    from services.optimizer.benchmark import (
    ...
    if str(getattr(session, "run_action", "train")) == "outer_rolling_oos":
        from services.optimizer.outer_rolling_oos import run_outer_rolling_oos
        try:
