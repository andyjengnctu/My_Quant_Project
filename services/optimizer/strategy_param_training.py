"""Optimizer-owned breakout strategy-parameter training service and legacy 4×2 gate.
...
def _parse_args(argv=None):
    ...
    parser.add_argument("--trials-per-fold", "--trials", dest="trials_per_fold", type=int, default=OPTIMIZER_OUTER_ROLLING_OOS_TRIALS_DEFAULT)
    parser.add_argument("--max-positions", type=int, default=DEFAULT_PORTFOLIO_MAX_POSITIONS)
    parser.add_argument("--rotation", choices=("off", "on"), default=DEFAULT_PORTFOLIO_ROTATION)
    parser.add_argument("--fixed-risk", type=float, default=DEFAULT_FIXED_RISK)
    parser.add_argument("--max-position-cap-pct", type=float, default=DEFAULT_MAX_POSITION_CAP_PCT)
    parser.add_argument(
        "--build-binary-pit",
...
def _load_baseline_contract(*, root: Path, args) -> dict[str, Any]:
    if str(getattr(args, "canonical_strategy_param_family", "") or "").strip():
...
def _run_optimizer_arm(*, root, args, settings, baseline_contract, model_artifact, output_dir, arm_id, training_dl_enabled, binary_pit):
    ...
    if canonical_current:
        ...
        active_param_dir = resolve_strategy_param_dir(
            root, family=canonical_family, evaluation_mode="rolling"
        )
        arm_dir = active_param_dir
        optimizer_output_dir = active_param_dir / "optimizer_runtime"
    else:
...
def _prepare_extending_roos_params(
    *,
    ...
    for date_text, members in historical_mapping.items():
        date = pd.Timestamp(str(date_text)).normalize()
        if date < transition:
            combined[date.strftime("%Y-%m-%d")] = copy.deepcopy(members)
    for date_text, members in current_mapping.items():
        date = pd.Timestamp(str(date_text)).normalize()
        if date >= transition:
            combined[date.strftime("%Y-%m-%d")] = copy.deepcopy(members)
    if not combined:
    ...
    manifest = {
        ...
        "source_artifacts": {
            ...
            "current": {
                "path": project_relative_display_path(current_path, project_root=root),
                "sha256": current_sha,
            },
        },
        "output_params": {
...
def run_param_adaptation_gate(*, project_root=PROJECT_ROOT, argv=None, canonical_strategy_param_family: str | None = None):
    args = _parse_args(argv)
    args.canonical_strategy_param_family = (
        None if canonical_strategy_param_family in (None, "")
        else str(canonical_strategy_param_family).strip().lower()
    )
    if args.canonical_strategy_param_family not in (None, "min"):
        raise ValueError("Min strategy parameter trainer目前canonical family只接受min")
    root = Path(project_root).resolve()
    settings = SimpleNamespace(seed=int(OPTIMIZER_RANDOM_SEED_DEFAULT))
...
def prepare_strategy_parameter_source(
    *,
    ...
    if quiet:
        argv.append("--quiet")
    return run_param_adaptation_gate(
        project_root=project_root,
        argv=argv,
