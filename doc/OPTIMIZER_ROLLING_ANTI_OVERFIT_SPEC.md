...
- `use_breakout_return_filter=True`：納入 `breakout_return_min`

但第一版仍：
- **不翻動** `use_bb / use_kc / use_vol / use_breakout_return_filter` 布林值本身
- 只檢查「已啟用結構下的數值參數局部穩健性」

### 3.5 條件式濾網合法鄰點規則
`vol_long_len / vol_breakout_mult / breakout_return_min` 分別依正式搜尋範圍產生鄰點：

```text
vol_long_len: 突破日前均量窗長
vol_breakout_mult: 突破日量需大於前均量的倍數
breakout_return_min: 突破日收盤相對前收漲幅門檻
```
