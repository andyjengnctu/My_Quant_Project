...
|---:|---|---|---|---|---|
| 1 | **MR-13V Nonlinear Direct HM/HS Head Model Gate**：MR-13U H-only無增益後，回到MR-13T shared tri-head，僅增加Direct H readout的固定一層nonlinear MLP capacity，檢驗joint intersection是否只是對目前shared latent線性不可分。 | **ACTIVE / IMPLEMENTED / RESULT_PENDING** | 固定MR-13T相同raw `300×10`、InceptionTime trunk、Raw Safety/Raw-MFE heads、S/U/H targets、Seed42、split、optimizer、full-list Delta-NDCG、三head等權loss與Raw-MFE Validation epoch selection；唯一scientific change為Direct H `Linear(128,2) → Linear(128,128)→ReLU→Linear(128,2)`。不掃hidden width/depth/activation/dropout/lambda，不加人工input，不用breakout/strategy state，不做OOS fitting。 | Primary Gate仍使用既有Model-specific Extension比較Direct H vs same-model Safety×Raw-MFE product。若Forward Pair/PR-AUC/Top10 enrichment相對MR-13T明顯提高且Breakout同方向、marginal S/U未崩，支持joint readout capacity blocker；若FAIL，停止head arithmetic/capacity微調，下一步才研究learned temporal pooling／raw-data joint representation architecture。 | 執行`[1] 訓練目前模型 → Forward-OOS標準Model SOP`；只做Seed42 frozen Model Gate，不建PIT/Strategy arm。 |
