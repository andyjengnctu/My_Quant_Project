...
|---:|---|---|---|---|---|
| 1 | **Extending-Window Rolling Fast Test**：用完整2016～2025 OOS但只做兩個60M folds，快速判斷active模型／策略是否值得投入Overnight。 | **ACTIVE / USER_PRIORITY** | cadence=`60M`、anchor=`2016-01-01`，folds固定=`2016～2020 / 2021～2025`；每fold仍從score_start前合法成熟歷史重新選epoch/refit。Fast不得promotion。Current strategy matrix=`C61/C58/C59/C60`。 | 任一fold/model Gate或策略轉化明顯失敗即可STOP；兩段都保有決策價值才進Overnight。 | 先以Fast完成current candidate最小必要Rolling證據。 |
| 2 | **Extending-Window Rolling Overnight Test**：完整12M annual refit evidence。 | **CONDITIONAL / AFTER_FAST** | period=`2016～2025`、10 folds、expanding history、12M cadence；既有合法annual folds/results沿原canonical paths REUSE。P2/P4 Extending schedules保持PIT-safe，不因mode重做optimizer。 | 只有Fast通過且完整年度穩定性仍可能改變GO/REJECT/promotion時執行；最終promotion只看Overnight。 | Fast GO後補足10-fold evidence。 |
| 3 | **Fixed-Window Rolling Stability Test**：控制歷史長度後檢查MR-13E/K/M跨年代learnability。 | **PLANNED / AFTER_EXTENDING_FAST** | fixed train window=`120M`、inner validation=`24M`；Fast同樣60M兩fold，必要時才做Overnight 12M十fold；輸出與Extending隔離。 | 若Fast已足以顯示固定window下結構差異，再決定是否需要Overnight；不作production performance Truth。 | Extending Fast之後按最小必要證據執行。 |
| 4 | **Extending-Window Rolling C60 Multi-seed Robustness Test**。 | **CONDITIONAL** | fixed=`C58`、stochastic=`C60`、seed_count=`4`、generator=`20260810`；Fast先用2 folds×4 seeds，Overnight才是10 folds×4 seeds。Fast/Overnight使用隔離的robustness/model-work roots；legacy robustness不補跑。 | 只有Seed42與Fast strategy evidence仍足以改變B2 GO/REJECT或promotion判斷才執行；Fast robustness若已失敗就不跑Overnight。 | 先Fast robustness；只有結果仍需要完整seed evidence才補Overnight。 |
