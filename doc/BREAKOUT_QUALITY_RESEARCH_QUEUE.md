...
| 4 | **Extending-Window Multi-seed Robustness Test**。 | **CONDITIONAL** | 與single-seed共用`extending_current` Compare Suite=`C61/C58/C59/C60`；fixed/stochastic不另寫arm清單，由model dependency自動解析為fixed C61/C58、stochastic C59/C60。seed_count=`4`、generator=`20260810`；OOS先用single 2021→latest fold×4 seeds，Rolling才是10 annual folds×4 seeds。C59每seed訓MR-13E；C60每seed同seed訓K+M。 | 只有Seed42與OOS strategy evidence仍可能改變GO/REJECT或promotion判斷才執行；OOS robustness若已足以否決就不跑Rolling。 | 同一四arm matrix先OOS robustness；必要才補Rolling。 |
