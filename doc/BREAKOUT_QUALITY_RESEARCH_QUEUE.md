...
|---:|---|---|---|---|---|
| 1 | **MR-13U Raw-data Direct HM/HS H-only Model Gate**：MR-13T Direct H在Daily Forward只有弱增量且Breakout方向反轉，先隔離Safety/MFE multi-task gradients，確認raw 300×10 + current InceptionTime本身能否直接學H。 | **ACTIVE / IMPLEMENTED / RESULT_PENDING** | 固定MR-13T相同raw `300×10`、InceptionTime trunk超參數、daily-universal universe、H=`1[S>=0.5 AND U>=0.5]`、Seed42、split、optimizer與full-list Delta-NDCG；唯一scientific change是移除Safety/MFE heads與loss，只保留Direct H single-head。Epoch selection=Validation HM/HS Pair，完全同值才以Validation global PR-AUC tie-break；不加人工input、不用breakout/strategy state、不做OOS fitting。 | 若Forward OOS與Breakout的Pair/PR-AUC/Top10/20 HM/HS enrichment都明顯高於MR-13T Direct H，支持multi-task interference，下一步才研究dedicated joint representation；若仍接近MR-13T弱水準且Breakout無增益，支持current raw representation/architecture不足，才重開raw-data joint architecture comparison。 | 執行`[1] 訓練目前模型 → Forward-OOS標準Model SOP`；只做Seed42 frozen H-only Gate，不建PIT/Strategy arm。 |
