...
| 4 | **Extending-Window Multi-seed Robustness Test**。 | **CONDITIONAL** | 與single-seed共用`extending_current` Compare Suite=`C61/C58/C59/C60`；fixed/stochastic不另寫arm清單，由model dependency自動解析為fixed C61/C58、stochastic C59/C60。seed_count=`4`、generator=`20260810`；OOS先用single 2021→latest fold×4 seeds，Rolling使用2021→latest的12M年度fold×4 seeds，fold數依latest合法日期動態決定。策略參數不隨model seed重訓，所有seed都REUSE同一Optimizer canonical family/mode artifact；C59每seed只重訓MR-13E，C60每seed同seed只重訓K+M。 | 只有Seed42與OOS strategy evidence仍可能改變GO/REJECT或promotion判斷才執行；OOS robustness若已足以否決就不跑Rolling。 | 同一四arm matrix先OOS robustness；必要才補Rolling。 |
