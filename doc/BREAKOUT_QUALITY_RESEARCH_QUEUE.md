...
| 2 | **Evaluation Framework Migration / Extending-Window Rolling**：建立2016→2025單一expanding、annual PIT-safe refit chain，取代active Selection/Frozen二分。 | **ACTIVE / USER_PRIORITY** | 舊PIT folds保留historical evidence；因解除2011 sample cutoff已改變training-data semantics，新Extending-Window從2016起依完整合法歷史重建，之後逐年延伸。Current strategy matrix固定保留Full/Min兩個DL-off策略體系基準：`C61 Full ROOS / C58 Min ROOS / C59 MR-13E / C60 MR-13K+MR-13M`。`P2_EXTENDING`與`P4_EXTENDING`都只stitch既有合法historical/current rolling schedules，不重做跨期optimizer；production仍C42/C44。Execution維持單fold串行；complete-host prefetch與2-fold同GPU process parallelism均已因實機更慢而退役，current feeding保留feature-only ordered prefetch。後續execution改善先用模型訓練`Timing Mode`建立改善前baseline，再以相同fold/seed做candidate A/B；只有selected epoch、model-state與scores等exact-result guard全數一致的改善才可升成current default。2025 MR-13K/Seed42 baseline=`10:10.2`；Candidate 1=`pairwise_sync_consolidation_v1`實機=`10:06.1 / 1.007x / -0.68%`且bitwise exact，但改善幅度不足，已`NOT_PROMOTED`；Candidate 2=`pairwise_weight_vector_v2`實機=`09:32.9 / 1.065x / -6.12%`且selected epoch/model-state/PIT scores皆bitwise exact，已`PROMOTED_TO_CANONICAL_EXECUTION`，Timing-only flags已退役。 | Seed42 Extending-Window artifacts與C61/C58/C59/C60能完整跑通且每foldinformation cutoff、parameter schedule、primary/secondary source均PIT-safe；execution candidate必須bitwise exact且wall-clock有淨改善，否則直接REJECT並維持canonical路徑。 | Candidate 2已結案並升為current execution；若要開始下一個獨立execution candidate，先在`[1] 模型訓練 → [7] Timing Mode`明確重建baseline，使baseline代表已promote的current path。 |
