...
|---:|---|---|---|---|---|
| 1 | **SR-C74 Safety × Conditional-MFE Joint Selector**：C71-C73已確認Safety constraint會降低HM/LS、提高High-Safety與平均曝險，但HM/HS只小幅增加，且High-MFE/RoMD隨constraint變強而下降。使用者停止threshold tuning，先測兩個既有MR-13R heads能否透過parameter-free joint ranking把basket直接推向兩者都高。 | **ACTIVE / IMPLEMENTED / RESULT_PENDING** | 固定MR-13R checkpoint/PIT source、Min base-finalist-best、all-off、No-K/No-R0、canonical sizing/cash/orderability/execution與max positions=10。不設Safety gate、不掃threshold、不訓練新模型；每日orderable Raw Safety與Conditional-MFE各自轉average-zero-based percentile，唯一selector score=`Safety_pct × ConditionalMFE_pct`。 | Primary=`C74-C71`；並看C74相對C72/C73是否能在較低HM/LS下保留更多High-MFE、提高HM/HS，且RoMD/Exposure不惡化。若HM/HS沒有實質提升或只是落在C71-C73既有trade-off內，停止selector arithmetic並進Joint-Signal/Capital/Drawdown Audit；若明顯提升，才考慮把joint mechanism作下一個controlled selector base。 | Strategy Compare current suite=`C61/C58/C59/C64/C66/C71/C72/C73/C74`；先跑single-seed OOS+Rolling，不進Multi-seed。 |
| 2 | **Primary-preserving MR-13P runtime condition**：C64 hard safety floor已REJECT；只有Stage Attribution顯示主要瓶頸是resource conversion且MR-13P mechanism仍相關時才回頭測minimum-primary-regret secondary application。 | **CONDITIONAL / AFTER_C74_RESULT** | 固定MR-13P模型與dual-head PIT，不改Target/architecture/loss；不做score fusion、固定權重、OOS-derived threshold或basket-wide safety floor。 | 只有C71加入Raw Safety gate後仍無法同時保留High-MFE與改善Safety/path conversion、且MR-13P mechanism仍可能改變決策時才GO；仍以`+2R前初始Stop↓`與Full-MFE preservation為主。 | C71 OOS+Rolling後再決定，不平行執行。 |
| 3 | **Conditional model formulation後續**：只有Stage Attribution顯示Raw Top-K本身已明顯缺乏High-MFE，才設計下一個單一conditional變體。 | **CONDITIONAL / AFTER_C74_RESULT** | 保持daily-universal strategy-agnostic、一次只改一個conditional mechanism；禁止掃權重／threshold或回到raw scalar MFE-adverse composite。 | 必須由C71顯示同一MR-13R Raw Safety auxiliary直接gate後仍無法把upside轉成安全EV，才允許新增conditional model formulation；否則停止擴張。 | C71 OOS+Rolling後做。 |
...
| 7 | **Extending-Window Multi-seed Robustness Test**。 | **CONDITIONAL / AFTER_BETTER_CONVERSION_CANDIDATE** | 與single-seed共用`extending_current` Compare Suite與same-seed SSOT。 | 只有C74完成single-seed OOS+Rolling且joint selector顯示有決策價值後才投入。 | 延後。 |
