...
|---:|---|---|---|---|---|
| 1 | **Representation-level controlled experiment**：既有300×OHLCV/benchmark representation是否只抓到Safety與MFE的近鏡像predictable component，而漏掉真實存在的joint upper-right資訊？ | **PLANNED / ID_NOT_ASSIGNED** | 以MR-13S為control，固定S/U targets、daily-universal universe、Seed42、split、full-list Delta-NDCG、duo-head與epoch-selection；下一輪一次只改一個strategy-agnostic causal input-representation mechanism，不使用OOS結果fit scalar weight/threshold/calibration。 | 必須保留marginal Safety/MFE learnability，同時讓predicted Safety↔MFE relation明顯脫離MR-13S極端反向並恢復S5×M5、S4+×M4+實質support；Forward OOS仍只作frozen Gate。若representation change只提高marginal rho但joint geometry仍collapse，STOP該mechanism。 | Research SOP/UI engineering refactor完成後再定義最小representation experiment；未實作前不預占新MR。 |
...
- 2026-08-26：在下一個model改善前先完成Research UI/SOP engineering refactor：不隱藏選單；Model訓練後一鍵固定SOP；Strategy OOS/Rolling維持分開並新增同層one-click consistency；執行前直接顯示plan、不設獨立settings/artifact submenu；Audit分四個Reusable families與concrete One-time AUD IDs。此工程不改research priority或scientific semantics。
