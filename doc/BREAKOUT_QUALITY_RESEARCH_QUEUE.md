...
|---:|---|---|---|---|---|
| 1 | **SR-C71 MR-13R Raw-Safety Gate No-K / No-R0**：C70已證明No-K/No-R0能拉高Conditional-MFE/Full-MFE但主要轉成HM/LS；現在只新增同一MR-13R Raw Safety eligibility，直接測absolute Safety head能否把upside從HM/LS移到HM/HS。 | **ACTIVE / IMPLEMENTED / RESULT_PENDING** | C71與C70共用同一MR-13R checkpoint/PIT source、final Conditional-MFE model_score、Min base-finalist-best、all-off、No-K/No-R0、canonical sizing/cash/orderability/execution與max positions=10。唯一change=`raw_safety_score`同日**orderable candidate cross-section** average-zero-based percentile `>=0.50`才eligible；eligible內primary rank完全沿C70。不加Conditional-MFE threshold、不改R0/K、不重訓模型。MR-13R PIT新增Raw Safety欄位只可用既有fold checkpoint rescore。 | 先跑single-seed OOS + Rolling。Mechanism優先：C70 HM/LS OOS/Rolling=`42.58/40.26%`必須顯著下降，HM/HS與High-Safety應提高，High-MFE/Full-MFE不能回落到失去upside；Adverse/Realized EV應改善。若Safety改善但Exposure更低，另判斷是否仍需資金部署constraint；最後才看Return/MDD/RoMD。 | Strategy Compare current suite=`C61/C58/C59/C64/C66/C70/C71`；Primary=`C71-C70`。不進multi-seed直到OOS+Rolling足以決策。 |
...
| 7 | **Extending-Window Multi-seed Robustness Test**。 | **CONDITIONAL / AFTER_BETTER_CONVERSION_CANDIDATE** | 與single-seed共用`extending_current` Compare Suite與same-seed SSOT。 | 只有C71（或後續candidate）完成single-seed OOS+Rolling且仍值得驗證穩健性時才投入。 | 延後。 |
