...
| 4 | **Extending-Window Multi-seed Robustness Test：end-to-end benchmark alignment**。 | **ACTIVE / ROUND1_CONTRACT_DONE / ROUND2_PENDING** | 與single-seed共用`extending_current`六arm Compare Suite。唯一benchmark題庫由`config/training_policy.py`管理：`benchmark_id=end_to_end_v1`、4 seeds、generator=`20260810`、resolved=`693545351/2014432738/104504529/762252050`、strategy trials=`300/fold`。C61/C58/C59/C60全部使用per-seed benchmark strategy params；C59/C60再以同一seed訓MR-13E或K+M；C62/C63只作production finalists-agree consensus reference。OOS/ Rolling同seed的2021初始param必須完全相同，OOS freeze、Rolling 2022+ annual refit。Round1 execution刻意BLOCKED，禁止fallback回Seed42 production params。 | Round2只有在benchmark strategy artifacts與same-seed model artifacts全部identity對齊後才允許replay；每seed先算C61-C58、C59-C58、C59-C61、C60-C58、C60-C61、C60-C59 paired deltas，再跨4 seeds aggregate。OOS已足以否決則不跑Rolling。 | **Round2：由canonical Optimizer建立4 seeds×Full/Min benchmark params（300 trials/fold），同seed建立E/K/M模型後replay；不得覆寫production params。** |
