...
| 4 | **Extending-Window Multi-seed Robustness Test：end-to-end benchmark alignment**。 | **ACTIVE / RESEARCH_ORCHESTRATION_FIX_IMPLEMENTED / FORMAL_RERUN_PENDING / EVIDENCE_PENDING** | 與single-seed共用`extending_current`六arm Compare Suite。唯一benchmark題庫由`config/training_policy.py`管理：`benchmark_id=end_to_end_v1`；seed count／generator／resolved sequence均只由該config當前值解析，Research Queue不得複製目前數值；題庫只擁有seed identity，strategy trials/window/cadence/parallel全部直接繼承canonical Optimizer；`trials/fold`只由`OPTIMIZER_OUTER_ROLLING_OOS_TRIALS_DEFAULT`即時解析，Research Queue不保存目前數值。C61/C62/C58/C63/C59/C60六個arms全部使用per-seed benchmark strategy params；C62/C63是同seedOptimizer search發布的`base-finalists-agree` policy arms，不是固定production reference；C59/C60再以同一seed訓MR-13E或K+M。每seed/family只建立一份`2021→latest`完整benchmark strategy schedule JSON；OOS/Rolling共讀同一實體工件，OOS只在replay讀取時freeze 2021合法member，Rolling按effective date使用完整schedule，禁止2021-only參數訓練或2022+ tail stitch。模型端使用benchmark-only fitting-identity checkpoint cache：E/K/M同seed的任意OOS／Rolling fold只要fitting contract完全一致就使用同一checkpoint SHA，各mode只依自己的score horizon重評score；cache以fitting identity定址並保留所有合法fold，不存在2021／initial特例。per-mode checkpoint/scores可照retention清除，但shared fitting cache保留；仍禁止fallback回Seed42 production params。 | Runtime只有在benchmark strategy artifacts與same-seed model artifacts全部identity對齊後才允許該seed replay；每seed對`C62-C61 / C63-C58 / C62-C63 / C61-C58 / C59-C58 / C59-C61 / C60-C58 / C60-C61 / C60-C59`九個suite contrasts全部作same-seed paired deltas，再跨configured benchmark seeds aggregate。strategy schedule一次交給canonical Rolling Optimizer建立全部年度fold並沿用其parallel policy；shared model cache若fitting identity不符就不得重用，identity相同但checkpoint SHA不同直接FAIL。OOS已足以否決則不跑Rolling。 | **先執行OOS benchmark長跑取得current config-defined seed evidence；若結果仍可能改變決策，再執行Rolling。Rolling應REUSE同一完整strategy schedule，模型fold則逐fold依fitting identity REUSE既有shared checkpoint；只有cache缺少的fitting identities才新增訓練。** |
