...
|---:|---|---|---|---|---|
| 1 | **MR-13X Joint-Min Learned Temporal Attention Pooling Gate**：MR-13W的`min(S,U)`在Validation可學但Forward/Breakout幾乎失效，先測Joint branch是否因global mean pooling丟失局部path information；只改pooling，不同時換整套backbone。 | **ACTIVE / IMPLEMENTED / RESULT_PENDING** | 固定MR-13W raw `300×10`、InceptionTime trunk、S/U/Jmin targets、Safety/Raw-MFE global-average heads、128→128→2 ReLU joint MLP、Seed42、split、optimizer、full-list Delta-NDCG、三head等權loss與Raw-MFE Validation epoch selection。唯一scientific change：Joint branch改為`1×1 scalar attention scorer → softmax(time) → weighted temporal sum`。不加positional encoding、多頭attention、temperature、dropout或attention hyperparameter sweep。 | Primary Gate沿用Joint-Min Extension。相對MR-13W，Forward Joint-Min rho/Pair與Top10/20 Min需明顯改善，Breakout Top10 Min必須高於Pop=`0.3267`且Pair離開random；Top10 Safety/MFE兩邊須維持joint floor，marginal Safety/MFE不可崩。若FAIL，停止InceptionTime head/pooling局部修補，下一步進ModernTCN／Patch Transformer raw-data architecture comparison；不再試另一種attention pooling小變體。 | 執行`[1] 訓練目前模型 → Forward-OOS標準Model SOP`；只做Seed42 frozen representation Gate，不建PIT/Strategy arm。 |
