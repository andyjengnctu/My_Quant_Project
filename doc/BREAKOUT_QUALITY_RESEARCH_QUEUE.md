...
|---:|---|---|---|---|---|
| 1 | **MR-13W Continuous Joint-Min Target Model Gate**：binary HM/HS把剛過中線與深右上樣本視為同一positive；在改temporal representation前，先用同一MR-13V architecture直接學`min(S,U)`，測target information loss是否才是Top-tail轉換不足的主因。 | **ACTIVE / IMPLEMENTED / RESULT_PENDING** | 固定MR-13V raw `300×10`、InceptionTime trunk、Raw Safety/Raw-MFE heads、128→128→2 ReLU joint head、Seed42、split、optimizer、full-list Delta-NDCG、三head等權loss與Raw-MFE Validation epoch selection；唯一fitting change為joint target `H=1[S>=0.5,U>=0.5] → Jmin=min(S,U)`。不得對min rerank、不得加0.5 threshold/soft-min/product/harmonic mean、不得用OOS挑aggregation。 | Primary Gate看Forward/Breakout Joint-Min Dailyρ/Pair與Top10/20 actual min；Top10 mean Safety、mean MFE都應維持高floor，binary HM/HS enrichment只作interpretation。若GO，支持binary target information loss是主要blocker並保留raw InceptionTime；若FAIL，停止target aggregation微調，下一步進learned temporal pooling／raw-data joint representation。 | 執行`[1] 訓練目前模型 → Forward-OOS標準Model SOP`；只做Seed42 frozen target Gate，不建PIT/Strategy arm。 |
