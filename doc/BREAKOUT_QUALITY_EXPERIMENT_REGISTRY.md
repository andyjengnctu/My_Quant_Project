...
| Current model research | `MR-13E` | Daily Universal Full-list Delta-NDCG-weighted Pairwise Ranker / `daily_universal_no_time_full_list_ndcg_pairwise`；固定MR-13A全部daily-universal data／Target／architecture／optimizer／epoch-selection，唯一scientific change為同日RankNet pair依目前預測完整榜單交換造成的raw-percentile `abs(Delta NDCG)`加權 | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_AUTHORIZED／NO_RUNTIME_DL_SOURCE。Seed42 selected epoch=`1`；Forward all-stock daily/global rho=`0.2151/0.1042`、pair=`57.42%`、Top-bottom spread=`+0.8234R`、Top-K Lift=`+0.9951R`、Boundary gap=`+0.0376R`；breakout daily/global rho=`0.1721/0.1535`、pair=`57.43%`、Top-bottom spread=`+1.4294R`、Top-K Lift=`+0.4489R`、Boundary gap=`+0.1961R`。13E同時提升13系列最高Daily rho／Pair，all-stock Top-K接近13A且breakout Top-K高於13A/13C/13D；但global rho與部分boundary仍未全面勝出，故只授權與MR-13C／MR-13D使用同一Selection PIT contract比較，不建立`DL-CONT13E`／`SR-*`。`MR-12B / DL-CONT12B`仍為CURRENT_RUNTIME_ANCHOR。 |
| Daily DL research infrastructure | `ContinuousRankerResearchSpec` / profile-driven dispatcher | Continuous experiment profile只描述scientific profile；額外research metadata集中保存`model_research_id / trainer_family / score_semantic_id / pairwise_reduction`，event與daily trainer共享learning API但各自擁有sample-universe orchestration | IMPLEMENTED／MR13C-D-E_PIT_BATCH_READY／PROFILE_DRIVEN。MR-12B與MR-13A固定`equal_pair_weight`；MR-13B歷史gap weighting已淘汰；MR-13C重用canonical percentile-MSE；MR-13D=`upper_tail_relevance_weighted`；MR-13E=`full_list_delta_ndcg_weighted`。13C/13D/13E均已通過Seed42 Forward Model Gate，模型研究選單可用同一Dataset／Target／Seed／period／fold contract依序建立各自PIT工件並並列表現；不建立人工總分、不挑seed。Strategy arm仍只綁一個`dl_id`；PIT結果前不建立新的`DL-*`或`SR-C*`。 |
...
| `MR-13C` | Daily Universal Percentile Regression / `daily_universal_no_time_percentile_mse` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_AUTHORIZED／NO_RUNTIME_DL_SOURCE。Seed42：selected epoch=`1`；Forward all-stock daily/global rho=`0.1877/0.1239`、pair=`56.38%`、Top-bottom spread=`+0.6886R`、Top-K Lift=`+0.3870R`；breakout daily/global rho=`0.1565/0.1509`、pair=`56.69%`、Top-bottom spread=`+1.4150R`、Top-K Lift=`+0.3735R`。相較MR-13A，daily rho略升但global/top-tail concentration下降，因此不是全面勝出；因全市場與breakout仍保有明確正向OOS排序訊號，授權進Selection PIT Model Gate以檢查MR-13A曾失敗的跨期穩定性。PIT結果前不得建立`DL-CONT13C`或任何`SR-C*`。 |
| `MR-13D` | Daily Universal Upper-tail Relevance-weighted Pairwise Ranker / `daily_universal_no_time_upper_tail_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_AUTHORIZED／NO_RUNTIME_DL_SOURCE。Seed42 selected epoch=`1`；Forward all-stock daily/global rho=`0.1690/0.1453`、pair=`55.81%`、Top-bottom spread=`+1.1685R`、Top-K Lift=`+0.7986R`、Boundary gap=`+0.0283R`；breakout daily/global rho=`0.1529/0.1576`、pair=`55.69%`、Top-bottom spread=`+1.3050R`、Top-K Lift=`+0.3298R`、Boundary gap=`+0.0307R`。相較MR-13C，all-stock Top-K／boundary明顯回升但daily rho下降，breakout tail沒有同步改善；屬正向且互補trade-off，授權進Selection PIT但不promotion。 |
| `MR-13E` | Daily Universal Full-list Delta-NDCG-weighted Pairwise Ranker / `daily_universal_no_time_full_list_ndcg_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_AUTHORIZED／NO_RUNTIME_DL_SOURCE。Seed42 selected epoch=`1`；Forward all-stock daily/global rho=`0.2151/0.1042`、pair=`57.42%`、Top-bottom spread=`+0.8234R`、Top-K Lift=`+0.9951R`、Boundary gap=`+0.0376R`；breakout daily/global rho=`0.1721/0.1535`、pair=`57.43%`、Top-bottom spread=`+1.4294R`、Top-K Lift=`+0.4489R`、Boundary gap=`+0.1961R`。這是目前13系列最均衡的Forward候選，但global rho／boundary並未全面勝出13A/13C，故只授權與13C/13D一起做Selection PIT Model Gate，不promotion、不建立`DL-CONT13E`／`SR-C*`。 |
