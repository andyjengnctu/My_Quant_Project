...
| Current model research | `MR-12B` | No-time all-event within-day Pairwise RankNet ranker；固定all-event Target／InceptionTime，以同日pairwise logistic直接學cross-sectional ordering | RESULT_AVAILABLE／MODEL_ECONOMIC_IMPROVEMENT_SUPPORTED；C19/C20 controlled replay均支持；P2 Fixed-K OOS亦顯示MR-12B較MR-12A改善NDCG／Top-K lift／boundary。Dynamic-K修正score-event-date後重跑仍為94/223個common-complete days（42.15%），且`score_event_date != signal_date`為0，證明首輪弱勢不是score-date join造成；在該C17-reference、mean K=1.34、common-complete raw-score子集上MR-12B Boundary=43.94%、B-A=-12.53pp。此段不等同正式max-DL selector basket：正式runtime允許partial-score並套K/R0 repair／feasible-ascent，且C19/C20使用各自path-dependent replay state；故目前保留為LIMITED_DIAGNOSTIC，不否定MR-12B anchor。下一步先做K-stratified與runtime selector/action attribution，再決定新MR。 |
