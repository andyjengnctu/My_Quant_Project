...
| Current model research | `MR-12B` | No-time all-event within-day Pairwise RankNet ranker；固定all-event Target／InceptionTime，以同日pairwise logistic直接學cross-sectional ordering | RESULT_AVAILABLE／PIT_MODEL_VALIDATION_PASS／MODEL_ECONOMIC_IMPROVEMENT_SUPPORTED；Selection PIT 2011-01-01～2020-12-31共10 folds、23,932/23,932 groups、coverage `100%`，PASS-only global rho=`0.2336`、mean daily rho=`0.1859`、top-bottom spread=`1.1386R`；年度rho>0=`10/10`、spread>0=`9/10`，Gate=`PASS`。此audit是MR-12B單模型絕對Gate，證明歷史PIT排序訊號穩定為正，但**不等同MR-12B在每個PIT fold都優於MR-12A**；B-A比較仍需paired PIT報表才可回答。`drift=True`只表示至少一組相鄰fold的score平均值位移達`1.0 pooled SD`，不是ranking方向失敗；策略層仍須用Selection PIT scores做無前視經濟驗證。Forward-OOS與C19/C20 controlled replay同樣支持MR-12B；P2 cross-score-date mismatch已排除，原始event-target Dynamic-K不得用來修改Pairwise loss。下一步進入Selection PIT策略績效驗證；若需解釋portfolio action差異，使用trade-date canonical counterfactual/realized outcome。 |
...
| Current Selection PIT strategy source | `DL-CONT12B-PIT` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` Selection point-in-time score | IMPLEMENTED／STRATEGY_RESULT_PENDING；runtime只重用已通過PIT Gate的2011～2020 score，正式經濟驗證固定2014～2020 historical P2 Min ROOS，Strategy Compare不得自動重訓PIT模型 |
...
| `DL-CONT12B-PIT` | `CONT12B_PIT` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` | Selection point-in-time continuous score；每個歷史日期只使用當時合法fold模型輸出；Strategy Compare只讀score／manifest／audit且要求PIT model Gate PASS，不得自動訓練或重建 | IMPLEMENTED／STRATEGY_RESULT_PENDING |
...
| `SR-C23` | Selection Min ROOS PIT baseline | 固定2014～2020 historical A2-teacher P2 Min ROOS active params、rules全關、DL off；作Selection PIT策略經濟驗證共同baseline | IMPLEMENTED／RESULT_PENDING；只重用既有lookahead-safe P2 historical active params，不觸發optimizer |
| `SR-C24` | Selection PIT: MR-12B minimum-repair | 與SR-C23完全相同historical P2 params；唯一新增為`DL-CONT12B-PIT`，selector完全沿用SR-C17 minimum-repair K/R0 contract | IMPLEMENTED／RESULT_PENDING；固定score source=`selection_point_in_time`，Strategy Compare不得重訓模型 |
| `SR-C25` | Selection PIT: MR-12B feasible-ascent | 與SR-C23完全相同historical P2 params；使用`DL-CONT12B-PIT`並完全沿用SR-C18 feasible-ascent K/R0 contract | IMPLEMENTED／RESULT_PENDING；用於檢查PIT ranking能否在較Max selector轉化成portfolio economics |
