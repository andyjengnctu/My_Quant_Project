...
| Current strategy research arm | `SR-C17` | Max-DL constrained basket + `DL-CONT12A` frozen continuous score | IMPLEMENTED／RESULT_PENDING；Min ROOS只提供同日預留單數K與exact reserved-capital floor，basket membership只由DL score決定；純DL Top-K不合法時才作deterministic minimum-repair，basket內執行順序維持Min ROOS rank |
| 最新策略結果 | `SR-C16` | Capital-preserving + `DL-CONT12A` all-event continuous score | RESULT_AVAILABLE／RESOURCE_CONTRACT_PASSED／NOT_PROMOTED；2021-01-01～2025-12-22 Return=175.15%、MDD=15.86%、RoMD=11.05、EV=0.48R；相對C15 Return +6.46pp但MDD +1.04pp、RoMD -0.34、EV -0.15R。盤前DL-selection days=270、selector changed days=179、selected-count delta=+70、reserved-capital delta=+1,361,933、resource violation days=0，證明資源保護機制成立但不升格策略基準 |
...
| `SR-C17` | Min ROOS: All-event Continuous max-DL constrained basket | 固定`PARAM-P2 / DL-CONT12A / MR-12A`；Min ROOS只建立同日K與reserved-capital floor，DL score決定K-stock basket membership；純DL Top-K若不合法，以最多K次deterministic minimum-repair替換Top-K成員；basket內正式執行順序沿用Min ROOS rank，且實際盤前只允許K筆預留單 | IMPLEMENTED／RESULT_PENDING；唯一新變數為selector，正式比較收斂為C3／C16／C17與C17-C16／C17-C3；不得依forward-OOS結果再調repair規則或新增numeric selector參數 |
...
17. `SR-C16`正式比較矩陣只保留`C3 / C15 / C16`，核心contrast為`C16-C15`（隔離resource-preserving selector本身）與`C16-C3`（相對Min ROOS基準）；結果取得前狀態固定`IMPLEMENTED / RESULT_PENDING`。
