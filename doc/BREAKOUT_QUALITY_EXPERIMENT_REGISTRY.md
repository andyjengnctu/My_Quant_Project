...
| 最新已完成Audit結果 | `AUD-c23-c25-pit-realization` | `SR-C23` vs `SR-C24 / SR-C25` Selection PIT realization/capture attribution | RESULT_AVAILABLE／REALIZATION_GAP_CONFIRMED／PARAM_ADAPTATION_NOT_SUPPORTED；C24/C25 exclusive selection R=`-35.78R/-19.52R`，主要損失為winner capture不足；fill/sizing/holding/exposure未達mechanical-gap門檻，下一個`AUD-c23-c25-pit-fold-runtime`已實作待結果 |
...
| `AUD-c23-c25-pit-fold-runtime` | `c23-c25-pit-fold-runtime` | `SR-C23` vs `SR-C24 / SR-C25` + `DL-CONT12B-PIT` fold identity | IMPLEMENTED／RESULT_PENDING | Read-only檢查orderable pool是否混合不同PIT fold model score，並將exclusive selection R／winner capture拆成single-fold vs mixed-fold及fold transition window；不得做score calibration或據此修改OOS模型／selector |
...
25. `AUD-c23-c25-pit-fold-runtime`已實作、結果待執行；它只讀C23/C24/C25 completed replay與合法PIT score/audit，量測mixed-fold days、cross-fold pair share、fold boundary ±config days及exclusive winner/loser R集中度。只有實際損失明顯集中於mixed-fold／fold transition，才有資格形成Selection-only normalization/runtime假說；否則回到Target／realized outcome語意研究。
