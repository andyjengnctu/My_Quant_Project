...
| Current model research | `MR-13B` | Daily Universal Target-gap-weighted Pairwise Ranker / `daily_universal_no_time_pairwise_gap_weighted`；沿用MR-13A全部daily-universal sample／target／architecture／optimizer／epoch-selection semantics，唯一learning變更為同日RankNet pair依`abs(daily target percentile差)`加權、date內正規化，降低近似target pair對梯度的稀釋 | IMPLEMENTED／AWAITING_MODEL_GATE／NO_RUNTIME_DL_SOURCE。MR-13B尚未取得任何Selection/OOS結果，不得預先判定有效。原先候選的單純equal-date reduction在實作前重新檢查`_date_coherent_batches()`後判定接近no-op：Daily Universal每日期常大於batch size，optimizer step本來就多為完整單日；因此不占用MR-13B於近乎無效變更，改以無magic threshold的target-rank-distance weighting作唯一scientific variable。`MR-12B / DL-CONT12B`仍為CURRENT_RUNTIME_ANCHOR，MR-13A歷史結果與NOT_PROMOTED狀態保留。 |
| Daily DL research infrastructure | `ContinuousRankerResearchSpec` / profile-driven dispatcher | Continuous experiment profile只描述scientific profile；額外research metadata集中保存`model_research_id / trainer_family / score_semantic_id / pairwise_reduction`，event與daily trainer共享learning API但各自擁有sample-universe orchestration | IMPLEMENTED／MR13B_PROFILE_READY／FORMAL_CONTRACT_FIX_APPLIED／AWAITING_FORMAL_RERUN。MR-12B與MR-13A固定`equal_pair_weight`不變；MR-13B由獨立profile明確選擇`target_gap_weighted_within_date`，未複製trainer。Strategy arm仍只綁一個`dl_id`；MR-13B model Gate前不建立新的`DL-*`或`SR-C*`，多DL融合留待有第二score role與fusion semantics後再研究。 |
...
| `MR-13B` | Daily Universal Target-gap-weighted Pairwise Ranker / `daily_universal_no_time_pairwise_gap_weighted` | IMPLEMENTED／AWAITING_MODEL_GATE／NO_RUNTIME_DL_SOURCE。固定MR-13A的daily eligible stock-day universe、`daily_opportunity_no_time_r_v1`、300×10 sequence、`ARCH-inception_time_v1`、Adam/LR/weight decay、inner validation、selected-epoch final refit與`mean_daily_spearman` epoch metric；唯一變更為RankNet同日non-tied pair loss依`abs(daily target percentile差)`加權並於date內正規化，不使用固定gap threshold。目標是降低daily-universal大量近似排序pair的supervision稀釋；結果前不得建立DL runtime source或策略arm。 |

`MR-13A`、`MR-13B` 已正式占用。後續daily-universal target、loss、architecture或training-data semantics若再變更，必須使用新的 `MR-*`，不得覆寫13A/13B。
