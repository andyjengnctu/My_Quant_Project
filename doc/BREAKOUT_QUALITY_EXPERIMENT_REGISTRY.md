...
| 最新已完成Audit結果 | `AUD-c23-c25-pit-fold-runtime` | `SR-C23` vs `SR-C24 / SR-C25` Selection PIT fold/runtime attribution | RESULT_AVAILABLE／FOLD_DRIFT_NOT_PRIMARY_CAUSE；C24 mixed-fold=`+13.91R`、single-fold=`-46.05R`、boundary outside=`-37.95R`；C25 mixed-fold=`-4.25R`、single-fold=`-10.65R`、boundary outside=`-24.45R`，負R未集中於mixed-fold或fold transition；下一個`AUD-c23-c25-pit-target-realization`已實作待結果 |
...
| `AUD-c23-c25-pit-target-realization` | `c23-c25-pit-target-realization` | `SR-C23` vs `SR-C24 / SR-C25` + `DL-CONT12B-PIT` + original event Target | IMPLEMENTED／RESULT_PENDING | 只讀實際成交exclusive trades，對齊PIT score、原始event Target與realized R，並依signal→entry calendar age quantile分層；用來區分event Target隨候選老化失真與Target公式本身對正式trade-path R失配。未成交／未選候選不建立counterfactual R。 |
...
26. `AUD-c23-c25-pit-target-realization`已實作、結果待執行；它只讀實際成交exclusive trades，對齊PIT score、原始event Target與realized R，按signal→entry age quantile拆分Target／Realized／win rate與Selection ΔR。若負R明顯集中高age bucket，才支持event Target老化／延續候選語意失真假說；若各age bucket皆負，則優先判定Target公式本身與正式trade-path R不對齊。不得重啟已停止的11J counterfactual。
