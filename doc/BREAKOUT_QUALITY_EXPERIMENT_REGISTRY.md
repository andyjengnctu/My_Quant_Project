...
| 最新已完成Audit結果 | `AUD-forward-robustness-portfolio-translation` | `SR-C29 / MR-13A` vs `SR-C20 / MR-12B` Forward 8-seed trade-set／risk-dollar／slot／wealth translation attribution | RESULT_AVAILABLE／ALL_SEEDS_COMPLETE／DIRECT_PAIR_R_EDGE_CONFIRMED／WITHIN_SET_WEIGHTING_DILUTION_CONFIRMED／RISK_BINDING_SOURCE_REBUILD_REQUIRED／MR13A_NOT_PROMOTED。schema v4 exact bridge：Exclusive ΔPnL Mean=`+173,208.15`，equal-risk selection effect Mean=`+355,214.76`、average-risk scale effect Mean=`+14,995.80`、within-set weighting effect Mean=`-197,002.42`、uncovered=`0`、residual=`0`；S1/S8的正selection effect分別被within-set weighting `-166,717/-605,665`反轉／大幅侵蝕，S6則由scale `+432,811`與within-set weighting `+377,298`覆蓋負selection。故下一步不是改MR-13A，而是同一Audit schema v5加入entry-execution risk utilization／binding attribution；compact source schema v2新增execution sidecar，需以相同8 seeds／同fingerprint重建Audit evidence後再只讀分析。 |
...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；保留all-stock daily universal方向，feature-eligible ticker/date皆可取得daily score；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet與mean daily Spearman選epoch | RESULT_AVAILABLE／ITERATIVE_RESEARCH_ACTIVE／MATCHED_SELECTION_R_EDGE／DIRECT_PAIR_R_EDGE_CONFIRMED／PORTFOLIO_TRANSLATION_HETEROGENEOUS／WITHIN_SET_WEIGHTING_DILUTION_CONFIRMED／NOT_PROMOTED。8-seed Forward中MR-13A RoMD Mean=`9.97`高於MR-12B=`8.21`但同seed僅4勝4敗；ΔDL選擇R 6勝2敗、Mean=`+50.64R`。全8-seed Portfolio Audit schema v4確認equal-risk selection effect Mean=`+355,214.76`，但within-set weighting effect Mean=`-197,002.42`吃掉大量edge，average-risk scale Mean僅`+14,995.80`；S1/S8為selection正但dollar translation負，S6為selection負但risk allocation反轉成正PnL。下一步固定診斷actual risk utilization與binding mechanism，不得先改Target／loss／architecture／hyperparameter、擴seed、跑Selection PIT multi-seed或依Forward結果直接新增sizing規則。 |
...
| Current continuous DL anchor | `DL-CONT12B` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` frozen OOS continuous score | CURRENT_RUNTIME_ANCHOR／MR13A_MODEL_EDGE_NOT_YET_PORTFOLIO_STABLE。seed 42與8-seed歷史結果保留；MR-13A已取得matched selection/direct-pair trade-set edge，但schema v4顯示within-set risk weighting高度異質，故DL-CONT12B繼續維持canonical runtime anchor，直到entry-execution risk utilization／binding attribution釐清可泛化mechanism；不得以單seed、獨立Mean、selection R、Audit平均PnL或單一描述性相關直接promotion。 |
