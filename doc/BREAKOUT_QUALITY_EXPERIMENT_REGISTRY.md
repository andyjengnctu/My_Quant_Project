...
| Current strategy research arm | `SR-C18` | Max-DL feasible-ascent + `DL-CONT12A` frozen continuous score | IMPLEMENTED／RESULT_PENDING；與C17使用相同K／R0、MR-12A與Min ROOS basket內執行順序；C17合法seed之後持續做best-feasible single-swap DL改善直到1-swap local optimum，capital只作hard feasibility；exact global search因真實候選規模計算時間不可接受而不進production |
