...
| Current model research | `MR-12B` | No-time all-event within-day Pairwise RankNet ranker；固定all-event Target／InceptionTime，以同日pairwise logistic直接學cross-sectional ordering | RESULT_AVAILABLE／MODEL_ECONOMIC_IMPROVEMENT_SUPPORTED；C19/C20 controlled replay均支持；P2 Fixed-K OOS亦顯示MR-12B較MR-12A改善NDCG／Top-K lift／boundary。首輪Dynamic-K因診斷錯用`signal_date`而非runtime保存的`breakout_quality_score_date`對score，coverage僅42.15%，該Dynamic-K數字作廢並待修正後重跑；MR-12B仍維持目前model research anchor |
