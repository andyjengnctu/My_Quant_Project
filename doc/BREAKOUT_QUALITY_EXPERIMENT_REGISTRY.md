...
| Strategy params baseline | `PARAM-P2 / Min ROOS` | rules全關、DL-off；每fold只搜尋`high_len`＋`atr_len/atr_buy_tol/atr_times_init/atr_times_trail`，其餘optimizer維度由canonical config/schema固定；trials直接讀`config/training_policy.py` | ACTIVE strategy parameter baseline／2026-08-09 semantic correction；舊4-ATR且high_len凍結工件只供歷史重現，不得作current Min ROOS cache |
...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；每個feature-eligible ticker/date皆可取得daily score，future target只決定training／audit validity；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet pairwise logistic與mean daily Spearman選epoch | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_PASS／STAGE3_PIT_RUNTIME_READY／SELECTION_STRATEGY_RESULT_PENDING／NOT_PROMOTED_OVER_MR12B。Stage 1 full-market OOS daily rho=0.1755、pair=56.12%；breakout-candidate OOS daily rho=0.1490、pair=56.91%、Top-K Target/Lift=1.6198R/+0.4056R。Stage 2 Selection PIT 2013-04-01～2020-12-31、8 folds、target-valid groups=778,532、coverage=100%；all-stock global/daily rho=0.0783/0.1111、pair=53.84%、spread=+0.3319R，年度rho>0與spread>0均8/8；breakout slice global/daily rho=0.0580/0.0784、pair=53.98%、Top-K Lift=+0.1102R、Boundary=51.49%，PIT模型Gate PASS。Stage 3 preflight確認舊778,532-row PIT table的score presence仍受future-target完整性限制，且舊runtime固定以breakout signal_date查score；兩者不影響Stage 2 model-audit結論，但該舊PIT table不得直接作策略runtime source。已實作feature-history-only score eligibility與daily information-date lookup；2026-08-09已完成8-fold checkpoint重評與PIT Audit，score groups仍為778,532、coverage=100%，模型指標與Stage 2一致，表示本Selection期間沒有新增feature-eligible/target-invalid score rows；row count不作contract Gate。C27/C28現可執行Selection策略比較。Model-research active仍為MR-13A；strategy workflow active仍為MR-12B。 |
...
| Current Selection PIT strategy source | `DL-CONT12B-PIT` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` Selection point-in-time score | RESULT_AVAILABLE／PIT_GATE_PASS／STRATEGY_NOT_PROMOTED；既有C23/C24/C25結果使用已淘汰的4-ATR、high_len凍結P2，僅保留read-only attribution；current 5-field Min ROOS下須與C27/C28同批重跑，取得新結果前不得沿用舊策略經濟結論 |
| Current Selection runtime experiment | `SR-C26` | `SR-C25` feasible-ascent + Selection-frozen 22-calendar-day stale-score membership guard；candidate保留、只禁止stale score驅動membership change | RESULT_AVAILABLE／STALE_GUARD_SUPPORTED／NOT_PROMOTED／RUNTIME_MICROTUNING_STOPPED；C26-C25：Return `+4.32pp`、EV `+0.08R`、same-param selection R `+27.51R`，證明22日guard有效修復C25；但C26-C23仍Return `-7.71pp`、MDD `+1.66pp`、RoMD `-0.59`。後續risk-dollar Audit又確認C26-only risk-weighted R=`0.21R`低於C23-only=`0.27R`，而common `-41.6k` PnL幾乎完全來自risk-size/path effect而非ΔR；沒有可泛化的事前runtime機械原因支持再微調，故不進Forward-OOS、22日維持凍結並結束C26 runtime微調鏈 |
| Pending Selection PIT source | `DL-CONT13A-PIT` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` daily Selection PIT score | RESULT_AVAILABLE／PIT_MODEL_GATE_PASS／STRATEGY_RUNTIME_READY／SELECTION_STRATEGY_RESULT_PENDING／NOT_PROMOTED；策略端只接受`score_eligibility_contract=feature_history_only`的新PIT manifest，每個盤前決策查最新已完成交易日score；舊Stage 2 target-complete-only PIT table會fail-fast拒絕。Strategy Compare只允許以既有合法PIT fold score/checkpoint自動重建scores／manifest／audit，任何fold需要模型訓練時必須停止並導向模型研究入口 |
| Pending Selection strategy translation | `SR-C27 / SR-C28` | C27完全沿用C24 minimum-repair、C28完全沿用C25 feasible-ascent；唯一DL source改為`DL-CONT13A-PIT`，不繼承C26 22-day stale guard | IMPLEMENTED／READY_FOR_SELECTION_COMPARE／RESULT_PENDING；C27-C24與C28-C25隔離純model/source差異，C27/C28-C23檢查portfolio economics，C28-C27檢查selector轉化；取得Selection結果前不得升格MR-13A或進ROOS |
...
| ID | 名稱 | 狀態 |
|---|---|---|
| `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_PASS／STAGE3_PIT_RUNTIME_READY／SELECTION_STRATEGY_RESULT_PENDING。Stage 1：all-stock OOS daily/global rho=0.1755/0.1761、pair=56.12%；breakout slice daily rho=0.1490、pair=56.91%、K=10 raw Target/Lift=1.6198R/+0.4056R、Boundary=54.02%。Stage 2：2013-04-01～2020-12-31、8 folds、778,532 target-valid groups、100% coverage；all-stock PIT global/daily rho=0.0783/0.1111、pair=53.84%、spread=+0.3319R、年度rho/spread正向8/8；breakout slice global/daily rho=0.0580/0.0784、pair=53.98%、Top-K Lift=+0.1102R、Boundary=51.49%，Gate PASS。Stage 3 preflight將score eligibility修正為feature-history-only並將daily runtime lookup改為最新已完成交易日；舊manifest不可直接當strategy source。2026-08-09已完成8-fold checkpoint重評與Audit，score groups仍為778,532且coverage=100%；因score cardinality不必然因contract改變而增加，正式Gate改以manifest contract＋runtime lookup語意為準。`DL-CONT12B / MR-12B`仍為current validated continuous anchor。 |
...
| `SR-C27` | Selection PIT: MR-13A daily minimum-repair | 與SR-C24完全相同historical P2 params、K/R0、minimum-repair selector與execution order；唯一DL source差異為`DL-CONT12B-PIT → DL-CONT13A-PIT`，盤前每日使用最新已完成交易日score | IMPLEMENTED／READY_FOR_SELECTION_COMPARE／RESULT_PENDING；此ID屬MR-13A新model-translation branch，C26結案時「不新增C27」只表示不再延伸C26 runtime微調。C27不繼承C26 22-day stale guard；主對照C27-C24隔離model/source，C27-C23檢查相對DL-off portfolio economics |
| `SR-C28` | Selection PIT: MR-13A daily feasible-ascent | 與SR-C25完全相同historical P2 params、K/R0、feasible-ascent selector與execution order；唯一DL source差異為`DL-CONT12B-PIT → DL-CONT13A-PIT`，盤前每日使用最新已完成交易日score | IMPLEMENTED／READY_FOR_SELECTION_COMPARE／RESULT_PENDING；不繼承C26 22-day stale guard。主對照C28-C25隔離model/source，C28-C23檢查portfolio economics，C28-C27檢查相同daily source下selector轉化 |
