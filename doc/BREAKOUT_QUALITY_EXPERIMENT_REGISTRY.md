...
| `MR-13E` | Daily Universal Full-list Delta-NDCG-weighted Pairwise Ranker / `daily_universal_no_time_full_list_ndcg_pairwise` | RESULT_AVAILABLE／MODEL_GATES_PASS／SELECTION_SINGLE_SEED_STRATEGY_GATE_PASS／FORWARD_SINGLE_SEED_STRATEGY_GATE_FAIL_VS_MR12B／NOT_PROMOTED／RESOURCE_TRANSLATION_GAP_IDENTIFIED／SELECTOR_STAGE_TRANSLATION_AUDIT_AUTHORIZED。模型層Selection/Forward皆維持正edge；策略層Selection C35=`153.96%/RoMD 7.74/DL選擇R +35.96R`，Forward C36=`128.77%/7.49/-66.22R`。`AUD-mr13e-orderable-feasible-alignment`顯示Forward 13E在真正orderable pool的Top-N/Repair Lift=`1.0996R/1.1401R`均勝12B=`0.8223R/0.7975R`，但Repair Action−Raw多損失`0.3673R`；故目前不授權13E 8-seed，先精確拆minimum-repair／ascent／action／fill。 |
...
| `AUD-mr13e-selector-stage-translation` | `mr13e-selector-stage-translation` | `MR-12B / MR-13A / MR-13E` Selection PIT + Forward completed Strategy Compare selector trace + execution sidecars | IMPLEMENTED／READ_ONLY／SOURCE_REPLAY_REFRESH_REQUIRED／AWAITING_RESULT | 正式score-ranking replay新增research-only canonical `score_ranking_selector_trace.csv`，只持久化既有Max-DL selector在同一決策路徑中的`raw_top_n / minimum_repair_seed / feasible_ascent_final` membership；entry action與actual fill沿用canonical execution sidecar。Audit以共同`daily_opportunity_no_time_r_v1`只在post-replay join Future Target，逐層計算Raw→Repair、Repair→Ascent、Ascent→Action、Action→Fill的target delta／score delta／membership overlap，並另看repair days；Forward相對12B最負的額外stage delta直接標為dominant gap，不使用加權總分或人工threshold。Trace sink為optional、不得改final basket／sizing／execution；舊pair缺trace只需一次strategy replay refresh，不重訓模型或重建scores。 |
