...
| Current model research | `MR-13C` | Daily Universal Percentile Regression / `daily_universal_no_time_percentile_mse`；固定MR-13A的daily-universal sample universe、`daily_opportunity_no_time_r_v1`、300×10 input、architecture、optimizer與epoch-selection，唯一scientific change為objective由same-date pairwise ranking改成直接回歸same-date target percentile的MSE | IMPLEMENTED／AWAITING_MODEL_GATE／NO_RUNTIME_DL_SOURCE。MR-13C只重用既有canonical `daily_percentile_regression` training path，不建立第二套trainer；結果前不得建立PIT、`DL-CONT13C`或`SR-*`。MR-13B已於Forward model Gate淘汰且不再微調gap weighting；`MR-12B / DL-CONT12B`仍為CURRENT_RUNTIME_ANCHOR。 |
| Daily DL research infrastructure | `ContinuousRankerResearchSpec` / profile-driven dispatcher | Continuous experiment profile只描述scientific profile；額外research metadata集中保存`model_research_id / trainer_family / score_semantic_id / pairwise_reduction`，event與daily trainer共享learning API但各自擁有sample-universe orchestration | IMPLEMENTED／MR13C_PROFILE_READY／PROFILE_DRIVEN。MR-12B與MR-13A固定`equal_pair_weight`；MR-13B保留歷史`target_gap_weighted_within_date`但已淘汰；MR-13C直接重用既有canonical percentile-MSE objective，不複製trainer。Strategy arm仍只綁一個`dl_id`；MR-13C model Gate前不建立新的`DL-*`或`SR-C*`，多DL融合留待有第二score role與fusion semantics後再研究。 |
...
| `MR-13C` | Daily Universal Percentile Regression / `daily_universal_no_time_percentile_mse` | IMPLEMENTED／AWAITING_MODEL_GATE／NO_RUNTIME_DL_SOURCE。固定MR-13A的daily eligible stock-day universe、`daily_opportunity_no_time_r_v1`、300×10 sequence、`ARCH-inception_time_v1`、Adam/LR/weight decay、inner validation、selected-epoch final refit與`mean_daily_spearman` epoch metric；唯一scientific change為training objective從same-date RankNet pairwise logistic改成直接回歸same-date target percentile的MSE。使用既有canonical percentile-regression training path，不新增architecture、Target、candidate membership或runtime fusion；結果前不得建立PIT／`DL-CONT13C`／`SR-C*`。 |

`MR-13A`、`MR-13B`、`MR-13C` 已正式占用。後續daily-universal target、loss、architecture或training-data semantics若再變更，必須使用新的 `MR-*`，不得覆寫13A/13B/13C。
