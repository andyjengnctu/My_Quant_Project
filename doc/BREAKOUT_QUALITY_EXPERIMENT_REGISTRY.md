...
| `MR-13E` | Daily Universal Full-list Delta-NDCG-weighted Pairwise Ranker / `daily_universal_no_time_full_list_ndcg_pairwise` | RESULT_AVAILABLE／MODEL_GATES_PASS／SELECTION_SINGLE_SEED_STRATEGY_GATE_PASS／FORWARD_SINGLE_SEED_STRATEGY_GATE_FAIL_VS_MR12B／NOT_PROMOTED／ORDERABLE_FEASIBLE_ALIGNMENT_AUDIT_AUTHORIZED。模型層Selection/Forward皆維持正edge；策略層Selection C35=`153.96%/RoMD 7.74/DL選擇R +35.96R`，Forward C36=`128.77%/7.49/-66.22R`。Forward相對C20/MR-12B=`-61.49pp Return/-2.65 RoMD/-102.16R DL選擇R`，故目前不授權13E 8-seed；先用正式read-only Audit區分orderable ranking gap與resource translation gap。 |
...
| `DL-CONT13E` | `CONT13E` | `MR-13E / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_full_list_ndcg_pairwise` | Frozen Forward-OOS daily continuous score；策略盤前使用最新已完成交易日score，runtime score eligibility沿用feature-history-only contract | RESULT_AVAILABLE／FORWARD_SINGLE_SEED_GATE_FAIL_VS_MR12B／NOT_PROMOTED。已完成C36 source-only Forward策略Gate；相對MR-12B明顯落後，source僅保留供read-only alignment Audit與研究，不取代`DL-CONT12B`。 |
...
| `AUD-mr13e-orderable-feasible-alignment` | `mr13e-orderable-feasible-alignment` | `MR-12B / MR-13A / MR-13E` Selection PIT + Forward completed Strategy Compare sidecars | IMPLEMENTED／READ_ONLY／SOURCE_REPLAY_REFRESH_REQUIRED／AWAITING_RESULT。每個trade day只在實際orderable competition set計算score→共同`daily_opportunity_no_time_r_v1`的Daily rho／pair／Top-N lift／oracle retention／top-vs-rest；N直接取當日pre-market order limit/action count，不用固定K。另把direct-feasible／repair／feasible-ascent分層，並以canonical `score_ranking_execution.csv`比較raw score Top-N與實際chosen pre-market action的target/score差。2026-08-14確認舊一般正式score-ranking pair未保證保存execution sidecar；不得以post-fill `score_ranking_selected_buys.csv`代替。正式pair artifact contract現要求execution sidecar，舊cache缺此檔只需對相應DL pair做一次strategy replay refresh，模型／PIT／OOS scores與scientific identity不失效。Future target僅post-replay join；Audit本身仍不train、不score、不replay。 |
