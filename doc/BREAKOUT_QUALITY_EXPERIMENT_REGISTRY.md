...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；保留all-stock daily universal方向，feature-eligible ticker/date皆可取得daily score；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet與mean daily Spearman選epoch | RESULT_AVAILABLE／CROSS_PERIOD_MULTI_SEED_EDGE_NOT_STABLE／FORWARD_SELECTION_EDGE_SUPPORTED／SELECTION_PIT_RELATIVE_EDGE_NOT_CONFIRMED／NOT_PROMOTED。Forward 8-seed：MR-13A vs MR-12B ΔDL選擇R 6勝2敗、Mean=`+50.64R`，ΔRoMD 4勝4敗、Mean=`+1.76`；Selection PIT 8-seed則反轉為ΔDL選擇R 3勝5敗、Mean=`-20.76R`，ΔRoMD 2勝6敗、Mean=`-0.93`。Selection PIT ranking/RoMD方向一致=`7/8`，表示該時段主問題是ranking/selection edge本身反轉，而非Forward已知的portfolio translation heterogeneity。schema v5 Forward binding attribution仍顯示無可泛化的13A-specific sizing constraint。`AUD-cross-period-year-regime-attribution`已完成並確認Selection負edge廣泛分布於5/7年度，不是單一年份集中。長期研究方向維持strategy-agnostic Daily Score，不退回event-only；MR-13B前先完成profile-driven multi-DL trainer metadata／loss-reduction SSOT，再以獨立MR逐一改training semantics，不同scientific variables不得同批混改；MR-12B daily inference只保留可選causal control。 |
| Daily DL research infrastructure | `ContinuousRankerResearchSpec` / profile-driven dispatcher | Continuous experiment profile只描述scientific profile；額外research metadata集中保存`model_research_id / trainer_family / score_semantic_id / pairwise_reduction`，event與daily trainer共享learning API但各自擁有sample-universe orchestration | IMPLEMENTED／PRE_MR13B_ARCHITECTURE_READY／FORMAL_CONTRACT_FIX_APPLIED／AWAITING_FORMAL_RERUN。既有MR-12B與MR-13A仍固定`equal_pair_weight`且profile manifest identity不變；future equal-date等aggregation必須由新的profile／MR明確選擇。2026-08-13首輪formal僅失敗於兩個舊synthetic以trainer source字串判斷CLI profile可用性；已改為驗證`SUPPORTED_CONTINUOUS_RANKER_RESEARCH_PROFILES`＋`ContinuousRankerResearchSpec.trainer_family`＋既有CLI registration，targeted contract全PASS，尚待下一次`apps/run_bundle.py`正式重驗。Strategy arm目前仍只綁一個`dl_id`，多DL融合等真正有第二個score role與fusion semantics時再擴充，避免污染單模型controlled experiment。 |
...
13系列後續採profile-driven trainer：新增Daily MR只新增獨立experiment profile＋research spec並選擇其training reduction／target contract，不再複製`train_daily_ranker.py`。目前尚未正式登記MR-13B；開始實驗時必須再依本Registry取得新ID。多個DL「研究版本並存」已支援，但同一strategy arm同時融合多個DL尚未啟用，因fusion本身屬新的SR scientific variable。
