...
| Current model research | `MR-13D` | Daily Universal Upper-tail Relevance-weighted Pairwise Ranker / `daily_universal_no_time_upper_tail_pairwise`；固定MR-13A的daily-universal sample universe、`daily_opportunity_no_time_r_v1`、300×10 input、architecture、optimizer與epoch-selection，唯一scientific change為同日RankNet pair依兩端daily target percentile算術平均作upper-tail relevance權重 | IMPLEMENTED／AWAITING_FORWARD_MODEL_GATE／NO_RUNTIME_DL_SOURCE。權重公式不含`lambda`、Top-K、boundary、threshold或exponent，並保留MR-13A原本pair aggregation；先做Seed42 Forward Model Gate，若保有實質排序訊號，再與已通過Forward Gate的MR-13C使用同一Selection PIT period/fold contract一起做Model Gate。結果前不得建立`DL-CONT13D`／`SR-*`；`MR-12B / DL-CONT12B`仍為CURRENT_RUNTIME_ANCHOR。 |
| Daily DL research infrastructure | `ContinuousRankerResearchSpec` / profile-driven dispatcher | Continuous experiment profile只描述scientific profile；額外research metadata集中保存`model_research_id / trainer_family / score_semantic_id / pairwise_reduction`，event與daily trainer共享learning API但各自擁有sample-universe orchestration | IMPLEMENTED／MR13D_PROFILE_READY／PROFILE_DRIVEN。MR-12B與MR-13A固定`equal_pair_weight`；MR-13B歷史`target_gap_weighted_within_date`已淘汰；MR-13C重用canonical percentile-MSE；MR-13D新增`upper_tail_relevance_weighted`且不複製trainer。Strategy arm仍只綁一個`dl_id`；13D Forward與13C/13D PIT Model Gate結果前不建立新的`DL-*`或`SR-C*`，多DL融合留待有第二score role與fusion semantics後再研究。 |
...
| `MR-13D` | Daily Universal Upper-tail Relevance-weighted Pairwise Ranker / `daily_universal_no_time_upper_tail_pairwise` | IMPLEMENTED／AWAITING_FORWARD_MODEL_GATE／NO_RUNTIME_DL_SOURCE。固定MR-13A全部daily-universal data／Target／architecture／optimizer／epoch-selection，只將同日non-tied RankNet pair由等權改成`(percentile_i + percentile_j) / 2` relevance權重；高-high pair自然高於低-low pair，不引入`lambda`、Top-K、boundary、threshold、exponent或breakout membership，且保留MR-13A原pair aggregation。先做Seed42 Forward Gate；若支持，再與MR-13C共用同一Selection PIT period/folds做Model Gate。 |

`MR-13A`、`MR-13B`、`MR-13C`、`MR-13D` 已正式占用。後續daily-universal target、loss、architecture或training-data semantics若再變更，必須使用新的 `MR-*`，不得覆寫13A/13B/13C/13D。
