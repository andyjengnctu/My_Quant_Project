...
| 最新已完成Audit結果 | `AUD-c23-c26-pit-portfolio-translation` | `SR-C26` vs `SR-C23 / SR-C25` Selection PIT R→PnL／wealth translation attribution | RESULT_AVAILABLE／R_TO_DOLLAR_TRANSLATION_GAP_CONFIRMED／RISK_DOLLAR_EXTENSION_IMPLEMENTED_PENDING_RERUN；C26-C23 exclusive selection ΔR=`+7.99R`但exclusive ΔPnL=`-35,524.03`、common trades ΔPnL=`-41,604.23`、all-trade ΔPnL=`-77,128.26`，final relative wealth=`-3.39%`；C26-C25則exclusive ΔR=`+27.51R`、exclusive ΔPnL=`+57,383.45`、all-trade ΔPnL=`+43,180.29`，確認22日guard本身在R與dollar PnL方向皆有效。現有結果仍不足以區分exclusive risk-dollar weighting與common-trade downstream sizing，故同一Audit已擴充implied initial-risk與common PnL對稱分解，待只讀重跑；C26仍NOT_PROMOTED、不進Forward-OOS |
...
| Current Selection runtime experiment | `SR-C26` | `SR-C25` feasible-ascent + Selection-frozen 22-calendar-day stale-score membership guard；candidate保留、只禁止stale score驅動membership change | RESULT_AVAILABLE／STALE_GUARD_SUPPORTED／NOT_PROMOTED；C26-C25：Return `+4.32pp`、EV `+0.08R`、same-param selection R `+27.51R`，證明22日guard有效修復C25；但C26-C23仍Return `-7.71pp`、MDD `+1.66pp`、RoMD `-0.59`，雖EV `+0.05R`、selection R `+7.99R`，故Selection portfolio gate未過、不進Forward-OOS |
...
| `SR-C26` | Selection PIT: MR-12B feasible-ascent stale-score membership guard | 完全沿用SR-C25的historical P2 params、`DL-CONT12B-PIT`、K/R0與feasible-ascent；唯一runtime變更為Selection預先凍結`score age > 22 calendar days`的有效舊PIT score不得參與DL造成的basket membership change。候選不刪除、不過期、不hard reject，仍保留Min ROOS membership／orderability；fresh↔fresh feasible-ascent維持原C25語意 | RESULT_AVAILABLE／STALE_GUARD_SUPPORTED／NOT_PROMOTED／CUTOFF_FROZEN_22D；C26-C25 Return `+4.32pp`、EV `+0.08R`、selection R `+27.51R`，但C26-C23 Return `-7.71pp`、MDD `+1.66pp`、RoMD `-0.59`，故不進Forward-OOS且22日門檻維持凍結 |
...
| `AUD-c23-c26-pit-portfolio-translation` | `c23-c26-pit-portfolio-translation` | `SR-C26` vs `SR-C23 / SR-C25` | RESULT_AVAILABLE／R_TO_DOLLAR_TRANSLATION_GAP_CONFIRMED／RISK_DOLLAR_EXTENSION_IMPLEMENTED_PENDING_RERUN | C26-C23：exclusive selection ΔR=`+7.99R`但exclusive ΔPnL=`-35,524.03`、common ΔPnL=`-41,604.23`、all-trade ΔPnL=`-77,128.26`、final relative wealth=`-3.39%`；C26-C25則exclusive ΔR=`+27.51R`、exclusive ΔPnL=`+57,383.45`、all-trade ΔPnL=`+43,180.29`，證明stale guard在dollar層亦有效。為區分未加權R與實際risk dollars，以及common-trade是否只是wealth／sizing下游放大，同一Audit已新增PnL÷R implied initial-risk、winner/loser risk-weighting與common `Δ(R×Risk)`對稱分解；只讀既有replay，不調22日門檻、不進OOS。 |
...
28. `SR-C26` Selection結果已取得：相對C25 Return `+4.32pp`、RoMD `+0.04`、EV `+0.08R`、same-param selection R `+27.51R`，underfilled end days `-40`、position-gap slot-days `-146`，證明stale-score guard方向有效；但相對C23仍Return `-7.71pp`、MDD `+1.66pp`、RoMD `-0.59`，僅EV `+0.05R`與selection R `+7.99R`為正，因此`STALE_GUARD_SUPPORTED / NOT_PROMOTED`，**不進Forward-OOS，也不得調22日門檻**。下一步只讀`AUD-c23-c26-pit-portfolio-translation`拆R→PnL／wealth轉化：exclusive/common trade PnL、capital geometry、slot occupancy與compounding path；若無可泛化機械原因，C26即維持Selection研究結果、不再細調runtime。
