...
| 最新已完成Audit結果 | `AUD-forward-robustness-portfolio-translation` | `SR-C29 / MR-13A` vs `SR-C20 / MR-12B` Forward 8-seed trade-set／risk-dollar／slot／wealth／entry-execution attribution | RESULT_AVAILABLE／ALL_SEEDS_COMPLETE／DIRECT_PAIR_R_EDGE_CONFIRMED／WITHIN_SET_WEIGHTING_DILUTION_CONFIRMED／NO_GENERALIZABLE_BINDING_DIFFERENTIAL／MR13A_NOT_PROMOTED。schema v5：Actual risk-util gap Mean=`-1.41pp`；MR-13A-only Winner−Loser actual utilization Mean=`+0.03pp`、MR-12B-only=`-0.64pp`；Cash binding=`851/856`、Position-cap=`286/284`、Risk-cap=`1360/1383`、Capital=`0/0`、MaxQty=`0/0`、Lot-rounding=`1360/1328`（13A/12B）。兩arm binding profile近似，未找到可泛化的13A-specific sizing/capital constraint；因此不新增score-weighted sizing、risk multiplier或position-cap SR。下一個既有正式gate改為Selection PIT Multi-seed robustness，以相同預設8 seeds／generator_seed=20260810確認MR-13A edge是否跨Selection與Forward時段成立。 |
...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；保留all-stock daily universal方向，feature-eligible ticker/date皆可取得daily score；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet與mean daily Spearman選epoch | RESULT_AVAILABLE／ITERATIVE_RESEARCH_ACTIVE／MATCHED_SELECTION_R_EDGE／DIRECT_PAIR_R_EDGE_CONFIRMED／PORTFOLIO_TRANSLATION_HETEROGENEOUS／WITHIN_SET_WEIGHTING_DILUTION_CONFIRMED／NO_GENERALIZABLE_BINDING_DIFFERENTIAL／NOT_PROMOTED。8-seed Forward中RoMD Mean=`9.97`高於MR-12B=`8.21`但同seed僅4勝4敗；ΔDL選擇R 6勝2敗、Mean=`+50.64R`。schema v4 equal-risk selection effect Mean=`+355,214.76`、within-set weighting=`-197,002.42`；schema v5進一步顯示兩arm actual risk utilization與cash/risk/position/lot binding profile沒有一致13A-specific差異，故目前無證據支持直接新增sizing/allocation SR。下一步改為既有Selection PIT Multi-seed robustness（8／20260810）確認model/selection edge跨時段seed robustness；在結果前不得改Target／loss／architecture／hyperparameter或依Forward Audit新增portfolio規則。 |
...
| Current continuous DL anchor | `DL-CONT12B` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` frozen OOS continuous score | CURRENT_RUNTIME_ANCHOR／MR13A_MODEL_EDGE_SUPPORTED_BUT_PORTFOLIO_STABILITY_INCOMPLETE。MR-13A已取得Forward matched selection/direct-pair trade-set edge；schema v5未找到可泛化的MR-13A-specific risk-utilization／binding缺陷，因此不以OOS Audit結果微調sizing。DL-CONT12B繼續維持canonical runtime anchor；下一個決策證據為Selection PIT Multi-seed robustness（8 seeds／generator_seed=20260810），不得以單seed、獨立Mean、Audit平均PnL或單一描述性相關直接promotion。 |
