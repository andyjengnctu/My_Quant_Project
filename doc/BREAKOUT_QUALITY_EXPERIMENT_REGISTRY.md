...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；保留all-stock daily universal方向，feature-eligible ticker/date皆可取得daily score；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet與mean daily Spearman選epoch | RESULT_AVAILABLE／ITERATIVE_RESEARCH_ACTIVE／SELECTION_STRATEGY_PASS／FORWARD_OOS_REPLAY_REQUIRED／PROMOTION_PENDING_RECHECK。Selection current五欄Min ROOS下C28 Return=151.88%、MDD=18.86%、RoMD=8.05、EV=0.71R，C28-C25 source-only Return +50.33pp、RoMD +2.89、EV +0.19R、same-param selection R +83.72R。既有Forward C20/C29結果使用target-complete-only score universe，score availability受future target horizon影響，已降級為PRE_FIX historical；frozen checkpoints仍有效，須先以future-independent inference universe重建Forward scores再重跑controlled replay，重驗後才決定是否取代MR-12B。 |
...
| Current Forward-OOS strategy matrix | `SR-C1 / C3 / C20 / C29` | 核心四arm顯示名稱統一為`Full ROOS`、`Min ROOS`、`Min MR-12B`、`Min MR-13A`；Forward-OOS階段由profile頁首標示，MR-12B／MR-13A arms皆固定使用feasible-ascent，Full只作完整策略體系baseline，MR-13A改善研究聚焦同一Min universe下的純DL source contrast | IMPLEMENTED／REPLAY_REQUIRED。舊共同期間2021-01-01～2025-12-22與C1/C3/C20/C29數值屬PRE_FIX historical，因Forward score存在性曾錯誤要求future target完整而截短runtime universe；frozen model checkpoints不變，但`DL-CONT12B`／`DL-CONT13A` Forward score artifacts須以feature-history-only inference eligibility重建，之後C20/C29與核心contrasts=`C1-C3`,`C20-C3`,`C29-C3`,`C29-C20`全部重跑。C30/C31維持永久歷史inactive，不擴大current comparison。 |
...
| ID | 名稱 | 狀態 |
|---|---|---|
| `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_PASS／STAGE3_PIT_RUNTIME_READY／SELECTION_STRATEGY_GATE_PASS／FORWARD_OOS_REPLAY_REQUIRED。Stage 1與Stage 2模型Gate、Selection PIT與C28 Selection strategy結果維持有效；frozen Forward checkpoint亦維持有效。舊Forward strategy replay使用target-complete-only score universe，runtime score presence受future horizon控制，故舊C20/C29只保留PRE_FIX歷史參考；修正後須從frozen checkpoint重建future-independent Forward scores並重跑controlled strategy Gate，完成前不得宣告MR-13A Forward fail或升級正式anchor。 |
...
| `DL-CONT12B` | `CONT12B` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` | Frozen OOS all-event continuous model checkpoint；training為同日pairwise ranking，runtime score為softmax PASS probability；模型權重與model Gate維持有效 | FORWARD_SCORE_REBUILD_REQUIRED；舊Forward score table只涵蓋target-evaluable rows，runtime coverage受future target horizon影響；以frozen checkpoint重建feature-history-only inference universe後再重跑C20，無需因本修正重訓模型 |
...
| `DL-CONT13A` | `CONT13A` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` | Frozen Forward-OOS daily continuous model checkpoint；策略盤前使用最新已完成交易日score；模型權重與Forward model Gate維持有效 | FORWARD_SCORE_REBUILD_REQUIRED／PROMOTION_PENDING_RECHECK；舊daily Forward score table只涵蓋target-evaluable rows並在future horizon前截尾，須由frozen checkpoint重建完整prediction-time eligible universe，再與`DL-CONT12B`做同Min universe純source controlled replay；不得以舊2021～2025 C29結果選loss weight／hyperparameter |
...
| `SR-C20` | Min ROOS: MR-12B max-DL feasible-ascent | current五欄Min ROOS、all-off rules、frozen feasible-ascent；K/R0與reserved-capital floor由同一Min DL-off baseline逐日建立；score source=`DL-CONT12B` | PRE_FIX_RESULT_ARCHIVED／REPLAY_REQUIRED；舊2021-01-01～2025-12-22結果使用target-complete-only Forward score universe，Return=172.40%、MDD=18.75%、RoMD=9.20、EV=1.08R、selection R=+40.61R只供歷史追溯，不再作current Forward anchor。frozen MR-12B checkpoint不變；重建future-independent Forward scores後以相同selector／Min params重跑。 |
...
| `SR-C29` | Forward-OOS: Min ROOS + MR-13A daily feasible-ascent | current五欄Min ROOS、all-off rules、frozen feasible-ascent與daily information-date lookup；K/R0與reserved-capital floor由同一C3 Min DL-off baseline逐日建立；score source=`DL-CONT13A` | PRE_FIX_RESULT_ARCHIVED／REPLAY_REQUIRED／ACTIVE_MR13A_CHALLENGER；舊2021-01-01～2025-12-22 Return=71.86%、MDD=15.71%、RoMD=4.57、EV=0.46R、selection R=-163.52R及C29-C20差異均使用target-complete-only Forward score universe，不再作current Gate結論。frozen MR-13A checkpoint與C28 selector語意維持；重建future-independent scores後重跑，完成前不得宣告Forward fail或用OOS調參。 |
