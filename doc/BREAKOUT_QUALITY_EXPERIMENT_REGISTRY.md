...
| `DL-CONT13A` | `CONT13A` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` | Frozen Forward-OOS daily continuous model checkpoint；策略盤前使用最新已完成交易日score；runtime score eligibility只依feature history、不要求future target | RESULT_AVAILABLE／FORWARD_STRATEGY_GATE_FAIL／NOT_PROMOTED；future-independent Forward score已重建並完成C29 replay，2021-01-01～2026-03-02 Return=83.81%、RoMD=5.34、EV=0.52R、selection R=-183.42R；相對DL-CONT12B/C20 Return -106.45pp、RoMD -4.81、EV -0.65R、selection R -219.35R。舊target-complete-only score只保留PRE_FIX歷史追溯。 |
...
| `AUD-forward-robustness-portfolio-translation` | `forward-robustness-portfolio-translation` | `MR-13A` vs `MR-12B` Forward 8-seed robustness | IMPLEMENTED／COMPACT_SOURCE_READY／SELECTION_BASIS_FIX_APPLIED／AWAITING_AUDIT_RERUN／READ_ONLY／ALL_SEEDS_REQUIRED | schema v6已證明ranking→RoMD只部分轉化；正式實作固定涵蓋全部8個matched seeds，不得挑S1/S7/S8或best/worst seeds。底層common/exclusive trade、risk-dollar decomposition、slot occupancy與wealth path直接reuse既有canonical strategy-attribution primitive；robustness在刪除full replay前只永久保存active trades/equity/daily capacity/selected buys的gzip compact source，不保存完整scores/orderable/replay tree。舊`2f70dbe73dcf3fed` scientific result已於2026-08-12以相同8 seeds／scientific fingerprint完成16/16 compact source重建，無需再次train/replay。首次Audit的match-key分叉已修正；其後S1仍被一致性gate擋下，進一步確認該gate本身跨越不同attribution basis：robustness同seed`ΔDL選擇R`是MR-13A與MR-12B各自相對共同DL-off baseline的direct-selection R之差，Audit的direct-pair exclusive ΔR則是兩個DL arms直接trade partition，兩者不得強制相等。現已移除錯誤equality gate並以Audit result schema v2分欄保存baseline-relative ranking edge、direct-pair exclusive/common/all-trade ΔR與basis gap；compact unit仍只接受`VERIFIED`，Audit本身只讀、不train、不replay、不回寫scientific result。尚未取得修正後Audit結果，不得預先promotion或淘汰MR-13A。 |
