...
| `AUD-mr13e-orderable-feasible-alignment` | `mr13e-orderable-feasible-alignment` | `MR-12B / MR-13A / MR-13E` Selection PIT + Forward completed Strategy Compare sidecars | IMPLEMENTED／READ_ONLY／AWAITING_RESULT。每個trade day只在實際orderable competition set計算score→共同`daily_opportunity_no_time_r_v1`的Daily rho／pair／Top-N lift／oracle retention／top-vs-rest；N直接取當日pre-market order limit/action count，不用固定K。另把direct-feasible／repair／feasible-ascent分層，並以execution sidecar比較raw score Top-N與實際chosen action的target/score差。Future target僅post-replay join；不train、不score、不replay，也不宣稱既有工件未保存的逐swap counterfactual identity。 |
