...
| Current Selection strategy matrix | `SR-C32 / C23 / C25 / C28 / C35` | Current enabled Selection matrix維持Full/Min baseline、MR-12B、MR-13A、MR-13E；已完成的C37 Expected-PnL只保留historical identity/result，不再列入current enabled profile。 | C37結果已定案`REJECTED`：Return=`84.06%`、MDD=`21.87%`、RoMD=`3.84`，相對C35 Return=`-69.90pp`、RoMD=`-3.90`；依事前Gate不進C38。 |
| Current Forward-OOS strategy matrix | `SR-C1 / C3 / C20 / C29 / C36` | C1/C3/C20/C29/C36既有結果保留；C38 implementation只保留該受控實驗的可重現identity，不列入current enabled matrix。 | `SR-C38 NOT_EXECUTED_AFTER_C37_REJECTED／ROBUSTNESS_OFF`。因Selection C37已明顯輸C35與C23，依預先Gate不查看C38 Forward；MR-13F direct-R Huber後續亦因magnitude error輸給frozen constant而REJECT，current下一步為`MR-13G` raw-R MSE Forward model gate。 |
...
| `MR-13G` | Daily Universal Direct-R Mean Regression / `daily_universal_no_time_r_mse` | IMPLEMENTED／AWAITING_FORWARD_MODEL_RESULT／NO_PIT／NO_RUNTIME_DL_SOURCE。固定MR-13F的Dataset、daily eligible sample、raw-R target、InceptionTime、optimizer/LR/weight decay、Seed42、Selection/Validation/Forward split、two-logit Predicted-R output與無clipping/no weighting；唯一scientific change=`huber_raw_r(delta=1R)` → `mse_raw_r`，epoch由Validation Huber改為Validation raw-R MSE最小值（tie-break Daily Spearman）。目的不是先追ranking，而是檢驗MSE population optimum對`E[R|X]`的conditional-mean對齊能否打贏pre-OOS frozen constant並改善magnitude calibration。第一Gate只跑Seed42 Forward。 |
