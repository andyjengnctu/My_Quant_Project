...
| 最新已完成Audit結果 | `AUD-forward-robustness-portfolio-translation` | `SR-C29 / MR-13A` vs `SR-C20 / MR-12B` Forward 8-seed trade-set／risk-dollar／slot／wealth／entry-execution attribution | RESULT_AVAILABLE／ALL_SEEDS_COMPLETE／DIRECT_PAIR_R_EDGE_CONFIRMED／WITHIN_SET_WEIGHTING_DILUTION_CONFIRMED／NO_GENERALIZABLE_BINDING_DIFFERENTIAL／MR13A_NOT_PROMOTED。schema v5：Actual risk-util gap Mean=`-1.41pp`；MR-13A-only Winner−Loser actual utilization Mean=`+0.03pp`、MR-12B-only=`-0.64pp`；兩arm binding profile近似，未找到可泛化的13A-specific sizing/capital constraint。後續Selection PIT 8-seed已完成並顯示relative edge跨時段反轉，因此本Forward Audit不再作promotion主證據，也不得據此新增sizing SR。 |
| Latest cross-period robustness | `MR-13A` vs `MR-12B` / same generated 8 seeds | Selection PIT 2014-01-01～2020-12-31 vs Forward-OOS 2021-01-01～2026-03-02 | RESULT_AVAILABLE／CROSS_PERIOD_DIRECTION_REVERSAL。Selection PIT：MR-13A−MR-12B ΔDL選擇R Mean=`-20.76R`（3勝5敗）、ΔRoMD Mean=`-0.93`（2勝6敗）、方向一致=`7/8`；Forward：ΔDL選擇R Mean=`+50.64R`（6勝2敗）、ΔRoMD Mean=`+1.76`（4勝4敗）、方向一致=`4/8`。因此MR-13A relative edge沒有跨兩個時段穩定成立，current runtime anchor維持MR-12B；cross-period／year-regime read-only Audit已實作，下一步執行`AUD-cross-period-year-regime-attribution`。 |
...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；保留all-stock daily universal方向，feature-eligible ticker/date皆可取得daily score；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet與mean daily Spearman選epoch | RESULT_AVAILABLE／CROSS_PERIOD_MULTI_SEED_EDGE_NOT_STABLE／FORWARD_SELECTION_EDGE_SUPPORTED／SELECTION_PIT_RELATIVE_EDGE_NOT_CONFIRMED／NOT_PROMOTED。Forward 8-seed：MR-13A vs MR-12B ΔDL選擇R 6勝2敗、Mean=`+50.64R`，ΔRoMD 4勝4敗、Mean=`+1.76`；Selection PIT 8-seed則反轉為ΔDL選擇R 3勝5敗、Mean=`-20.76R`，ΔRoMD 2勝6敗、Mean=`-0.93`。Selection PIT ranking/RoMD方向一致=`7/8`，表示該時段主問題是ranking/selection edge本身反轉，而非Forward已知的portfolio translation heterogeneity。schema v5 Forward binding attribution仍顯示無可泛化的13A-specific sizing constraint。`AUD-cross-period-year-regime-attribution`已實作並等待正式run，將以既有工件定位2014–2020相對2021–2026的edge翻轉來源；結果前不改Target／loss／architecture／hyperparameter、不新增sizing SR、不挑seed。 |
...
| Current Selection strategy matrix | `SR-C32 / C23 / C25 / C28` | 核心四arm顯示名稱統一為`Full ROOS`、`Min ROOS`、`Min MR-12B`、`Min MR-13A`；Selection階段由profile頁首標示，MR-12B／MR-13A arms皆固定使用feasible-ascent，Full只保留獨立策略體系baseline | IMPLEMENTED／RESULT_AVAILABLE／MULTI_SEED_COMPLETE；2014-01-01～2020-12-31。固定baseline：Full Return/RoMD=`132.25%/8.34`、Min=`107.98%/5.25`。8-seed：MR-12B Return/MDD/RoMD=`117.08%/19.85%/5.96`、DL選擇R=`64.08R`；MR-13A=`102.98%/20.51%/5.03`、DL選擇R=`43.32R`。Matched MR-13A−MR-12B：ΔRoMD=`-0.93`（2勝6敗）、ΔDL選擇R=`-20.76R`（3勝5敗）；ranking/RoMD方向一致=`7/8`。單seedC28高績效仍保留歷史，但不得代表multi-seed robustness。 |
...
| `AUD-cross-period-year-regime-attribution` | `cross-period-year-regime-attribution` | `MR-13A` vs `MR-12B` Selection PIT + Forward-OOS 8-seed robustness | IMPLEMENTED／READ_ONLY／ALL_SAME_SEEDS_REQUIRED／AWAITING_AUDIT_RUN | 只讀兩個已完成robustness與逐檔SHA驗證compact attribution；要求兩period的resolved seeds、seed_count、seed generator完全一致。Period aggregate保留正式baseline-relative ΔDL選擇R；逐年度只輸出same-seed ΔReturn與MR-13A vs MR-12B direct-pair exclusive trade R/PnL／trade counts，因目前沒有baseline-relative逐年selection-R永久工件，兩種basis不得強制相等。目的只定位edge flip是否集中於特定年份，Audit不得train／score／replay／建立regime gate或回流training。 |
