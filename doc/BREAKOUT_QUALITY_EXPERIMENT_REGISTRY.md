...
| Current model research | `MR-12B` | No-time all-event within-day Pairwise RankNet ranker；固定all-event Target／InceptionTime，以同日pairwise logistic直接學cross-sectional ordering | RESULT_AVAILABLE／MODEL_ECONOMIC_IMPROVEMENT_SUPPORTED；C19/C20 controlled replay均支持。P2完整Forward-OOS prefix sweep顯示K=1/2/3/5/10 NDCG Δ(B-A)皆為正，K=1為`+0.0477`、Boundary `+5.21pp`。C17 common-complete K=1的負向反轉只在orderable universe出現（Event NDCG Δ=`+0.0532`；Orderable=`-0.0886`）。最新score-date comparability又否定跨date score尺度失配：Cross-score-date Pair Δ=`+2.49pp`、K=1 Cross=`+2.14pp`，MR-12B均優於MR-12A。同時orderable候選score-age>0=`100%`、median=`14`天、mean=`20.22`天；目前Dynamic-K的`target_raw_r`仍錨定原始score-event date，不是trade-date剩餘機會，因此不得據此修改Pairwise loss。下一步先完成MR-12B Selection PIT穩定性；若要解釋portfolio action差異，應改用trade-date canonical counterfactual/realized outcome，而非繼續細調原始event-target P2。 |
