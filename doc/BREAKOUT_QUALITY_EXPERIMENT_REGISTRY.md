...
| `AUD-c23-c25-pit-realization` | `c23-c25-pit-realization` | `SR-C23` vs `SR-C24 / SR-C25` | IMPLEMENTED／RESULT_PENDING | 只讀既有Selection PIT replay，整合exclusive trade、wealth／capital geometry、slot occupancy與Target→Realized capture；不重跑portfolio、不重訓、不跑optimizer。用於解釋C24/C25 Target改善但selection R／Return惡化的realization gap |
