...
| 最新已完成Audit結果 | `AUD-forward-robustness-portfolio-translation` | `SR-C29 / MR-13A` vs `SR-C20 / MR-12B` Forward 8-seed trade-set／risk-dollar／slot／wealth translation attribution | RESULT_AVAILABLE／ALL_SEEDS_COMPLETE／DIRECT_PAIR_R_EDGE_CONFIRMED／DOLLAR_TRANSLATION_HETEROGENEOUS／MR13A_NOT_PROMOTED。8 seeds的direct-pair Exclusive ΔR Mean=`+50.64R`、Exclusive ΔPnL Mean=`+173,208.15`、Common risk-size effect Mean=`+18,544.82`、ΔGap slot-days Mean=`-99`；3 seeds=`TRANSLATION_SUCCESS`、2=`TRADE_SET_DOLLAR_TRANSLATION`、1=`MDD_WEALTH_PATH`、1=`PORTFOLIO_OVERRIDE`、1=`ALIGNED_DECLINE`。S1/S8皆為exclusive ΔR正但exclusive ΔPnL負（S1 `+11.59R/-103,422`；S8 `+74.70R/-434,639`）。schema v3 R→Dollar bridge進一步顯示S1的MR-13A-only／MR-12B-only RW-R=`0.476R/0.547R`、Avg risk=`6,684/6,792`，S8=`0.821R/0.900R`、Avg risk=`6,290/7,699`；反例S6雖exclusive ΔR=`-41.02R`，但RW-R=`0.832R/0.655R`、Avg risk=`8,152/6,066`且Exclusive ΔPnL=`+520,269`。因此下一個未解問題已縮到exclusive trade的risk-dollar配置，而不是再改MR-13A ranking objective；但aggregate Exclusive RW-R gap Mean仍為`+0.121R`，不能直接假設全域sizing規則有缺陷。MR-12B繼續作current runtime anchor；同一Audit report schema v4已IMPLEMENTED，直接用既有trade-level R與implied risk做exact decomposition，將Exclusive ΔPnL拆為equal-risk selection、average-risk scale、within-set R×risk weighting與uncovered effect並要求residual閉合；目前AWAITING_AUDIT_RERUN，不train、不replay、不擴seed。 |
...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；保留all-stock daily universal方向，feature-eligible ticker/date皆可取得daily score；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet與mean daily Spearman選epoch | RESULT_AVAILABLE／ITERATIVE_RESEARCH_ACTIVE／SELECTION_STRATEGY_PASS／SINGLE_SEED_FORWARD_FAIL／MULTI_SEED_REOPENED／MATCHED_SELECTION_R_EDGE／MATCHED_ROMD_INCONCLUSIVE／RANKING_TO_PORTFOLIO_PARTIAL_TRANSLATION／TRADE_PATH_ATTRIBUTION_REQUIRED／NOT_PROMOTED。Selection current五欄Min ROOS下C28 Return=151.88%、MDD=18.86%、RoMD=8.05、EV=0.71R，C28-C25 source-only Return +50.33pp、RoMD +2.89、EV +0.19R、same-param selection R +83.72R。2026-08-12正式8-seed Forward robustness（seed_count=8、generator_seed=20260810）中MR-13A Return/MDD/RoMD/EV Mean=`148.18%/15.47%/9.97/0.94R`、DL選擇R Mean=`-54.70R`，高於MR-12B的`129.01%/15.87%/8.21/0.77R`與`-105.34R`；同seedΔDL選擇R為6勝2敗、Mean/Median/Std=`+50.64R/+46.36R/77.12R`、P25=`+4.25R`。schema v6進一步確認6個ΔDL選擇R>0 seeds中只有3個ΔRoMD>0，ranking／RoMD方向一致=`4/8`、相反=`4/8`；Spearman(ΔDL選擇R,ΔRoMD)=`0.476`、Spearman(ΔDL選擇R,ΔReturn)=`0.333`。S1/S7/S8屬ranking↑但RoMD↓，S6則ranking↓但RoMD↑；全8-seed portfolio Audit已完成，direct-pair Exclusive ΔR Mean=`+50.64R`、Exclusive ΔPnL Mean=`+173,208.15`、Common risk-size effect Mean=`+18,544.82`、ΔGap slot-days Mean=`-99`，但S1/S8仍出現exclusive ΔR正而exclusive ΔPnL負，證明ranking edge在平均上有經濟轉化、但risk-dollar translation具明顯seed heterogeneity。schema v3已顯示S1/S8皆為ΣR正但RW-R較MR-12B低，S6則ΣR較差但RW-R與dollar PnL反而較佳；aggregate Exclusive RW-R gap Mean仍為+0.121R，故不得直接下結論為全域sizing缺陷。下一步固定用既有trade-level R與implied risk做exact R→Dollar decomposition，把Exclusive ΔPnL拆為equal-risk selection、average-risk scale、within-set R×risk weighting與uncovered effect；不得先擴seed、跑Selection PIT multi-seed、開新MR、做ensemble或依Forward結果調hyperparameter。 |
...
| Current continuous DL anchor | `DL-CONT12B` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` frozen OOS continuous score | SOURCE_SUPPORTED_FOR_MODEL_RESEARCH／CANONICAL_SEED42_FORWARD_ANCHOR／CURRENT_RUNTIME_ANCHOR／MR13A_MATCHED_SELECTION_R_EDGE_NOT_YET_PORTFOLIO_STABLE。future-independent Forward score重建後，seed 42的C20於2021-01-01～2026-03-02 Return=190.26%、MDD=18.75%、RoMD=10.15、EV=1.17R、same-param selection R=+35.93R；2026-08-12正式8-seed robustness中MR-12B Return/MDD/RoMD/EV Mean=`129.01%/15.87%/8.21/0.77R`、DL選擇R Mean=`-105.34R`，低於MR-13A。MR-13A同seedΔDL選擇R雖6勝2敗且Median=`+46.36R`，schema v6只有3/6 ranking↑ seeds同步ΔRoMD>0；全8-seed trade/path Audit進一步顯示平均Exclusive ΔPnL=`+173,208.15`但S1/S8仍為正exclusive ΔR、負exclusive ΔPnL，故DL-CONT12B繼續維持canonical runtime anchor，直到exclusive risk-dollar bridge的可泛化機制被釐清；不得把seed 42、獨立分布Mean、selection R、Audit平均PnL或描述性Spearman單一指標直接當作promotion gate。 |
...
| Current Selection strategy matrix | `SR-C32 / C23 / C25 / C28` | 核心四arm顯示名稱統一為`Full ROOS`、`Min ROOS`、`Min MR-12B`、`Min MR-13A`；Selection階段由profile頁首標示，MR-12B／MR-13A arms皆固定使用feasible-ascent，Full只保留獨立策略體系baseline，不再把Full+DL arms列入current MR-13A改善矩陣 | IMPLEMENTED／RESULT_AVAILABLE；2014-01-01～2020-12-31。C32 Full baseline Return=132.25%、MDD=15.85%、RoMD=8.34、EV=0.71R；C23 Min baseline=107.98%/20.57%/5.25/0.42R；C25 Min+MR-12B=101.55%/19.66%/5.17/0.52R；C28 Min+MR-13A=151.88%/18.86%/8.05/0.71R。Current核心contrasts=`C32-C23`,`C25-C23`,`C28-C23`,`C28-C25`；C33/C34保留永久歷史結果但inactive。Selection與Forward-OOS仍由永久雙profile隔離，新run寫各自output root，legacy runs只讀作cache。 |
| Current Forward-OOS strategy matrix | `SR-C1 / C3 / C20 / C29` | 核心四arm顯示名稱統一為`Full ROOS`、`Min ROOS`、`Min MR-12B`、`Min MR-13A`；Forward-OOS階段由profile頁首標示，MR-12B／MR-13A arms皆固定使用feasible-ascent，Full只作完整策略體系baseline，MR-13A改善研究聚焦同一Min universe下的純DL source contrast | IMPLEMENTED／RESULT_AVAILABLE／POST_FIX_CURRENT。共同期間2021-01-01～2026-03-02，score eligibility已改為feature-history-only且future target不再控制runtime score presence。C1 Full=129.08%/17.41%/7.42/EV0.53R；C3 Min=101.60%/13.12%/7.74/1.02R；C20 Min+MR-12B=190.26%/18.75%/10.15/1.17R；C29 Min+MR-13A=83.81%/15.71%/5.34/0.52R。C29-C20 source-only Return -106.45pp、RoMD -4.81、EV -0.65R、selection R -219.35R；current anchor維持MR-12B，MR-13A不升格。C30/C31維持永久歷史inactive。 |
...
| ID | 名稱 | 狀態 |
|---|---|---|
| `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_PASS／STAGE3_PIT_RUNTIME_READY／SELECTION_STRATEGY_GATE_PASS／MULTI_SEED_REOPENED／MATCHED_SELECTION_R_EDGE／MATCHED_ROMD_INCONCLUSIVE／PORTFOLIO_ATTRIBUTION_COMPLETE／DOLLAR_TRANSLATION_HETEROGENEOUS／NOT_PROMOTED。seed 42 的future-independent Forward replay仍是有效單一run證據：2021-01-01～2026-03-02 C29 Return=83.81%、MDD=15.71%、RoMD=5.34、EV=0.52R、selection R=-183.42R；相對C20 Return -106.45pp、RoMD -4.81、EV -0.65R、selection R -219.35R。但正式8-seed robustness已推翻「單一seed可代表模型普遍Forward失敗」的外推：MR-13A同seedΔDL選擇R 6勝2敗、Median=+46.36R，且全8-seed Audit的direct-pair Exclusive ΔR Mean=`+50.64R`、Exclusive ΔPnL Mean=`+173,208.15`、Common risk-size effect Mean=`+18,544.82`、ΔGap slot-days Mean=`-99`，支持ranking／trade-set edge在平均上具有經濟轉化；然而S1/S8仍為正exclusive ΔR、負exclusive ΔPnL，且ΔRoMD只有4勝4敗，故尚不能升格runtime anchor。下一步只做既有risk-weighted R／initial-risk bridge診斷，不擴seed、不跑Selection PIT multi-seed、不開新MR，也不得依Forward結果選training hyperparameter或best seed。 |
...
| `AUD-forward-robustness-portfolio-translation` | `forward-robustness-portfolio-translation` | `MR-13A` vs `MR-12B` Forward 8-seed robustness | RESULT_AVAILABLE／READ_ONLY／ALL_SEEDS_COMPLETE／DIRECT_PAIR_R_EDGE_CONFIRMED／DOLLAR_TRANSLATION_HETEROGENEOUS | 8/8 matched seeds已完成：Ranking↑=`6/8`、Ranking↑且RoMD↑=`3/6`、ranking/RoMD同方向=`4/8`；direct-pair Exclusive ΔR Mean=`+50.64R`、selection basis gap Mean=`0.00R`、Exclusive ΔPnL Mean=`+173,208.15`、Common risk-size effect Mean=`+18,544.82`、ΔGap slot-days Mean=`-99`。Driver=`TRANSLATION_SUCCESS 3`、`TRADE_SET_DOLLAR_TRANSLATION 2`、`MDD_WEALTH_PATH 1`、`PORTFOLIO_OVERRIDE 1`、`ALIGNED_DECLINE 1`。S1=`+11.59R/-103,422`、S8=`+74.70R/-434,639`顯示正exclusive R仍可因risk-dollar配置轉為負PnL；下一步沿用同Audit已計算的exclusive risk-weighted R、implied initial risk及winner/loser risk summary作R→Dollar bridge，無需train/replay或新Audit ID。 |
