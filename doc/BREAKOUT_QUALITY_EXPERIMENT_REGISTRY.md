...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；每個合法ticker/date皆可成為sample，candidate membership不進training sample selection；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet pairwise logistic與mean daily Spearman選epoch | IMPLEMENTED／RESULT_PENDING／FORWARD_OOS_MODEL_GATE_ONLY；stage 1只做全市場daily stock-day與breakout-candidate slice的forward-OOS模型驗證，不建立PIT、不接selector／ROOS。Model-research active profile獨立指向MR-13A；strategy workflow active profile仍維持MR-12B，避免尚無PIT的MR-13A滲入既有策略預設。Daily feature以canonical OHLCV NumPy lazy materialization；stock window不經逐sample DataFrame切片，benchmark同一交易日normalized window由RAM cache重用；持久工件不複製expanded 300×10 daily feature bank。原先排隊的`MR-12B vs MR-12A same-fold paired Selection PIT`暫不執行；使用者明確改採daily-universal模型方向。MR-12B仍保留current validated continuous DL anchor，MR-13A在結果通過前不得取代。 |
...
| ID | 名稱 | 狀態 |
|---|---|---|
| `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise` | IMPLEMENTED／RESULT_PENDING；sample scope=`daily_eligible_stock_days`，target=`daily_opportunity_no_time_r_v1`。固定沿用`ARCH-inception_time_v1`、300×10 sequence、40-day adverse-first risk-adjusted opportunity核心、pairwise logistic與mean daily Spearman；唯一研究層變更為breakout-event sample universe→每日合法stock-day，以及對應不依賴breakout/high_len/level/candidate membership的daily target identity。Stage 1只允許checkpoint後forward-OOS模型Gate：全市場與breakout-candidate slice；PIT／strategy source／ROOS均未授權。Performance-only修正以canonical NumPy array lazy window取代逐sample Pandas feature materialization，benchmark normalized window依benchmark position快取，training以config-driven 2-batch單CPU-worker prefetch與GPU計算重疊；feature值契約、sample universe、target、loss與batch ordering均不變。 |
