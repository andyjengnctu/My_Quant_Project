...
| `AUD-mr13e-minimum-repair-mechanism` | `mr13e-minimum-repair-mechanism` | `MR-12B / MR-13A / MR-13E` Selection PIT + Forward repair days；completed selector trace + repair mechanism sidecar | IMPLEMENTED／READ_ONLY／SOURCE_REPLAY_REFRESH_REQUIRED／AWAITING_RESULT | Strategy Compare僅在顯式research diagnostic sink下，於production selector完成後使用同一canonical cash-capped reservation simulator與同一K/R0做exact frozen-score oracle；一個best-first搜尋求最少replacement distance下最高quality合法basket，另一個求全域highest-quality合法K-basket。Future Target完全不參與oracle，只在Audit post-replay attribution。另保存production accepted repair swaps與before/after count/reserve deficit，讓Audit區分`RESOURCE_CONSTRAINT_EXACT_OPTIMUM`、`GREEDY_REPAIR_EXTRA_REPLACEMENTS`、`GREEDY_REPAIR_SCORE_GAP_AT_MIN_DISTANCE`、`MULTI_SWAP_LOCAL_SEARCH_GAP`；不設state cap、lambda、threshold或target-guided selection。舊正式pair缺`score_ranking_repair_mechanism.csv`只需一次strategy replay refresh。 |
