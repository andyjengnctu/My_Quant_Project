...
| `AUD-mr13e-orderable-feasible-alignment` | `mr13e-orderable-feasible-alignment` | `MR-12B / MR-13A / MR-13E` Selection PIT + Forward completed Strategy Compare sidecars | RESULT_AVAILABLE／READ_ONLY／RESOURCE_TRANSLATION_GAP | Selection 13E Daily rho/Pair=`0.1261/54.39%`勝12B=`0.0994/53.41%`，但Top-N/Repair Lift=`0.2989R/0.2710R`低於12B=`0.5826R/0.6423R`；Forward則13E Daily rho/Pair/Top-N/Repair Lift=`0.1698/55.93%/1.0996R/1.1401R`全面高於12B=`0.1586/55.52%/0.8223R/0.7975R`。然而Forward Repair Action−Raw 13E=`-1.3132R`、12B=`-0.9459R`，13E額外損失=`-0.3673R`，故正式classification=`RESOURCE_TRANSLATION_GAP`。Audit只讀canonical orderable/capacity/execution sidecars並post-replay join共同Future Target；已完成後在config關閉，由下一層selector-stage Audit接棒。 |
| `AUD-mr13e-selector-stage-translation` | `mr13e-selector-stage-translation` | `MR-12B / MR-13A / MR-13E` Selection PIT + Forward completed Strategy Compare selector trace + execution sidecars | RESULT_AVAILABLE／READ_ONLY／DOMINANT_EXTRA_LOSS_AT_RAW_TO_REPAIR | Forward repair days：12B Raw→Repair=`-0.9292R`、13A=`-1.4038R`、13E=`-1.3763R`；13E相對12B額外`-0.4471R`。其後13E−12B Repair→Ascent=`-0.0177R`、Ascent→Action=`0`、Action→Fill=`-0.0018R`，故feasible-ascent／entry／fill不是主要斷點。Selection則13E Raw→Repair=`-0.6420R`優於12B=`-0.9358R`，顯示repair傷害具跨期差異，不能據Forward直接改selector。Audit已完成後在config關閉；下一層以exact frozen-score oracle區分resource incompatibility與search heuristic deficiency。 |
