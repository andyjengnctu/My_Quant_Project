...
| Current Forward-OOS strategy matrix | `SR-C1 / C3 / C20 / C29 / C36` | C1/C3/C20/C29/C36既有結果保留；C38 implementation只保留該受控實驗的可重現identity，不列入current enabled matrix。 | `SR-C38 NOT_EXECUTED_AFTER_C37_REJECTED／ROBUSTNESS_OFF`。因Selection C37已明顯輸C35與C23，依預先Gate不查看C38 Forward；下一步改做新模型`MR-13F` direct-R Forward model gate。 |
...
| `MR-13F` | Daily Universal Direct-R Huber Regression / `daily_universal_no_time_r_huber` | RESULT_AVAILABLE／HOLD_FOR_DIRECT_R_CALIBRATION_AUDIT／NO_PIT／NO_RUNTIME_DL_SOURCE。Seed42 Forward selected epoch=`2`；OOS Huber=`0.7888`、MAE=`1.1547R`、bias=`-0.6505R`、Daily rho=`0.1611`、Global rho=`0.1310`、Pair=`55.52%`、Top-K Lift=`+0.7392R`；breakout Daily rho=`0.0919`、Pair=`54.80%`、Top-K Lift=`+0.2472R`。相對MR-13E純ranking較弱，但direct-R研究真正待決策的是magnitude/calibration；下一步只跑read-only `AUD-mr13f-direct-r-calibration`，不建PIT/runtime source。 |

`MR-13A`、`MR-13B`、`MR-13C`、`MR-13D`、`MR-13E`、`MR-13F` 已正式占用。後續daily-universal target、loss、architecture或training-data semantics若再變更，必須使用新的 `MR-*`，不得覆寫13A～13F。
...
| `AUD-mr13f-direct-r-calibration` | `mr13f-direct-r-calibration` | `MR-13F` frozen Seed42 Forward report + `daily_ranker_oos_scores.csv.gz` | IMPLEMENTED／READ_ONLY／AWAITING_RESULT／NO_PIT／NO_RUNTIME_CALIBRATION | 待決策問題只有一個：MR-13F的`-0.6505R` bias是可描述的level/scale calibration問題，還是Predicted-R magnitude本身不比pre-OOS constant baseline可靠。Audit以Selection-inner Validation target mean作frozen constant；OOS只post-hoc比較Huber/MAE/RMSE、`Actual=intercept+slope×Predicted`、Predicted-R quantile bucket單調性與`Predicted R >0`/`<=0`的實際R，不設定人工slope門檻，也禁止把OOS fitted係數直接回灌runtime。 |
