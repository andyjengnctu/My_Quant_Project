...
| Current Selection PIT strategy source | `DL-CONT12B-PIT` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` Selection point-in-time score | RESULT_AVAILABLE／PIT_GATE_PASS／STRATEGY_NOT_PROMOTED；既有C23/C24/C25結果使用已淘汰的4-ATR、high_len凍結P2，僅保留read-only attribution；current 5-field Min ROOS下須與C27/C28同批重跑。PIT top-level工件缺失時，Strategy Compare只可在Dataset／Target等model upstream已就緒時作checkpoint-only重建；缺Dataset／Label／Target或任一PIT fold checkpoint時，必須由模型訓練工作類型依config準備；該模型工作類型以resume重用相容fold並只補訓缺失／不相容PIT fold，Strategy Compare不得跨界建立或訓練 |
