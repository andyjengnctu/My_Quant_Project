...
| Current model research | `MR-12B` | No-time all-event within-day Pairwise RankNet ranker；固定all-event Target／InceptionTime，以同日pairwise logistic直接學cross-sectional ordering | RESULT_AVAILABLE／MODEL_ECONOMIC_IMPROVEMENT_SUPPORTED；C19/C20 controlled replay均支持。P2完整Forward-OOS prefix sweep排除「一般性Top-1弱」：K=1/2/3/5/10的NDCG Δ(B-A)皆為正，K=1為`+0.0477`、Boundary `+5.21pp`、Top-1 Lift `0.8244R vs 0.5069R`。最新reference-subset attribution再把C17 common-complete K=1反轉定位到orderable universe：同一71個reference dates中，完整event universe NDCG Δ=`+0.0532`、Boundary Δ=`+4.69pp`，但C17 orderable universe NDCG Δ=`-0.0886`、Boundary Δ=`-19.72pp`；因此date/regime本身不是主因，弱勢由candidate／portfolio-path conditioning引入。下一步只讀檢查orderable pool是否混合不同`score_event_date` cohort，直接比較same-score-date與cross-score-date pair concordance；因MR-12B training只約束同日pairs，完成此歸因前不新增MR、不改pairwise loss。 |
