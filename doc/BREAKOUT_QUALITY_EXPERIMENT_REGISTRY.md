...
| 最新已完成Audit結果 | `AUD-c23-c26-pit-portfolio-translation` | `SR-C26` vs `SR-C23 / SR-C25` Selection PIT R→PnL／wealth translation attribution | RESULT_AVAILABLE／RISK_DOLLAR_PATH_DECOMPOSITION_CONFIRMED／C26_NOT_PROMOTED／RUNTIME_MICROTUNING_STOPPED；C26-C23 exclusive selection ΔR=`+7.99R`但exclusive ΔPnL=`-35,524.03`，C26-only risk-weighted R=`0.21R`低於C23-only=`0.27R`；206筆common trades的ΔR約`0R`、ΔPnL=`-41,604.23`，其中risk-size/path effect=`-41,612.25`、R-difference effect僅`+8.02`、residual約0，證明common loss不是trade outcome差異而是既有portfolio path／trade-specific sizing的下游結果。C26-C25仍有exclusive ΔR=`+27.51R`、exclusive ΔPnL=`+57,383.45`，故22日guard本身有效但不足以通過C23 portfolio gate；沒有新的可事前觀測泛化mechanism支持再調selector、fixed risk或22日門檻，C26不進Forward-OOS並停止此runtime微調鏈 |
...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；每個feature-eligible ticker/date皆可取得daily score，future target只決定training／audit validity；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet pairwise logistic與mean daily Spearman選epoch | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_PASS／STAGE3_PIT_RUNTIME_READY／SELECTION_STRATEGY_GATE_PASS／FORWARD_OOS_STRATEGY_AUTHORIZED／NOT_YET_PROMOTED_OVER_MR12B。Stage 1 full-market OOS daily rho=0.1755、pair=56.12%；breakout-candidate OOS daily rho=0.1490、pair=56.91%、Top-K Target/Lift=1.6198R/+0.4056R。Stage 2 Selection PIT 2013-04-01～2020-12-31、8 folds、target-valid groups=778,532、coverage=100%；all-stock global/daily rho=0.0783/0.1111、pair=53.84%、spread=+0.3319R，年度rho>0與spread>0均8/8；breakout slice global/daily rho=0.0580/0.0784、pair=53.98%、Top-K Lift=+0.1102R、Boundary=51.49%，PIT模型Gate PASS。Stage 3 runtime契約已改為feature-history-only score eligibility與最新已完成交易日lookup。Current五欄Min ROOS下Selection策略結果：C27 minimum-repair Return=102.26%、MDD=21.57%、RoMD=4.74、EV=0.50R，仍低於C23 baseline；C28 feasible-ascent Return=151.88%、MDD=18.86%、RoMD=8.05、EV=0.71R。C28-C25 source-only：Return +50.33pp、MDD -0.80pp、RoMD +2.89、EV +0.19R、same-param selection R +83.72R；C28-C23：Return +43.90pp、MDD -1.71pp、RoMD +2.80、EV +0.29R、selection R +109.91R，且年度Return相對C23改善6/7。Selection支持MR-13A daily source搭配frozen feasible-ascent進Forward-OOS，但正式continuous anchor仍待Forward-OOS策略Gate。 |
...
| Current Selection PIT daily source | `DL-CONT13A-PIT` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` daily Selection PIT score | RESULT_AVAILABLE／PIT_MODEL_GATE_PASS／STRATEGY_RUNTIME_READY／SELECTION_STRATEGY_GATE_PASS；策略端只接受`score_eligibility_contract=feature_history_only`的新PIT manifest，每個盤前決策查最新已完成交易日score。Current五欄Min ROOS下，C28-C25 source-only Return +50.33pp、RoMD +2.89、EV +0.19R、same-param selection R +83.72R，支持daily source進Forward-OOS controlled validation；PIT source本身只供Selection，不作forward runtime source |
| Latest Selection strategy translation | `SR-C27 / SR-C28` | C27完全沿用C24 minimum-repair、C28完全沿用C25 feasible-ascent；唯一DL source改為`DL-CONT13A-PIT`，不繼承C26 22-day stale guard | RESULT_AVAILABLE／C27_NOT_PROMOTED／C28_SELECTION_GATE_PASS／C28_FORWARD_OOS_AUTHORIZED。C27-C24 Return +19.27pp但C27-C23仍Return -5.73pp、MDD +1.00pp、RoMD -0.51；C28-C25 Return +50.33pp、MDD -0.80pp、RoMD +2.89、EV +0.19R、selection R +83.72R；C28-C23 Return +43.90pp、MDD -1.71pp、RoMD +2.80、EV +0.29R、selection R +109.91R；C28-C27 Return +49.62pp、MDD -2.71pp、RoMD +3.31、EV +0.21R，故只凍結C28 feasible-ascent路徑進Forward-OOS，不再Selection內調selector／stale cutoff／capital objective |
| Current Selection strategy matrix | `SR-C23 / C25 / C28 / C32 / C33 / C34` | 2×3：historical Min/Full ROOS × DL-off/MR-12B feasible-ascent/MR-13A feasible-ascent；Min row重用current五欄Selection結果，Full row使用獨立`PARAM-P4` historical Full rolling active params與formal rules；兩列的K/R0與reserved-capital floor皆由同param-source／rule-policy的DL-off baseline逐日建立 | IMPLEMENTED／FULL_ROW_RESULT_PENDING；Selection profile固定2014-01-01～2020-12-31，重用既有`DL-CONT12B-PIT`與`DL-CONT13A-PIT`，不重選模型、不修改selector。核心Full-row source-only contrast=`C34-C33`；C32-C23與C34-C28只可解讀為Min/Full完整策略體系interaction。Selection與Forward-OOS由兩個永久共存的Strategy Compare profiles隔離；新結果分別寫入獨立output root，但cache resolver可依config唯讀掃描舊`outputs/strategy_compare/runs/`以重用既有pair，不得再靠改寫同一組enabled arms切換階段 |
| Current Forward-OOS strategy matrix | `SR-C1 / C3 / C20 / C29 / C30 / C31` | 2×3：Min/Full ROOS × DL-off/MR-12B feasible-ascent/MR-13A feasible-ascent；feasible-ascent的K/R0與reserved-capital floor一律由同param-source／rule-policy的DL-off baseline逐日建立 | IMPLEMENTED／RESULT_PENDING；共同期間由`DL-CONT12B`與`DL-CONT13A` frozen Forward-OOS runtime coverage交集自動決定；核心source-only contrasts=`C29-C20`與`C31-C30`。Forward-OOS score artifact contract必須依profile sample scope解析：event ranker使用`continuous_ranker_scores.csv`且只取`split=oos`，MR-13A daily ranker使用trainer的canonical `daily_ranker_oos_scores.csv.gz`且整份即為OOS daily universe，不得要求event-only `split`欄。Strategy Compare前置會自動建立可由既有真理工件確定產生的參數／推論工件，但不得訓練模型權重；模型訓練工作類型的config-driven「準備策略比較所需模型工件」必須涵蓋目前enabled arms需要的Selection PIT與Forward-OOS continuous sources，ready contract直接REUSE、缺少／不相容Forward-OOS model則由模型工作類型補訓；不得依結果修改selector或模型 |
...
| ID | 名稱 | 狀態 |
|---|---|---|
| `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_PASS／STAGE3_PIT_RUNTIME_READY／SELECTION_STRATEGY_GATE_PASS／FORWARD_OOS_STRATEGY_AUTHORIZED。Stage 1：all-stock OOS daily/global rho=0.1755/0.1761、pair=56.12%；breakout slice daily rho=0.1490、pair=56.91%、K=10 raw Target/Lift=1.6198R/+0.4056R、Boundary=54.02%。Stage 2：2013-04-01～2020-12-31、8 folds、778,532 target-valid groups、100% coverage；all-stock PIT global/daily rho=0.0783/0.1111、pair=53.84%、spread=+0.3319R、年度rho/spread正向8/8；breakout slice global/daily rho=0.0580/0.0784、pair=53.98%、Top-K Lift=+0.1102R、Boundary=51.49%，Gate PASS。Stage 3 daily PIT runtime contract完成後，current五欄Min ROOS Selection比較中C28 feasible-ascent達Return 151.88%、MDD 18.86%、RoMD 8.05、EV 0.71R；相對C25 source-only Return +50.33pp／RoMD +2.89／EV +0.19R，相對C23 baseline Return +43.90pp／MDD -1.71pp／RoMD +2.80／EV +0.29R。Selection Gate PASS並授權凍結C28語意進Forward-OOS；正式取代`DL-CONT12B / MR-12B`仍須Forward-OOS策略結果。 |
...
| `DL-CONT13A` | `CONT13A` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` | Frozen Forward-OOS daily continuous score；策略盤前使用最新已完成交易日score；供current Min/Full ROOS × frozen feasible-ascent controlled validation | IMPLEMENTED／RESULT_PENDING／NOT_YET_PROMOTED；Strategy Compare只重用既有MR-13A Forward-OOS score，不選模或訓練模型 |
...
| `SR-C20` | Min ROOS: MR-12B max-DL feasible-ascent | frozen feasible-ascent；K/R0與reserved-capital floor均由同一current Min ROOS DL-off baseline逐日建立；score source=`DL-CONT12B` | RESULT_AVAILABLE historical／CURRENT_FIVE_FIELD_RETEST_PENDING；舊Return=179.54%、MDD=14.48%、RoMD=12.40、EV=0.84R來自先前Min ROOS universe，只供歷史參考；本次2×3 Forward-OOS矩陣須以current五欄Min ROOS重新計算，不得直接重用舊數值作結論 |
...
| `SR-C29` | Forward-OOS: Min ROOS + MR-13A daily feasible-ascent | 凍結SR-C28的feasible-ascent與daily information-date lookup；K/R0與reserved-capital floor改由同一current五欄Min ROOS DL-off baseline逐日建立；score source=`DL-CONT13A` | IMPLEMENTED／RESULT_PENDING／FORWARD_OOS_CANDIDATE；與C20形成Min-row純DL source contrast，禁止依Forward-OOS結果修改selector、score cutoff、stale guard、capital objective或模型 |
| `SR-C30` | Forward-OOS: Full ROOS + MR-12B feasible-ascent | Full ROOS active params與formal rules；K/R0及reserved-capital floor由同參數DL-off C1逐日建立；frozen feasible-ascent + `DL-CONT12B` | IMPLEMENTED／RESULT_PENDING；與C1比較DL經濟效果，與C31形成Full-row純DL source contrast |
| `SR-C31` | Forward-OOS: Full ROOS + MR-13A daily feasible-ascent | 與C30完全相同Full ROOS active params、formal rules、same-param K/R0 hard feasibility與frozen feasible-ascent；唯一DL source改為`DL-CONT13A` | IMPLEMENTED／RESULT_PENDING；與C30為純DL source contrast，與C29僅作Min/Full完整策略體系interaction，不解讀為單一參數效果 |
| `SR-C32` | Selection PIT: Full ROOS baseline | `PARAM-P4` historical Full ROOS rolling active params；formal rules；DL-off；2014～2020無前視active-param replay，作Selection Full row共同baseline | IMPLEMENTED／RESULT_PENDING；不得使用2021+ `full_roos` forward工件倒灌Selection；與C23只作Min/Full完整策略體系interaction |
| `SR-C33` | Selection PIT: Full ROOS + MR-12B feasible-ascent | 與C32完全相同`PARAM-P4`、formal rules與same-param K/R0 hard feasibility；frozen feasible-ascent + `DL-CONT12B-PIT` | IMPLEMENTED／RESULT_PENDING；與C32比較MR-12B策略增益，與C34形成Full-row純DL source contrast |
| `SR-C34` | Selection PIT: Full ROOS + MR-13A daily feasible-ascent | 與C33完全相同`PARAM-P4`、formal rules、same-param K/R0 hard feasibility與frozen feasible-ascent；唯一DL source改為`DL-CONT13A-PIT` | IMPLEMENTED／RESULT_PENDING／SELECTION_FULL_ROW_CANDIDATE；核心Gate為C34-C33及C34-C32；不得依Selection結果再調selector、score cutoff、capital objective或模型 |
...
34. 下一個Forward-OOS strategy Gate已實作為2×3矩陣：`SR-C1/C3`分別為Full/Min DL-off baseline；`SR-C20/C29`為current五欄Min ROOS + frozen feasible-ascent + `DL-CONT12B/DL-CONT13A`；`SR-C30/C31`為Full ROOS + 同一frozen feasible-ascent + `DL-CONT12B/DL-CONT13A`。Max-DL K/R0與reserved-capital floor不再以Min-specific文字定義，而是一律由同`param_source/rule_policy`的DL-off baseline逐日建立，因此Full arms不得借用Min ROOS資源底線。共同比較期間由兩個Forward-OOS score sources的runtime coverage交集自動決定。核心source-only contrasts為`C29-C20`與`C31-C30`；跨Min/Full只可解讀為完整策略體系interaction，不得宣稱為單一參數或rule效果。狀態=`IMPLEMENTED / RESULT_PENDING`。
35. 在Forward-OOS正式Gate前補對稱Selection Full row：新增`PARAM-P4` historical Full ROOS rolling active params及`SR-C32/C33/C34`，分別為Full DL-off、Full + MR-12B feasible-ascent、Full + MR-13A feasible-ascent。Selection固定2014-01-01～2020-12-31並重用既有CONT12B/CONT13A PIT；不重訓模型、不重新搜尋selector。Full-row核心source-only contrast為`C34-C33`，C34相對C32用來判斷MR-13A是否能轉成Full portfolio economics；跨C32-C23/C34-C28只能解讀為完整策略體系interaction。狀態=`IMPLEMENTED / RESULT_PENDING`。
