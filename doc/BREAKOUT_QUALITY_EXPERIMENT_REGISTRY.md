...
| `MR-13F` | Daily Universal Direct-R Huber Regression / `daily_universal_no_time_r_huber` | IMPLEMENTED／AWAITING_FORWARD_MODEL_RESULT／NO_PIT／NO_RUNTIME_DL_SOURCE。固定13E的daily sample universe、raw target identity、architecture、optimizer、seed與split；唯一learning-semantic change為直接以two-logit margin預測raw R並使用Huber loss delta=`1R`，epoch依Validation Huber loss選擇。Forward model report同時輸出Huber/MAE/RMSE/Bias與daily Spearman/pair/Top-K diagnostics；OOS不參與loss或epoch selection。 |
...
| `AUD-mr13e-minimum-repair-mechanism` | `mr13e-minimum-repair-mechanism` | `MR-12B / MR-13A / MR-13E` Selection PIT + Forward repair days；completed selector trace + repair search-certificate sidecar | IMPLEMENTED／READ_ONLY／SOURCE_REPLAY_REFRESH_REQUIRED／AWAITING_RESULT／GLOBAL_COMBINATORIAL_ORACLE_WITHDRAWN_FOR_SCALE | 2026-08-14真實Selection replay證明無state-cap的全K-combination best-first exact oracle在數十候選、K近10時可進入不可接受的`C(n,K)`狀態爆炸，因此撤回該engineering設計，不以timeout/node cap近似冒充exact。Strategy Compare現在只序列化production minimum-repair本來就完整枚舉的single-swap搜尋證據：每step記錄evaluated/progress/feasible swap counts、accepted out/in、before/after count/reserve deficit與frozen score；若raw不合法且`repair_steps=1`，可exact證明minimum replacement distance=1且production取全部feasible one-swap中最高frozen-score basket；`repair_steps>1`則標`MULTI_STEP_GREEDY_PATH_UNRESOLVED`，feasible-ascent仍只宣稱1-swap local optimum。Future Target僅在Audit post-replay attribution。新canonical sidecar=`score_ranking_repair_search_certificate.csv`，舊pair缺檔只需一次strategy replay refresh。 |
