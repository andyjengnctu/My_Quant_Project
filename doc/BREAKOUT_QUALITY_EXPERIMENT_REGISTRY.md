...
| Planned Forward-OOS daily source | `DL-CONT13A` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` frozen Forward-OOS daily continuous score；盤前使用最新已完成交易日score | PLANNED／SOURCE_ID_RESERVED；只允許沿用已完成MR-13A Forward-OOS權重／score contract，不由Strategy Compare選模或訓練；用於C28 Selection通過後的Forward-OOS strategy Gate |
...
| `DL-CONT13A` | `CONT13A` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` | Frozen Forward-OOS daily continuous score；策略盤前使用最新已完成交易日score；只供Selection已凍結之SR-C29 controlled validation | PLANNED／SOURCE_ID_RESERVED／NOT_YET_PROMOTED |
...
| `SR-C29` | Forward-OOS: MR-13A daily feasible-ascent | 凍結SR-C28的Min ROOS K/R0、feasible-ascent selector、execution order與daily information-date lookup；唯一期間／DL runtime source切換為2021+ `DL-CONT13A` frozen Forward-OOS score | PLANNED／FORWARD_OOS_CANDIDATE；主對照固定為current五欄Min ROOS DL-off baseline與同selector的MR-12B Forward-OOS arm，禁止依Selection或Forward-OOS結果修改selector、score cutoff、stale guard、capital objective或模型 |
