...
| Current model research | `MR-12B` | No-time all-event within-day Pairwise RankNet ranker；固定all-event Target／InceptionTime，以同日pairwise logistic直接學cross-sectional ordering | RESULT_AVAILABLE／MODEL_ECONOMIC_IMPROVEMENT_SUPPORTED；C19/C20 controlled replay均支持；P2 Fixed-K=10 OOS亦顯示MR-12B較MR-12A改善NDCG／Top-K lift／boundary。Dynamic-K strict common-complete為94/223日，但candidate-level complete coverage與C17 runtime scored-candidate coverage皆97.52%，表示day coverage低主要來自少量缺分散落多日。K-stratified已確認弱勢高度集中K=1：eligible 179/223日、common-complete 71/94日；K=1 MR-12B vs MR-12A NDCG `0.4997 vs 0.5883`（Δ `-0.0886`）、Boundary `38.03% vs 57.75%`（Δ `-19.72pp`）、Lift `0.0425R vs 0.4903R`。K=2/3的NDCG與Boundary反而偏向MR-12B，但僅13/5日，不足以採用。此段仍是C17-reference raw-score診斷，不等同正式resource-feasible basket。下一步先在完整forward-OOS共同候選做config-driven Fixed-K prefix sweep（K=1/2/3/5/10），區分一般Top-1弱點與C17 path/subset效應；歸因完成前不新增MR、不改pairwise loss。 |
