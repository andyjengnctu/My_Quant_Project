...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；每個合法ticker/date皆可成為sample，candidate membership不進training sample selection；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet pairwise logistic與mean daily Spearman選epoch | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／STAGE2_PIT_IMPLEMENTED_PENDING_RESULT／STRATEGY_NOT_CONNECTED。Stage 1 full-market OOS：daily rho=0.1755、pair=56.12%、Top 10% Target=1.8261R、Bottom 10%=0.4846R；breakout-candidate OOS：17,346 groups、603 competition days、daily rho=0.1490、pair=56.91%、Top-K Target=1.6198R、Lift=+0.4056R、Oracle overlap=61.04%、Boundary=54.02%、gap=+0.2158R。相對MR-12B candidate anchor，raw Top-K/Lift近乎持平、pair/boundary略升但daily rho略降，因此只足以授權Stage 2 historical Selection PIT，不代表已取代MR-12B。MR-13A NDCG因Stage 1 candidate slice沿用full-daily-universe percentile relevance，不能作與MR-12B event-universe NDCG的嚴格head-to-head。Model-research active仍為MR-13A；strategy workflow active仍為MR-12B。 |
...
| ID | 名稱 | 狀態 |
|---|---|---|
| `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／STAGE2_PIT_IMPLEMENTED_PENDING_RESULT；sample scope=`daily_eligible_stock_days`，target=`daily_opportunity_no_time_r_v1`。Stage 1以1,578,349 samples／558 tickers完成forward-OOS：all-stock OOS daily rho=0.1755、global rho=0.1761、pair=56.12%；breakout slice daily rho=0.1490、pair=56.91%，K=10 raw Target/Lift=1.6198R/+0.4056R，Boundary=54.02%。Stage 2沿用同一MR identity與既有expanding-window PIT trainer，sample provider改由profile scope自動路由；daily primary audit為all-valid stock-days，breakout candidate為第二層diagnostic；PIT score仍禁止forward-OOS runtime，strategy source／ROOS尚未授權。`DL-CONT12B / MR-12B`仍為current validated continuous anchor。 |
