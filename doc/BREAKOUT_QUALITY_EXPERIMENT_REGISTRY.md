...
| Pending Selection PIT source | `DL-CONT13A-PIT` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` daily Selection PIT score | IMPLEMENTED／PIT_MODEL_GATE_PASS／STRATEGY_RUNTIME_REBUILD_REQUIRED／RESULT_PENDING／NOT_PROMOTED；策略端只接受`score_eligibility_contract=feature_history_only`的新PIT manifest，每個盤前決策查最新已完成交易日score；舊Stage 2 target-complete-only PIT table會fail-fast拒絕 |
| Pending Selection strategy translation | `SR-C27 / SR-C28` | C27完全沿用C24 minimum-repair、C28完全沿用C25 feasible-ascent；唯一DL source改為`DL-CONT13A-PIT`，不繼承C26 22-day stale guard | IMPLEMENTED／RESULT_PENDING／PIT_REBUILD_REQUIRED；C27-C24與C28-C25隔離純model/source差異，C27/C28-C23檢查portfolio economics，C28-C27檢查selector轉化；取得Selection結果前不得升格MR-13A或進ROOS |
...
| ID | 名稱 | 狀態 |
|---|---|---|
| `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise` | RESULT_AVAILABLE／FORWARD_OOS_MODEL_GATE_PASS／SELECTION_PIT_MODEL_GATE_PASS／STAGE3_IMPLEMENTED_PENDING_REBUILD_AND_STRATEGY_RESULT。Stage 1：all-stock OOS daily/global rho=0.1755/0.1761、pair=56.12%；breakout slice daily rho=0.1490、pair=56.91%、K=10 raw Target/Lift=1.6198R/+0.4056R、Boundary=54.02%。Stage 2：2013-04-01～2020-12-31、8 folds、778,532 target-valid groups、100% coverage；all-stock PIT global/daily rho=0.0783/0.1111、pair=53.84%、spread=+0.3319R、年度rho/spread正向8/8；breakout slice global/daily rho=0.0580/0.0784、pair=53.98%、Top-K Lift=+0.1102R、Boundary=51.49%，Gate PASS。Stage 3 preflight將score eligibility修正為feature-history-only並將daily runtime lookup改為最新已完成交易日；舊778,532-row PIT可保留作model-audit evidence但不可直接當strategy source，必須重建。`DL-CONT12B / MR-12B`仍為current validated continuous anchor。 |
...
| `SR-C27` | Selection PIT: MR-13A daily minimum-repair | 與SR-C24完全相同historical P2 params、K/R0、minimum-repair selector與execution order；唯一DL source差異為`DL-CONT12B-PIT → DL-CONT13A-PIT`，盤前每日使用最新已完成交易日score | IMPLEMENTED／RESULT_PENDING／PIT_REBUILD_REQUIRED；此ID屬MR-13A新model-translation branch，C26結案時「不新增C27」只表示不再延伸C26 runtime微調。C27不繼承C26 22-day stale guard；主對照C27-C24隔離model/source，C27-C23檢查相對DL-off portfolio economics |
| `SR-C28` | Selection PIT: MR-13A daily feasible-ascent | 與SR-C25完全相同historical P2 params、K/R0、feasible-ascent selector與execution order；唯一DL source差異為`DL-CONT12B-PIT → DL-CONT13A-PIT`，盤前每日使用最新已完成交易日score | IMPLEMENTED／RESULT_PENDING／PIT_REBUILD_REQUIRED；不繼承C26 22-day stale guard。主對照C28-C25隔離model/source，C28-C23檢查portfolio economics，C28-C27檢查相同daily source下selector轉化 |
