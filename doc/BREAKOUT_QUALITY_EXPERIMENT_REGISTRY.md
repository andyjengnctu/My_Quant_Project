...
| Current model research | `MR-12B` | No-time all-event within-day Pairwise RankNet ranker；固定all-event Target／InceptionTime，以同日pairwise logistic直接學cross-sectional ordering | RESULT_AVAILABLE／MODEL_ECONOMIC_IMPROVEMENT_SUPPORTED；C19/C20 controlled replay均支持。P2完整Forward-OOS prefix sweep已排除「MR-12B一般性Top-1弱」假說：K=1/2/3/5/10的NDCG Δ(B-A)皆為正，分別`+0.0477/+0.0327/+0.0309/+0.0281/+0.0290`；K=1 Boundary亦`+5.21pp`、Top-1 Lift `0.8244R vs 0.5069R`。因此C17-reference common-complete K=1子集中的NDCG `-0.0886`、Boundary `-19.72pp`、Lift `-0.4478R`不能解讀為Pairwise本身Top-1 objective mismatch；較可能是reference-date/regime conditioning或C17 orderable candidate／portfolio-state conditioning。下一步固定同一批reference dates與實際K，比較完整OOS event-group universe vs C17 orderable universe，先拆date effect與candidate/path effect；完成前不新增MR、不改pairwise loss。 |
