...
| Pending Selection PIT source | `DL-CONT13A-PIT` | `MR-13A / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_pairwise` daily Selection PIT score | RESULT_AVAILABLE／PIT_MODEL_GATE_PASS／STRATEGY_RUNTIME_READY／SELECTION_STRATEGY_RESULT_PENDING／NOT_PROMOTED；策略端只接受`score_eligibility_contract=feature_history_only`的新PIT manifest，每個盤前決策查最新已完成交易日score；舊Stage 2 target-complete-only PIT table會fail-fast拒絕 |
