...
| Current Selection strategy matrix | `SR-C32 / C23 / C25 / C28 / C35 / C37` | 既有單seedGate保留C32/C23/C25/C28/C35；新增`SR-C37`只重用frozen MR-13E PIT score，將daily score percentile以expanding/PIT成熟target校準為Expected R，並把basket objective由score-sum改成`Σ(Expected R × canonical planned initial risk)`；historical Min params、K/R0、sizing、cash與execution全部固定。 | C25/C28/C35既有結果不變；C37=`IMPLEMENTED／AWAITING_SELECTION_STRATEGY_RESULT／ROBUSTNESS_OFF`。Expected-R calibration每個Selection策略年只使用該年以前且`label_eval_end_date < cutoff`的成熟資料；負Expected R不改K/R0，避免同一輪混入少買／留現金語意。 |
| Current Forward-OOS strategy matrix | `SR-C1 / C3 / C20 / C29 / C36 / C38` | 既有C1/C3/C20/C29/C36結果保留；`SR-C38`是C37的Forward對應，使用frozen `DL-CONT13E` score與只由Forward frozen OOS `execution_start`前Selection PIT成熟target fit的Expected-R mapping（目前Forward contract為2021-01-01），basket objective同為Expected Dollar PnL；K/R0、sizing、cash與execution與C36固定相同。 | C20/C29/C36既有結果不變；C38=`IMPLEMENTED／FORWARD_EXECUTION_CONDITIONAL_ON_C37_SELECTION_DECISION／ROBUSTNESS_OFF`，不得用Forward target fit calibration。研究流程先執行C37 Selection，再決定是否執行C38；仍不promotion MR-13E、不做13E 8-seed。 |
