...
| `AUD-c23-c26-pit-portfolio-translation` | `c23-c26-pit-portfolio-translation` | `SR-C26` vs `SR-C23 / SR-C25` | IMPLEMENTED／RESULT_PENDING | C26已把same-param selection R修到相對C23 `+7.99R`，但Return仍`-7.71pp`；本Audit只讀既有replay，使用既有strategy attribution primitive拆Exclusive selection ΔPnL、Common trades ΔPnL、capital geometry、slot occupancy與wealth path，判斷R改善未轉成portfolio wealth的來源；不重跑portfolio、不改22日門檻、不進OOS。 |
