...
| `AUD-forward-robustness-portfolio-translation` | `forward-robustness-portfolio-translation` | `MR-13A` vs `MR-12B` Forward 8-seed robustness | PLANNED／READ_ONLY／ALL_SEEDS_REQUIRED | schema v6已證明ranking→RoMD只部分轉化；此Audit必須同時涵蓋8個matched seeds，不得只挑S1/S7/S8或best/worst seeds。目標固定拆解active trade-set common/exclusive、realized R、risk-dollar sizing、slot/capital geometry與wealth/MDD path，回答ranking edge為何只在3/6正ranking seeds轉成正RoMD。若既有robustness run未保留足夠replay detail，Audit不得從`seed_results.csv`猜交易層結論；應由同一scientific contract重新產生可稽核的compact attribution source，而非永久保留完整model/score/replay樹。 |
