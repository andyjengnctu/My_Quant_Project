...
| Current model research | `MR-13A` | Daily Universal No-time Pairwise Ranker / `daily_universal_no_time_pairwise`；保留all-stock daily universal方向，feature-eligible ticker/date皆可取得daily score；沿用300×10 sequence、InceptionTime、40-day adverse-first no-time opportunity-R、within-day RankNet與mean daily Spearman選epoch | RESULT_AVAILABLE／ITERATIVE_RESEARCH_ACTIVE／SELECTION_STRATEGY_PASS／SINGLE_SEED_FORWARD_FAIL／MULTI_SEED_REOPENED／MATCHED_ROMD_INCONCLUSIVE／MATCHED_SELECTION_R_PENDING／NOT_PROMOTED。Selection current五欄Min ROOS下C28 Return=151.88%、MDD=18.86%、RoMD=8.05、EV=0.71R，C28-C25 source-only Return +50.33pp、RoMD +2.89、EV +0.19R、same-param selection R +83.72R。修正Forward score eligibility後，canonical seed 42 replay的C29 Return=83.81%、MDD=15.71%、RoMD=5.34、EV=0.52R；但2026-08-12正式8-seed robustness（seed_count=8、generator_seed=20260810）中MR-13A Return/MDD/RoMD/EV Mean=`148.18%/15.47%/9.97/0.94R`、DL選擇R Mean=`-54.70R`，高於MR-12B的`129.01%/15.87%/8.21/0.77R`與`-105.34R`。RoMD同seed仍為4勝4敗，ΔRoMD Mean/Median/Std=`+1.76/+1.48/4.47`，故平均與中位數改善尚未形成一致seed優勢；年度右勝左為2021～2026依序`6/8,5/8,5/8,7/8,5/8,3/8`，改善亦非單一年度獨占。下一個判定只需由既有`seed_results.csv`補出同seedΔDL選擇R；不得挑best seed、做ensemble或依Forward結果調hyperparameter。 |
...
| Current continuous DL anchor | `DL-CONT12B` | `MR-12B / ARCH-inception_time_v1 / PROFILE-strategy_aligned_no_time_all_event_pairwise` frozen OOS continuous score | SOURCE_SUPPORTED_FOR_MODEL_RESEARCH／CANONICAL_SEED42_FORWARD_ANCHOR／MULTI_SEED_SUPERIORITY_NOT_ESTABLISHED。future-independent Forward score重建後，seed 42的C20於2021-01-01～2026-03-02 Return=190.26%、MDD=18.75%、RoMD=10.15、EV=1.17R、same-param selection R=+35.93R；2026-08-12正式8-seed robustness則MR-12B Return/MDD/RoMD/EV Mean=`129.01%/15.87%/8.21/0.77R`、DL選擇R Mean=`-105.34R`，低於MR-13A的`148.18%/15.47%/9.97/0.94R`與`-54.70R`，但RoMD同seed仍4勝4敗。DL-CONT12B因此維持canonical runtime anchor，直到同seedΔDL選擇R完成後再決定是否有足夠證據改變anchor；不得把seed 42或獨立分布Mean單獨視為跨seed模型優勢。 |
