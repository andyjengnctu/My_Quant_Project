...
| Current Selection strategy matrix | `SR-C32 / C23 / C25 / C28 / C35 / C39` | Current enabled Selection matrix維持Full/Min baseline、MR-12B、MR-13A、MR-13E，新增Selection-only C39；C37 Expected-PnL只保留historical identity/result。C39 frozen重用`DL-CONT13E-PIT`，同C35的Min params、K/R0、sizing、cash、orderability與execution，只把objective改成`Σ(Expected Excess-R × canonical planned initial risk)`。Expected Excess-R由MR-13E daily percentile對`raw R − same-day full daily-eligible target-valid mean R`的expanding/PIT nondecreasing isotonic mapping取得。 | IMPLEMENTED／AWAITING_SELECTION_RESULT／ROBUSTNESS_OFF／NO_FORWARD_ARM_YET。C39先只回答移除共同absolute-R baseline後，relative rank alpha×deployable risk是否能比C35更好保留MR-13E edge；Selection不支持就停止，不建立C40。 |
| Current Forward-OOS strategy matrix | `SR-C1 / C3 / C20 / C29 / C36` | C1/C3/C20/C29/C36既有結果保留；C38 historical implementation只供舊Expected-PnL實驗重現，不列入current enabled matrix；C39目前沒有Forward對應arm。 | `NO_NEW_FORWARD_ARM_PENDING_C39_SELECTION`。MR-13F/13G direct raw-R regression均已REJECT/no-PIT；新的relative Excess-Alpha假說必須先通過C39 Selection，未通過前不得建立C40或查看新Forward結果。 |
...
| `MR-13E` | Daily Universal Full-list Delta-NDCG-weighted Pairwise Ranker / `daily_universal_no_time_full_list_ndcg_pairwise` | RESULT_AVAILABLE／MODEL_GATES_PASS／SELECTION_SINGLE_SEED_STRATEGY_GATE_PASS／FORWARD_SINGLE_SEED_STRATEGY_GATE_FAIL_VS_MR12B／NOT_PROMOTED／RESOURCE_TRANSLATION_GAP_IDENTIFIED／RAW_TO_MINIMUM_REPAIR_GAP_IDENTIFIED／MINIMUM_REPAIR_MECHANISM_AUDIT_AUTHORIZED。模型層Selection/Forward皆維持正edge；策略層Selection C35=`153.96%/RoMD 7.74/DL選擇R +35.96R`，Forward C36=`128.77%/7.49/-66.22R`。Orderable Audit顯示Forward raw Top-N/Repair Lift勝12B；selector-stage Audit進一步證明主要額外損失集中Raw→minimum-repair：13E=`-1.3763R`、12B=`-0.9292R`、差=`-0.4471R`，後續ascent/action/fill差很小。Selection同stage則13E損失少於12B，故目前不授權13E 8-seed，也不直接改repair；先用production exhaustive single-swap certificate判定1-step resource constraint，並把multi-step greedy path明確留為未解區段。 |
...
`MR-13A`、`MR-13B`、`MR-13C`、`MR-13D`、`MR-13E`、`MR-13F`、`MR-13G` 已正式占用。後續daily-universal target、loss、architecture或training-data semantics若再變更，必須使用新的 `MR-*`，不得覆寫13A～13G。
...
| `DL-CONT13E-PIT` | `CONT13E_PIT` | `MR-13E / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_full_list_ndcg_pairwise` | Daily Selection PIT continuous score；每個盤前決策只使用最新已完成交易日score，score eligibility沿用daily feature-history-only contract | RESULT_AVAILABLE／SELECTION_SINGLE_SEED_GATE_PASS／NOT_PROMOTED。已完成C35 source-only Selection策略Gate；source保留供read-only alignment Audit與後續受控研究，不因Selection單期結果加入multi-seed。 |
| `DL-CONT13E` | `CONT13E` | `MR-13E / ARCH-inception_time_v1 / PROFILE-daily_universal_no_time_full_list_ndcg_pairwise` | Frozen Forward-OOS daily continuous score；策略盤前使用最新已完成交易日score，runtime score eligibility沿用feature-history-only contract | RESULT_AVAILABLE／FORWARD_SINGLE_SEED_GATE_FAIL_VS_MR12B／NOT_PROMOTED。已完成C36 source-only Forward策略Gate；相對MR-12B明顯落後，source僅保留供read-only alignment／selector-stage／minimum-repair mechanism Audit與研究，不取代`DL-CONT12B`。 |
...
| `SR-C35` | Selection PIT: Min ROOS + MR-13E daily feasible-ascent | 與`SR-C25/SR-C28`完全相同historical Min ROOS params、all-off rules、K/R0、feasible-ascent與execution；唯一DL source=`DL-CONT13E-PIT` | RESULT_AVAILABLE／SELECTION_SINGLE_SEED_GATE_PASS／ROBUSTNESS_OFF。Return=`153.96%`、MDD=`19.88%`、RoMD=`7.74`、EV=`0.54R`、DL選擇R=`+35.96R`；相對C25 Return/RoMD/DL選擇R=`+52.41pp/+2.58/+9.77R`，但相對C28 RoMD/EV/DL選擇R=`-0.31/-0.17R/-73.96R`。 |
| `SR-C36` | Forward-OOS: Min ROOS + MR-13E daily feasible-ascent | 與`SR-C20/SR-C29`完全相同current Min ROOS、all-off rules、K/R0、feasible-ascent與execution；唯一DL source=`DL-CONT13E` | RESULT_AVAILABLE／FORWARD_SINGLE_SEED_GATE_FAIL_VS_MR12B／ROBUSTNESS_OFF。Return=`128.77%`、MDD=`17.18%`、RoMD=`7.49`、EV=`0.87R`、DL選擇R=`-66.22R`；相對C29明顯改善，但相對C20 Return/RoMD/EV/DL選擇R=`-61.49pp/-2.65/-0.30R/-102.16R`。 |
...
| `SR-C39` | Selection PIT: Frozen MR-13E Relative Excess-Alpha feasible-ascent | 重用`DL-CONT13E-PIT` frozen score；先在完整canonical daily-eligible target-valid universe定義`Excess R = raw R − same-day mean raw R`，再以每個策略年cutoff以前且`label_eval_end_date < cutoff`的成熟PIT rows fit daily percentile→Expected Excess-R nondecreasing isotonic PAVA。與C35固定同一historical Min params、K/R0、sizing、cash、orderability、execution，只把basket objective改為`Σ(Expected Excess-R × canonical planned initial risk)`；negative excess只影響quality objective，不改K/R0。 | IMPLEMENTED／AWAITING_SELECTION_RESULT／ROBUSTNESS_OFF／SELECTION_ONLY。C39不建立Forward arm；只有Selection證據支持後，才另占新的SR-C ID做Forward。 |
