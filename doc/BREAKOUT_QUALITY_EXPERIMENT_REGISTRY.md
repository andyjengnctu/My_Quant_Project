...
| `AUD-forward-robustness-portfolio-translation` | `forward-robustness-portfolio-translation` | `MR-13A` vs `MR-12B` Forward 8-seed robustness | IMPLEMENTED／AWAITING_COMPACT_SOURCE_REBUILD／READ_ONLY／ALL_SEEDS_REQUIRED | schema v6已證明ranking→RoMD只部分轉化；正式實作固定涵蓋全部8個matched seeds，不得挑S1/S7/S8或best/worst seeds。底層common/exclusive trade、risk-dollar decomposition、slot occupancy與wealth path直接reuse既有canonical strategy-attribution primitive；robustness在刪除full replay前只永久保存active trades/equity/daily capacity/selected buys的gzip compact source，不保存完整scores/orderable/replay tree。舊`2f70dbe73dcf3fed` scientific result已完成但compact source尚缺時，同一robustness入口以相同8 seeds／scientific fingerprint重建工件，且核對selected epoch／fold count、全部正式策略metrics與逐年度Return；既有observation若已有model/score SHA亦須exact一致，舊schema沒有SHA時不得假造歷史hash。compact unit先為`PENDING`，全部一致後才標記`VERIFIED`；未驗證或中斷unit不得被resume/Audit視為READY。Audit本身只讀，不train、不replay。尚未取得新Audit結果，不得預先promotion或淘汰MR-13A。 |
