...
- Selection Full ROOS使用獨立`PARAM-P4` historical rolling active params：2014～2020、120m train／12m OOS、canonical Full optimizer search space、TP/DL/History threshold依current optimizer policy固定OFF；不得以2021+ `full_roos` forward active params倒灌Selection。Selection Full historical row為C32/C33/C34，P4與三個scientific IDs永久保留；current Selection核心references為C32 Full DL-off baseline、C23 Min baseline與C42 Min MR-13E exact constrained；MR-13K Seed42 Forward GO後新增C53 Min MR-13K exact constrained作Selection source-only controlled arm。MR-13H C51/C52與C33/C34及C25/C28/C46/C47/C48等已結案研究arm保留歷史重現但inactive；engine支援standalone DL-off comparator，因此C32不必掛隱藏Full+DL arm。
- Selection與Forward-OOS由兩個永久共存的Strategy Compare profiles隔離；current核心比較為Selection=`C32/C23/C42/C53`、Forward=`C1/C3/C44/C54/C55`。C53/C54分別是MR-13K相對MR-13E的Selection/Forward source-only exact controlled arm；C55是Forward-only Plan B，完整沿用C54的MR-13K primary score、Min/all-off、K/R0、canonical cash/sizing/orderability/execution與exact branch-and-bound，只增加MR-13M frozen low-adverse score的baseline-relative safety hard floor：選中basket的13M score coverage與score-sum不得低於同日DL-off Min ROOS baseline。13M只作constraint，不進primary objective、不做score fusion、不設固定threshold。C55不加入current 8-seed robustness或production；只有Forward GO才允許建立13M Selection PIT與Selection dual-model arm。MR-13H C51/C52已結案移至historical compatibility。Full只作DL-off完整策略體系baseline，Min ROOS作同param/rule baseline，MR-13E exact constrained作current production allocator；其他已完成controls只保留歷史重現。
...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained。MR-13H no-breach Label已完成Selection/Forward robustness並結案為`VALID_LABEL_SIMPLIFICATION / NOT_SELECTED_FOR_PROMOTION`。MR-13K Pure-MFE與MR-13M low-adverse已分別證明upside與path-risk component都具高cross-sectional learnability；MR-13K的C42/C53與C44/C54 current 8-seed full-flow robustness仍獨立完成後再結案13K。MR-13L dual raw-R MSE、MR-13N single-model equal-rank composite、固定50/50 frozen expert fusion與MR-13O strict Pareto pair supervision皆已REJECT，因此Target／pair-definition微調路線停止。Current research分成兩層：Strategy Plan B=`C55`，以MR-13K作primary objective、MR-13M只作baseline-relative hard safety floor；Model Plan A列於`doc/BREAKOUT_QUALITY_RESEARCH_QUEUE.md`，固定MR-13H economic Target、只改parameter-free path-dynamics input representation。C55先做Seed42 Forward，PIT/robustness/production staged off；production candidate固定C42/C44。MR-13I/J risk-normalized/context路線已REJECT，只保留歷史重現。
