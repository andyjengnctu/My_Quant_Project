...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained。MR-13H no-breach Label已完成Selection/Forward 16-seed robustness並結案為`VALID_LABEL_SIMPLIFICATION / NOT_SELECTED_FOR_PROMOTION`。MR-13K Pure-MFE與MR-13M low-adverse已分別證明upside與path-risk component都具高cross-sectional learnability；MR-13K的C42/C53與C44/C54 current 8-seed full-flow robustness仍獨立完成後再結案13K。MR-13L dual raw-R MSE decomposition已在Forward model Gate REJECT。Model Research active profile現為MR-13N `daily_universal_full_horizon_equal_rank_mfe_low_adverse_full_list_ndcg_pairwise / inception_time_v1`：沿用相同40D complete horizon與earliest max-MFE peak，先將`MFE R`與`-adverse-to-peak R`各自在同日轉成[0,1] average-rank percentile，再固定0.5/0.5等權組合；training仍是Full-list Delta-NDCG weighted same-day RankNet，不掃lambda。MR-13N只做Seed42 Forward model Gate，PIT/strategy staged off。production candidate固定C42/C44。MR-13I/J risk-normalized/context路線已REJECT，只保留歷史重現。
- `services/breakout_quality/daily_target_comparison.py`是daily label simplification的正式read-only model-provider診斷：只從兩個config profile建立同一daily stock-day universe並比較Target ranking／breach diagnostics，輸出`outputs/filters/breakout_quality/<filter_id>/daily_target_comparison/`；不train、不fit、不建立PIT。`ContinuousRankerResearchSpec.reference_profile_name`仍代表可由Target-comparison入口直接比較的同量綱受控Target；`evaluation_reference_profile_name`只供`services/breakout_quality/train_daily_ranker.py`在checkpoint固定後對同一frozen score補reference Target OOS診斷，兩者都不得進learning或epoch selection。MR-13N使用後者對MR-13H economic Target做reference evaluation，因此不把unitless rank-composite誤送進R-unit Target comparison。
