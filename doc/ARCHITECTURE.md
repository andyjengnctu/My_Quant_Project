...
- `apps/breakout_quality.py` 是 breakout-quality 唯一正式使用者入口。互動選單只保留「模型研究與驗證」、「策略績效驗證」與「查看設定／工件狀態」三個完整流程；Dataset、單獨 train、export、audit 與 research-only 指令維持 CLI-only。模型與策略流程不自動硬串。泛用 workflow identity、PIT 日期／fold、Score source、策略參數 policy 與 buy-sort 名稱集中在 `config/breakout_quality_workflow.py`；模型架構、訓練數學與實驗 profile 仍由既有 policy／experiments 模組管理。`apps/breakout_quality_strategy_compare.py` 不再維護第二套 parser，只將參數轉送至 `apps/breakout_quality.py strategy-compare`。舊 `workflow` 與低階子命令為 CLI 相容功能，不出現在新互動選單。
