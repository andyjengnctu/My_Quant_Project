...
- `tools/debug/`：交易除錯子系統；`trade_log.py` 保留 CLI、資料集解析與 GUI 共用後端封裝，`backtest.py` 只保留 debug 回測主控 façade，買進 / 延續候選進場已拆到 `entry_flow.py`，出場 / 錯失賣出 / 期末結算已拆到 `exit_flow.py`，明細列建構與半倉停利價 helper 集中到 `log_rows.py`，`charting.py` 專責 K 線 payload / HTML 輸出 / GUI 內嵌 figure、可視區間 autoscale、中文字型 fallback 與 focus-window scoped render，`reporting.py` 專責 Excel / HTML artifact 與虧損摘要。
- `tools/gui/`：GUI 工作台子系統；`main.py` 只負責啟動，`workbench.py` 維護單一視窗與頁籤 registry，`single_stock_inspector.py` 承接單股回測檢視頁籤與內嵌圖表 canvas，圖表分頁預設優先顯示且成交量可切換、摘要與明細以獨立分頁承接，避免 GUI 與交易邏輯耦合。
