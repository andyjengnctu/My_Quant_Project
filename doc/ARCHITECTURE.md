...
- `run_meta_quality.py`：meta quality 工具；對 synthetic coverage suite 產出 line / branch coverage baseline，並正式檢查 checklist 主表、未完成摘要、已完成摘要的一致性；已納入 `apps/test_suite.py` / `run_all.py` 單一入口。
