...
│  ├─ breakout_quality_workflow.py    # 單一入口的模型／PIT Score／策略workflow設定
...
- `apps/breakout_quality.py` 是 breakout-quality 唯一正式使用者入口。互動選單只保留「模型研究與驗證」、「策略績效驗證」與「查看設定／工件狀態」三個完整流程；Dataset、單獨 train、export、audit 與 research-only 指令維持 CLI-only。模型與策略流程不自動硬串。`config/breakout_quality_workflow.py` 的experiment profile是流程派送單一來源：binary objective沿用既有classification workflow，continuous objective使用Selection PIT builder／audit；策略mode、Score source與buy-sort可設為`auto`依objective解析，也可明確覆寫。狀態頁只顯示目前objective相關工件。模型架構、訓練數學與experiment profile定義仍由既有policy／experiments模組管理。`apps/breakout_quality_strategy_compare.py` 不再維護第二套parser，只將參數轉送至 `apps/breakout_quality.py strategy-compare`。舊 `workflow` 與低階子命令為CLI相容功能，不出現在新互動選單。
