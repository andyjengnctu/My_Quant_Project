...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained。MR-13H no-breach Label已完成Selection/Forward 16-seed robustness並結案為`VALID_LABEL_SIMPLIFICATION / NOT_SELECTED_FOR_PROMOTION`。Model Research active profile現為MR-13K `daily_universal_full_horizon_pure_mfe_full_list_ndcg_pairwise / inception_time_v1`：沿用MR-13H完整40D earliest maximum High與breach diagnostic，adverse-to-peak仍計算但只作diagnostic，Target只保留favorable/R-scale。architecture、daily universe、Full-list Delta-NDCG loss與strategy allocator都不變；MR-13K Target Audit與Seed42 Forward已完成：OOS Daily rho=`0.3833`、Pair=`63.19%`，且同一score對MR-13H reference Target仍有rho=`0.2381`、Pair=`58.24%`；因此已授權單profile Selection PIT，並以C53對C42作同K/R0/exact source-only策略轉化。Forward strategy arm與multi-seed仍未授權，production candidate固定C42/C44。MR-13I/J risk-normalized/context路線已REJECT，只保留歷史重現。
