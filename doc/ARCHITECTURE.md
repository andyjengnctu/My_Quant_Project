...
│     ├─ full/<study|full|oos|rolling|trade>/
│     │  ├─ base_best.json / *_finalists_agree.json / ensemble_*.json
│     │  ├─ manifest.json
│     │  └─ trade/state/               # 只存在full/trade：active/candidate runtime state
│     └─ min/<oos|rolling>/
│        ├─ base_best.json / *_finalists_agree.json / ensemble_*.json
│        └─ manifest.json
└─ tools/
...
- `run_all.py` 在每次 formal staging run 以目前 `config/` defaults 生成隔離的 `formal_primary_params.json`，並透過 `V16_RUN_BEST_PARAMS_PATH` runtime override 傳給 dataset prep、consistency、chain、quick gate、ML smoke 與 meta quality；formal regression 不依賴、也不覆寫 canonical `models/strategy_params/full/trade/state/active.json`，且不要求 repository 或交付 ZIP 內建任何 runtime 產生的策略參數工件。
