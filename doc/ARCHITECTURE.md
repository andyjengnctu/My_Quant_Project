...
- Breakout-quality 所有 binary、continuous、pretraining 與PIT流程共用 `config/breakout_quality.py` 的單一非負整數 `BREAKOUT_QUALITY_RANDOM_SEED`；experiment profile只決定訓練目標與方法，不得隱式改變Seed。CLI `--seed`只作單次執行覆寫。
