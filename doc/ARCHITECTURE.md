...
- `tools/filters/breakout_quality/` 承接 dataset、training、score export、研究 metrics 與 report rendering 子系統實作；`evaluate.py` 是完整 JSON 稽核介面，`report.py` 只重用同一份 metrics，輸出表格化終端報表、Markdown 與完整報表 JSON，不另算第二套指標。報表預設納入 OOS，並依序呈現 Epoch 選擇、Selection／OOS Confusion Matrix、各資料區段比較、Selection/OOS 差異與部署判定。Confusion Matrix 中央只保留 TP／FN／FP／TN，右側依序列原始 PASS／REJECT 比例與列合計公式，底部依序列欄合計公式與模型 PASS／REJECT 比例；Precision、Recall、Specificity、NPV、Accuracy 以含公式與解釋的獨立分類品質表呈現，Precision 提升以含公式的篩選效果表呈現。區段與日期分欄，第 4、5 區使用相同正式指標名稱；終端色彩僅在互動式 TTY 自動啟用。training 會驗證 source-data inventory，來源已更新時拒絕沿用過期 dataset。直接 CLI 僅保留開發與既有指令相容，不再作為文件建議的正式入口。
