...
- `apps/breakout_quality.py` 是 breakout-quality 唯一正式使用者入口。互動選單只保留「模型研究與驗證」、「策略績效驗證」與「查看設定／工件狀態」三個完整流程；Dataset、單獨 train、export、audit 與 research-only 指令維持 CLI-only。模型與策略流程不自動硬串。`config/breakout_quality.py` 的experiment profile是流程派送單一來源：binary objective沿用既有classification workflow，continuous objective先共用同一個Dataset refresh step準備Full dataset／全部股票，再使用Selection PIT builder／audit；策略mode、Score source與buy-sort可設為`auto`依objective解析，也可明確覆寫。狀態頁只顯示目前objective相關工件。模型架構、Label、訓練參數、experiment profile定義、PIT與策略workflow全部集中在同一檔案；使用者設定置頂，profile類別、驗證、衍生值與helper集中於下半部。舊`breakout_quality_policy.py`、`breakout_quality_experiments.py`與`breakout_quality_workflow.py`已移除，所有程式只可import canonical module。`apps/breakout_quality.py strategy-compare`是唯一策略比較入口；CLI-only `strategy-filter-gate`的預設identity必須直接取自目前continuous-ranker workflow，禁止退回binary `unique_group_sampling` profile；CLI-only `strategy-dl-filter-gate`則必須明確使用binary canonical runtime identity與固定threshold；已刪除的舊`apps/breakout_quality_strategy_compare.py`不得恢復。舊 `workflow` 與低階子命令為CLI相容功能，不出現在新互動選單。
...
- `apps/breakout_quality.py strategy-dl-filter-gate` 是CLI-only的Binary DL Filter Replacement A／B／C／F入口。它兩次重用`strategy_compare.run_comparison`的hard-filter canonical runtime鏈：A／B沿用目前optional filters並只切換`use_breakout_quality_filter=False/True`；C／F同時把EMA、BB、Volume、breakout return與false-breakout五個optional entry filters固定為False，再只切換Binary DL hard filter。四組均固定原position-aware buy-sort，`use_breakout_quality_ranking=False`，不使用continuous Score、R3、Future Target或optimizer。Gate必須先做binary runtime preflight：`manifest.json`、`model.pt`、`split_assignments.csv`缺少時視為模型identity不完整，research scores／歷史報表不得冒充checkpoint；模型三件完整而只缺`scores.csv`時才允許只執行forward-OOS export。Preflight會列出相對路徑並搜尋其他位置的同identity manifest，但不自動搬移單檔或猜測缺失identity。Gate之後再驗證兩個pair共用相同runtime score／manifest、threshold、active-param來源、期間、benchmark、部位與rotation；`B−A`判斷疊加效果、`F−C`判斷替代效果、`F−A`作正式replacement比較，interaction=`(F−C)−(B−A)`只作機制診斷。輸出保存於binary model目錄下獨立`strategy_dl_filter_gate_<param_policy>_canonical_runtime/`，保留兩個pair的完整策略比較與trade attribution，再合併成A／B／C／F摘要。
