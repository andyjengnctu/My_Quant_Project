...
- 既有binary／canonical與pretraining流程保留`BREAKOUT_QUALITY_RANDOM_SEED=42`。本輪Selection PIT continuous-ranker研究依交接契約使用獨立且顯式的`BREAKOUT_QUALITY_WORKFLOW_RANDOM_SEED=1`；PIT manifest、fold fingerprint與策略score contract均鎖定此Seed，禁止與Seed 42工件resume或混接。CLI `--seed`只作單次執行覆寫。
