...
- 固定策略對照輸出位於 `outputs/filters/breakout_quality/<filter_id>/<model_architecture>/<experiment_profile>/strategy_compare/`；主檔為 `strategy_comparison.md`／`.json`，並保留兩組 equity、trades、daily capacity 與年度報酬 CSV。此目錄只存研究比較結果，不是正式 runtime score 來源。
