...
- `tools/trade_analysis/`：單股 trade-analysis 子系統；對外公開函式與部分 validator / output labels 暫保留 legacy `run_debug_*` / `debug_trade_log` 命名以維持相容性。`trade_log.py` 保留 CLI、資料集解析與 GUI 共用後端封裝，`backtest.py` 只保留單股分析主控 façade，買進 / 延續候選進場已拆到 `entry_flow.py`，出場 / 錯失賣出 / 期末結算已拆到 `exit_flow.py`，明細列建構與半倉停利價 helper 集中到 `log_rows.py`，`charting.py` 專責 K 線 payload / HTML 輸出 / GUI 內嵌 figure、完整歷史視窗導航、預設最近 18 個月視窗、台股紅漲綠跌色系、可視區間 autoscale、滑鼠滾輪縮放 / 左鍵拖曳平移、左上即時 hover OHLCV 顯示、買訊/賣訊半透明註記、停損/停利/限價/成交線（未成交前僅預覽限價）與右側摘要/右下狀態框、同圖成交量 overlay 與中文字型 fallback；GUI 預設 path 改為只回傳 chart payload、不預先輸出 HTML，以降低互動記憶體與載入成本，`reporting.py` 專責 Excel / HTML artifact 與虧損摘要。
