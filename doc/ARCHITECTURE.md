...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained；Model Research active profile則為MR-13H `daily_universal_full_horizon_no_breach_full_list_ndcg_pairwise / inception_time_v1`。MR-13H只改continuous Target：first risk-breach由path truncation改成diagnostic-only，完整40D取earliest maximum High並保留adverse-to-peak；不改architecture、daily universe、loss或strategy allocator。MR-13H已完成Forward Model Gate、Selection PIT與C51 Selection strategy；目前以C52 Forward與Selection/Forward same-generated-8-seed robustness完成全流程評估，production candidate仍固定C42/C44。MR-13I/J risk-normalized/context路線已在C49/C50 Selection strategy-conversion後REJECT，只保留歷史重現。
