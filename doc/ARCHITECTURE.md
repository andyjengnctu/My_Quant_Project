...
- 固定策略對照輸出位於 `outputs/filters/breakout_quality/<filter_id>/<model_architecture>/<experiment_profile>/strategy_compare/`；主檔為 `strategy_comparison.md`／`.html`／`.json`，並保留兩組 equity、trades、daily capacity、年度報酬，以及 `trade_attribution.md/.json`、交易配對、年度R歸因與兩組round-trip CSV。`tools/filters/breakout_quality/strategy_report_style.py`是策略診斷報表色彩SSOT：Markdown以🟢／🔴／🟡／⚪保留語意、終端使用ANSI色彩、HTML以green／red／yellow／gray CSS呈現；顏色只作可讀性標記，不改原始數值或決策。Score-ranking比較另由read-only `audit_score_ranking_capture.py`從既有transaction history與post-replay selected-target diagnostics重建每筆trade lifecycle，輸出`score_ranking_capture_audit.md`／`.html`／`.json`、兩組lifecycle CSV、年度與scenario CSV，分解投入／預留、stop distance、fill、holding、partial日曆天數、依daily-capacity交易日曆計算的真正尾倉slot-days、entry-date／月份集中度、可用時的產業集中度、exit reason、Realized R／Target capture與年度差異。`--capture-audit-only`只重建主報表及audit，不重跑portfolio；Future Target只可在replay後join。年度完整性必須同時通過既有市場邊界檢查且涵蓋一月至十二月；被回測終點截短的2026年不得標為完整年度。此目錄只存研究比較結果，不是正式 runtime score 來源。
