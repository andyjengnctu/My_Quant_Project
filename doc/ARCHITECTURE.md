...
- `apps/breakout_quality.py` 是Breakout Quality Dataset／Label／模型訓練與模型評估的正式入口；互動選單只保留模型研究、模型設定／工件狀態與返回。策略績效比較由獨立`apps/strategy_compare.py`負責，兩個App不得互相轉呼叫。模型流程在Prediction報表與forward-OOS模型score工件完成後停止，不執行portfolio replay、不訓練策略參數。舊版本化Gate與低階子命令只保留CLI相容或研究用途，不出現在正式模型選單。
