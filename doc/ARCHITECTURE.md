...
- canonical策略比較主工件為 `strategy_comparison.md`／`.json`及pair-level equity、trades、daily-capacity、selected diagnostics、年度報酬與round-trip attribution。`core/console_report.py`是全專案簡易報表格式SSOT，統一標題、段落、key-value、表格、狀態與專案根目錄相對工件路徑；Breakout Quality的`strategy_report_style.py`只保存該domain綠／紅／黃／灰判讀語意。所有Breakout Quality策略層正式結果都必須同時有console簡報與persistent Markdown；JSON／CSV／manifest只是同一結果的詳細工件，不另複製第二套指標。Strategy Compare pair的RUN與REUSE固定由canonical pair JSON payload呼叫同一renderer重新materialize Markdown與console簡報；cache缺少`strategy_comparison.md`即視為不完整、不得REUSE。`filters/breakout_quality/strategy_compare_engine.py`只保留pair orchestration、CLI與legacy import façade；canonical replay位於`strategy_compare_replay.py`，canonical pair summary/yearly/readable renderer位於`strategy_compare_reporting.py`，diagnostic join位於`strategy_compare_diagnostics.py`。engine不import Audit、不自動生成capture，也不提供`--capture-audit-only`。歷史score-ranking capture位於`tools/audit/portfolio/score_ranking_capture.py`，由需要它的研究流程在**已完成pair工件**上顯式materialize read-only報表；策略比較JSON不內嵌Audit payload。Future Target只可在replay後join。全專案formal Audit另輸出到`outputs/audit/<domain>/<audit>/runs/<timestamp>/`並更新`latest/`；目前`AUD-c15-strategy-attribution`從最新正式`outputs/strategy_compare`只讀C15/C3/C12，使用exact daily `Δ log wealth`歸因portfolio wealth path，並拆解selection changes、trade contribution、capital geometry、slot occupancy與2024集中度，不重跑portfolio、不回流runtime／training。
