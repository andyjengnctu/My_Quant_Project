...
- `apps/strategy_compare.py` 是正式策略績效比較入口；選單固定為「執行目前比較設定／查看目前比較設定與工件狀態」，不得出現C1～C6、TP1或其他特定版本名稱。`config/strategy_compare.py`逐項定義parameter sources、DL sources、arms與contrasts，每項以`enabled`開關；同一param source／rule policy的DL-off與DL-on controlled pair必須同時開關；不存在整套實驗ID。`core/strategy_comparison.py`只提供泛用設定資料結構、驗證與fingerprint；`filters/breakout_quality/strategy_comparison.py`把設定轉為Breakout Quality replay；canonical比較引擎位於`filters/breakout_quality/strategy_compare_engine.py`。舊`tools/filters/breakout_quality/strategy_compare.py`只作相容別名。正式比較只讀取既有模型、Scores與ROOS工件，缺件fail-fast，不建立Label、不訓練模型、不匯出缺少Scores、不執行optimizer。每次輸出以啟用arm IDs與config fingerprint自動建立`outputs/strategy_compare/runs/<timestamp>_<arms>_<fingerprint>/`，並更新`outputs/strategy_compare/latest/`；完整manifest保存設定snapshot與工件SHA256。
