...
- Selection Full ROOS使用獨立`PARAM-P4` historical rolling active params；current Selection核心references為C32 Full、C23 Min與production C42 MR-13E exact。C56 full-flow新增C57作Selection PIT counterpart：primary=`CONT13K_PIT`、secondary=`CONT13M_PIT`，兩者皆必須PIT-safe；C53 MR-13K source-only已退出current並移至historical compatibility。Engine支援standalone DL-off comparator，因此C32不必掛隱藏Full+DL arm。
- Selection與Forward-OOS由兩個永久共存的Strategy Compare profiles隔離；current核心比較為Selection=`C32/C23/C42/C57`、Forward=`C1/C3/C44/C56`。C57/C56是同一Plan B2 residual-safety architecture的PIT/Forward對稱arms：MR-13K仍是唯一objective，MR-13M只提供same-day rank OLS residual safety baseline-relative hard floor。C53/C54 source-only與C55 raw-safety已移至historical compatibility，不進current replay或robustness。Current 8-seed robustness fixed references為Selection C32/C23、Forward C1/C3；stochastic只測Selection C57與Forward C56，每個generated seed都同seed建立13K+13M兩個source再replay一次。Production仍C42/C44。
