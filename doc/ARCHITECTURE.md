...
- Selection與Forward-OOS由兩個永久共存的Strategy Compare profiles隔離；current核心比較為Selection=`C32/C23/C42/C53`、Forward=`C1/C3/C44`；C53是MR-13K相對MR-13E的Selection source-only exact controlled arm。MR-13K Forward strategy與multi-seed robustness尚未授權；MR-13H C51/C52已結案移至historical compatibility。Full只作DL-off完整策略體系baseline，Min ROOS作同param/rule baseline，MR-13E exact constrained作current DL allocator；其他已完成MR-12B/MR-13A/No-R0 controls全部只保留歷史重現。
...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained。MR-13H no-breach Label已完成Selection/Forward 16-seed robustness並結案為`VALID_LABEL_SIMPLIFICATION / NOT_SELECTED_FOR_PROMOTION`。MR-13K Pure-MFE與MR-13M low-adverse已分別證明upside與path-risk component都具高cross-sectional learnability；MR-13K的C42/C53與C44/C54 current 8-seed full-flow robustness仍獨立完成後再結案13K。MR-13L dual raw-R MSE、MR-13N single-model equal-rank composite與固定50/50 frozen expert fusion皆已REJECT。Model Research active profile現為MR-13O `daily_universal_full_horizon_pareto_mfe_low_adverse_pairwise / inception_time_v1`：40D complete horizon、earliest max-MFE peak、daily universe、300×10 sequence與MR-13H economic Target保持不變；training不建立新的scalar mix，而是在同日將MFE與low-adverse轉為component percentiles，只有一支股票在兩component都嚴格支配另一支時才形成RankNet pair，trade-off/tie pair無gradient、comparable pairs等權。MR-13O只做Seed42 Forward model Gate，PIT/strategy staged off；production candidate固定C42/C44。MR-13I/J risk-normalized/context路線已REJECT，只保留歷史重現。
