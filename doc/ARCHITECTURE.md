...
- `filters/breakout_quality/` 承接 feature/label、indexed feature-bank dataset storage、source-data inventory fingerprint、artifact contract、canonical path、外層 Selection/OOS split 與正式 runtime lookup；`models/` 子套件以單一 factory 管理 `tiny_cnn_v1`、`multiscale_cnn_v1`、`multiscale_cnn_v2`、`multiscale_cnn_v3`、`multiscale_cnn_v4`、`multiscale_cnn_v5` 與 `residual_tcn_v1`，版本化 `model_spec` 同時寫入 checkpoint 與 manifest；`multiscale_cnn_v2` 只在模型內讓短／中期 branch 使用 canonical OHLCV 的 Return／Delta 表示，`multiscale_cnn_v3` 再把其中個股 O/H/L/C 變化轉為相對同欄 0050 變化，兩者長期 branch 均保留 level 表示；`multiscale_cnn_v4` 與 `multiscale_cnn_v5` 都與 v1 使用完全相同的 Level input，只將 Long Branch channels 分別由 16 降為 8 與 12，參數量分別為 14,530 與 18,530。所有版本均不改 Dataset feature contract。正式 runtime 不執行 CNN，只讀目前 policy 架構的 canonical `scores.csv`。
