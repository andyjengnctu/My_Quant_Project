...
- `apps/research.py`：研究單一正式入口；主選單只選工作類型。模型訓練由`config/research.py`指定active model provider；策略參數最佳化目前仍由既有optimizer互動orchestrator承接；策略組合比較依`config/strategy_compare.py`執行並透過`services/portfolio_replay.py`重用canonical portfolio replay；策略參數訓練重用`services/optimizer/` canonical optimizer library，Breakout Quality training／PIT producer則由`services/breakout_quality/`承接，其中Multiple-seed robustness以Strategy Compare作UI/orchestrator、模型權重仍只由canonical continuous-ranker trainer建立；MR-12B／MR-13A／PIT／robustness的共用training／inference primitives經`services/breakout_quality/ranker_training.py` public API取得，避免workflow依賴trainer private helper。Robustness retention把scientific aggregate與Audit evidence分離：checkpoint／full score／完整replay tree可清除，但可在同一scientific fingerprint下永久保存active trades／equity／daily-capacity／selected-buys／entry-execution compact attribution source；舊completed observation缺source時由Strategy Compare重建；unit先為`PENDING`，只有scientific observation與training/artifact identity驗證通過後才升為`VERIFIED`，resume與Audit一律忽略未驗證unit。Audit依`config/audit.py`指定active module；`forward-robustness-portfolio-translation`只讀compact source，固定完整matched seeds並重用`c15_strategy_attribution`公開primitive；entry-execution sidecar由正式portfolio entry path記錄risk budget、實際initial risk與qty，再由Audit用canonical sizing primitive辨識risk/position/capital/MaxQty/lot/cash binding，Audit層不得train、replay或改寫sizing。
