...
- `tools/filters/breakout_quality/` 承接 dataset、training、score export、研究 metrics 與 report rendering 子系統實作；`evaluate.py` 是完整 JSON 稽核介面，`report.py` 只重用同一份 metrics，輸出表格化終端報表、Markdown 與完整報表 JSON，不另算第二套指標。報表將固定訓練參數併入標頭，後續依序呈現 Epoch 選擇、區段比較與部署判定；區段與日期分欄，部署判定只保留 OOS 依據、部署決策與不可回調限制。若未納入 OOS，report renderer 必須直接說明 OOS Confusion Matrix 與 Selection/OOS 差異不會產生。Confusion Matrix 只保留 TP／FN／FP／TN 與原始／模型邊際合計，所有衍生指標移至矩陣下方；各表統一使用「原始 PASS 比例、模型 PASS 比例、PASS Precision、PASS Recall、REJECT Specificity、REJECT NPV」等名稱。終端色彩僅在互動式 TTY 自動啟用，淡藍、綠、黃、紅分別標示選擇、正向、警告與失敗重點。training 會驗證 source-data inventory，來源已更新時拒絕沿用過期 dataset。直接 CLI 僅保留開發與既有指令相容，不再作為文件建議的正式入口。
