...
- Selection與Forward-OOS由兩個永久共存的Strategy Compare profiles隔離；current核心比較為Selection=`C32/C23/C42/C53`、Forward=`C1/C3/C44/C54/C55/C56`。C53/C54是MR-13K相對MR-13E的source-only exact controls。C55 raw 13M baseline safety floor已取得結果並證實會過度犧牲13K upside；C56為current Plan B2：每天在當日orderable候選把13K/13M frozen scores轉rank percentile，以含intercept OLS估計expected 13M rank given 13K rank，Residual Safety只作C3 baseline-relative coverage/score-sum hard floor；13K仍是唯一objective，無future Target、跨日fit、score fusion或absolute threshold。C55/C56均不加入8-seed robustness或production。MR-13H C51/C52已結案移至historical compatibility。Full只作DL-off完整策略體系baseline，Min ROOS作同param/rule baseline，MR-13E exact constrained作current production allocator；其他已完成controls只保留歷史重現。
