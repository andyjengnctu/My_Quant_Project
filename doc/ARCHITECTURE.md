...
- 訓練效能優化以結果一致為硬性契約：每個 epoch 的訓練 rows、shuffle、batch size、seed、optimizer 更新次數與單執行緒設定均不得改變。完整 Train／Validation／Selection 評估只改為 `BREAKOUT_QUALITY_EVALUATION_BATCH_SIZE` 控制的分批推論，仍使用全部 requested rows；Final refit 最後一輪已產生的完整 Selection metrics 直接作為 final metrics，不再做一次完全重複的推論。
