...
- `apps/breakout_quality.py` 是 breakout-quality 唯一正式使用者入口。互動選單只保留「模型研究與驗證」、「策略績效驗證」與「查看設定／工件狀態」三個完整流程；Dataset、單獨 train、export、audit 與 research-only 指令維持 CLI-only。Binary objective 的模型研究子選單提供「重新訓練」與「使用既有模型」兩條正式路徑；兩者都必須先顯示同一份OOS簡易模型報表，再匯出forward-OOS runtime scores並重用`strategy-compare`執行hard-filter對Baseline的正式績效比較。Continuous objective仍維持模型與策略分開。`config/breakout_quality.py` 的experiment profile是流程派送單一來源：binary objective沿用classification workflow，continuous objective先共用同一個Dataset refresh step準備Full dataset／全部股票，再使用Selection PIT builder／audit；策略mode、Score source與buy-sort可設為`auto`依objective解析，也可明確覆寫。狀態頁只顯示目前objective相關工件。模型架構、Label、訓練參數、experiment profile定義、PIT與策略workflow全部集中在同一檔案；使用者設定置頂，profile類別、驗證、衍生值與helper集中於下半部。舊`breakout_quality_policy.py`、`breakout_quality_experiments.py`與`breakout_quality_workflow.py`已移除，所有程式只可import canonical module。`apps/breakout_quality.py strategy-compare`是唯一策略比較入口；CLI-only `strategy-filter-gate`固定使用continuous-ranker identity，不隨正式主workflow切換；CLI-only `strategy-dl-filter-gate`固定使用binary canonical runtime identity與固定threshold；已刪除的舊`apps/breakout_quality_strategy_compare.py`不得恢復。舊 `workflow` 與低階子命令為CLI相容功能，不出現在新互動選單。
