...
- `apps/breakout_quality.py` 是Breakout Quality Dataset／Label／模型訓練、模型評估與正式只讀Audit的入口；互動選單只保留模型研究、Audit／診斷、模型設定／工件狀態與返回。策略績效比較由獨立`apps/strategy_compare.py`負責，兩個App不得互相轉呼叫。模型流程在Prediction報表與forward-OOS模型score工件完成後停止，不執行portfolio replay、不訓練策略參數。正式Audit由`config/audit.py`集中指定對象／來源／維度／輸出，`tools/filters/breakout_quality/audit_runner.py`只依config派送只讀Audit；第一個`audit_pass_quality.py`只讀既有strategy-compare候選／交易與Dataset Label，不重跑策略、不改Label／模型／runtime。舊版本化Gate與低階子命令只保留CLI相容或研究用途，不出現在正式模型選單。
...
- `tools/filters/breakout_quality/strategy_trade_path_label_gate.py` 是CLI-only三組策略Gate。它只接受既有模型與Prediction report，更新舊／新Label forward-OOS Scores後，以相同A2 active params、rules全關、原buy-sort、費稅、部位與rotation各跑一個hard-filter pair；兩次pair的no-DL base必須逐指標完全一致。合併報表固定輸出Base／Old／New、`New−Base`、`New−Old`、`Old−Base`、直接交易選擇R及年度結果；兩個pair的no-DL equity／trades／daily-capacity工件SHA256必須完全一致，不執行訓練或threshold tuning。
...
- `apps/strategy_compare.py` 是正式策略績效比較入口；選單固定為「執行目前比較設定／查看設定、工件與預計動作」，不得出現特定arm ID、TP1、A9或其他實驗版本名稱。`config/strategy_compare.py`逐項定義parameter sources、DL sources、arms、contrasts與preparation policy；每項以`enabled`或明確欄位調整，不存在整套實驗ID。同一param source／rule policy只定義一個共用DL-off基準，可掛一個或多個DL-on模型；各DL-on arm可獨立開關，runtime必須逐一與同一DL-off基準形成controlled pair，並驗證重複回放的基準摘要與年度報酬完全一致。`core/strategy_comparison.py`只提供泛用設定、驗證、依賴計畫與fingerprint；`filters/breakout_quality/strategy_compare_preparation.py`依config判定`READY／PREPARABLE／BLOCKED`，並透過`filters/breakout_quality/export_scores.py`、`strategy_optimizer_policy.py`與`strategy_param_training.py`正式共用服務補建既有模型的forward-OOS scores或比較所需策略參數；不建立Label、不選模型、不訓練模型權重。`filters/breakout_quality/strategy_comparison.py`完成一次確認後的前置編排與Breakout Quality replay；canonical比較引擎位於`filters/breakout_quality/strategy_compare_engine.py`。舊`tools/filters/breakout_quality/common.py`、`export_scores.py`、`strategy_dl_filter_param_adapt_gate.py`與`strategy_compare.py`只保留相容別名，不保存第二套正式邏輯。任一前置步驟失敗即停止回放、保留可接續工件並回報步驟與相對路徑。正式回放前須先由全部啟用DL runtime工件解析共同可比較期間，並驗證每個rolling active-param來源完整覆蓋該期間；歷史Label teacher params不得冒充forward績效比較參數。Min ROOS使用forward P2 DL-off-trained工件，Min-DL ROOS使用forward P3 DL-on-trained工件；缺少或identity／coverage不符時依config自動建立或接續。前置允許分波重新規劃，例如先建立forward scores取得正式期間，再建立因此顯露為缺少／過期的參數工件；所有來源READY後才開始第一個pair replay。目前config把TP1與既有A9列為兩個runtime DL sources。DL-off訓練的Full／Min參數可分別搭配TP1或A9作runtime比較；任何`trained_with_dl_id`非空的Min-DL參數，只允許搭配訓練時相同的DL runtime，禁止TP1-trained參數搭A9或A9-trained參數搭TP1。使用者可見名稱採`Min ROOS`、`Min ROOS: TP1-on`、`Min ROOS: A9-on`、`Min-TP1 ROOS`、`Min-TP1 ROOS: DL-on`、`Min-A9 ROOS`、`Min-A9 ROOS: DL-on`；內部identity仍保存完整DL版本。A9-aware P3工件獨立落在`models/research/breakout_quality/binary_dl_filter_param_adaptation/risk_only_rolling/p3_dl_on_trained/A9/`，不覆蓋既有TP1 P3。`direct_selection_delta_r`是相對同一param source／rule policy之DL-off基準的attribution，只能在相同參數與規則宇宙內比較；跨參數或跨rule-policy contrast不得把兩個不同baseline attribution相減後顯示為直接選擇效果。每次輸出以啟用arm IDs與config fingerprint建立`outputs/strategy_compare/runs/<timestamp>_<arms>_<fingerprint>/`並更新`latest/`，manifest保存設定snapshot、共同比較期間、前置計畫與工件SHA256。
- A9 Resource-aware Binary是candidate ranking的盤前overlay，不是第三套signal filter。`resource-aware-binary`與`resource-aware-binary-basket`都保留完整setup lifecycle與Min ROOS初始buy-sort；`core/portfolio_entries.py`在正式reserve前用同一`build_cash_capped_entry_plan()`／exact-accounting模擬Min ROOS。若free slots先用滿或沒有競爭候選，mode=`capital-utilization`且DL不得改單；只有free slots尚未用滿、仍有未選候選而現金已無法再建立合法cash-capped entry時，mode=`dl-selection`。C11 `resource-aware-binary`沿Min ROOS順位接受第一個維持cash-binding且提高PASS reserved capital的promotion；C12 `resource-aware-binary-basket`每一輪評估全部尚未promotion的A9 PASS候選，以exact cash-cap結果挑選當輪PASS reserved capital最高、其次PASS數較多、再以Min ROOS順位決勝的best-improvement，再從新basket重算下一輪。C12刻意不做指數級全子集合窮舉，避免明顯犧牲正式replay效率；兩者都不得新增利用率百分比、距限價bucket、權重參數、Future Target或candidate-day重新打分。
...
- `python -m tools.filters.breakout_quality.strategy_dl_filter_gate` 是CLI-only的Binary DL Filter Rule Ablation入口。它五次重用`strategy_compare.run_comparison`的hard-filter canonical runtime鏈，每層只切換`use_breakout_quality_filter=False/True`：A0／B0沿用原正式規則；A1／B1固定關閉EMA、BB、Volume、breakout return與false-breakout五個optional entry filters；A2／B2再固定`use_history_threshold=False`；A3／B3再固定`use_breakout_reclaim_reentry=False`；A4／B4再固定`use_kc=False`。十組均固定原position-aware buy-sort、`use_breakout_quality_ranking=False`、相同rolling active-param來源、runtime score／manifest、threshold、期間、benchmark、部位與rotation，不使用continuous Score、R3、Future Target或optimizer。合併報表以五列矩陣呈現每層A／B績效與`Bn−An`增量，另列A系列相對A0的規則消融、每層trade attribution及年度DL增量，避免列出大量跨組pair。輸出保存於binary model目錄下`strategy_dl_filter_rule_ablation_gate_<param_policy>_canonical_runtime/`。
...
- 固定策略對照輸出位於 `outputs/filters/breakout_quality/<filter_id>/<model_architecture>/<experiment_profile>/strategy_compare/`；主檔為 `strategy_comparison.md`／`.json`，並保留兩組 equity、trades、daily capacity、年度報酬，以及 `trade_attribution.md/.json`、交易配對、年度R歸因與兩組round-trip CSV。`filters/breakout_quality/console_report.py`是breakout-quality使用者可見簡易報表的格式SSOT，統一標題、段落、key-value、表格、狀態與專案根目錄相對工件路徑；`strategy_report_style.py`只保存綠／紅／黃／灰判讀語意，console以ANSI呈現、Markdown以🟢／🔴／🟡／⚪保留語意，不建立HTML或第二套指標。Score-ranking比較另由read-only `audit_score_ranking_capture.py`從既有transaction history與post-replay selected-target diagnostics重建每筆trade lifecycle，輸出`score_ranking_capture_audit.md`／`.json`、兩組lifecycle CSV、年度與scenario CSV，分解投入／預留、stop distance、fill、holding、partial日曆天數、依daily-capacity交易日曆計算的真正尾倉slot-days、entry-date／月份集中度、可用時的產業集中度、exit reason、Realized R／Target capture與年度差異。完整易讀策略比較與capture audit直接寫入console；舊版同目錄HTML在報表重建時刪除。`--capture-audit-only`只重建主報表及audit，不重跑portfolio；Future Target只可在replay後join。年度完整性必須同時通過既有市場邊界檢查且涵蓋一月至十二月；被回測終點截短的2026年不得標為完整年度。此目錄只存研究比較結果，不是正式 runtime score 來源。
