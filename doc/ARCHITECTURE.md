...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained。MR-13K Pure-MFE與MR-13M low-adverse已證明upside/path-risk component可學，C56 Seed42 Forward證明residual safety具有portfolio conversion價值。依2026-08-18使用者決策，下一個必要Gate不是Plan C-M，而是先完整完成C56 full-flow：Selection C57使用PIT-safe 13K+13M，Forward C56使用frozen 13K+13M；兩stage各跑generated 8-seed且每seed兩source同seed。C54/C55不同行測；production candidate固定C42/C44。Plan C-M與Plan A依Research Queue排在此Gate之後。
