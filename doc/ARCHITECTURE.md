...
- A9 Resource-aware Binary是candidate ranking的盤前overlay，不是第三套signal filter。`resource-aware-binary`保留完整setup lifecycle與Min ROOS初始buy-sort；`core/portfolio_entries.py`在正式reserve前用同一`build_cash_capped_entry_plan()`／exact-accounting模擬Min ROOS。若free slots先用滿或沒有競爭候選，mode=`capital-utilization`且DL不得改單；只有free slots尚未用滿、仍有未選候選而現金已無法再建立合法cash-capped entry時，mode=`dl-selection`。DL由Min ROOS基準逐一嘗試提前原始breakout A9 PASS候選，每次trial必須維持cash為binding resource並增加實際PASS reserved capital；找不到改善即保留Min ROOS。不得新增利用率百分比、距限價bucket、權重參數、Future Target或candidate-day重新打分。
