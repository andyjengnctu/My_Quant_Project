...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained。MR-13H no-breach Label已完成Selection/Forward 16-seed robustness並結案為`VALID_LABEL_SIMPLIFICATION / NOT_SELECTED_FOR_PROMOTION`。MR-13K Pure-MFE已證明upside-only Target的ranking learnability明顯提高，但Seed42 Selection/Forward portfolio translation均低於MR-13E；依current config，C42/C53與C44/C54的8-seed full-flow robustness仍獨立完成後再結案13K。MR-13L dual raw-R MSE decomposition已在Forward model Gate REJECT。Model Research active profile現為MR-13M `daily_universal_full_horizon_low_adverse_full_list_ndcg_pairwise / inception_time_v1`：沿用MR-13H/13K的40D complete horizon與earliest max-MFE peak，Target只取`-adverse-to-peak R`，MFE只定位peak、不提供reward；training仍是Full-list Delta-NDCG weighted same-day RankNet。MR-13M只做Seed42 Forward path-risk learnability Gate，PIT/strategy staged off。production candidate固定C42/C44。MR-13I/J risk-normalized/context路線已REJECT，只保留歷史重現。
