...
- Production daily ranker仍為MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / inception_time_v1` + exact K/R0 constrained。MR-13H no-breach Label已完成Selection/Forward 16-seed robustness並結案為`VALID_LABEL_SIMPLIFICATION / NOT_SELECTED_FOR_PROMOTION`。MR-13K Pure-MFE已證明upside-only Target的ranking learnability明顯提高，但Seed42 Selection/Forward portfolio translation均低於MR-13E；依使用者決策，C42/C53與C44/C54的16-seed full-flow robustness仍獨立完成後再結案13K。Model Research active profile現為MR-13L `daily_universal_full_horizon_mfe_adverse_dual_mse / inception_time_v1`：Target、40D horizon、fixed R scale、daily universe與architecture完全沿用MR-13H，只重新解釋既有2-output classifier head為`predicted adverse-to-peak R`與`predicted favorable MFE R`兩個primary regression outputs；training loss固定為兩分量raw-R MSE的同一mean reduction，無auxiliary loss／lambda，正式score固定為`Predicted MFE R - Predicted adverse R`。因此MR-13L是learning-formulation experiment而不是新Target或新architecture；先只做Seed42 Forward model Gate，PIT/strategy staged off。production candidate固定C42/C44。MR-13I/J risk-normalized/context路線已REJECT，只保留歷史重現。
- `services/breakout_quality/daily_target_comparison.py`是daily label simplification的正式read-only model-provider診斷：只從兩個config profile建立同一daily stock-day universe並比較Target ranking／breach diagnostics，輸出`outputs/filters/breakout_quality/<filter_id>/daily_target_comparison/`；不train、不fit、不建立PIT。`services/breakout_quality/train_daily_ranker.py`在checkpoint後可依`ContinuousRankerResearchSpec.reference_profile_name`補同一score對reference Target的common-target OOS診斷，reference不得進learning或epoch selection。
