...
- `apps/breakout_quality.py` 是 breakout-quality 唯一正式使用者入口。互動選單只保留「模型研究與驗證」、「策略績效驗證」與「查看設定／工件狀態」三個完整流程；Dataset、單獨 train、export、audit 與 research-only 指令維持 CLI-only。目前Binary模型研究子選單固定以獨立`filter_id=breakout_quality_a2_trade_path_v1`研究`a2_realized_trade_path_v1`，提供「建立新Label並重新訓練」、「使用既有模型更新預測報表」及「查看Label生命週期摘要」；兩條模型路徑都在Selection／OOS Prediction報表後停止，不自動匯出runtime scores或回放策略。策略經濟效果由CLI-only `strategy-trade-path-label-gate`比較A2 no-DL、舊9A Label DL與新trade-path Label DL。Continuous objective仍維持模型與策略分開。`config/breakout_quality.py` 的experiment profile仍是訓練profile單一來源；目前Binary研究Label identity由獨立filter contract固定，不改寫9A正式工件。`apps/breakout_quality.py strategy-compare`維持通用策略比較入口；臨時Gate仍為CLI-only。舊 `workflow` 與低階子命令為CLI相容功能，不出現在新互動選單。

- `filters/breakout_quality/trade_path_label.py` 與 `tools/filters/breakout_quality/build_trade_path_labels.py` 組成A2 realized trade-path Label鏈。Label identity與9A MFE／MAE契約隔離；Builder hardlink／copy既有300×10 feature bank，只重建event labels與events metadata。Teacher params由2014～2020 Selection rolling risk-only no-DL schedule與2021～2026既有P2 schedule合併，每日只解析當時已生效參數。模擬器直接重用`generate_signals`、normal／extended pre-market entry plan、shadow continuation cleanup、`execute_bar_step`及exact-accounting；初次miss buy只維持pending，同一原始event後續成交只產生一個終局Label，已成交淨Realized R正值為PASS、非正為REJECT，永未成交／shadow終止／新setup覆蓋／資料截止未結算為INVALID並排除訓練。`train.py`、Binary PIT builder與artifact validator依filter_id解析Label policy，禁止新模型與9A Dataset／manifest混接。
- `tools/filters/breakout_quality/strategy_trade_path_label_gate.py` 是CLI-only三組策略Gate。它只接受既有模型與Prediction report，更新舊／新Label forward-OOS Scores後，以相同A2 active params、rules全關、原buy-sort、費稅、部位與rotation各跑一個hard-filter pair；兩次pair的no-DL base必須逐指標完全一致。合併報表固定輸出Base／Old／New、`New−Base`、`New−Old`、`Old−Base`及年度結果，不執行訓練或threshold tuning。
...
- `apps/breakout_quality.py strategy-compare`：固定參數 no-filter 與 active breakout-quality filter 的策略經濟效果隔離比較正式入口；只允許 `use_breakout_quality_filter` 不同。
