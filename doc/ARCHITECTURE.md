...
- Selection Full ROOS使用獨立`PARAM-P4` historical rolling active params：2014～2020、120m train／12m OOS、canonical Full optimizer search space、TP/DL/History threshold依current optimizer policy固定OFF；不得以2021+ `full_roos` forward active params倒灌Selection。Selection Full historical row為C32/C33/C34，P4與三個scientific IDs永久保留；current Selection核心references為C32 Full DL-off baseline、C23 Min baseline與C42 Min MR-13E exact constrained；MR-13H Forward Gate通過後新增C51 Min MR-13H exact constrained作source-only controlled arm。C33/C34及C25/C28/C46/C47/C48等已結案研究arm保留歷史重現但inactive；engine支援standalone DL-off comparator，因此C32不必掛隱藏Full+DL arm。
- Selection與Forward-OOS由兩個永久共存的Strategy Compare profiles隔離；current核心比較為Selection=`C32/C23/C42/C51`、Forward=`C1/C3/C44/C52`；C51/C52分別是MR-13H相對MR-13E的Selection/Forward source-only exact controlled arms。Full只作DL-off完整策略體系baseline，Min ROOS作同param/rule baseline，MR-13E exact constrained作current DL allocator；其他已完成MR-12B/MR-13A/No-R0 controls全部只保留歷史重現。
