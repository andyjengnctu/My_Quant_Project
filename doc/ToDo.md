- Trading
    ...
    - 帳務中心還是不用在任意區域scroll up/down，但交易中心可以
    - 交易中心還是有在版面上的固定文字註解，應一致移到最下方的操作提示區; 持股決策表單最左邊少了單股回測檢示連結; 反之，在每個表單右下方的檢示選取股/在單股回測檢示是多餘的
    - 各中心的操作提示應固定在最下方狀態列，不用一個獨立的框佔版面，並且不會隨著scroll up/down改變位置
