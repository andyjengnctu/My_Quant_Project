...
訓練參數】 進場：突破買進 啟用 (突破 105 日新高)｜Re-entry 啟用（5日內站回 +0.80R）
　　　　     風控：ATR 20 日| 掛單 +4.5 ATR｜停損 -3.9 ATR｜追蹤 -3.7 ATR｜半倉停利 0.0%
　　　　     基礎濾網：布林(BB) 啟用（長14, 寬1.9x）｜阿肯那(KC) 啟用（長19, 寬2.0x）｜均量 啟用（突破日量 > 前10日均量 × 2.2）
            進階濾網：漲幅 啟用（突破日漲幅 > 8.0%）｜假突破 啟用（ATR%≤15.0%）｜EMA濾網 啟用（Close > EMA125）｜品質模型 關閉
　　　　     歷史門檻：關閉
