...
- TV-based UI
    - 賣出/停損資訊框加入最大回撤
    - 左上停利/停損/限價線圖示到右邊的區域，值不到重覆顯示，e.g. --- 停利: 185.25
    - 目前開啟與回到最新k線如圖1 (約留空1/2版本)，改成像圖2的位置 (約留空1/5版面)
    - 歷史績效符合 -> 符合歷史績效
    - 出現買入訊號才亮藍底，沒出現就黑底
    - 出現符合歷史績效才亮橙底，沒出現就黑底
    - 移除左上顯示"xxxx 單股回測k線交易檢示"
    - 移除開啟html鍵，也不需產生html
    - 移除執行摘要分項
    - 除了目前最上層的"單股回檢示"上層分頁，加入"投組回測檢視"頁上層分頁，來源是portifolio_sim.py
