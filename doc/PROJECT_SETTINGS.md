...
2. 每次開始前，必須先盤點必要資產：`<repo>/models/best_params.json`、`requirements` 所需套件。測試資料規則與搜尋順序以 D 節為準。
...
6. 再檢查／最嚴格檢查時，必須先執行 `apps/test_suite.py` 或 `tools/local_regression/run_all.py` 完整入口；僅得對 FAIL 步驟用 `tools/local_regression/run_all.py --only ...` 展開，不得在完整入口前先手動逐段重跑全部檢查。
