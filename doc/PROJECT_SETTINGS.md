...
12. `breakout_quality` 的時間驗證框架自 2026-08-18 起正式使用 **Extending-Window Rolling + Fixed-Window Rolling**，不再把 `Selection PIT` 與 `Frozen Forward-OOS` 視為兩個 current 頂層 Gate。正式規則如下：
   ...
   - **Rolling feeding acceleration 只可改 execution，不可改科學語意**：current default=`train_prefetch_batches=8`、`train_prefetch_workers=4`；CUDA使用pinned host staging、non-blocking H2D與獨立copy stream，使N+1 transfer可與N的forward/backward重疊。batch membership/order、same-date batching、seed、optimizer step、loss、deterministic algorithms與TF32設定不得因此改變。
