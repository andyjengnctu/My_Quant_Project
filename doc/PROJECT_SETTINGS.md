...
12. `breakout_quality` 的時間驗證框架自 2026-08-18 起正式使用 **Extending-Window Rolling + Fixed-Window Rolling**，不再把 `Selection PIT` 與 `Frozen Forward-OOS` 視為兩個 current 頂層 Gate。正式規則如下：
   - **Pre-Test 是正式研究漏斗第一層，但不是正式 Rolling evidence**：模型 Pre-Test 沿用原本單模型流程，以所有合法歷史資料在 Selection 內做 inner validation／epoch selection、完整 Selection refit，再只讀 2021+ OOS；Strategy Pre-Test直接 REUSE 該單模型 OOS scores做快速策略比較。Pre-Test只能回答「是否值得進 Rolling」，不得作 production promotion 或取代十fold Rolling。
   - **Extending-Window Rolling 是主要策略／實際操作 Truth**：正式 score period 固定為 `2016-01-01～2025-12-31`，共10個完整年度fold；training history 採 expanding/extending window，score fold／模型更新 cadence 預設 12 months。每個 fold 必須用該 score period 前且 Target 已完整成熟的所有合法資料做 inner validation／epoch selection 與 final refit，再只對下一段期間打分。
   ...
   - **Fixed-Window Rolling 是歷史穩定性／learnability 診斷，不是 production performance Truth**：正式 score period同樣固定為 `2016-01-01～2025-12-31`，共10個完整年度fold，使用相同 12M periodic refit 與 PIT legality，但完整fit history固定為可設定calendar window；canonical default=`120 months`。雖然2015已具備完整120M且benchmark 300-bar成熟，但current research governance選擇從2016開始，保留2016～2025十個完整年度fold即可。報表必須同列 train date span、train groups、tickers、validation groups、score groups；只有必要時才新增 matched-sample audit。
   ...
   - **Artifact isolation**：Extending-Window Rolling 與 Fixed-Window Rolling 不得共用會互相覆寫的 aggregate／fold output path；legacy annual folds／正式 Strategy Compare runs永久保留可追溯。
   ...
   - **Robustness**：current multi-seed只以 Extending-Window Rolling strategy 為主，canonical default=`seed_count=4`、`seed_generator_seed=20260810`；Fixed-Window Rolling先做Seed42，只有結果可能改變GO/REJECT時才補multi-seed。互動選單只提供各current robustness profile自己的入口，不再提供「一次執行全部 Multi-seed robustness」。legacy Selection/Frozen robustness不再補跑。
   ...
   - **Rolling fold-level parallelism 只可平行獨立fold，不可改fold語意**：current default=`fold_workers=2`，以`spawn`獨立process執行真正缺少的fold；父程序必須先完成REUSE／checkpoint-rescore／legacy migration判定，每個worker只能寫自己的fold目錄，完成後父程序再依既有fingerprint/hash驗證並按fold順序aggregate。worker count不得進scientific fingerprint，`fold_workers=1`必須等價於原serial流程。若外層流程本身已平行GPU model trainers（例如Multi-seed robustness），其nested PIT fold parallelism必須由config降為1，避免巢狀CUDA process oversubscription。
