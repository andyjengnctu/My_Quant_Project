...
9. 研究工作可由單一`apps/research.py`作為使用者正式入口，但模型訓練、策略參數最佳化與策略組合比較必須維持獨立工作類型與application/service責任分離；一般策略組合比較不得自行建立Label、選擇模型或訓練模型權重。Extending-Window／Fixed-Window rolling score、manifest、audit與Model Gate一律屬於`[1] 模型訓練`工作類型；Legacy Frozen Forward只保留歷史相容讀取，`[3] 策略組合比較`即使已有fold score／checkpoint也不得建立、重建或重跑PIT Gate。current Rolling工件缺少、identity/hash/coverage/期間或PIT資訊時點不合法時必須直接BLOCKED並導向`[1] 模型訓練 → Extending-Window Rolling Test → 對應 Fast／Overnight Test`；但**已完整完成且工件合法的PIT Model Gate品質FAIL屬研究診斷證據，不得自動取代最終策略績效驗證**。若current `config/strategy_compare.py`明確啟用該controlled research arm，Strategy Compare必須以WARN揭露Gate FAIL但仍可REUSE既有PIT工件進行strategy replay；此例外只供研究conversion/ablation，不構成production/runtime promotion資格，正式promotion仍須滿足其model/runtime Gate。一般策略組合比較仍可建立比較所需的策略參數工件。唯一明確例外為`Multiple-seed robustness`：因其scientific variable本身就是模型訓練seed、最終判讀對象是策略績效，可在`[3] 策略組合比較`下作獨立robustness orchestrator，且只能呼叫canonical model-training service建立隔離、暫存的per-seed權重／score後立即進同一策略replay；不得建立新Label、改target／architecture／loss／hyperparameter、覆寫canonical模型、挑best seed、做seed ensemble或把seed工件升級成正式模型。各工作類型不得複製核心訓練邏輯；特定model、實驗標的、比較arm與Audit module不得由選單選擇，必須由`config/`指定。
...
12. `breakout_quality` 的時間驗證框架自 2026-08-19 起正式收斂為 **Extending-Window Rolling Test + Fixed-Window Rolling Stability Test**，兩者都只有 `Fast Test` 與 `Overnight Test` 兩種執行深度；`Selection PIT`、`Frozen Forward-OOS`、舊 `Pre-Test` 都不得再作 current 頂層 Gate。正式規則如下：
   - **Fast Test 是 current 第一層 Rolling Gate**：score period固定為 `2016-01-01～2025-12-31`，cadence=`60 months`，fold anchor固定=`2016-01-01`，因此只形成兩個完整score folds：`2016-01-01～2020-12-31`、`2021-01-01～2025-12-31`。每個fold仍必須用score_start前且Target已完整成熟的合法資料重新做inner validation／epoch selection與final refit；Fast只回答「是否值得投入Overnight」，不得作production promotion或取代12M完整evidence。
   - **Overnight Test 是主要策略／實際操作 Truth**：score period同樣固定為 `2016-01-01～2025-12-31`，cadence=`12 months`，共10個完整年度fold；training history採expanding/extending window。既有合法12M canonical PIT folds與Strategy Compare結果保持原identity/path並可直接REUSE。
   - **Legacy Pre-Test退出current workflow**：原本Selection-refit→2021+ frozen OOS模型／策略快速Gate及其Cxx/MRxx結果永久保留為`LEGACY_PRE_TEST / HISTORICAL_EVIDENCE`，不刪除、不改ID，但不再出現在current互動選單、current model-artifact preparation或current Gate。
   - **Daily Universal training universe 不得再繼承 optimizer 的 historical `selection_start` cutoff**。最早可用 training stock-day 必須由原始資料與 feature contract 自動決定：個股與 benchmark 都必須具備完整 sequence lookback；目前300-bar契約下以 benchmark 第300個合法bar日期作共享下界，個股仍各自要求完整300 bars。任何因舊 optimizer period 造成的2011等人為截斷不得限制模型歷史資料。因此改變了 training-data semantics，舊2014～2020 PIT folds保留為historical evidence，但不得直接 REUSE 成新的 Extending-Window Rolling folds；2016起須依新完整歷史語意建立／驗證。
   - **Fixed-Window Rolling Stability Test 是歷史穩定性／learnability 診斷，不是 production performance Truth**：完整fit history固定為canonical `120 months`；Fast同樣用60M cadence與2016 anchor形成兩個fold，Overnight用12M cadence形成十個年度fold。兩種mode都保持相同PIT legality與24M inner validation；報表必須同列train date span、train groups、tickers、validation groups、score groups。
   - **OOS legality 以每個 prediction/fold 的 information cutoff 為準，不以某個人為年份切割為準**。對 score date `t`，所有 train features、已使用 Target 的 completion time、inner-validation／epoch-selection資訊、normalization/calibration、sample weighting、strategy parameter fitting與其他自動決策都必須在 `t` 前合法可得；不得利用 `t` 之後的結果回頭擬合、調整或選擇當下模型／參數。保留 inner validation；取消 Selection period 頂層語意不等於取消模型 selection mechanism。
   - **Frozen Forward 退出 active workflow**：既有 frozen checkpoints、Cxx/MRxx results、reports、manifests 與歷史 conclusions 永久保留，不刪除、不改 ID、不假裝成 Extending-Window Rolling；其 evidence 分類為 `LEGACY_FROZEN_FORWARD / HISTORICAL_EVIDENCE`。若需 model staleness，只做最小必要 model-age audit。
   - **歷史研究記錄不得改寫**：舊 Selection PIT／Frozen Forward 原始數值與同policy controlled contrasts保留；跨兩種舊policy的直接差值不得再單獨解讀為temporal stability／learnability。
   - **Strategy params 必須同樣 PIT-safe**：可 stitch 已存在且各自合法的 historical/current rolling parameter schedules，但不得利用後段績效重新最佳化前段；stitch 前驗證 rolling window／horizon／search-contract 相容與 schedule 無缺口。Extending-Window Rolling正式策略比較只取用2016-01-01～2025-12-31 schedule。
   - **Artifact isolation**：Fast 60M與Overnight 12M不得互相覆寫aggregate／fold output path；Extending與Fixed亦必須隔離。Overnight沿用既有canonical `point_in_time`與既有Strategy Compare output root以保留REUSE；Fast固定寫入獨立`point_in_time_fast_60m`／`fast_60m` namespace。Fast／Overnight是execution/evaluation mode，不新增MR/SR/Cxx scientific identity。
   - **研究與 production 分離**：production/runtime identity 在新 Extending-Window Rolling evidence、必要 robustness 與明確 promotion decision 完成前不得自動切換；目前既有 production C42/C44 只作既有production identity保存。
   - **Robustness**：current multi-seed只以Extending-Window Rolling strategy為主，canonical default=`seed_count=4`、`seed_generator_seed=20260810`；同一入口內先選Fast 60M或Overnight 12M。Fast robustness先以2 folds × 4 seeds作最小必要seed Gate；只有其結果仍可能改變GO/REJECT/promotion時才跑Overnight 10 folds × 4 seeds。Fixed-Window先做Seed42，必要時才補robustness。legacy Selection/Frozen robustness不再補跑。
   - **Rolling authorization 必須由 research identity 明確授權**：continuous profile即使技術上支援PIT，也只有Registry/config research spec的`selection_pit_authorized=True`才可實際執行Extending-Window／Fixed-Window Rolling。互動選單的Rolling工作類型入口不得因目前profile未授權、已REJECT或NO PIT而被隱藏；使用者選入後必須明確顯示BLOCKED／未授權原因並返回，不得把「隱藏選單」當成authorization。programmatic builder與audit仍不得繞過底層授權。
   - **Rolling training-history identity 必須data-driven且進入reuse contract**：Daily Universal manifest的available-history start與fold `source_contract.training_universe_start_date`固定取真正`training_universe_start_date`，不得回報optimizer `selection_start`。任何此欄位缺少或不同的舊fold都不得REUSE成current Rolling fold。
   ...
   - **Rolling feeding acceleration 只可改 execution，不可改科學語意**：current default=`train_prefetch_batches=8`、`train_prefetch_workers=4`；worker prefetch維持feature-only ordered materialization，CUDA使用pinned feature、context/target主執行緒staging、non-blocking H2D與獨立copy stream。已實測會使主執行緒在yield目前batch前等待更重`future.result()`的complete-host prefetch不得作current default。batch membership/order、same-date batching、seed、optimizer step、loss、deterministic algorithms與TF32設定不得因此改變。
   - **Rolling fold execution current default 固定為單fold串行**：2026-08-19曾以單張GPU同時執行2個獨立fold的`spawn` process parallelism；使用者實機觀察VRAM用滿、GPU約40%且總體更慢，因此該execution方案已退役，不得作current default或保留只為coverage的專屬實作／設定。Rolling仍維持父流程逐fold完成REUSE／checkpoint-rescore／legacy migration與缺fold補訓，已完成且identity/hash合法的fold照原resume contract重用；未來若重新研究cross-fold parallelism，必須以新的受控工程benchmark證明wall-clock淨改善，且不得改fold scientific identity。
