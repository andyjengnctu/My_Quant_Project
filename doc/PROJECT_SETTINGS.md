...
1. 若使用者只傳檔案、ZIP上來，沒有文字指示，預設就是代表完整檢查。
2. 檢查必須優先做完整盤點，一次找出所有問題，不得以多輪局部修補當成預設流程。
...
6. 使用者未上傳本地 `apps/test_suite.py` 產出的 bundle 時，代表目前的動態測試ALL PASSED，assistant 僅依本輪提供之程式、ZIP、檔案做靜態檢查。
7. 使用者有上傳本地 `apps/test_suite.py` 產出的 bundle 時，assistant 才依該結果檢查本地動態回歸問題。
8. 雖然`apps/test_suite.py` 限定在本地端測試，assistant 有責任在必要時(如修改測試編排、正式入口、核心統計口徑、錯誤路徑後)檢查 `apps/test_suite.py` 是否完整覆蓋應驗項目。
9. 檢查結束時，assistant 必須以使用者可見的整輪 UI 總處理時間作為主指標，用表格列出GPT 在各階段的耗時: 
    - 表格包含GPT 端各階段處理內容、各階段耗時、總耗時，格式為幾分:幾秒；
    - 階段耗時加總必需與總耗時一致，總耗時又必需盡可能與使用者可見的整輪 UI 總處理時間一致; 
    - 若單一階段同時包含多種不同工作，必須再細分為可判斷瓶頸的子階段，不得以過粗階段名稱掩蓋真正耗時來源
    - 不得以觀察、估計或近似描述替代，也不得以過粗階段名稱掩蓋真正時間瓶頸。
