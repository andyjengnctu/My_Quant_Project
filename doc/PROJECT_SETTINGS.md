...
8. GPT 回報檢查結果時，必須明確區分本輪已獨立確認、僅有本地 formal suite / checklist 覆蓋宣稱、以及本輪仍未能確認，不得混寫或誤標；使用者只要要求「完整檢查」，即強制進入固定檢查矩陣模式：須逐項列出本文件 A 至 E 全部條款、逐項填寫本輪驗證方式與結果，且每項均須標示為前述三類之一；任一條款未落表、未分類、或僅以摘要帶過者，一律視為未完成完整檢查，且不得使用「已完成完整檢查」或其他同義表述，只能明示為部分檢查並列出缺口。
