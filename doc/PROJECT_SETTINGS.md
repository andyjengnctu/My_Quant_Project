...
12. `breakout_quality` 的時間驗證框架自 2026-08-20 起正式收斂為 **Extending-Window Test + Fixed-Window Stability Test**；兩者都只有 `OOS Test` 與 `Rolling Test` 兩種執行模式。策略比較current頂層只保留 **Extending-Window Test** 與 **Extending-Window Multi-seed Robustness Test**；`Selection PIT`、`Frozen Forward-OOS`、舊 `Pre-Test`、舊 `Fast 60M` 都不得再作 current 頂層 Gate。正式規則如下：
   ...
   - **Robustness**：current multi-seed只以Extending-Window strategy為主，canonical default=`seed_count=4`、`seed_generator_seed=20260810`；同一入口內先選OOS Test或Rolling Test。兩mode都沿用與single-seed相同Compare Suite；目前fixed references由dependency自動解析為C61/C58，seed-sensitive arms自動解析為C59/C60，C59每seed訓MR-13E，C60每seed同seed訓MR-13K+MR-13M。OOS robustness只需1 fold × 4 seeds作最小必要seed Gate；只有其結果仍可能改變GO/REJECT/promotion時才跑Rolling Test的2021→latest 12M folds × 4 seeds（fold數由latest合法日期動態決定）。Fixed-Window先做Seed42，必要時才補robustness。legacy Selection/Frozen/Fast60 robustness不再補跑。
