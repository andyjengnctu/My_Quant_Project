...
12. `breakout_quality` 的時間驗證框架自 2026-08-20 起正式收斂為 **Extending-Window Test + Fixed-Window Stability Test**；兩者都只有 `OOS Test` 與 `Rolling Test` 兩種執行模式。策略比較current頂層只保留 **Extending-Window Test** 與 **Extending-Window Multi-seed Robustness Test**；`Selection PIT`、`Frozen Forward-OOS`、舊 `Pre-Test`、舊 `Fast 60M` 都不得再作 current 頂層 Gate。正式規則如下：
   ...
   - **Strategy params 必須同樣 PIT-safe**：Rolling Test可 stitch 已存在且各自合法的 historical/current rolling parameter schedules，但不得利用後段績效重新最佳化前段。**OOS Test不得沿用2021後逐年更新的rolling parameter members**；Full／Min都必須只取`2021-01-01`當下合法、亦即2020年底information cutoff前已完成fit的effective member，建立deterministic frozen parameter artifact並固定使用到最新。OOS frozen artifact必須pin source SHA／cutoff／source effective member／output SHA，且不得保留可被downstream誤讀的post-cutoff平行schedule。OOS策略比較期間由三個current PIT DL runtime工件的共同coverage動態解析為2021→最新；Rolling Test正式策略比較仍只取2016-01-01～2025-12-31 schedule。
   - **Artifact isolation**：OOS Test與Rolling Test不得互相覆寫aggregate／fold output path；Extending與Fixed亦必須隔離。Rolling Test沿用既有canonical `point_in_time`與既有Strategy Compare output root以保留REUSE；OOS Test固定寫入獨立`point_in_time_oos_2021_forward`與`outputs/strategy_compare/extending_window/oos_2021_forward` namespace；Multi-seed OOS亦使用獨立`oos_2021_forward` model-work/output roots。任何dual-model arm（目前C60 MR-13K primary + MR-13M residual safety）之primary／secondary score、manifest都必須解析到**同一evaluation mode namespace**，不得一邊OOS、一邊fallback到Rolling canonical。OOS／Rolling只是execution/evaluation mode，不新增MR／SR／Cxx scientific identity。
   ...
   - **Robustness**：current multi-seed只以Extending-Window strategy為主，canonical default=`seed_count=4`、`seed_generator_seed=20260810`；同一入口內先選OOS Test或Rolling Test。兩mode都沿用與single-seed相同Compare Suite；目前fixed references由dependency自動解析為C61/C58，seed-sensitive arms自動解析為C59/C60，C59每seed訓MR-13E，C60每seed同seed訓MR-13K+MR-13M。OOS robustness只需1 fold × 4 seeds作最小必要seed Gate；只有其結果仍可能改變GO/REJECT/promotion時才跑Rolling Test的10 folds × 4 seeds。Fixed-Window先做Seed42，必要時才補robustness。legacy Selection/Frozen/Fast60 robustness不再補跑。
