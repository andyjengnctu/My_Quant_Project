...
12. `breakout_quality` 的時間驗證框架自 2026-08-18 起正式使用 **Extending-Window Rolling + Fixed-Window Rolling**，不再把 `Selection PIT` 與 `Frozen Forward-OOS` 視為兩個 current 頂層 Gate。正式規則如下：
   - **Extending-Window Rolling 是主要策略／實際操作 Truth**：正式 score period 自 `2015-01-01` 起；training history 採 expanding/extending window，score fold／模型更新 cadence 預設 12 months。每個 fold 必須用該 score period 前且 Target 已完整成熟的所有合法資料做 inner validation／epoch selection 與 final refit，再只對下一段期間打分。
   - **Daily Universal training universe 不得再繼承 optimizer 的 historical `selection_start` cutoff**。最早可用 training stock-day 必須由原始資料與 feature contract 自動決定：個股與 benchmark 都必須具備完整 sequence lookback；目前300-bar契約下以 benchmark 第300個合法bar日期作共享下界，個股仍各自要求完整300 bars。任何因舊 optimizer period 造成的2011等人為截斷不得限制模型歷史資料。因此改變了 training-data semantics，舊2014～2020 PIT folds保留為historical evidence，但不得直接 REUSE 成新的 Extending-Window Rolling folds；2015起須依新完整歷史語意建立／驗證。
   - **Fixed-Window Rolling 是歷史穩定性／learnability 診斷，不是 production performance Truth**：正式 score period同樣自 `2015-01-01` 起，使用相同 12M periodic refit 與 PIT legality，但完整fit history固定為可設定calendar window；canonical default=`120 months`。2015是目前raw data下第一個可從1月1日起取得完整前120M且benchmark 300-bar已成熟的年度。報表必須同列 train date span、train groups、tickers、validation groups、score groups；只有必要時才新增 matched-sample audit。
   ...
   - **Strategy params 必須同樣 PIT-safe**：可 stitch 已存在且各自合法的 historical/current rolling parameter schedules，但不得利用後段績效重新最佳化前段；stitch 前驗證 rolling window／horizon／search-contract 相容與 schedule 無缺口。Extending-Window Rolling正式策略比較只從2015開始取用schedule。
