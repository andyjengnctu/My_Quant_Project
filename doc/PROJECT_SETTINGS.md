...
10. 單股回測與投組回測使用複利資金口徑；scanner 的參考投入 / 掛單股數改由 `scanner_live_capital` 控制，預設 2,000,000；歷史績效 filter 只用於投組層與 scanner，單股回測不得用自身歷史績效 filter 作為買入閘門; 仍保留'use_compounding'參數作為未來彈性。
