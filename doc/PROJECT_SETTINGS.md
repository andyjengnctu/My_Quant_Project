...
23. Strategy Compare 的 `R 預測／轉化` 人讀區固定合併成單一表，採「比較 arm 放列、指標放欄」格式；表頭第一層固定用三個分群超欄：`實際交易`、`模型預測`、`選股轉換`，其下欄位順序固定為：`實際交易＝平均R、中位R、Coverage、DL選擇R`；`模型預測＝Dailyρ、Globalρ、Pair一致、Top-R、Bottom-R、Top-BottomR`；`選股轉換＝RCE、Target mean R、Target %ile、Top-K、Opp gap`。`RCE`固定定義為同參數DL-on與DL-off之間「同一批canonical exclusive completed trades」的`realized mean-R edge ÷ Future Target mean-R edge`；exclusive trade partition必須使用canonical closed-trade match key，Future Target只在replay後由既有selected-target sidecar join到同一批交易。只有exclusive trade Target coverage完整、兩側皆有exclusive completed trades且Target mean-R edge`>0`時才顯示；100%代表預測的每筆交易Target品質edge等量轉成實際每筆交易R品質edge。`DL選擇R`仍獨立表示exclusive realized total-R效果，不得拿它除以不同trade count的Target total-R，也不得使用`EV差／overall Target mean差`等proxy。`Top-R`／`Bottom-R`必須直接重用既有模型驗證工件中最高／最低Score十分位的平均Target R，`Top-BottomR=Top-R−Bottom-R`，不得為報表另算第二套；欄位上色採每個metric在同表可比較arm間「綠＝最佳、紅＝最差、白＝其餘」，而不是reference delta。`定義`與`理想方向`改為表格下方註解，依三個分群列出。aggregate console、`strategy_comparison.md`與`strategy_diagnostics.md`必須重用同一canonical diagnostics payload與metric registry。
