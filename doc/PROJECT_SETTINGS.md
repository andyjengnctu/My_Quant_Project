...
10. 維護任何會被程式或檢查流程解析的文件、表格或摘要時，每列只能表達一個可唯一對應的原子事實；不得在同一列合併多個項目、入口、結果或不同層級的摘要，避免解析歧義。
11. 維護狀態追蹤或收斂歷程時，表格列只可記錄實際狀態變更；若僅為補充說明、probe、註記、背景或不改變最終狀態的資訊，必須改用表格外文字，不得覆蓋或混淆最終狀態判定。
