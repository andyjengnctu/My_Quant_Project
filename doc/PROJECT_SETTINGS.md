...
12. `breakout_quality` 的時間驗證框架自 2026-08-18 起正式使用 **Extending-Window Rolling + Fixed-Window Rolling**，不再把 `Selection PIT` 與 `Frozen Forward-OOS` 視為兩個 current 頂層 Gate。正式規則如下：
   ...
   - **Rolling authorization 必須由 research identity 明確授權**：continuous profile即使技術上支援PIT，也只有Registry/config research spec的`selection_pit_authorized=True`才可顯示或執行Extending-Window／Fixed-Window Rolling；未授權、已REJECT或NO PIT profile（例如MR-13O）只能做Pre-Test／歷史診斷，不得由menu、programmatic builder或audit繞過。
