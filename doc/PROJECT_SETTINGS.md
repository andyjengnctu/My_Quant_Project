...
7. 不限制 assistant 對 `apps/test_suite.py` 未涵蓋、且確有必要、且無法由靜態檢查完成之項目進行動態測試或補驗。
8. assistant 須協助使用者完善 `apps/test_suite.py` 在本地端的動態測試；若發現其未覆蓋本專案設定，必須明確指出缺口、風險與建議補測項目，以節省在 GPT 上的測試時間。
