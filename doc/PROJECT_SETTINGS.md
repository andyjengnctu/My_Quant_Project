...
12. `breakout_quality` 的時間驗證框架自 2026-08-20 起正式收斂為 **Extending-Window Test + Fixed-Window Stability Test**；兩者都只有 `OOS Test` 與 `Rolling Test` 兩種執行模式。策略比較current頂層只保留 **Extending-Window Test** 與 **Extending-Window Multi-seed Robustness Test**；`Selection PIT`、`Frozen Forward-OOS`、舊 `Pre-Test`、舊 `Fast 60M` 都不得再作 current 頂層 Gate。正式規則如下：
   ...
   - **Rolling Test 是主要策略／實際操作 Truth**：score period固定從`2021-01-01`開始，`score_end=auto（最新合法score date）`，cadence=`12 months`；training history採expanding/extending window並每個calendar fold重新做inner validation／epoch selection／final refit。OOS與Rolling因此使用相同2021→latest評估期間，主要差異只保留「2020 cutoff後是否持續年度refit」。既有canonical `point_in_time`的2021+合法annual folds必須直接REUSE，資料尾端可形成latest partial fold；2016～2020舊fold與舊2016～2025 aggregate只保留historical evidence。
   ...
   - **Fixed-Window Stability Test 是歷史穩定性／learnability 診斷，不是 production performance Truth**：完整fit history固定為canonical `120 months`。OOS Test同樣只訓練一次並score `2021～最新`；Rolling Test同樣score `2021～最新`並以12M cadence逐fold refit。兩種mode都保持相同PIT legality與24M inner validation；報表必須同列train date span、train groups、tickers、validation groups、score groups。
   ...
   - **Strategy params 必須同樣 PIT-safe，且只有一個current artifact domain**：Optimizer是唯一current producer，正式工件固定由`models/strategy_params/{full|min}/{oos|rolling}/`解析；`base_best / *_finalists_agree / ensemble_*`等first-class parameter policies、seed、trial count、training policy fingerprint與payload SHA都屬同一Optimizer-owned manifest lineage。Rolling Test只消費2021→latest各fold cutoff合法的canonical rolling schedule；不得由Research另做stitch或重新最佳化。**OOS Test不得沿用2021後逐年更新的rolling members**；若canonical OOS工件不存在，只有Optimizer service可從同family rolling schedule的`2021-01-01`合法effective member做deterministic 2020-cutoff freeze／migration並pin source SHA、cutoff、source effective member與output SHA，Research不得持有第二套freeze邏輯。舊root `models/base*.json`、`models/full_*.json`、`models/trade_*.json`、`models/oos_*.json`、`models/roos_*.json`、`models/run_best_params.json`、`models/candidate_*`與`models/research/...`參數只作historical compatibility／一次性migration source，不得成為current REUSE truth；migration完成後current discovery/runtime/writer不得再掃描、fallback或寫回`models/*.json` root。Trade active/candidate state固定位於`models/strategy_params/full/trade/state/`，其atomic write／backup／manifest refresh亦只由Optimizer repository持有。OOS與Rolling策略比較期間都由current PIT DL runtime工件共同coverage動態解析為2021→latest；差異只在OOS參數固定、Rolling參數按各fold PIT-safe schedule更新。
   - **Artifact isolation**：OOS Test與Rolling Test不得互相覆寫aggregate／fold output path；Extending與Fixed亦必須隔離。Rolling model fold store仍沿用各自既有Extending／Fixed PIT fold store以直接REUSE既有2021+ annual folds；若current score period由舊2016～2025縮為2021→latest，刷新任何top-level aggregate前必須先snapshot superseded aggregate/audit，2016～2020 fold不可刪除。Current Rolling Strategy Compare與Robustness則固定使用新的`rolling_2021_forward` output/model-work namespace，避免覆寫或誤REUSE舊2016～2025 aggregate。OOS Test固定寫入獨立`point_in_time_oos_2021_forward`與`outputs/strategy_compare/extending_window/oos_2021_forward` namespace；Multi-seed OOS亦使用獨立`oos_2021_forward` model-work/output roots。任何dual-model arm（目前C60 MR-13K primary + MR-13M residual safety）之primary／secondary score、manifest都必須解析到**同一evaluation mode namespace**，不得一邊OOS、一邊fallback到Rolling canonical。OOS／Rolling只是execution/evaluation mode，不新增MR／SR／Cxx scientific identity。
   ...
   - **Compare Suite單一真理**：current Extending strategy的比較對象與contrasts只能由`config/strategy_compare.py`的一個Compare Suite定義；OOS Test、Rolling Test、single-seed與multi-seed都只能引用同一`suite_id`，不得各自維護arm／contrast清單。Current suite固定解析同一scientific matrix（目前C61/C62/C58/C63/C59/C60及其九個contrasts；C61/C58/C59/C60固定`base-finalist-best`，C62/C63固定`base-finalists-agree`），evaluation mode只可改cutoff／fold／param binding／artifact namespace與人讀suffix。Robustness的fixed/stochastic執行角色不得另寫current arm ID清單：無model dependency的suite arm自動視為fixed reference，只回放一次；有model dependency的suite arm自動視為seed-sensitive，逐seed建立其全部primary／secondary model sources後replay。任何current mode解析出的arm集合不同於suite、或robustness membership不等於suite依dependency推導的partition都必須FAIL。
   - **Current人讀命名**：suite保存不帶evaluation階段的策略base name；OOS renderer固定加`OOS` suffix（例如`Full OOS / Min OOS`），Rolling renderer固定加`Rolling` suffix（例如`Full Rolling / Min Rolling`）。`ROOS`只保留在legacy/historical identity與工件相容語意，不得再作current OOS／Rolling報表的比較對象名稱。
   - **Robustness**：current multi-seed只以Extending-Window strategy為主，canonical default=`seed_count=4`、`seed_generator_seed=20260810`；同一入口內先選OOS Test或Rolling Test。兩mode都沿用與single-seed相同Compare Suite；目前fixed references由dependency自動解析為C61/C62/C58/C63，seed-sensitive arms自動解析為C59/C60，C59每seed訓MR-13E，C60每seed同seed訓MR-13K+MR-13M。OOS robustness只需1 fold × 4 seeds作最小必要seed Gate；只有其結果仍可能改變GO/REJECT/promotion時才跑Rolling Test的2021→latest 12M folds × 4 seeds（fold數由latest合法日期動態決定）。Fixed-Window先做Seed42，必要時才補robustness。legacy Selection/Frozen/Fast60 robustness不再補跑。
   ...
   - **Current strategy-parameter identity**：current Strategy Compare只可解析四個canonical source identity：`full_oos / min_oos / full_rolling / min_rolling`，其實體工件一律位於`models/strategy_params/{full|min}/{oos|rolling}/`並由Optimizer service唯一管理。`extending_min_roos / P2_EXTENDING`、`extending_full_roos`、`oos_min_roos`、`oos_full_roos`、`operational_min_roos / P2_OPERATIONAL`均只可作historical compatibility identity，不得再作current config/runtime/artifact truth。
