...
12. `breakout_quality` 的時間驗證框架自 2026-08-18 起改為 **Operational Rolling + Fixed-Window Stability**，不再把 `Selection PIT` 與 `Frozen Forward-OOS` 視為兩個 current 頂層 Gate。正式規則如下：
   - **Operational Rolling 是主要策略／實際操作 Truth**：training history 採 expanding window，score fold／模型更新 cadence 預設 12 months；每個 fold 必須用該 score period 前且 Target 已完整成熟的所有合法資料做 inner validation／epoch selection 與 final refit，再對下一段期間打分。2014～2020 既有合法 expanding PIT folds 在 contract 相容時直接 REUSE，之後以同一語意延伸至目前可驗證期間；不得在 2021 人為切換成 frozen model policy。
   - **Fixed-Window Stability 是歷史穩定性／learnability 診斷，不是 production performance Truth**：使用相同 periodic refit 與 PIT legality，但 training history 固定為可設定的 calendar window；第一版 canonical default=`120 months`。報表必須同列 train date span、train groups、tickers、validation groups、score groups，因相同 calendar window 仍可能有不同 sample volume；只有必要時才新增 matched-sample audit。
   ...
   - **Frozen Forward 退出 active workflow**：既有 frozen checkpoints、Cxx/MRxx results、reports、manifests 與歷史 conclusions 永久保留，不刪除、不改 ID、不假裝成 Operational Rolling；其 evidence 重新分類為 `LEGACY_FROZEN_FORWARD / HISTORICAL_EVIDENCE`。未來若需研究 model staleness，以最小必要的 model-age audit 進行，不再要求每個新模型執行多年 frozen strategy Gate 或 frozen robustness。
   - **歷史研究記錄採 migration、不得改寫歷史**：舊 `Selection PIT 2014–2020` 可重新分類為 historical expanding annual-refit evidence；舊 Frozen Forward 保留原 policy 標記。同一 evaluation policy／同一期間內的 controlled contrasts 仍有效；任何直接以舊 Selection 與 Frozen Forward 的差異推論 temporal stability／learnability 的結論都必須標記為 policy-confounded 並重新解讀，不得刪除原始數值。
   - **Operational strategy params 也必須維持 PIT legality**：migration 可 stitch 已存在且各自合法的 historical/current rolling parameter schedules，但不得利用後段績效重新最佳化前段；stitch 前必須驗證 rolling window／horizon／search-contract 相容與 schedule 無缺口。
   - **Artifact isolation**：Operational expanding rolling 與 Fixed-Window Stability 不得共用會互相覆寫的 fold identity／output path。第一次將 canonical expanding aggregate 從舊 historical Selection period 延伸到後續年份時，必須保留舊 combined aggregate 的 historical snapshot；舊 annual folds／正式 Strategy Compare runs 永久保留並可追溯。
   - **研究與 production 分離**：production/runtime identity 在新 Operational Rolling evidence、必要 robustness 與明確 promotion decision 完成前不得自動切換；目前既有 production C42/C44 只作既有 production identity 保存，不因 evaluation-framework migration 自動升降格。
   - **Robustness**：current multi-seed 應以 Operational Rolling strategy 為主；Fixed-Window Stability 先做 Seed42，只有結果可能改變 GO/REJECT 時才補 multi-seed。已被 framework migration 取代的 legacy Selection/Frozen full-flow robustness 不再為了完成舊矩陣而消耗算力。
