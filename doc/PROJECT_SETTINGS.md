...
2. 測試資料正式來源依序為：
   - 本輪使用者或系統另外提供的測試資料 ZIP / 專案資料
   - 已可直接使用的 `/data/tw_stock_data_vip_reduced`
3. reduced 動態測試、主流程 smoke test、consistency、最嚴格檢查前，必須先搜尋本輪或專案中已提供的 `tw_stock_data_vip_reduced`；不得只因程式包 ZIP 內沒有該資料就判定缺資料。
4. 程式包 ZIP 不包含測試資料屬正常設計，不得視為缺檔、專案缺陷或使用者未提供資料。
5. 驗證一律以 `tw_stock_data_vip_reduced` 為準；除非使用者明確要求，否則不做 `tw_stock_data_vip` full dataset 測試。
6. 若 reduced 測試資料最終不可用，只需在驗證結果中寫「reduced 動態驗證未執行」；不得解讀為程式包缺檔、專案缺陷或使用者未提供資料。
7. 若需外部套件，應依 `requirements` 自行安裝後再驗證；不得因缺套件就略過本應執行的動態測試。
