...
9. 檢查結束時，assistant 必須以 GPT 端自記的整輪工作區間總歷時作為主指標，用表格列出各階段處理內容、耗時、各階段耗時總和；時間格式為幾分:幾秒，不得以觀察、估計或近似描述替代。
10. GPT 端階段切分必須覆蓋主要耗時來源；各階段耗時總和若與 GPT 端整輪工作區間總歷時不一致，必須列出差額與來源，不得讓主要耗時落在未說明區間，也不得以過粗階段名稱掩蓋真正瓶頸。
