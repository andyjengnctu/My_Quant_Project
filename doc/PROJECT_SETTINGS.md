...
5. 當使用者提供最新程式ZIP與對應的本地 `apps/test_suite.py` bundle 時，可不重複執行 bundle已覆蓋且可由報告證明的驗證；僅在bundle缺項、顯示 FAIL、與 ZIP 不一致、本輪修改點未覆蓋、或使用者明確要求時補驗。
6. 只有在「已完成完整盤點、問題已一次列全、已全部修補、已整輪回歸、且無未揭露未執行驗證項或環境問題」時，才可寫「本輪已全部修好」。
