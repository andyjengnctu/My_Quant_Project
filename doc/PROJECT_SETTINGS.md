...
12. `breakout_quality` 的時間驗證框架自 2026-08-20 起正式收斂為 **Extending-Window Test + Fixed-Window Stability Test**；兩者都只有 `OOS Test` 與 `Rolling Test` 兩種執行模式。策略比較current頂層只保留 **Extending-Window Test** 與 **Extending-Window Multi-seed Robustness Test**；`Selection PIT`、`Frozen Forward-OOS`、舊 `Pre-Test`、舊 `Fast 60M` 都不得再作 current 頂層 Gate。正式規則如下：
   ...
   - **Strategy params 必須同樣 PIT-safe**：Rolling Test可 stitch 已存在且各自合法的 historical/current rolling parameter schedules，但不得利用後段績效重新最佳化前段；current strategy replay只消費2021→latest共同period對應的effective members。**OOS Test不得沿用2021後逐年更新的rolling parameter members**；Full／Min都必須只取`2021-01-01`當下合法、亦即2020年底information cutoff前已完成fit的effective member，建立deterministic frozen parameter artifact並固定使用到最新。OOS frozen artifact必須pin source SHA／cutoff／source effective member／output SHA，且不得保留可被downstream誤讀的post-cutoff平行schedule。OOS與Rolling策略比較期間都由三個current PIT DL runtime工件的共同coverage動態解析為2021→latest；差異僅在OOS參數固定、Rolling參數按各fold PIT-safe schedule更新。
