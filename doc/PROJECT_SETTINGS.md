...
2. 使用者只要要求「完整檢查」，GPT 即強制進入固定檢查矩陣模式，須逐項列出並檢查本文件 A 至 E 全部條款，以及本文件未列出但應被檢查的項目，以表格逐項填寫本輪驗證方式與結果；只有當前述全部事項皆於本輪完成獨立確認時，方得稱為已完成完整檢查；任一事項未完成獨立確認、未列出、未填寫驗證方式或結果，一律視為未完成完整檢查。
