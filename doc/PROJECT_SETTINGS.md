...
8. 全專案唯一初始停損基準為 `init_sl`。
9. 盤前 frozen plan 一律以 `L / init_sl / T` 定義；`T` 由 `L` 與 `init_sl` 盤前決定，不得依實際成交價、盤中資訊或 trailing 重算。
10. `trailing_stop` 只屬於持倉後動態保護，不屬於初始風險定義，不得回寫或覆蓋 frozen 初始停損。
11. 候選資格、是否掛單、是否成交、是否 miss buy、歷史績效統計，必須分層定義，不得混用。
12. 單股回測與投組回測使用複利資金口徑；scanner 的參考投入 / 掛單股數改由 `scanner_live_capital` 控制，預設 2,000,000；歷史績效 filter 只用於投組層與 scanner，單股回測不得用自身歷史績效 filter 作為買入閘門。
13. 不得另引入 `S0` 或其他第二套初始風險基準。
