...
9. 核心規則、帳務、價格與統計的衍生結果，必須共享同一規則來源；顯示、重建、排序、比較與驗證不得分叉。
10. 跨模組所需上下文必須以明確介面傳遞；不得依賴隱式狀態、自由變數或私有特例。
11. 共享契約有變動時，相關實作、呼叫端、使用端、驗證與文件必須同輪同步。
12. 非核心路徑仍須服從正式規則，不得因顯示、fallback 或 optional dependency 而繞開。
13. validator 與 oracle 只驗 invariant、契約與單一真理來源，不重寫分叉實作。
