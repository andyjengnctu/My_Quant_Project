...
| 項目 | 目前狀態 |
|---|---|
| 基準 ZIP | `test-branch-1_20260813_202833_b72d444.zip`；SHA256 `6891174c512f7579ebf25cf4c01992f10e6c5bc560ce1a1446e4e00d0cbc3676`。此基準已完成MR-13C／MR-13D／MR-13E同contract Selection PIT Model Gate batch；本輪只同步研究結果與下一Gate判定，不改Dataset／Target／architecture／strategy runtime。 |
| SHA256／最新結果 | MR-13E Seed42 Forward all-stock daily rho=`0.2151`、Top-K Lift=`+0.9951R`；breakout daily rho=`0.1721`、Top-K Lift=`+0.4489R`。同contract Selection PIT 2013-04-01～2020-12-31 coverage=`100%`：all-stock daily/global rho=`0.1627/0.0533`、pair=`55.75%`、Top-K Lift=`+0.4456R`，breakout daily rho=`0.1019`、Top-K Lift=`+0.1513R`，年度rho/spread皆`8/8`正，Gate PASS。13E在13C/13D/13E shared PIT主要ranking與top-tail證據整體最強，下一步授權13E單獨進controlled strategy gate；MR-12B／DL-CONT12B仍是runtime anchor。 |
| 程式版本範圍 | Active architecture仍為`inception_time_v1`；Model Research active profile=`daily_universal_no_time_full_list_ndcg_pairwise / MR-13E`，Strategy workflow仍固定`strategy_aligned_no_time_all_event_pairwise / MR-12B`。其他淘汰architecture與profile只保留歷史重現。 |
| Policy 預設 | filter=`breakout_quality_v1`、architecture=`inception_time_v1`、Seed=`42`；Model Research profile=`daily_universal_no_time_full_list_ndcg_pairwise`；Strategy workflow profile=`strategy_aligned_no_time_all_event_pairwise`。PIT score start=`auto`、fold=`12` months、inner validation=`24` months。Binary `unique_group_sampling / threshold 0.5`仍保留其原本正式identity，但不代表目前continuous model-research active profile。 |
| 當前最佳實證模型 | Continuous model research目前優先候選為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise`，已通過Seed42 Forward與Selection PIT Model Gate，但尚未通過策略／multi-seed gate；正式runtime anchor仍是`MR-12B / DL-CONT12B`。Binary 9A `inception_time_v1 / unique_group_sampling / threshold 0.5`與8F歷史基準維持原identity。 |
