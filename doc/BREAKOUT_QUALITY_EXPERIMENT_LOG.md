...
| 項目 | 目前狀態 |
|---|---|
| 基準 ZIP | `test-branch-1_20260818_100207_a009b17.zip`，SHA256 `82a44233325d97281f4a62ccfc59b8f7a777a2ad3d2a84f349357de5e88f3d74`。此基準含MR-13O Seed42 Forward結果：Pareto只有弱edge並已REJECT；本輪在此基準上實作Plan B `SR-C55`（MR-13K primary objective + MR-13M baseline-relative safety hard floor）與系統化Research Queue。 |
| SHA256／最新結果 | MR-13E仍是production anchor。MR-13O Seed42 Forward Pareto=`51.81%`、economic Daily rho=`0.0782`、breakout Pareto=`50.30%`，已判`REJECTED_AT_FORWARD_MODEL_GATE / NO_PIT`，Target／pair-definition路線停止。MR-13M low-adverse Frozen Forward score改以`DL-CONT13M`作C55 secondary safety source；C55只在Forward測Plan B，結果待跑。MR-13K current 8-seed Selection/Forward strategy robustness仍獨立繼續。 |
| 程式版本範圍 | Active sequence architecture仍為`inception_time_v1`；MR-13O已Forward REJECT且不授權PIT。current Strategy Compare schema=`39`：Selection=`C32/C23/C42/C53`；Forward=`C1/C3/C44/C54/C55`。C55是Forward-only Plan B；production workflow profile仍為`daily_universal_no_time_full_list_ndcg_pairwise / MR-13E`，runtime candidate仍C42/C44。MR-13K C42/C53與C44/C54 current 8-seed robustness不含C55。 |
...
| 當前最佳實證模型 | Production anchor仍為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise` + Min/all-off + exact K/R0 constrained。MR-13K single-seed Selection/Forward translation低於MR-13E但8-seed robustness仍完成；MR-13M證明path-risk component可學；MR-13L/N/O與fixed equal-rank fusion均已REJECT。下一步拆成兩方案：先跑Plan B C55策略guardrail；Plan A則固定MR-13H Target只改input representation，列入`doc/BREAKOUT_QUALITY_RESEARCH_QUEUE.md`。 |
