...
| 當前最佳實證模型 | Production anchor仍為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise` + Min/all-off + exact K/R0 constrained。MR-13K single-seed Selection/Forward strategy translation低於MR-13E但依使用者決策完成current 8-seed robustness；MR-13L已Forward REJECT；MR-13M只確認path-risk component可學且不做standalone PIT；MR-13N與fixed equal-rank frozen fusion均已REJECT。MR-13O為目前唯一active model research，結果前不影響strategy/runtime。 |
