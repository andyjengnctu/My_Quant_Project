...
| 當前最佳實證模型 | Production anchor仍為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise` + Min/all-off + exact K/R0 constrained。MR-13K single-seed Selection/Forward strategy translation低於MR-13E但依使用者決策完成current 8-seed robustness；MR-13L已Forward REJECT；MR-13M只確認path-risk component可學且不做standalone PIT；MR-13N與fixed equal-rank frozen fusion均已REJECT；MR-13O Pareto-dominance pairwise亦已Forward REJECT且不進PIT。MFE/adverse Target／loss重組主線到此停止，下一輪若繼續必須轉向新的input representation／可觀測資訊，而非再掃scalar mix、pair mask或既有300×10單模型architecture。 |
...
### 2026-08-18 — MR-13O Seed42 Forward結果：Pareto supervision可泛化但edge過弱，Target／pair-definition主線停止

- 程式基準：`test-branch-1_20260818_100207_a009b17.zip`，SHA256=`82a44233325d97281f4a62ccfc59b8f7a777a2ad3d2a84f349357de5e88f3d74`；fresh extraction=`/tmp/mqp13o_result`（僅GPT工作環境）。依`doc/PROJECT_SETTINGS.md`先讀Registry／Log並核對MR-13O target、strict Pareto pair mask、loss與epoch-selection實作後判讀；本輪不執行`apps/run_bundle.py`或`apps/test_suite.py`。
- Seed42結果：selected epoch=`1`；獨立選模Validation Pareto concordance=`0.5114`、該epoch economic Daily rho=`0.0896`。Final refit後原Validation economic rho=`0.1709`因原Validation rows已納入完整Selection重訓，只作描述值，不作泛化證據。
- Frozen Forward：all-stock economic Daily/Global rho=`0.0782/0.1008`、Pair=`52.59%`、Top10=`1.2409R`、Bottom10=`0.6367R`、spread=`+0.6042R`、Top-K Lift=`+0.1120R`；Pareto concordance=`0.5181`。Breakout slice economic Daily/Global rho=`0.0376/0.0506`、Pair=`52.67%`、spread=`+0.4248R`、Top-K Lift=`+0.1147R`；Pareto concordance=`0.5030`。
- 判讀：Validation Pareto `0.5114`到Forward `0.5181`沒有典型泛化崩落，代表strict-dominance supervision的方向可重現，但實際edge僅約高於隨機`1～2pp`，breakout slice更只高`0.3pp`；economic rho／Pair／Top-K亦遠低於MR-13H及MR-13K對同economic reference的既有結果。故問題不是「13O overfit」，而是現有300×10 representation對joint high-MFE + low-adverse dominance提供的可學辨識力不足。
- Decision：`MR-13O = RESULT_AVAILABLE / REJECTED_AT_FORWARD_MODEL_GATE / NO_PIT / NO_RUNTIME_DL_SOURCE`。MR-13K/M/N/O與frozen fusion已依序排除pure-MFE-only portfolio translation、raw-R dual regression、single-model equal-rank、fixed expert fusion與strict Pareto-pair supervision；停止繼續調MFE/adverse mix、lambda、pair mask、dominance threshold或Pareto weighting。
- 下一步治理：不重開9B ModernTCN、9F Patch Transformer或既有300×10單模型architecture橫向搜尋，因歷史9B～9F已正式淘汰該方向。若繼續此研究問題，新的scientific variable必須是**input representation／可觀測資訊**，並固定economic Target與策略層；候選實驗在正式建立新MR identity前仍須先查Registry。MR-13K Selection/Forward current 8-seed robustness繼續獨立完成；production/runtime維持MR-13E C42/C44。
