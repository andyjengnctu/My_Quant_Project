...
| 項目 | 目前狀態 |
|---|---|
| 基準 ZIP | `test-branch-1_20260817_003828_4bca445.zip`，SHA256 `fd4f399225f0362b5cd7729ee4e5086de01a04d526d014252b9e86cde82d7eb9`；覆蓋`mr13j_closure_patch_20260817.zip`，SHA256 `eec8ee4f75cd1932853669ea88e07b95c4cd05052e25dc18f14a412333140554`。MR-13J/C49/C50已結案；本輪在此基準實作MR-13H label simplification，尚未取得MR-13H模型結果。 |
| SHA256／最新結果 | MR-13E已完成Forward／Selection PIT、exact K/R0 Strategy Gate、8-seed robustness與Runtime Integration promotion，production anchor為MR-13E。最新MR-13J strategy-conversion：C49同K/R0 exact未優於C42，C50 No-R0顯著降低Return/RoMD/曝險，risk-context/No-R0路線REJECT。MR-13H僅IMPLEMENTED，尚無Label Audit或Forward結果。 |
| 程式版本範圍 | Active sequence architecture仍為`inception_time_v1`；Model Research active profile=`daily_universal_full_horizon_no_breach_full_list_ndcg_pairwise / MR-13H`，production workflow profile=`daily_universal_no_time_full_list_ndcg_pairwise / MR-13E`。MR-13H未通過Forward Gate前不建立PIT/runtime source。其他淘汰architecture/profile只保留歷史重現。 |
| Policy 預設 | filter=`breakout_quality_v1`、architecture=`inception_time_v1`、Seed=`42`；Model Research profile=`daily_universal_full_horizon_no_breach_full_list_ndcg_pairwise`；Production workflow profile=`daily_universal_no_time_full_list_ndcg_pairwise`。PIT score start=`auto`、fold=`12` months、inner validation=`24` months；MR-13H尚未加入PIT Gate profiles。 |
| 當前最佳實證模型 | 已promotion實證anchor為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise` + Min/all-off + exact K/R0 constrained；current新研究為`MR-13H` label simplification，尚無結果，不得預先取代MR-13E。 |
