...
## 2026-08-12 — Legacy Model／Artifact Isolation Batch 9：active training與historical reconstruction物理分流

- 使用者最新程式基準：`test-branch-1_20260812_112756_cc3f1fa.zip`；SHA256 `e9340c180ef712fc6af609b86261df52e35827330216cbcc1450cfd250d25184`。本輪只做architecture/infrastructure isolation，不新增或修改任何`MR-*`、`ARCH-*`、`DL-*`、Target、loss、seed、Selection／Forward期間、score schema或策略語意。
- 問題：雖`ACTIVE_MODEL_ARCHITECTURES`已存在，formal binary／continuous trainer仍直接使用可解析／建立全部22個historical architectures的`get_model_spec()`／`build_model()`；`models/factory.py`與artifact/application import graph亦會在current path eager import legacy MOMENT／Mantis／TS2Vec builder／contract，與Registry「legacy read-only compatibility、不得正式新訓練」不完全一致。
- 新增`filters/breakout_quality/models/active.py`作正式新訓練唯一API，只接受`ACTIVE_MODEL_ARCHITECTURES`；新增`models/legacy_compatibility.py`集中historical builder dispatch，`models/factory.py`降為compatibility façade，只有遇到legacy architecture才lazy delegate；`models/runtime.py`承接`require_torch`與trainable-parameter count共用helper。`models/spec.py`保留全部歷史規格但MOMENT／Mantis external contract改為branch-local lazy import。
- `services/breakout_quality/train.py`與`train_continuous_ranker.py`改用active model API；continuous CLI的`--model-architecture` choices直接由`ACTIVE_MODEL_ARCHITECTURES`驅動，legacy architecture在正式新訓練入口fail-fast。Binary trainer current config仍由`contract.py` active normalization保護。Historical inference／PIT／score export維持compatibility factory，因此舊checkpoint／manifest仍可讀。
- Binary trainer移除historical pretrained-encoder loader與每次formal train都會經過的legacy compatibility call；current training直接以`pretrained_encoder_state=None`建立active model。正式`apps/research.py model`亦移除`build-pretrain-dataset`／`pretrain` command registration；底層TS2Vec pretraining modules只保留historical compatibility，不再是current Research service。
- `filters/breakout_quality/artifacts.py`與Breakout Quality application的MOMENT／Mantis external contract改為historical branch lazy import；全新process單獨import active API、binary trainer、continuous trainer與application均不載入MOMENT／Mantis／TS2Vec model/pretrained modules。
- Identity保護：對baseline與修改版全部22個`SUPPORTED_MODEL_ARCHITECTURES`逐一比較`get_model_spec(...).as_manifest_payload()`，22/22完全相同；不改既有checkpoint／manifest identity。
...
## 2026-08-12｜Infrastructure：最新程式完整獨立檢查／dead code、test hygiene、SSOT closure

- **基準**：`test-branch-1_20260812_123312_ccb6589(1).zip`；SHA256 `51b9ea51c564c715ffd40ad798f87e535920e2e782ea7332c802f9cd4d5e305a`；fresh extract `/mnt/data/stock_full_audit_latest`。本輪依`doc/PROJECT_SETTINGS.md` A～E矩陣重新獨立檢查，不沿用Batch 10結論。
- **Import graph真實缺口**：既有cycle validator module index只包含`apps/core/tools`，因此漏檢`services.breakout_quality.ranker_training -> train_continuous_ranker -> train_daily_ranker -> ranker_training`真循環。修正meta graph為`apps/config/core/filters/services/strategies/tools`完整source layers；新增`services/breakout_quality/ranker_cli.py`作profile dispatcher，event/daily trainer不再互相import。正式完整graph恢復0 cycle、formal lower layers對`apps/tools` reverse import為0。
- **Bundle安全順序**：`apps/run_bundle.py`原先先commit再formal test，失敗可能留下未驗證commit。改為stage → `apps/test_suite.py` → PASS後commit → `apps/package_zip.py` from verified HEAD；正式contract直接驗證test必須早於commit且commit早於package。
- **Dead／temporary cleanup**：移除14個無current consumer／無historical reconstruction必要性的檔案：`filters/breakout_quality/mantis_pretrained.py`、`moment_pretrained.py`、`filters/registry.py`、`tools/local_regression/run_scanner_terminal_guard.py`、已無formal consumer的`tools/validate/synthetic_breakout_quality_cases.py`、`tools/filters/breakout_quality/train_daily_ranker.py`alias、`tools/optimizer/{objective,objective_filters,objective_profiles,objective_runner,param_cache,profile,trial_inputs}.py` compatibility aliases、`tools/portfolio_sim/runtime_common.py` alias。仍保留`build_pretraining_dataset.py`、`pretrain.py`、`pretraining_store.py`等具有historical reproduction consumer的compatibility tooling。
- **SSOT closure**：`is_insufficient_data_error`與兩種environment flag解析集中`core/runtime_utils.py`；generic file SHA與canonical JSON SHA集中`core/file_integrity.py`；跨平台path判定集中`core/path_utils.py`；trade attribution／portfolio Audit共用的optional-text與JSON-native serialization集中`core/serialization_utils.py`；precomputed signal tuple／execution-context lookup分別集中`core/signal_utils.py`／`core/position_step.py`；Strategy Compare JSON/path resolution集中`strategy_compare_sources.py`；Audit finite normalization集中`tools/audit/primitives.py`；active/rolling parameter JSON/date parsing重用`core/active_param_ensemble.py`；continuous ranker source-data-end集中`continuous_ranker_data.py`；daily/event ranker execution plan集中`continuous_ranker_pipeline.py`；optimizer local-min dependency empty stats與seed-ensemble policy payload各收斂單一owner；PIT JSON-native serialization集中`pit_primitives.py`。critical-helper meta contract同步擴充canonical owners，防止複製實作復活。
- **Dead config API**：移除零consumer的`config/training_policy.py::build_optimizer_train_test_policy_snapshot`與`config/strategy_compare.py::get_strategy_comparison_catalog_counts`；未發現其他高信心current設定死API。
...
## 2026-08-12 — Forward robustness portfolio Audit 第二次S1 gate：確認為 attribution basis 混淆；修正 read-only Audit 語意

- 最新程式基準：`test-branch-1_20260812_203815_e6f8237.zip`；SHA256 `f7ac1d5c9ea2165098859815bb56b5e23c9a0dcb93425284a6f0befb3d7cf860`。既有Forward robustness scientific fingerprint=`2f70dbe73dcf3fed`、seed_count=8、generator_seed=20260810與16/16 VERIFIED compact attribution source不變；本輪不重訓、不replay、不改Dataset／Target／architecture／loss／selector／策略accounting。
- 使用者在canonical closed-trade match-key修正後直接重跑`AUD-forward-robustness-portfolio-translation`，S1仍被「compact attribution exclusive selection R與robustness aggregate同seedΔDL選擇R不一致」gate擋下。這證明前一輪match-key分叉確實是需修的SSOT問題，但不是此equality gate持續失敗的唯一原因。
- 重新追溯兩邊正式公式後確認comparison basis不同：robustness `direct_selection_r`是每個DL arm各自相對同param／同rule-policy DL-off baseline的`exclusive_selection_delta_r`；報表同seed`ΔDL選擇R`再做MR-13A − MR-12B。Portfolio Audit的`exclusive_selection_delta_r`則是直接把MR-13A與MR-12B兩個active trade sets做common/exclusive partition。因common-trade R差、與baseline的partition邊界等皆可不同，`f(13A,baseline)-f(12B,baseline)`不等於`f(13A,12B)`，原equality gate在數學上無成立契約。
- 修正：保留robustness `delta_direct_selection_r`作「baseline-relative ranking edge」；Audit直接pair attribution改以`direct_pair_exclusive_delta_r`命名，並新增`direct_pair_common_delta_r`、`direct_pair_all_trade_delta_r`與`selection_basis_gap_r = direct_pair_exclusive_delta_r - delta_direct_selection_r`作診斷。C15 canonical pair summary同步保存common/all-trade ΔR；移除跨basis equality gate，但不放寬compact unit VERIFIED／SHA／all-seed／read-only等任何source integrity gate。Audit result schema `1 → 2`只反映輸出欄位語意修正，不影響scientific fingerprint。
...
## 2026-08-12 — Forward robustness portfolio Audit schema v3：R→Dollar bridge確認為seed-specific risk allocation heterogeneity

- 最新程式基準：`test-branch-1_20260812_211440_34a43ba.zip`；SHA256 `4602f49a7546f307ed3b4150c6c2011fedb498f27d60118300f75a0ede8b7c5f`。使用者重跑既有`AUD-forward-robustness-portfolio-translation` schema v3；scientific fingerprint=`2f70dbe73dcf3fed`、seed_count=8、generator_seed=`20260810`、16/16 VERIFIED compact source不變；Audit只讀既有工件，不train、不replay。
- 既有aggregate維持：Ranking↑=`6/8`、Ranking↑且RoMD↑=`3/6`、Direct-pair Exclusive ΔR Mean=`+50.64R`、Exclusive ΔPnL Mean=`+173,208.15`、Common risk-size effect Mean=`+18,544.82`、ΔGap slot-days Mean=`-99.00`。
- schema v3新增的exclusive risk bridge顯示：S1 MR-13A-only／MR-12B-only `ΣR=143.33/131.74R`，但`RW-R=0.476/0.547R`、Avg risk=`6,684/6,792`，故Exclusive ΔPnL=`-103,422`；S8 `ΣR=286.63/211.94R`，但`RW-R=0.821/0.900R`、Avg risk=`6,290/7,699`，Exclusive ΔPnL=`-434,639`。反例S6雖`ΣR=187.79/228.80R`，但`RW-R=0.832/0.655R`、Avg risk=`8,152/6,066`，Exclusive ΔPnL=`+520,269`。
- 8-seed aggregate `Exclusive RW-R gap Mean=+0.121R`，因此MR-13A並不存在「平均risk weighting必然較差」的證據；問題是seed-specific R×risk配置異質性。現階段不得直接新增score-weighted sizing、risk multiplier或其他SR規則，否則會把diagnostic observation直接變成OOS-tuned策略。
- 下一步沿用同一`AUD-forward-robustness-portfolio-translation`，report schema v4只做read-only exact decomposition，不新增Audit／SR／MR identity：對每seed的exclusive trades，以兩arm所有可識別initial-risk交易的pooled mean risk作reference，將covered Exclusive ΔPnL精確拆成 `(1) equal-risk selection effect = q_ref × ΔΣR`、`(2) average-risk scale effect`、`(3) within-set R×risk weighting effect`；risk不可由PnL/R識別的交易另列`uncovered ΔPnL`。四項加總必須閉合actual Exclusive ΔPnL，residual fail-fast／報表顯示。
- 判讀順序預先固定：若S1/S8主要負在average-risk scale，才研究capital/risk-budget路徑；若主要負在within-set weighting，才研究為何canonical ATR/cash sizing與realized R呈不利配置；若equal-risk selection本身轉負，則回到trade-set economics。schema v4仍不得作promotion gate或回流training semantics，且不需重跑robustness。


## 2026-08-12 — Forward robustness portfolio Audit schema v4：within-set risk weighting為主要edge稀釋來源；實作entry-execution binding attribution

- 最新程式基準：`test-branch-1_20260812_213111_826092c.zip`；SHA256 `e17d9e35648a67bfcb15bbfe417dd0302ab92c564d928f8a6c5568dd80719798`。使用者重跑既有`AUD-forward-robustness-portfolio-translation` schema v4；scientific fingerprint=`2f70dbe73dcf3fed`、seed_count=8、generator_seed=`20260810`與原16/16 scientific observations不變；Audit只讀既有compact source，不train、不replay。
- schema v4 exact decomposition完全閉合：8-seed Exclusive ΔPnL Mean=`+173,208.15` = equal-risk selection effect Mean=`+355,214.76` + average-risk scale effect Mean=`+14,995.80` + within-set R×risk weighting effect Mean=`-197,002.42` + uncovered=`0`；residual Mean=`0.000000`。因此MR-13A的equal-risk trade-set edge在平均上明確為正，但約55%的selection effect被within-set risk weighting吃掉，主未解問題已從模型ranking移到實際risk-dollar配置。
- seed-level機制：S1 equal-risk selection=`+78,069`、scale=`-14,774`、within-set=`-166,717`，actual Exclusive ΔPnL=`-103,422`；S8=`+523,425/-352,399/-605,665`→`-434,639`；S3雖selection=`+1,528,579`且最終PnL=`+733,325`，within-set仍=`-1,012,222`；S6 selection=`-289,839`，卻由scale=`+432,811`與within-set=`+377,298`反轉為`+520,269`。這些結果不支持直接調高／調低全域fixed risk，也不支持把Forward diagnostic直接變成新的SR規則。
- 下一步固定為同一Audit的Risk Utilization / Binding Attribution，不新增MR／SR／Audit ID。為取得正式entry execution evidence，`core/portfolio_entries.py`只在呼叫者提供sidecar時記錄每筆entry execution的candidate/chosen/filled qty、sizing capital、effective entry budget、available cash、max_qty、limit/stop、實際initial risk與execution order；不改候選排序、qty決策、成交、帳務或正式runtime semantics。
- `strategy_compare_diagnostics._flatten_entry_execution_rows()`只用canonical `calc_position_size()`做counterfactual：原始risk＋position cap、移除risk cap、移除position cap、同時移除兩者，再依正式MaxQty與board-lot policy標記`RISK_CAP / POSITION_CAP / RISK_POSITION_TIE / CAPITAL / MAX_QTY / LOT_ROUNDING / ENTRY_BUDGET_CASH`；risk budget固定由該筆`candidate_sizing_capital × fixed_risk`，actual utilization使用正式position保存的`initial_risk_total_milli`，不得用Audit另造近似風險公式。
- Multiple-seed compact attribution schema `1 → 2`：永久source在既有`trades/equity/daily_capacity/selected_buys`外增加`execution.csv.gz`；仍不保存full scores、orderable universe、checkpoint或完整replay tree。舊schema-v1 16 units因此視為Audit evidence缺件，必須由同一Forward robustness入口以原fingerprint／原8 seeds重建；原`seed_results.csv` scientific observations仍是truth，rebuild後照既有selected epoch／fold count／策略metrics／年度報酬／可得hash驗證，只有完全一致才標`VERIFIED`。
- Audit result schema `4 → 5`：全seed exclusive closed trades依project-wide canonical actual-trade key join到filled entry execution sidecar，輸出pre-cash candidate risk utilization→actual risk utilization、winner-minus-loser utilization，以及cash／risk cap／position cap／capital／MaxQty／lot binding counts與binding signatures；缺任何exclusive trade execution attribution直接fail-fast。execution sidecar另保留`execution_order`，canonical occurrence identity依正式執行順序而非signal date或報表排序建立。
- Scientific status不變：MR-13A維持model-research edge supported但`NOT_PROMOTED`；MR-12B／DL-CONT12B繼續作current runtime anchor。只有schema v5 binding結果能決定是否存在可泛化的portfolio allocation mechanism；結果前不得直接新增score-weighted sizing、risk multiplier、position-cap調整或其他SR-C變更。
- 使用者操作：套用本批後先由`Research → 策略組合比較 → Forward-OOS Multi-seed robustness → 執行`重建compact schema-v2的16個execution evidence units；完成且16/16 VERIFIED後，才由`Research → Audit／診斷 → 執行目前 Audit 設定`讀取schema v5。這次重建不是新scientific experiment，且不得刪除原`outputs/strategy_compare/robustness/2f70dbe73dcf3fed/`。


## 2026-08-12 — Forward robustness portfolio Audit schema v5結果：未找到可泛化的MR-13A-specific sizing／binding機制

- 最新程式基準：`test-branch-1_20260812_222249_c1cf30b.zip`；SHA256 `b6588961ca026f2e476b19f9cfe40133f9b878f78ff06893308d63796f08c1d8`。使用者以原Forward robustness scientific fingerprint=`2f70dbe73dcf3fed`、seed_count=8、generator_seed=`20260810`重建compact attribution schema v2並執行既有`AUD-forward-robustness-portfolio-translation` schema v5；16/16 scientific observations與既有結果一致，Audit只讀compact source，不train、不replay。
- 既有model/trade-set證據不變：MR-13A vs MR-12B同seedΔDL選擇R=`6/8`勝、Mean=`+50.64R`；RoMD=`4/8`勝、Mean=`+1.76`；Direct-pair Exclusive ΔR Mean=`+50.64R`；Exclusive ΔPnL Mean=`+173,208.15`。schema v4 exact bridge仍為equal-risk selection effect Mean=`+355,214.76`、average-risk scale=`+14,995.80`、within-set weighting=`-197,002.42`、residual=`0`。
- schema v5 entry-execution結果：Actual risk-util gap Mean（MR-13A-only − MR-12B-only）=`-1.41pp`；MR-13A-only Winner−Loser actual utilization Mean=`+0.03pp`，MR-12B-only=`-0.64pp`。Binding totals（13A/12B）：Cash=`851/856`、Position cap=`286/284`、Risk cap=`1360/1383`、Risk/Position tie=`0/0`、Capital=`0/0`、MaxQty=`0/0`、Lot rounding=`1360/1328`。
- Seed-level actual utilization仍異質：S1=`47.8%/45.7%`、S2=`42.1%/47.4%`、S3=`50.5%/47.3%`、S4=`45.8%/47.7%`、S5=`44.7%/48.2%`、S6=`52.0%/43.3%`、S7=`44.1%/51.2%`、S8=`41.8%/49.2%`（13A-only/12B-only）。S1/S8的negative dollar translation沒有對應到跨seed一致的cash/risk/position/lot binding差異；S6則以較高actual utilization伴隨positive portfolio override，證明risk allocation能影響結果，但沒有單一constraint可從8-seed evidence直接泛化成策略規則。
- 判定：`AUD-forward-robustness-portfolio-translation = RESULT_AVAILABLE / NO_GENERALIZABLE_BINDING_DIFFERENTIAL`。MR-13A的model/selection edge維持supported，但portfolio RoMD robustness仍不一致，因此`MR-13A = NOT_PROMOTED`、`DL-CONT12B / MR-12B = CURRENT_RUNTIME_ANCHOR`。不得從本Audit直接新增score-weighted sizing、risk multiplier、position-cap或cash-allocation SR；否則等同把Forward OOS attribution結果回寫成tuned策略。
- 下一步解除先前「暫不跑Selection PIT multi-seed」限制：使用既有`Selection PIT Multi-seed robustness`正式workflow，固定預設`seed_count=8`、`generator_seed=20260810`，相同scientific condition下比較MR-12B／MR-13A。目的不是修Forward portfolio，而是確認MR-13A的model/selection edge是否跨Selection與Forward兩個時段都具seed robustness。結果前不擴16 seeds、不開MR-14、不新增SR-C35，也不改MR-13A training semantics。


## 2026-08-13 — Selection PIT Multi-seed長跑程式深查：修正partial-fold resume與heartbeat可觀測性

- 程式基準：`test-branch-1_20260812_232724_7b18ee4.zip`；SHA256 `bda014d7b4f4bea846e2008a84704849ef86cb2123903986581d11f21fbe09ba`。使用者執行Selection PIT Multi-seed robustness，固定`seed_count=8`、`generator_seed=20260810`、GPU trainers=2、CPU replay=1；2014-01-01～2020-12-31共7 PIT folds/model/seed、112 fold trainings。執行中MR-13A單seed已觀察約20分鐘，要求確認長時間heartbeat是否代表卡住。
- 深查確認沒有共享fold輸出路徑、file lock、training/replay互鎖或queue deadlock：每個arm/seed使用獨立isolated root，7 folds在各trainer subprocess內串行，兩trainer subprocess只共享同一CUDA裝置，CPU replay固定單worker；目前長時間本身符合MR-13A PIT 7-fold工作量，未發現會使當前run必然卡死的correctness blocker。
- 發現一項resumability缺陷：`point_in_time_scores` canonical builder本身支援fold-level `resume=True`，但multi-seed `_train_one_unit()`在完整seed aggregate尚未形成時會先刪除整個`model_dir`，連已完成的partial fold checkpoint／score一併刪除，導致中斷後無法沿用合法fold，違反失敗保留可接續工件契約。修正後Selection PIT只清除不完整top-level aggregate score／coverage／manifest並保留`folds/`；非PIT trainer維持原本clean-start語意。
- 發現一項可觀測性缺口：parent heartbeat原本只顯示trainer subprocess存活時間，child compact stdout又被導向非TTY `train.log`，PIT fold `InlineProgress.update()`不會寫入log，因此使用者無法從主console知道fold是否持續完成；而training全部結束後若只剩CPU replay，原heartbeat條件也完全停止。修正後parent直接從isolated `folds/*/manifest.json`顯示`PIT saved folds=N/7`，並在training或replay/queue任一仍活躍時每個config interval持續heartbeat；不增加自動kill timeout，避免以未驗證magic duration誤殺合法長fold。
- Scientific condition完全不變：Dataset／Target／architecture／loss／hyperparameter／seed set／PIT fold policy／selector／strategy replay皆未修改；execution/monitoring修正不進scientific fingerprint。當前已啟動的舊process可繼續跑；若中斷，套用本修正後再由同一Selection PIT robustness入口可保留現有partial folds並由canonical fingerprint/hash檢查決定可重用項目。

## 2026-08-13 — Selection PIT Multi-seed實際卡死：MR-13A per-seed replay誤做daily-universal future-target join

- 程式基準：`test-branch-1_20260812_232724_7b18ee4.zip`；SHA256 `bda014d7b4f4bea846e2008a84704849ef86cb2123903986581d11f21fbe09ba`。使用者實跑Selection PIT Multi-seed robustness後，16個training units皆完成，但只有前2個MR-12B units產生`[DONE]`；其後約8小時沒有新的`[DONE]`，GPU/CPU皆閒置。
- Scheduler對照確認真正阻塞點不是trainer：`CPU replay workers=1`下，MR-13A seed1 training完成後被送入唯一replay worker；該replay沒有返回，因此後續MR-12B seed3～8與MR-13A seed2～8雖全部training完成，replay jobs只會累積於queue，最終形成「全部TRAIN DONE但程序不退出」。
- 根因位於`strategy_compare_engine.run_comparison()`的Selection PIT post-replay diagnostic：任何`score_source=selection_point_in_time`都無條件呼叫`_selection_target_lookup()`。對MR-13A `daily_universal_no_time_pairwise`，此helper會重新呼叫`load_daily_universal_ranker_data()`，逐ticker掃描canonical OHLCV並重建整個daily eligible stock-day target universe，再與PIT scores/orderable/selected做future-target join。這是offline future-target diagnostics，不參與runtime排序、策略績效、direct selection R、trade attribution或compact attribution；在multi-seed每個replay重做既浪費大量I/O/RAM，也會把單一CPU replay worker長時間堵死。
- 修正：`run_comparison()`新增execution-only `capture_selection_target_diagnostics=True`，一般Strategy Compare維持原行為；Multiple-seed robustness明確傳`False`，只跳過post-replay future-target diagnostic join，仍完整保存策略metrics、yearly returns、trade/equity/capacity/selected/execution compact evidence與direct selection R。metadata新增`selection_target_diagnostics_captured`供稽核。此開關不進scientific fingerprint。
- 同批整合上一輪已發現的resilience修正：Selection PIT incomplete seed不再刪除合法partial `folds/`，parent heartbeat顯示`PIT saved folds=N/7`，training結束但仍有CPU replay/queue時亦持續heartbeat；replay submit時間也正式記錄，避免顯示0秒假進度。
- 當前卡住的舊process應停止後再套用修正。原`seed_results.csv`已完成的前2 units會由resume排除；其餘已完成training artifacts仍在isolated work dirs時由`reuse_completed`先驗證並顯示`[TRAIN REUSE]`，因此正常情況不需重跑112 fold trainings，只需完成剩餘replay與必要compact attribution。若強制終止前某工件不完整，canonical validator只重建該unit。
- Scientific condition不變：Selection period、8 seeds、generator_seed=20260810、7 folds、Dataset、Target、architecture、loss、epoch selection、selector與portfolio replay semantics皆未改變；本修正只移除multi-seed不需要的post-hoc target diagnostic與改善resume/observability，不構成新MR/SR/Audit實驗。

## 2026-08-13 — Selection PIT Multi-seed replay第二層效能根因：isolated PIT score override每次candidate lookup重讀整份CSV

- 程式基準：`test-branch-1_20260813_085538_7794659.zip`；SHA256 `f10206e125b6e1eb1ddeafff04366ea0a878365878ce708cd335cd338d941cf0`。使用者套用前一批「停用multi-seed post-replay future-target diagnostic＋resume/heartbeat」修正後重新執行Selection PIT Multi-seed robustness；16個training artifacts均成功REUSE，但第一個`MR-13A seed1` CPU replay仍超過27分鐘，`replay queued=13`、GPU=0、CPU約單核心負載，證明仍有replay hot path未排除。
- 深查確認真正per-candidate hot loop位於`filters/breakout_quality/ranking_score_store.py`：multi-seed isolated Selection PIT透過`score_path_override`查分時，`lookup_selection_point_in_time_candidate_score()`每次候選查詢都呼叫`load_selection_point_in_time_score_table_from_path()`；該loader原本沒有任何cache，因此每次lookup都重新read CSV、型別轉換、完整schema／唯一性驗證、建立`ticker/date` MultiIndex。canonical非override loader本身有`lru_cache`，Continuous OOS override loader也已有cache，故此為isolated Selection PIT路徑的實作遺漏；MR-13A daily-universal PIT score表特別大，因此症狀遠比MR-12B event PIT嚴重。
- 修正採file-revision-aware cache，不以單純path永久快取：public `load_selection_point_in_time_score_table_from_path()`先取得score／manifest的`mtime_ns + size`，再進入私有LRU cache；相同不可變revision只parse一次，檔案在resume/rebuild中原地改寫時signature變更會自動失效，避免舊DataFrame污染重建結果。cache只保存少量近期revision，避免16個seed大表全部常駐記憶體。
- 新增isolated PIT synthetic regression：同一score/manifest revision重複load必須回傳同一cached DataFrame；原路徑改寫成不同size/value後必須載入新DataFrame且讀到新score。此修正只改score artifact讀取效率，不改PIT score內容、lookup date、ranking、selector、strategy replay、seed、fold或scientific fingerprint。
- 操作判定：目前仍使用舊process的27+分鐘replay應停止；套用本修正後由同一Selection PIT Multi-seed robustness入口重啟。既有2個completed observations與16個training artifacts均保留／REUSE，正常情況只需完成剩餘replay，不重跑112 fold trainings。

## 2026-08-13 — Selection PIT 8-seed robustness完成：MR-13A relative edge跨時段反轉，維持MR-12B runtime anchor

- 最新使用者程式基準：`test-branch-1_20260813_085538_7794659(2).zip`；SHA256 `f10206e125b6e1eb1ddeafff04366ea0a878365878ce708cd335cd338d941cf0`。Selection PIT robustness scientific condition固定為2014-01-01～2020-12-31、`seed_count=8`、`generator_seed=20260810`、MR-12B／MR-13A各7 PIT folds/seed、同一Min feasible-ascent strategy replay。使用者先前完成的16個training artifacts全部成功REUSE；isolated PIT score cache修正後MR-13A replay降至約01:42～01:50，完整16/16 observations在約20分鐘replay階段完成。
- Selection PIT 8-seed aggregate：MR-12B Return/MDD/RoMD=`117.08%/19.85%/5.96`、DL選擇R Mean=`64.08R`；MR-13A=`102.98%/20.51%/5.03`、DL選擇R Mean=`43.32R`。Matched MR-13A−MR-12B：ΔRoMD=`-0.93`、Median=`-0.96`、Std=`1.94`，MR-13A勝`2/8`；ΔDL選擇R=`-20.76R`、Median=`-16.59R`、Std=`35.37R`，MR-13A勝`3/8`。Ranking/RoMD方向一致=`7/8`，Spearman(ΔDL選擇R,ΔRoMD)=`0.381`、Spearman(ΔDL選擇R,ΔReturn)=`0.452`。
- 年度strategy return差異（MR-13A − MR-12B）：2014 `+0.06pp`、2015 `-2.41pp`、2016 `-0.27pp`、2017 `-7.48pp`（0/8 seeds勝）、2018 `+2.28pp`（7/8勝）、2019 `-0.25pp`、2020 `+0.18pp`。目前最明顯負向regime集中於2017，但年度Return本身不足以證明ranking edge翻轉原因，不能直接依2017調training或strategy。
- 與既有Forward 8-seed形成正式cross-period reversal：Forward 2021-01-01～2026-03-02中MR-13A−MR-12B ΔDL選擇R Mean=`+50.64R`、6勝2敗，ΔRoMD Mean=`+1.76`、4勝4敗；Selection PIT則分別=`-20.76R`／3勝5敗與`-0.93`／2勝6敗。故MR-13A的daily-universal relative ranking/selection edge沒有跨Selection與Forward兩時段穩定成立。
- 機制判定也與Forward不同：Forward主要是ranking edge平均為正、但portfolio translation heterogeneous；Selection PIT則ranking/RoMD方向一致7/8，且ranking edge本身平均為負。因此Selection時段不應再優先研究sizing／capital translation；核心問題回到daily-universal vs event-based training universe在不同歷史regime下的relative ranking泛化。
- Scientific status：`MR-13A = RESULT_AVAILABLE / CROSS_PERIOD_MULTI_SEED_EDGE_NOT_STABLE / NOT_PROMOTED`；`DL-CONT12B / MR-12B = CURRENT_RUNTIME_ANCHOR`。既有MR-13A單模型Gate與單seedC28/C29結果保留為有效歷史觀察，但不得再外推為跨seed／跨period superiority。不得挑best seed、不得以單一年度或Forward Audit直接調loss／Target／architecture／hyperparameter，也不新增sizing SR。
- 下一步先使用兩邊既有robustness permanent artifacts做read-only cross-period／year-regime attribution：以同一8 generated seeds比較每period的matched ΔDL選擇R、ΔReturn、ΔMDD、ΔRoMD，並由compact attribution source拆出年度／regime trade-set realized-R差異；目的只定位edge flip是否集中於特定年份／市場狀態。只有在可重現的regime mechanism被確認後，才評估是否建立新的`MR-*` training-semantics實驗；目前不直接開MR-14。

## 2026-08-13 — Cross-period / Year-regime read-only attribution 實作完成，等待正式 Audit run

- 程式基準：`test-branch-1_20260813_121915_24da0a8.zip`；SHA256 `04618f5678d1b4397739dd6ea882dfbf7aeabe064ba074d51da2926d0bb5f8d2`。Selection PIT 8-seed robustness已16/16 compact attribution ready且可1秒全REUSE；既有Forward robustness亦有完整8-seed compact attribution，因此下一步不再train或replay。
- 新增正式Audit identity：`AUD-cross-period-year-regime-attribution / cross-period-year-regime-attribution`，source固定為已完成`selection_pit`與`forward_oos` multi-seed robustness；兩period必須使用完全相同`resolved_seeds`、`seed_count`與`seed_generator_seed`，任一不一致即BLOCKED。
- Audit-side multi-seed robustness run resolution、seed coverage、compact schema/fingerprint/VERIFIED狀態與trades/equity/daily-capacity/selected/execution逐檔SHA驗證集中到`tools/audit/sources/multi_seed_robustness.py`。既有Forward portfolio-translation Audit同步改用此public resolver，避免Selection／Forward／Cross-period各維護第二套artifact validation。
- Attribution basis刻意分層：period aggregate的`ΔDL選擇R`沿用`seed_results.csv`中兩arm各自相對共同DL-off baseline的direct-selection R再相減；逐年度永久工件目前沒有baseline-relative selection-R，因此year bucket只使用same-seed annual return與MR-13A vs MR-12B direct-pair exclusive trade R/PnL、candidate-only/comparator-only R與trade counts。兩種basis明確分欄，禁止強制相等或把direct-pair年度R宣稱成逐年`ΔDL選擇R`。
- 報表輸出同seed跨period ranking/RoMD方向、逐年度direct-pair trade-set attribution、period aggregate與classification；若Selection baseline-relative ΔDL選擇R Mean<0且Forward>0，標記`RANKING_EDGE_DIRECTION_REVERSAL`。年度worst/best只作read-only定位，不建立year/regime runtime gate、不挑best seed、不回流Target/loss/architecture/hyperparameter。
- Formal Audit framework direct synthetic新增cross-period contract：固定Selection ranking edge為負、Forward為正，驗證`RANKING_EDGE_DIRECTION_REVERSAL`、2017 worst-year識別與yearly basis note，且Forward/Cross-period Audit必須引用同一public robustness source resolver／compact adapter；目前Audit framework共20 checks、0 fail。
- 狀態：`IMPLEMENTED / READ_ONLY / ALL_SAME_SEEDS_REQUIRED / AWAITING_AUDIT_RUN`。下一步由`Research → Audit／診斷`執行目前Audit設定；結果前`MR-13A`維持`NOT_PROMOTED`、`DL-CONT12B / MR-12B`維持current runtime anchor。

## 2026-08-13 — Cross-period / Year-regime Audit完成：Selection負edge廣泛分布，下一步改測MR-12B daily-inference bridge

- 程式基準：`test-branch-1_20260813_123854_7984fda.zip`；SHA256 `1cf5e4eb06b08833b0d578a51ba7dfb4bcada9b99c1de49e080331d78ae4e4bf`。正式`AUD-cross-period-year-regime-attribution`使用Selection fingerprint=`bc496e102dd5348e`、Forward fingerprint=`2f70dbe73dcf3fed`、同一8 generated seeds與`seed_generator_seed=20260810`，全程read-only，不train／score／replay。
- Period aggregate再次確認relative ranking edge方向反轉：Selection PIT MR-13A−MR-12B `ΔDL選擇R Mean=-20.76R`、`ΔRoMD Mean=-0.93`；Forward-OOS為`+50.64R`與`+1.76`。Classification=`RANKING_EDGE_DIRECTION_REVERSAL`。
- 逐年度direct-pair trade-set證據排除「單一2017 regime造成全部失敗」：Selection 2014～2020 Exclusive ΔR符號為5負2正；2014=`-13.96R`、2015=`+1.72R`、2016=`-1.53R`、2017=`-7.71R`、2018=`+10.51R`、2019=`-1.85R`、2020=`-7.93R`。Worst year改為2014，且只占Selection負年度R的`42.3%`；Forward 2021～2026則為5正1負。故Selection弱勢是廣泛period-level instability，不應建立year gate或只針對2017調模型。
- 程式語意核對：MR-12B與MR-13A同用`ARCH-inception_time_v1`、300×10 stock+0050 normalized OHLCV、40-bar adverse-first no-time opportunity公式、within-day RankNet pairwise logistic loss與`mean_daily_spearman` epoch selection；`continuous_target.py`明確定義daily target future-path mechanics與MR-12 no-time target一致。MR-13A相對MR-12B的核心scientific變更是training sample universe由`breakout_event_groups`擴為`daily_eligible_stock_days`，而策略端同時取得daily score refresh。
- 因此下一個最乾淨的causal test不直接建立新`MR-*`：先用**既有MR-12B權重做daily-universal inference bridge**，令training semantics完全維持MR-12B，只把score universe／refresh timing改成與MR-13A相同的feature-eligible daily stock-days。這屬新的`DL-*` score source／`SR-C*` runtime usage研究，而不是新模型。
- 預定判讀：若MR-12B daily-inference bridge在Selection與Forward都維持MR-12B的relative robustness，則daily scoring本身可行，MR-13A問題主要來自universal training-pair dilution；若bridge本身也顯著惡化，才表示event-trained模型對off-event daily inference不具穩定可移植性，之後才值得建立新的candidate-aware daily training `MR-*`。在bridge前不改Target／loss／architecture／hyperparameter，不新增year/regime gate或sizing SR。


## 2026-08-13 — MR-13B前多DL架構盤點：Continuous ranker改為profile-driven，Daily Score主線不退回event-only

- 使用者研究方向確認：長期DL介面維持「每日對所有feature-eligible股票產生strategy-agnostic Daily Score」，目標是讓breakout與未來非-breakout策略共用同一daily model layer；MR-13A跨期不穩不代表Daily Score架構退役。既有`MR-12B / DL-CONT12B`只維持目前runtime anchor，不改變Daily model research主線。
- 架構盤點發現目前可同時註冊多個`StrategyDLSource`與多個MR/profile artifact，output path亦由architecture＋experiment profile隔離，因此「多個DL研究版本並存／A-B比較」本來就可行；但`StrategyComparisonArm`仍只有一個`dl_id`、runtime candidate也只有一組ranking score/rank。這個限制目前刻意保留：同一策略同時融合多個DL需要先定義score role、fusion／gating與promotion semantics，屬新的strategy scientific variable，不應在MR-13B前預先加入。
- 真正需要在MR-13B前修正的是trainer ownership：`services/breakout_quality/train_daily_ranker.py`原本把report／phase／runtime reason直接寫死`MR-13A`，`train_continuous_ranker._profile_contract()`也以profile name長串`if`綁MR identity。若直接新增13B/13C，會自然複製MR-specific trainer與loss分支，增加維護與語意漂移風險。
- 新增`ContinuousRankerResearchSpec` SSOT，依experiment profile集中保存`model_research_id / experiment_name / phase / trainer_family / target_description / objective_description / metric_scope / score_semantic_id / pairwise_reduction`。Research metadata刻意與`BreakoutQualityExperimentProfile.as_manifest_payload()`分離，避免僅為治理欄位而改寫既有profile artifact scientific identity。
- `ranker_cli.py`改依research spec的`trainer_family`分派event／daily orchestrator；`train_daily_ranker.py`不再含MR-13A/profile硬編，target identity與report identity皆由active profile/spec解析。`train_continuous_ranker.py`的CLI supported profiles與profile contract同樣改由spec registry驅動；新增continuous profile若漏登research spec，formal CLI不會默默接受。
- Pairwise ranking aggregation也改為profile-owned reduction SSOT。既有MR-12B與MR-13A都明確保持`equal_pair_weight`，其loss數值與修改前相同；基礎API同時支援`pairwise_reduction`擴充點；目前所有既有pairwise profile仍只允許`equal_pair_weight`。新的equal-date或其他reduction要等正式建立新MR/profile時再實作其完整batch／gradient contract，因此本批不是MR-13B scientific run，也不改現有模型結果。
- Synthetic contract新增：所有continuous profiles必須一一有research spec；Daily trainer與dispatcher不得重新硬編`MR-13A`／`daily_universal_no_time_pairwise`；MR-12B現行pairwise artifact contract保持equal-pair；並釘死既有pairwise profiles不得在沒有新MR identity下改變reduction。既有continuous profile manifest payload與MR-12B／MR-13A training semantics須逐一與修改前完全一致。
- 下一步：先完成本架構整理的本機formal double check；之後若開始MR-13B，只新增新的Daily experiment profile＋research spec並一次只改一個training semantic。MR-12B daily-inference bridge降為可選causal control，不再作Daily model主研究路線的前置Gate。

## 2026-08-13 — Multi-DL architecture 首輪 formal：兩個舊 source-string synthetic 漂移，改回契約式驗證

- 使用者本機 `apps/run_bundle.py` 首輪正式 double check：quick gate PASS、chain checks PASS、ml smoke PASS；consistency FAIL 2 checks，meta quality只因 coverage synthetic suite沿用同一2個FAIL而失敗。正式 bundle=`to_chatgpt_bundle_20260813_155049_1f782576.zip`；兩個FAIL均位於舊 continuous-ranker synthetic：`pass_conditional_ranker_is_cli_only_and_uses_existing_command`與`pass_conditional_ranker_reuses_existing_cli_and_is_not_in_menu`。
- 根因不是CLI/profile功能消失。B201架構整理已把continuous profile support與trainer ownership從`train_continuous_ranker.py`字串分支移到`config/breakout_quality.py::ContinuousRankerResearchSpec`；`ranker_cli.py`仍是既有`train-continuous-ranker` command owner，`parse_args()`仍以`SUPPORTED_CONTINUOUS_RANKER_RESEARCH_PROFILES`作`--experiment-profile` choices，MR-11G PASS-conditional profile仍登記為event trainer且未進互動menu。
- 舊B177/T274 synthetic卻以`"STRATEGY_ALIGNED_NO_TIME_PASS_MAGNITUDE_MSE_PROFILE" in ranker_source`及`"choices=(" in ranker_source`判定CLI可用性，因此在SSOT重構後形成implementation-shape假性FAIL。修正後改直接驗證：PASS profile存在於`SUPPORTED_CONTINUOUS_RANKER_RESEARCH_PROFILES`、其`ContinuousRankerResearchSpec.trainer_family == event`、`COMMAND_MODULES["train-continuous-ranker"] == services.breakout_quality.ranker_cli`、互動menu仍無MR-11G/profile硬編。
...
## 2026-08-13 — MR-13C Forward Model Gate：Daily Spearman改善、Top-tail集中度下降，授權Selection PIT跨期驗證

- 本機模型結果來源：`test-branch-1_20260813_181447_e5e7812.zip`；SHA256 `c67d92341eedc0f4e98eca8dc97191df3fb63c33fb5b85c5f4fdcb3dfb063ec1`。本次Forward結果屬迭代研究OOS證據，不宣稱untouched holdout。
- Profile=`daily_universal_no_time_percentile_mse`；MR=`MR-13C`；sample scope=`daily_eligible_stock_days`；Target=`daily_opportunity_no_time_r_v1`；Architecture=`inception_time_v1`；Seed=`42`。相較MR-13A唯一scientific change仍是training objective由same-date RankNet pairwise logistic改為直接回歸same-date target percentile的MSE。
- Epoch選擇：epoch1 Validation daily rho=`0.0492`為最佳；epoch2=`-0.0341`，故selected epoch=`1`。完整Selection refit固定1 epoch。重訓後原Validation daily rho=`0.2687`因原Validation rows已進final refit，只作描述，不是獨立泛化證據。
- Forward all-stock OOS：groups=`608,204`；daily rho=`0.1877`、global rho=`0.1239`、pair=`56.38%`；Top10% Target=`1.2072R`、Bottom10%=`0.5186R`，spread=`+0.6886R`；NDCG@10=`0.5569`、Top-K Target=`1.3888R`、Lift=`+0.3870R`、Oracle overlap=`3.60%`、Boundary=`48.28%`、Boundary gap=`-0.0746R`。
- Breakout-candidate OOS：groups=`17,346`；daily rho=`0.1565`、global rho=`0.1509`、pair=`56.69%`；Top10% Target=`1.8536R`、Bottom10%=`0.4386R`，spread=`+1.4150R`；NDCG@10=`0.7067`、Top-K Target=`1.5877R`、Lift=`+0.3735R`、Oracle overlap=`59.95%`、Boundary=`52.69%`、Boundary gap=`+0.2172R`。
- 與MR-13A同口徑Forward比較：all-stock daily rho `+0.0122`、pair `+0.26pp`，但global rho `-0.0522`、Top-bottom spread `-0.6529R`、Top-K Lift `-0.6487R`、Boundary gap `-0.1781R`；breakout daily rho `+0.0075`，但global rho `-0.0385`、pair `-0.22pp`、Top-bottom spread `-0.3902R`、Top-K Lift `-0.0321R`，Boundary gap則`+0.0014R`近乎持平。
- 判定：`MR-13C = RESULT_AVAILABLE / FORWARD_OOS_MODEL_GATE_PASS / SELECTION_PIT_MODEL_GATE_AUTHORIZED / NO_RUNTIME_DL_SOURCE`。這不是MR-13C全面勝過MR-13A；較準確結論是MSE讓平均daily ordering略改善、但top-tail concentration明顯變弱。由於MR-13A真正的主要缺陷是Selection 2014-2020與Forward 2021-2026跨期方向反轉，MR-13C現在最有資訊價值的下一步是**Selection PIT模型Gate**，確認percentile regression是否改善跨期穩定性。
- 下一步只執行`Research → 模型訓練 → 建立／更新 Selection PIT Scores → PIT模型驗證`；不建立`DL-CONT13C`、策略arm或multi-seed robustness。若Selection PIT仍出現廣泛弱排序，MR-13C停止於model layer；若PIT跨期明顯改善，再討論runtime source與後續8-seed robustness。


## 2026-08-13 — MR-13D實作：Daily Universal Upper-tail Relevance-weighted Pairwise Ranker

- 程式基準：`test-branch-1_20260813_181447_e5e7812.zip`；SHA256 `c67d92341eedc0f4e98eca8dc97191df3fb63c33fb5b85c5f4fdcb3dfb063ec1`，並先同步MR-13C Forward Model Gate結果。MR-13C保有正向Forward排序訊號但Top-tail concentration低於MR-13A，因此已授權Selection PIT；使用者希望先設計另一個不含magic number的upper-tail候選，再讓13C/13D一起進PIT。
- `MR-13D / PROFILE-daily_universal_no_time_upper_tail_pairwise`固定MR-13A的sample universe=`daily_eligible_stock_days`、Target=`daily_opportunity_no_time_r_v1`、300×10 stock+0050 input、`ARCH-inception_time_v1`、optimizer/LR/weight decay、inner validation、selected-epoch final refit與`mean_daily_spearman` epoch selection。
- 唯一scientific change：同日non-tied RankNet pair的logistic loss由MR-13A等權改成pair relevance權重 `w_ij = (percentile_i + percentile_j) / 2`。所有contributing pairs仍依MR-13A原本pair aggregation作加權平均；因此沒有順帶切換成MR-13B的equal-date reduction。
- 此公式只使用canonical 0～1 daily target percentile；`/2`為算術平均定義，不是可調hyperparameter。沒有`lambda`、Top-K=10、boundary=3、percentile threshold、exponent、gap floor、breakout candidate membership或strategy feature。高-high pair自然比低-low pair取得較高loss relevance，目的是研究strategy-agnostic upper-tail local discrimination，而不是把目前portfolio K寫入model training。
...
## 2026-08-13 — MR-13C／13D／13E Selection PIT Model Gate結果：MR-13E取得跨期單seed模型層優先候選

- 結果程式基準：`test-branch-1_20260813_202833_b72d444.zip`；SHA256 `6891174c512f7579ebf25cf4c01992f10e6c5bc560ce1a1446e4e00d0cbc3676`。三模型使用完全相同的filter=`breakout_quality_v1`、architecture=`inception_time_v1`、Target=`daily_opportunity_no_time_r_v1`、sample scope=`daily_eligible_stock_days`、Seed=`42`、PIT period=`2013-04-01～2020-12-31`、fold=`12` months、inner validation=`24` months；各自獨立訓練與PIT工件。
- Coverage：三者皆`778,532/778,532 = 100%`、8 folds。MR-13C PIT all-stock daily/global rho=`0.0841/0.0463`、pair=`53.15%`、Top-K Lift=`+0.1550R`、Boundary gap=`+0.0858R`；年度rho/spread=`7/8, 7/8`正；breakout daily rho=`0.0773`、Top-K Lift=`+0.1139R`。Gate PASS。
- MR-13D PIT all-stock daily/global rho=`0.0842/0.0630`、pair=`52.85%`、Top-K Lift=`+0.1776R`、Boundary gap=`-0.0093R`；年度rho/spread=`7/8, 6/8`正；breakout daily rho=`0.0233`、Top-K Lift=`+0.0657R`。Gate PASS。
- MR-13E PIT all-stock daily/global rho=`0.1627/0.0533`、pair=`55.75%`、Top-bottom spread=`+0.2813R`、Top-K Lift=`+0.4456R`、Boundary gap=`-0.0043R`；年度rho/spread皆`8/8`正。Breakout global/daily rho=`0.0833/0.1019`、pair=`54.77%`、Top-K Lift=`+0.1513R`。Gate PASS。
- Shared PIT比較中，13E的all-stock Daily rho比13C／13D高約`+0.0786/+0.0785`，Pair高`+2.60pp/+2.90pp`，Top-K Lift高`+0.2906R/+0.2680R`；breakout Daily rho也最高。13D的all-stock global rho略高，13C的Boundary gap較佳，但兩者沒有抵銷13E在主要daily ranking、pair、top-tail與breakout slice的整體優勢。
...
## 2026-08-13 — MR-13E single-seed strategy Gate結果與orderable／feasible alignment Audit

- Audit實作程式基準：`test-branch-1_20260813_230500_c0bbfde.zip`；SHA256 `95cfe895c1d57f61716bd05f02b7b4774e6585fb562703bee52f85b2fa56862b`。本輪只新增read-only Audit／source resolver／synthetic contract與結果文件同步，不改trainer、score source、selector、portfolio engine、sizing或execution。
- 策略Gate結果來自同一source-only controlled compare：Selection PIT固定historical Min params／all-off／feasible-ascent，Forward固定current Min params／all-off／feasible-ascent；MR-12B、MR-13A、MR-13E唯一差異為DL score source。`MR-12B / DL-CONT12B`仍為CURRENT_RUNTIME_ANCHOR。
- Selection 2014-01-01～2020-12-31：C25 MR-12B Return/MDD/RoMD/EV/DL選擇R=`101.55%/19.66%/5.17/0.52R/+26.19R`；C28 MR-13A=`151.88%/18.86%/8.05/0.71R/+109.91R`；C35 MR-13E=`153.96%/19.88%/7.74/0.54R/+35.96R`。13E相對12B Return=`+52.41pp`、RoMD=`+2.58`、DL選擇R=`+9.77R`，Selection Gate PASS；但相對13A Return僅`+2.08pp`，RoMD=`-0.31`、EV=`-0.17R`、DL選擇R=`-73.96R`，不能宣稱13E全面優於13A。
- Forward 2021-01-01～2026-03-02：C20 MR-12B=`190.26%/18.75%/10.15/1.17R/+35.93R`；C29 MR-13A=`83.81%/15.71%/5.34/0.52R/-183.42R`；C36 MR-13E=`128.77%/17.18%/7.49/0.87R/-66.22R`。13E相對13A Return/RoMD/EV/DL選擇R=`+44.96pp/+2.16/+0.35R/+117.20R`，但相對12B=`-61.49pp/-2.65/-0.30R/-102.16R`，故Forward Gate FAIL vs MR-12B，13E不promotion且不直接加入8-seed robustness。
- Resource-aware diagnostics顯示raw score basket通常不能直接落地：Selection C35 DL days=`274`、direct-score-feasible=`13`、repair=`261`、feasible-ascent改善=`17`；Forward C36 DL days=`242`、direct=`16`、repair=`226`、ascent改善=`21`。12B/13A也使用完全相同selector，因此上述現象本身不證明selector造成13E失敗，但顯示全市場model Top-K不能直接代表實際buy-list competition quality。
- 下一步新增正式read-only `AUD-mr13e-orderable-feasible-alignment`，不train、不score、不replay。Selection讀C25/C28/C35、Forward讀C20/C29/C36完成的Strategy Compare `orderable_candidates / daily_capacity / execution / strategy_comparison`工件；共同Future Target固定用`daily_opportunity_no_time_r_v1`，以trade_date之前最新已完成交易日join，且只在replay完成後加入Audit，Future Target不得進runtime decision。
- Ranking層每天只看**實際orderable competition set**，計算Daily Spearman、pair accuracy、Top-N target lift、oracle retention與top-vs-rest pair accuracy；N直接取當日`Resource_Aware_Pre_Market_Order_Limit`，缺時fallback baseline selected count，不寫固定K=10。分層固定為all／direct-feasible／repair／feasible-ascent。
- Translation層以`score_ranking_execution.csv`的`chosen_qty>0`當實際action candidates；每天用實際action count作N，比較raw score Top-N與actual action set的overlap、Action−Raw target R與Action−Raw score，並同樣拆direct／repair／ascent。既有sidecar未永久保存minimum-repair seed與每一步swap membership，因此Audit只定位raw orderable ranking vs resource/action translation，不宣稱逐swap counterfactual identity。
- 自動classification只用方向比較，不建立人工加權總分：若13E Forward all/repair orderable Top-N lift都低於12B則`ORDERABLE_RANKING_ALIGNMENT_GAP`；若orderable ranking不弱但repair Action−Raw target較差則`RESOURCE_TRANSLATION_GAP`；其他為`MIXED_ORDERABLE_AND_TRANSLATION`。

## 2026-08-14 — MR-13E orderable／feasible Audit來源sidecar契約修正

- 程式基準：`test-branch-1_20260814_004543_94e4928.zip`；SHA256 `f7ab929015a879c81cf61ade9043db18eb8a47e69ed24a07f2ef0175c48b7e15`。使用者執行`AUD-mr13e-orderable-feasible-alignment`狀態頁時，Audit被`selection_pit C25`缺`score_ranking_execution.csv`阻擋。
- 根因是Audit來源契約與既有一般正式Strategy Compare artifact contract不一致：`run_comparison(..., capture_execution_diagnostics=False)`原本只在特定attribution／multi-seed來源明確要求時保存pre-market execution diagnostics，一般C25/C28/C35與C20/C29/C36正式pair不保證存在`score_ranking_execution.csv`。因此這是Audit實作時對舊正式工件能力的錯誤假設，不是C25/C36策略結果失效。
- `score_ranking_selected_buys.csv`不能作為替代來源：它只包含實際fill後買進，會把盤前selector已選中但未成交的action漏掉，造成resource/feasible selection與fill/execution混層；舊`orderable_candidates + daily_capacity`也缺完整pre-market cash／sizing-equity／active-param selector inputs，不能無歧義重建final chosen basket。
- 修正正式score-ranking pair artifact contract：一般Strategy Compare只要`comparison_mode=score_ranking`即開啟`capture_execution_diagnostics=True`，並把`score_ranking_execution.csv`列入pair cache required files。舊完成pair若缺此sidecar，cache resolver只將該DL pair視為需要一次replay refresh；shared baseline、params、continuous models、PIT scores與Forward frozen scores仍可REUSE，且config/scientific fingerprint不因sidecar持久化契約改變。
- Audit仍固定read-only：它不自行replay、不從selected-buys猜action。使用者套用修正後先重新執行Selection PIT與Forward Strategy Compare，使C25/C28/C35及C20/C29/C36補齊canonical pre-market execution sidecar；之後Audit狀態應轉READY並直接消費這些正式工件。
- Direct contract新增`formal_score_ranking_pairs_persist_pre_market_execution_sidecar_and_cache_requires_it`，同時驗證normal orchestration會capture execution、engine會寫sidecar、cache reuse會要求sidecar；orderable/feasible Audit synthetic仍驗證dynamic N與actual chosen action semantics。
- 本輪不改模型訓練、score、selector、sizing、portfolio execution或任何既有Strategy Compare結果，只補正式artifact observability／cache completeness contract。

## 2026-08-14 — MR-13E orderable／feasible alignment結果：Forward raw edge存在，斷點定位為resource translation

- 使用者先依正式Strategy Compare artifact contract refresh C25/C28/C35與C20/C29/C36，使canonical `score_ranking_execution.csv`齊備；`AUD-mr13e-orderable-feasible-alignment`之後由BLOCKED轉READY並完整執行。Audit全程只讀completed Strategy Compare artifacts，不train、不score、不replay；共同Future Target固定`daily_opportunity_no_time_r_v1`，只在replay後依trade_date之前一已完成交易日join。
- Selection PIT orderable set：C25/MR-12B Daily rho/Pair/Top-N Lift=`0.0994/53.41%/+0.5826R`，repair rho/Lift=`0.1182/+0.6423R`；C28/MR-13A=`0.0636/52.07%/+0.2661R`，repair=`0.0679/+0.2899R`；C35/MR-13E=`0.1261/54.39%/+0.2989R`，repair=`0.1246/+0.2710R`。因此13E在Selection的broad ranking勝12B，但真正最前端Top-N與repair-day lift仍低於12B，Selection仍有top-tail cross-period stability問題。
- Forward-OOS orderable set：C20/MR-12B Daily rho/Pair/Top-N Lift=`0.1586/55.52%/+0.8223R`，repair rho/Lift=`0.1596/+0.7975R`；C29/MR-13A=`0.1351/54.83%/+1.0243R`，repair=`0.1639/+1.0734R`；C36/MR-13E=`0.1698/55.93%/+1.0996R`，repair=`0.1699/+1.1401R`。13E相對12B Forward Top-N Lift=`+0.2773R`、repair-day Lift=`+0.3426R`，因此「13E只會排全市場、進真正breakout/orderable pool就失效」假說被否定。
- 真正Forward斷點出現在raw→action translation：Raw→Action overlap C20/C29/C36=`0.1742/0.1493/0.1309`；Action−Raw Target=`-0.8091R/-1.2096R/-1.1685R`；repair-day Action−Raw=`-0.9459R/-1.3807R/-1.3132R`。13E相對12B在repair-day多損失`-0.3673R`，即使raw orderable ranking更強，轉成實際entry action時保留的Future Target更少。
- 正式classification=`RESOURCE_TRANSLATION_GAP`。但既有工件只保存raw orderable、aggregate repair/ascent diagnostics與final execution action，沒有永久保存`minimum-repair seed`與每一步accepted swap membership，因此不能把整段translation gap直接歸罪於feasible-ascent；目前只可定位為raw ranking到resource/action basket之間的缺口。
- Research policy維持：MR-13E不promotion、不進8-seed、不開始13F；下一步先精確拆selector transformation stage。只有定位minimum-repair／ascent／action／fill何者造成13E相對12B額外損失後，才討論是否需要改selector或model training semantics。

## 2026-08-14 — MR-13E selector-stage translation trace：Raw→Repair→Ascent→Action→Fill只增診斷、不改決策

- 新增正式read-only `AUD-mr13e-selector-stage-translation`。目的只回答前一Audit留下的未識別段：13E相對12B Forward repair-day額外`-0.3673R` translation loss究竟主要發生在`Raw Top-N → minimum-repair seed`、`repair → feasible-ascent final`、`ascent → entry action`或`action → actual fill`。
- Max-DL selector在既有diagnostics中新增transient `_selector_trace_baskets`：`raw_top_n`直接保存原本score Top-N，`minimum_repair_seed`保存既有minimum-repair合法basket，`feasible_ascent_final`保存既有one-swap local optimum final basket。Trace只引用當次in-memory candidate object，只有顯式replay trace sink存在時才由portfolio engine透過既有candidate replay snapshot序列化；沒有trace sink時原runtime執行路徑與回傳action完全不變。
- Strategy Compare score-ranking正式pair新增canonical `score_ranking_selector_trace.csv`，欄位只保存stage／rank／ticker／trade_date／signal_date／score及當日pre-market order limit、direct-feasible、repair/ascent step counts，不保存Future Target。Pair cache將selector trace列入正式required artifact；因此舊C25/C28/C35與C20/C29/C36若缺trace，下一次Strategy Compare只需各自replay refresh一次，models、PIT/OOS scores、params與scientific fingerprint維持REUSE／不變。
- 新Audit只讀`score_ranking_orderable_candidates.csv + score_ranking_selector_trace.csv + score_ranking_execution.csv + daily_capacity + strategy_comparison.json`。`raw_top_n / minimum_repair_seed / feasible_ascent_final`來自trace，`entry_action`定義為execution sidecar `chosen_qty>0`，`actual_fill`只以`filled_qty>0`定義，避免CSV字串`"False"`經`astype(bool)`被誤判為True。
...
## 2026-08-14 — Minimum-repair global exact oracle scale incident：撤回全K-combination搜尋，改production single-swap certificate

- 使用者在`test-branch-1_20260814_165400_e6449d8.zip`執行正式整合流程：`apps/run_bundle.py`完整PASS（quick gate／consistency／chain checks／ml smoke／meta quality全PASS），之後Selection PIT Strategy Compare進入`score_ranking` replay並長時間停在2018-02-02附近。這不是formal failure，也不是模型重訓；卡點來自上一輪research-only minimum-repair global exact oracle。
- 根因是oracle以所有orderable candidates選K的組合做無state-cap best-first exact search；真實候選常為數十檔且K接近10，狀態空間為`C(n,K)`，例如`n=60,K=10`約`7.5e10`個basket。Best-first雖可early stop，但repair-day feasibility若位於較後順位，仍可展開不可接受數量的states。這個設計違反既有架構「C17/C18不得在真實replay做指數級全子集合窮舉」的工程原則，因此撤回；不以node cap、timeout、beam width等近似冒充exact。
- `AUD-mr13e-minimum-repair-mechanism` identity保留，但實作改為scalable production search certificate。Production minimum-repair本來就對每個accepted step完整枚舉所有`current out × candidate in` single swaps，以canonical cash-capped reservation重算並在deficit-improving trials中取最高frozen-score move；diagnostic現在只記錄每step的`evaluated_swap_count / progress_swap_count / feasible_swap_count`、accepted out/in、before/after count/reserve deficit及score，不再另跑全K-combination oracle。
- 可證明範圍：raw Top-N已知不合法且`repair_steps=1`時，第一步已完整枚舉所有single swaps，任何feasible one-swap的deficit必為`(0,0)`且會納入候選，因此production選到的是**minimum replacement distance=1下最高frozen-score feasible basket**，分類`EXACT_ONE_SWAP_RESOURCE_CONSTRAINT`。若`repair_steps>1`，每一步仍是當下exhaustive best single-swap progress，但不同多步路徑／global multi-swap optimum不再宣稱已解，分類`MULTI_STEP_GREEDY_PATH_UNRESOLVED`；若greedy repair最終回退baseline則`BASELINE_FALLBACK_AFTER_GREEDY_REPAIR`。Feasible-ascent仍只宣稱1-swap local optimum。
- Sidecar改名為`score_ranking_repair_search_certificate.csv`以避免舊exact-oracle schema被cache誤當相容；completed pair缺新sidecar時只需replay refresh，models、PIT/Forward scores、params與strategy scientific fingerprint不變。
- Read-only Audit以三類repair day的Raw→Repair post-replay Future Target貢獻拆解13E相對12B額外損失；若extra loss主要落在`EXACT_ONE_SWAP_RESOURCE_CONSTRAINT`，可支持1-step resource incompatibility為主要來源；若主要落在`MULTI_STEP_GREEDY_PATH_UNRESOLVED`，則只能判定multi-step greedy path仍是下一個需研究區段，不能把結果誤寫成global search deficiency已證明。
- 本輪不改MR-13E model、K/R0、production minimum-repair move selection、feasible-ascent、sizing或execution。使用者目前卡住的舊程序可直接Ctrl+C；formal PASS已完成，不需因oracle incident重訓模型。

## 2026-08-14 — Legacy Audit／臨時研究程式瘦身與自動維護規則

- 程式基準：`test-branch-1_20260814_174306_6978779.zip`；SHA256 `b514130c30ba64665e1f45005e5fe38f84099cce951ad2e7a1a5a0929b917106`。本輪不是新增scientific experiment，而是對已完成研究 implementation 做生命周期收斂；既有研究結果、ID與歷史證據不重算、不改判定。
- `config/audit.py`由14個formal Audit定義收斂為只保留目前仍待決策的`AUD-mr13e-minimum-repair-mechanism`；已完成的A9、C15、PIT attribution、Forward/cross-period、orderable alignment與selector-stage Audit implementation，以及11H～11K一次性research diagnostics與只服務它們的helpers／dedicated synthetic contracts退出current tree。共刪除20個Python檔、11,462行；`tools/audit/`由17,337行降至5,721行。
- `tools/validate/synthetic_breakout_quality_audit_cases.py`由4,333行縮至1,289行，只保留目前仍支援的11C～11F/PIT utility與active minimum-repair formal Audit契約；retired checklist項目由`DONE`轉`N/A`，歷史G紀錄保留，不再要求已結案validator永久存在。
- `doc/PROJECT_SETTINGS.md`新增臨時研究程式disposable lifecycle：研究決策完成、Registry／Log已有證據且current runtime／active Audit／必要compatibility無依賴後，implementation、config/catalog registration及專屬tests/helpers必須退役；不允許用永久`enabled=False`、historical CLI或coverage-only理由累積。
...
## 2026-08-15 — SR-C39 Selection result → SR-C40 Excess-Alpha No-R0 ablation

- SR-C39正式Selection PIT結果已完成，run=`20260815_014807_C32-C23-C25-C28-C35-C39_1f8085b42d35`。C39 frozen重用MR-13E PIT與PIT isotonic Expected Excess-R calibration，保留C35相同historical Min params、K/R0、sizing、cash、orderability與execution，只把basket objective改為`Σ(Expected Excess-R × canonical planned initial risk)`。
- C39主要結果：Return=`131.44%`、MDD=`20.21%`、RoMD=`6.50`、Annual=`12.74%`、EV=`0.58R`、DL selection R=`+51.02R`；C35為Return=`153.96%`、MDD=`19.88%`、RoMD=`7.74`、EV=`0.54R`、DL selection R=`+35.96R`。C39−C35=`-22.52pp Return / +0.32pp MDD / -1.24 RoMD / +0.03R EV / +15.06R DL selection R`。因此C39不能取代C35，也不得建立Forward arm。
- Resource diagnostics顯示C39仍受Min ROOS resource-preservation強烈影響：C35/C39的DL days=`274/275`、raw-direct-feasible days=`13/7`、minimum-repair days=`261/268`、actual changed-order days=`129/132`、reserved-capital delta=`+533,890/+716,817`。因此C39的失敗不能乾淨區分「Expected Excess-R×Risk objective本身不好」或「objective已含planned risk後又被R0 hard floor + R0 repair二次強調capacity」。
- 新受控實驗=`SR-C40 / Min MR-13E Excess-Alpha No-R0`，程式基準=`test-branch-1_20260815_014353_6a458df.zip`，SHA256=`98cfeec4868933f0334a9cf47e8c3e5336c875dfb2a2fb2eeacdf363e78b675d` + 本輪patch。
- C40與C39固定相同：`DL-CONT13E-PIT`、Expected Excess-R calibration identity、historical Min params、K、`Expected Excess-R × canonical planned initial risk` objective、sizing、actual cash、orderability、execution。唯一controlled change為`preserve_r0=False`、`r0_minimum_repair=False`。
- No-R0精確語意：不是取消真正資金限制。raw objective Top-K若在canonical cash-capped reservation下能保留K則直接接受，即使reserved capital低於baseline R0；若raw Top-K因真正cash feasibility無法保留K，只允許同參數Min ROOS baseline basket作K-only cash-feasible seed，再做Expected Excess-Alpha one-for-one ascent。seed只保證K/actual-cash feasibility，不做R0 restoration；`minimum_repair_steps/evaluations`必須保持0，`resource_preservation_required=False`。
- Decision contrasts：`C40-C39`只回答R0 hard floor + R0-driven repair純效果；`C40-C35`回答移除R0干擾後，Excess-Alpha×Risk objective是否真的優於原始MR-13E score objective。若C40只勝C39但仍輸C35，代表R0確有傷害但rank×capacity objective仍不值得採用；若C40仍未改善則停止此路線。Selection結果支持前不建立Forward arm、不跑multi-seed、不改模型或calibration。

## 2026-08-15 — SR-C40 Selection REJECT → SR-C41 Exact Constrained Excess-Alpha solver

- SR-C40正式Selection PIT結果已完成，run=`20260815_031604_C32-C23-C25-C28-C35-C39-C40_07556294e644`。C40與C39固定同一MR-13E PIT、Expected Excess-R calibration、historical Min params、K、Excess-Alpha×canonical planned-risk objective、sizing/cash/orderability/execution，只移除R0 hard floor與R0-driven minimum repair。
- C40主要結果：Return=`73.68%`、MDD=`18.83%`、RoMD=`3.91`、Annual=`8.21%`、EV=`0.27R`、Exposure=`59.30%`、DL selection R=`-81.67R`。相對C39：Return=`-57.76pp`、MDD=`-1.37pp`、RoMD=`-2.59`、Annual=`-4.53pp`、EV=`-0.31R`、Exposure=`-28.81pp`、DL selection R=`-132.69R`。因此No-R0明確REJECT，不建立Forward arm。
- 同時C40提供關鍵機制證據：R0 repair days由C39=`268`降為`0`、direct feasible days由`7`升為`288`；post-hoc Selected Target percentile=`0.6311`、top-K retention=`0.2148`、Target mean=`0.9951R`、opportunity gap=`3.0435R`，均顯示移除R0/repair後selection quality提高。然而總reserved-capital delta=`-73,969,424`、Exposure只有`59.30%`，說明R0具有必要capital-deployment功能，不能直接刪除。
- 新受控實驗=`SR-C41 / Min MR-13E Excess-Alpha Constrained`，程式基準=`test-branch-1_20260815_031548_4cb2516.zip`，SHA256=`f7ea9d0fe8b6dff5b07c397aeba84dca82729a1aa2474eccceaf5d428d6e36d0` + 本輪patch。
- C41與C39固定相同：MR-13E PIT、Expected Excess-R mapping、historical Min params、K/R0、`Σ(Expected Excess-R × canonical planned initial risk)` objective、canonical sizing/cash/orderability/execution。唯一controlled change為solver：C39的`raw Top-K → deterministic R0 minimum repair → best 1-swap feasible ascent`改成完整候選universe deterministic exact branch-and-bound，直接求K/R0/canonical-cash feasible global optimum。
- Exact solver contract：C39 feasible-ascent只提供incumbent lower bound，不能裁掉任何candidate membership；每個include state由canonical cash-capped reservation重新計算planned risk與reserved capital。Pruning只允許候選數不足、standalone maximum reserve不足、或admissible optimistic Excess-Alpha upper bound已不可能勝incumbent；禁止candidate truncation、beam width、time budget、lambda、R0比例或OOS-tuned tolerance。搜尋完整結束才輸出`constrained_solver_optimality_certified=True`。
- GPT獨立oracle：固定小型case直接暴力枚舉全部K-baskets，C41與oracle完全相同；另以30個randomized 8-candidate/K=3 case逐一完整枚舉，30/30皆與C41一致。另固定一個10-candidate/K=4 multi-swap trap：C39停在`MS2/MS5/MS8/MS9`，C41取得`MS0/MS3/MS8/MS9`，需同時替換2檔且canonical Excess-Alpha嚴格更高；2,000個小型randomized可行案例中另有11例C39/C41 membership不同且全部由C41取得更高objective，只作solver distinctness驗證。60-candidate synthetic benchmark在K=5/8/10的branch-and-bound states保持有限且均完成certificate；此benchmark只驗工程可行性，不作scientific gate。
- Decision Gate：只跑Selection C41，不建Forward、不跑multi-seed。主要contrast=`C41-C39`純solver effect與`C41-C35`最終Excess-Alpha策略經濟Gate。若exact constrained仍低於C35，停止`rank × capacity` objective路線；若明顯改善C39且勝C35，才另輪決定是否建立Forward arm。

## 2026-08-15 — SR-C41 Selection REJECT → SR-C42 MR-13E Score Exact Constrained

- SR-C41正式Selection PIT結果已完成，run=`20260815_034944_C32-C23-C25-C28-C35-C39-C40-C41_a3076d792ad2`。C41固定C39相同MR-13E PIT、Expected Excess-R calibration、historical Min params、K/R0、`Σ(Expected Excess-R × canonical planned initial risk)` objective、sizing/cash/orderability/execution，只把repair+1-swap heuristic替換為完整候選exact constrained branch-and-bound。
- C41主要結果：Return=`115.23%`、MDD=`20.65%`、RoMD=`5.58`、Annual=`11.58%`、EV=`0.51R`、Exposure=`88.20%`、DL selection R=`+26.86R`。相對C39：Return=`-16.21pp`、MDD=`+0.44pp`、RoMD=`-0.92`、Annual=`-1.16pp`、EV=`-0.07R`、Exposure=`+0.09pp`、DL selection R=`-24.16R`；相對C35：Return=`-38.73pp`、MDD=`+0.77pp`、RoMD=`-2.16`、Annual=`-2.67pp`、EV=`-0.03R`、Exposure=`-0.13pp`、DL selection R=`-9.10R`。
- Solver evidence：C41 repair/ascent eval均=`0`，Exact states=`66,283`、pruned=`31,288`、certified days=`1,018`；Exposure已恢復至與C35/C39相近水準，仍明顯落後。因此C39 failure不能再歸因於minimum-repair/1-swap local optimum，`Expected Excess-R × PlannedRisk` additive basket objective正式`REJECTED`；C39/C40/C41都不得建立Forward或multi-seed arm。
- 尚未回答的最小必要問題：exact constrained solver本身是否能改善目前最強的原始MR-13E score selector。既有C35=`score + K/R0 + repair/1-swap`，C41=`Excess-Alpha + K/R0 + exact`，因此仍缺`score + K/R0 + exact`這一格；不能由C41結果判定exact solver對score objective的效果。
- 新受控實驗=`SR-C42 / Min MR-13E Constrained`，程式基準=`test-branch-1_20260815_034918_37e4db2.zip`，SHA256=`33f604ded3eea285ad2c2ce4ffce30291c0cfce9e882738989d4da1ec65d5ac4` + 本輪patch。
- C42與C35固定相同：`DL-CONT13E-PIT`、historical Min params、K/R0、原始MR-13E continuous score objective、canonical sizing/cash/orderability/execution。唯一controlled change：C35的`score Top-K → deterministic minimum repair → best feasible 1-swap ascent`改為C41同源generic exact branch-and-bound，在完整候選universe直接最大化lexicographic MR-13E score quality subject to exact K/R0/canonical cash feasibility。
- C42不依賴Expected-R或Expected Excess-R calibration，不新增mapping、lambda、R0比例、candidate truncation、beam width或time cutoff。C41/C42共用同一exact solver owner，只由`objective_mode=excess_alpha/score`切換objective，避免solver implementation分叉。
- Decision Gate：只跑Selection。`C42-C35`為純solver effect；`C42-C41`在同MR-13E PIT、K/R0、exact solver與execution下只隔離score vs Excess-Alpha objective。若C42未明顯改善C35，停止selector exact-solver研究線；若明顯改善，才另輪決定是否建立Forward對應arm。

## 2026-08-15 — SR-C42 Selection PASS → SR-C43 MR-13A Score Exact Constrained

- SR-C42正式Selection PIT結果已完成，run=`20260815_041835_C32-C23-C25-C28-C35-C39-C40-C41-C42_d81955de516a`。C42固定C35相同`DL-CONT13E-PIT`、historical Min params、K/R0、原始MR-13E score、sizing/cash/orderability/execution，只把repair+1-swap heuristic替換為同源exact constrained branch-and-bound。
- C42主要結果：Return=`199.32%`、MDD=`19.88%`、RoMD=`10.03`、Annual=`16.96%`、EV=`0.75R`、Exposure=`88.16%`、DL selection R=`+117.16R`。相對C35：Return=`+45.36pp`、MDD=`0.00pp`、RoMD=`+2.28`、Annual=`+2.71pp`、EV=`+0.21R`、Exposure=`-0.17pp`、DL selection R=`+81.20R`。因此MR-13E pure score exact constrained solver Selection Gate明確PASS。
- Solver evidence：C42 repair/ascent正式path均=`0`，Exact states=`10,407`、pruned=`3,603`、certified days=`914`；selector total=`2206.33ms`、Median/day=`0.526ms`、P95/day=`4.983ms`，未出現不可接受的工程成本。C42相對C41同exact solver但objective改回原始score後Return=`+84.09pp`、RoMD=`+4.45`，進一步支持Expected Excess-R×Risk objective已REJECT而exact solver本身有價值。
- 尚缺的最小必要control：C28 MR-13A亦使用舊`Top-K → R0 minimum repair → 1-swap` heuristic；既然C42已證明solver可大幅改變MR-13E策略結果，不能再直接用C28 vs C42判定13A/13E純模型差異。故Forward C42暫緩一輪，先補13A exact constrained Selection。
- 新受控實驗=`SR-C43 / Min MR-13A Constrained`，程式基準=`test-branch-1_20260815_041825_439b851.zip`，SHA256=`627cdc64168c4d82b9a8c12c8c92efa1504f64597bdf4ada2403755b847d0533` + 本輪patch。
- C43與C28固定相同：`DL-CONT13A-PIT`、historical Min params、K/R0、原始MR-13A continuous score objective、canonical sizing/cash/orderability/execution。唯一controlled change：C28 repair+1-swap heuristic改為C42同一generic exact branch-and-bound，`objective_mode=score`，完整candidate universe直接求K/R0/canonical-cash feasible global optimum。
- C43不依賴Expected-R/Expected Excess-R calibration，不新增lambda、R0比例、candidate truncation、beam width、time cutoff，不重訓MR-13A、不重建PIT；robustness=`off`、Selection-only、無Forward arm。MR-12B C25/C20保留為historical/Forward anchor，不新增12B constrained。
- Decision Gate：`C43-C28`回答MR-13A純solver effect；`C42-C43`在同K/R0、同exact solver與execution下回答MR-13E vs MR-13A純DL source effect。C43結果取得後即停止新增Selection selector變體，直接決定哪一個daily exact-constrained candidate進single-seed frozen Forward。

## 2026-08-15 — SR-C43 Selection NOT PROMOTED → SR-C44 MR-13E Forward Score Exact Constrained

- SR-C43正式Selection PIT結果已完成，run=`20260815_044157_C32-C23-C25-C28-C35-C39-C40-C41-C42-C43_cf6bf772b3af`。C43固定C28相同`DL-CONT13A-PIT`、historical Min params、K/R0、原始MR-13A score、sizing/cash/orderability/execution，只把repair+1-swap heuristic替換為C42同源exact constrained branch-and-bound。
- C43主要結果：Return=`146.22%`、MDD=`18.86%`、RoMD=`7.75`、Annual=`13.74%`、EV=`0.68R`、Exposure=`88.09%`、DL selection R=`+103.25R`。相對C28：Return=`-5.66pp`、MDD=`0.00pp`、RoMD=`-0.30`、Annual=`-0.37pp`、EV=`-0.03R`、Exposure=`+0.29pp`、DL selection R=`-6.67R`；因此MR-13A exact constrained不提供promotion證據，不建Forward arm。
- Exact solver evidence：C43 repair/ascent eval=`0/0`，Exact states=`10,831`、pruned=`3,600`、certified days=`849`；同一exact solver下C42−C43=`+53.10pp Return / +1.02pp MDD / +2.27 RoMD / +3.22pp Annual / +0.07R EV / +13.91R DL selection R`，支持MR-13E source在此solver contract下明顯優於MR-13A。這也證明C42的Selection改善不是exact solver對所有daily model的普遍偏差。
- 新受控實驗=`SR-C44 / Min MR-13E Constrained Forward-OOS`。Effective source basis=`test-branch-1_20260815_041825_439b851(1).zip`，SHA256=`627cdc64168c4d82b9a8c12c8c92efa1504f64597bdf4ada2403755b847d0533` + 已成功執行的SR-C43 patch + 本輪patch；使用者本輪提供的ZIP本身仍是C43前source，因此只疊加已被正式Selection輸出證明已執行的C43 patch，不推測其他本機變更。
...
## 2026-08-15 — SR-C44 Forward PASS → Selection / Forward 8-seed exact-constrained robustness activated

- SR-C44正式Forward-OOS結果完成，run=`20260815_053617_C1-C3-C20-C29-C36-C44_182b73f08149`。C44固定C36相同`DL-CONT13E` frozen Forward score、current Min ROOS params、K/R0、原始continuous score objective、sizing/cash/orderability/execution，只把repair+1-swap heuristic替換為generic exact constrained branch-and-bound。
- C44主要結果：Return=`214.29%`、MDD=`15.13%`、RoMD=`14.17`、Annual=`24.86%`、EV=`1.03R`、Exposure=`92.80%`。相對C36：Return=`+85.52pp`、MDD=`-2.06pp`、RoMD=`+6.67`、Annual=`+7.46pp`、EV=`+0.16R`；相對MR-12B C20：Return=`+24.03pp`、MDD=`-3.62pp`、RoMD=`+4.02`、Annual=`+1.91pp`。因此C44 single-seed Forward Gate PASS，但current runtime anchor在multi-seed完成前仍維持MR-12B。
- 下一個最小必要Gate依stage順序固定為`[3] Selection PIT Multi-seed robustness`後`[4] Forward-OOS Multi-seed robustness`。兩stage均沿用`seed_count=8`、`seed_generator_seed=20260810`、deterministic generated seeds；禁止best-seed selection、seed-specific調參或結果後改objective。
- Selection robustness matrix由robustness profile顯式指定：fixed=`C32 Full ROOS / C23 Min ROOS`；stochastic=`C25 MR-12B / C35 MR-13E heuristic / C42 MR-13E exact constrained`。Forward robustness matrix同樣由robustness profile顯式指定：fixed=`C1 Full ROOS / C3 Min ROOS`；stochastic=`C20 MR-12B / C36 MR-13E heuristic / C44 MR-13E exact constrained`。MR-13A C28/C29與C43不進本輪，C39/C40/C41亦不回流；single-seed arms既有`robustness_role`與Strategy Compare schema/fingerprint不因本輪robustness activation改寫。
- Same-seed paired reporting改為config-driven，只保留兩個決策contrast：`MR-13E exact vs MR-12B anchor`與`MR-13E exact vs same-source heuristic`；每個contrast獨立輸出RoMD wins/delta、DL selection R translation、cross-seed distribution與yearly same-seed delta，不再假設stochastic arms固定為2個，也不建立全排列比較噪音。
- Training orchestration按`(dl_id, seed)`去重。同stage有3 stochastic arms×8 seeds=`24` strategy replay observations，但只有2 unique DL sources×8=`16` model trainings：Selection為`CONT12B_PIT`與`CONT13E_PIT`，Forward為`CONT12B`與`CONT13E`。同一MR-13E seed只訓練/產生score一次，再fan-out到heuristic與exact runtime replay；shared training artifacts在全部fan-out replay完成後才依retention policy清理，避免第一個arm提前刪除第二個arm仍需的score/checkpoint。
- 此輪只改robustness orchestration/reporting與robustness profile membership；single-seed arm identity／`robustness_role`／Strategy Compare schema維持既有值，因此[1]/[2] scientific fingerprint不因啟用[3]/[4]而漂移。不改MR-13E/MR-12B training semantics、dataset/target、exact solver、K/R0、cash/sizing/execution，也不新增新的SR-C arm。結果尚未產生，不預先判定promotion。

## 2026-08-15 — 8-seed exact robustness closed → SR-C45 Full ROOS MR-13E Exact Selection

- 前一輪Selection PIT與Forward-OOS same-generated-8-seed robustness均已完成，固定`seed_count=8`、`seed_generator_seed=20260810`，沒有best-seed selection或seed-specific調參。Forward主要結果：MR-12B C20 RoMD Mean=`8.21`、MR-13E heuristic C36=`10.75`、MR-13E exact C44=`10.80`；C44-C20 same-seed ΔRoMD Mean=`+2.59`且C44勝=`6/8`，支持MR-13E model family相對MR-12B；C44-C36 ΔRoMD Mean僅=`+0.05`且雙方各勝=`4/8`，不支持宣稱exact solver本身具有跨seed穩健績效優勢。
- 因此後續把MR-13E Exact當作主要research architecture base的理由限定為scientific definition：完整candidate universe下直接求`K / R0 / canonical cash feasibility`的global optimum並輸出optimality certificate；不是把8-seed結果解讀成exact必然勝heuristic。MR-12B暫維持current runtime anchor／benchmark。
- 新受控實驗=`SR-C45 / Selection PIT: Full ROOS + Frozen MR-13E Score Exact Constrained`。程式基準=`test-branch-1_20260815_062408_fd2bde7(1).zip`，SHA256=`57f2ae2636b2c38d53c33c1b5b53930c2eb49d429a4987e4c40c326009bbbd9b` + 本輪patch。
- C45不新增任何selector implementation。完全重用C42的`DL-CONT13E-PIT`、原始`Σ MR-13E score` objective、`resource-aware-continuous-score-constrained-optimal`與`exact_branch_and_bound_v1`；`dl_runtime_options`與C42 byte-semantic一致：`preserve_k_r0=True / constrained_solver=exact_branch_and_bound_v1 / selection_only=True`。
...
## 2026-08-15 — Strategy Compare planner architecture consolidation

- 本輪是engineering consolidation，不新增MR/DL/SR/PARAM/AUD identity，也不改任何scientific result。程式基準=`test-branch-1_20260815_160315_924cf0e.zip`，SHA256=`ff63cea57833e7e75691e21f30371eb27d41ead409e65f421f096023ace77328` + 本輪patch。待解決問題來自minimal active matrix後連續暴露的兩個orchestration regression：historical arm退役使current arm失去frozen-score provenance，以及score recovery後status已READY但comparison period仍為None。
- 新增`filters/breakout_quality/strategy_compare_plan.py`作正式resolved-plan boundary。`ResolvedComparisonPlan`在UI或replay取得status前強制驗證`overall_status/preparation_plan/comparison_ready`一致；只要聲稱READY，就必須同時存在合法comparison period、所有required parameter bindings、每個DL-on arm的current source／completed pair／frozen-score binding之一，以及已解析replay cache。READY invariant不再延後到真正replay才檢查。
- `apps/research.py`的同一次「顯示執行計畫 → 使用者確認 → run」只解析一次`ResolvedComparisonPlan`，並把同一plan直接交給`run_strategy_comparison()`；run不再為READY狀態重新收集artifact/replay cache。若前置屬PREPARABLE，builder完成後才重新resolve一個完整final plan，再進replay。舊`_collect_ready_status_after_preparation()`事故型status補丁helper退役。
- completed-pair continuous-score provenance discovery與active-arm membership解耦。active matrix只決定哪些DL artifact有需求；provenance resolver直接合併目前replay-cache entries與config-declared historical completed runs，依DL identity、project-root path、stored/actual SHA、profile、required-start／first-scored-event／available-through coverage逐項驗證。結果先形成immutable `ResolvedContinuousScoreBinding`，再投影回compatibility status dict；historical arm是否仍enabled不得影響同一frozen artifact能否被證明。
- 新增獨立`synthetic_breakout_quality_strategy_plan_cases.py`，不硬編current historical arm ID：動態找current DL-on arm與同source retired arm，以isolated archived score fixture驗證「退役unrelated same-source arm前後artifact SHA/path/period完全不變」；同時驗證Resolved plan不可被外部status copy反向修改，以及`READY + comparison_period=None`必須在execution前直接拒絕。此generic transition contract取代繼續堆疊C36/C44事故型helper測試。
- Targeted independent checks：new resolved-plan transition=`5/5 PASS`、Daily PIT runtime=`9/9 PASS`、MR-13E source gate=`4/4 PASS`、synthetic registry metadata=`18/18 PASS`。既有config-driven Strategy Compare direct validator=`113/114 PASS`；唯一failure=`completed_pair_cache_uses_replay_identity_not_whole_config_and_invalidates_on_score_sha`已在未修改原始ZIP上獨立重跑亦為同一`113/114`，確認不是本輪引入，故本輪不為綠燈改動不相關cache semantics。
- Scientific boundary不變：Selection active=`C32/C23/C42`、Forward active=`C1/C3/C44`；MR-13E model、raw-score objective、exact constrained solver、K/R0、cash/sizing/execution、Full-transfer REJECT與既有single/multi-seed結果全部不變。


## 2026-08-15 — Strategy Compare preparation ownership split / validator responsibility reduction

- 本輪是engineering P2 consolidation，不新增MR/DL/SR/PARAM/AUD identity，也不改scientific result。程式基準=`test-branch-1_20260815_162242_8b142c7.zip`，SHA256=`018656db1e83fc513b538236f8ebd42eab86554aab130377255e91f6b4bc6c4e` + 本輪patch。
- 根因不是新增runtime bug，而是上一輪已建立`ResolvedComparisonPlan`邊界後，preparation內部仍由單一`collect_artifact_status()`承擔required-source解析、model-upstream readiness、Selection/Forward DL artifact、Expected-R／Expected-Excess-R calibration、comparison period與parameter artifact preflight；原函式約943行，builder execution又與status planner同module，讓後續ownership與測試impact mapping難以局部判定。
- `filters/breakout_quality/strategy_compare_preparation_status.py`成為artifact readiness／period／parameter coverage／preparation-plan建構的單一owner；`strategy_compare_preparation.py`縮為builder execution＋dependency-wave runner façade並保留原public imports相容。`collect_artifact_status()`本體由約943行降為75行，依序呼叫required-source、DL artifact、Expected-R calibration、Expected-Excess-R calibration、parameter artifact五個專責helper；不新增第二套formula或artifact判定。
- 行為等價檢查直接以未修改原ZIP與refactor後程式分別解析`selection_pit`、`forward_oos`，逐項比較overall/READY、comparison period/source、所有preparation actions與dependencies/producer、parameter status/coverage/SHA、DL status/coverage、artifact identity path/SHA，兩profile結果byte-meaning相同（normalized payload equality=True）。
- 既有config-driven synthetic中的preparation dependency/multi-wave/parameter-preflight 8項不複製新case，而是原check原名搬到`synthetic_breakout_quality_strategy_preparation_cases.py`專責helper，再由原validator聚合；check數不增加。`synthetic_cases.py`impact mapping同步加入新的status owner與validator helper，`meta_quality_targets.py`把strategy plan/preparation implementation owners納入Breakout Quality coverage targets，避免拆檔後因change-selection或coverage target漏登而形成盲區。
- Targeted independent checks：preparation contract=`8/8 PASS`、resolved-plan transition=`5/5 PASS`、Daily PIT runtime=`9/9 PASS`、MR-13E source gate=`4/4 PASS`。使用synthetic `V16StrategyParams()`執行config-driven app contract為`113/114 PASS`；唯一failure=`completed_pair_cache_uses_replay_identity_not_whole_config_and_invalidates_on_score_sha`在未修改原ZIP上亦為同一failure，確認非本輪引入，故本輪不改不相關pair-cache semantics。
- Scientific/runtime boundary不變：Selection active=`C32/C23/C42`、Forward active=`C1/C3/C44`；MR-13E score、exact constrained solver、K/R0、cash/sizing/execution、historical artifact provenance與既有single/multi-seed結果全部不變。

## 2026-08-15 — Strategy Compare P2-B DL artifact / completed-pair reuse ownership split

- 本輪是engineering P2-B consolidation，不新增MR/DL/SR/PARAM/AUD identity，也不改scientific result。程式基準=`test-branch-1_20260815_163824_05203cc.zip`，SHA256=`5021ded2af56d9843b3d763de1ef860b75aad5124e92e3f75a2473c7482fcc07` + 本輪patch。
- `filters/breakout_quality/strategy_compare_dl_artifacts.py`成為Strategy Compare DL artifact demand／source-specific readiness owner。原`strategy_compare_preparation_status.py`中的required DL source解析與約490行DL artifact branch搬入新owner，並拆成model-upstream dependency、Selection PIT、continuous-ranker OOS、一般canonical runtime四個責任；`strategy_compare_preparation_status.py`只聚合DL結果、calibration、period、parameter status與preparation plan。Selection PIT、Forward continuous OOS與isolated TP1 canonical-runtime三類normalized planning payload均與未修改原ZIP完全一致。
- completed-pair/cache/reuse synthetic不再持續堆在巨型`synthetic_breakout_quality_strategy_app_cases.py`。既有6項contract原check identity搬入`synthetic_breakout_quality_strategy_reuse_cases.py`專責helper，再由原config-driven validator聚合；不新增C36/C44事故型重複case。config-driven validator使用正式`V16StrategyParams()`執行為`114/114 PASS`。
...
## 2026-08-15 — P1 MR-13E Exact runtime integration Gate

- 本輪執行P1，不新增MR/DL/SR/PARAM/AUD scientific identity，也不重訓模型。程式基準=`test-branch-1_20260815_171129_7a74c71(1).zip`，SHA256=`81c07b04c106717eda5d8a49516336596abd933d0979640e05d538169a56e2bc` + 本輪patch。目標不是直接把`BREAKOUT_QUALITY_WORKFLOW_EXPERIMENT_PROFILE`改成MR-13E，而是先驗證「Min ROOS + MR-13E daily score + K/R0/canonical-cash constrained exact selector」整套已研究策略語意能否升格；單改profile只會得到一般score-ranking，與C42/C44不等價，因此禁止視為promotion。
- `config/strategy_compare.py`新增config-driven `StrategyRuntimeIntegrationSettings`。Gate不硬編C42/C44：Selection/Forward各自動態解析唯一active DL candidate與同param/rule DL-off baseline，要求兩stage同一filter/architecture/experiment profile、Selection=`selection_point_in_time`、Forward=`continuous_ranker_oos`、daily eligible-stock-day information-date、Min/all-off、`resource-aware-continuous-score-constrained-optimal`、`preserve_k_r0=True`、`exact_branch_and_bound_v1`，且只允許`selection_only`依stage不同。
- 新增`filters/breakout_quality/runtime_integration_gate.py`與正式入口`apps/research.py compare integration [run|status|latest]`；互動Strategy Compare選單由config顯示`Runtime 整合 Gate`。Gate為read-only evidence decision：不建Label、不訓練模型、不重跑Strategy Compare、不自動修改runtime default；`run`只持久化`outputs/strategy_compare/runtime_integration/.../runtime_integration.json|md`，console與Markdown使用同一payload。
- Promotion必要證據：兩stage canonical Strategy Compare中candidate RoMD高於同param/rule Min baseline；每個實際進入exact owner的active day皆有optimality certificate，certificate-required days定義為`resource_aware_dl_selection_days + resource_aware_capital_utilization_days`，`resource_aware_max_dl_eligible_days`只作非平凡candidate-competition診斷，不作certificate分母；K/R0 preservation與order-count violation皆為0，selector max latency不得超過config上限10秒；目前Selection/Forward promotion candidate實際使用的param與DL artifacts必須READY；同profile的Full benchmark不是runtime prerequisite，不得因其缺工件誤BLOCK promotion。active matrix瘦身不使既有結果失效：single-run evidence依candidate/baseline pair scientific semantics搜尋目前stage output與profile宣告的`reuse_output_roots` historical run，而不是要求整張舊matrix fingerprint與current matrix完全相同。
- Runtime-anchor robustness特別不得用目前slim robustness的「MR-13E vs fixed Min」取代。Gate會在既有robustness output tree搜尋同`seed_count=8 / generator_seed=20260810 / resolved_seeds`、同Min strategy universe，且同時包含current exact candidate與目前`BREAKOUT_QUALITY_WORKFLOW_EXPERIMENT_PROFILE`（現為MR-12B）的historical canonical summary；要求candidate對anchor same-seed RoMD mean delta>0且勝出seed數嚴格過半。這可直接重用既有Selection `6/8, +0.19`與Forward `6/8, +2.59`類型證據，不為P1重訓已退役MR-12B controls。
- Gate decision固定三態：任何scientific/runtime必要條件FAIL=`NO_GO`；缺少/stale canonical evidence或目前工件=`BLOCKED`；全部PASS=`GO`。在本GPT ZIP環境因刻意不含`models/`與`outputs/`，實跑`compare integration status`得到語意／anti-lookahead／core runtime contract PASS，但evidence與operational checks正確為BLOCKED，總決策=`BLOCKED`，沒有誤升格。隔離完整假工件測試得到`GO`；將兩stage anchor robustness改為4/8且mean delta≤0時得到`NO_GO`，確認三態判斷可區分。
- 本輪新增正式synthetic contract覆蓋runtime integration candidate identity、MR-13E exact跨stage語意、current MR-12B anchor在Gate=`GO`前保持分離、GO/BLOCKED/NO_GO三態優先序，以及historical `reuse_output_roots`重用／不得在Gate內修改workflow default；targeted結果=`4/4 PASS`。既有config-driven Strategy Compare=`114/114 PASS`、Daily PIT runtime=`9/9 PASS`、Dataset/Research CLI=`164/164 PASS`。
- Scientific boundary：本輪只完成promotion decision mechanism，沒有宣稱runtime已升格。只有使用者本機canonical artifacts執行Gate得到`GO`後，才允許下一個獨立變更把runtime default從DL-CONT12B/MR-12B切到Gate輸出的MR-13E Exact candidate contract。

## 2026-08-15 — P1 Runtime Integration Gate certificate denominator fix

- 使用者在完整本機canonical artifacts執行第一次Gate，run=`20260815_174451_b2e20ba60849`，除Selection/Forward `exact_optimality_certificate`兩項外，其餘semantics、anti-lookahead、single-run RoMD、selector latency、same-seed robustness與operational readiness全部PASS，總決策顯示`NO_GO`。此`NO_GO`後續被判定為Gate engineering false negative，不是MR-13E Exact研究失敗。
- 根因：Gate schema v1把`resource_aware_max_dl_eligible_days`誤當「必須取得exact certificate的全部天數」，並要求`certified_days == max_dl_eligible_days`。但exact owner在`candidate_count <= K`、沒有非平凡membership競爭等情況仍會回傳可證明的trivial optimum，這些天在summary記為`capital-utilization`且`constrained_solver_optimality_certified=True`，因此`certified_days`可合法大於`max_dl_eligible_days`。
- Gate schema升為v2。正確certificate-required days固定為`resource_aware_dl_selection_days + resource_aware_capital_utilization_days`；只有這兩類代表exact policy真正進入domain owner。`Max_DL_Eligible`保留為diagnostic-only，僅表示其中需要非平凡candidate competition的子集合。required fields缺失／無可驗證active day回`BLOCKED`；`certified_days != required_days`、order-count violation或K/R0 preservation violation才回`FAIL/NO_GO`。
- 新增synthetic regression：`DL-selection=5 / capital-utilization=3 / Max-DL eligible=5 / certified=8`必須PASS，直接釘死「certified合法大於Max-DL eligible」；另驗證order violation=`FAIL`、summary required field缺失=`BLOCKED`，避免把真違規與舊報表缺證據混為一談。
- 2026-08-15 17:44:51第一次Gate的`NO_GO`永久標記`INVALIDATED_GATE_DENOMINATOR_BUG`，不得用來否決promotion；MR-12B仍維持current runtime anchor，需套用修正版後由相同正式選單重新執行Gate取得新的canonical `GO / NO_GO / BLOCKED`決策。
## 2026-08-15 — P1 MR-13E Exact runtime promotion approved and production wiring

- 使用者在完整本機canonical artifacts套用certificate denominator修正版後，Runtime Integration Gate run=`20260815_175208_210428b4f953`得到`GO`；semantics、anti-lookahead、Selection/Forward canonical result identity與RoMD、exact optimality certificate、selector latency、same-generated-8-seed anchor robustness、operational readiness全部PASS。這是P1正式promotion decision；pre-promotion anchor固定為MR-12B，不因workflow切到MR-13E後跟著變更。
- `BREAKOUT_QUALITY_WORKFLOW_EXPERIMENT_PROFILE`正式由`strategy_aligned_no_time_all_event_pairwise`升格為`daily_universal_no_time_full_list_ndcg_pairwise`。production runtime contract另由同一workflow SSOT固定：`canonical_runtime` daily score、`resource-aware-continuous-score-constrained-optimal`、`preserve_k_r0=True`、`constrained_solver=exact_branch_and_bound_v1`、`selection_only=False`。Strategy Compare仍用stage-specific context，研究與production ownership不混用。
- 正式production score不覆寫C44 frozen Forward-OOS canonical score。新增獨立`models/runtime/breakout_quality/<filter>/<architecture>/<profile>/scores.csv + runtime_manifest.json`；由同一frozen accepted MR-13E checkpoint從model information cutoff起，以當日及以前OHLCV因果延伸至目前source-data tail。runtime manifest保存source model manifest/checkpoint SHA，載入時重新核對，防止score與model identity漂移。
- 新增`apps/research.py compare integration promote`與互動選單「套用／更新正式 Runtime」。此action先重新要求current Gate=`GO`，再refresh production scores；其後從Forward `min_roos` / `base-finalist-best` canonical active-param ensemble取最新唯一member，materialize為正式`models/run_best_params.json`，rule policy固定all-off，`use_breakout_quality_filter=False`、`use_breakout_quality_ranking=True`。舊run_best存在時先永久備份，再atomic replace；promotion JSON/Markdown持久化至`outputs/strategy_compare/runtime_integration/promotion/...`。
- Scanner仍只負責candidate discovery／參考資金，不假裝具有portfolio K/R0/cash state；正式basket membership與exact constrained ownership仍在`core/portfolio_entry_selection.py`。production score的**全域日期coverage過期／超界**時fail-fast並明示重新執行「套用／更新正式 Runtime」，不使用stale score fallback；但在有效coverage內個別ticker/date沒有score時，維持C44既有`score unavailable`語意，候選不因缺score被hard reject，而是在exact ranking中排到有score候選之後。
- Scientific boundary不變：這次promotion只適用**Min ROOS/all-off + MR-13E + exact K/R0 constrained**整套策略architecture。MR-13E相對MR-12B的historical 8-seed strategy RoMD證據為Selection `6/8, ΔMean=+0.19`、Forward `6/8, ΔMean=+2.59`；exact-vs-heuristic兩stage仍僅`4/8`，Full transfer C45仍REJECT，因此不得宣稱exact solver或MR-13E source在所有策略體系普遍優勝。

## 2026-08-15 — P1 production daily-ranker score path closure after first promotion execution

- 使用者套用promotion patch後，從正式互動選單`Research → 策略組合比較 → Runtime 整合 Gate → 套用／更新正式 Runtime`首次執行時，`filters/breakout_quality/export_scores.py`的`--experiment-profile` argparse仍只接受舊classification profile集合，導致promoted profile `daily_universal_no_time_full_list_ndcg_pairwise`在真正score refresh前即被拒絕。最新程式基準=`test-branch-1_20260815_183049_4e89507.zip`，SHA256=`3e94481c6c8fc95f3e86e7471295e863b3864957510373c4dcecf003df06c2f1`；同輪formal failure bundle顯示consistency唯一FAIL與meta-quality四個coverage FAIL另有同一synthetic提前中止根因。
- Promotion score exporter不是只放寬argparse choices：舊`run_export()`後續仍固定走binary `load_model_artifact_contract + event dataset + softmax PASS`語意，對MR-13E會在下一層再次失敗或產生錯誤score universe。因此新增daily continuous workflow export branch：由profile SSOT辨識continuous objective，讀canonical `load_continuous_ranker_oos_contract`與同一frozen checkpoint，重用`load_daily_universal_ranker_data`及shared `ranker_training.predict_scores`，輸出真正的`ticker/date/model_score` production table；Forward-OOS C44 research score仍immutable。model build同時補齊architecture argument，避免promotion下一步才出現model-construction error。
- Daily production inference允許在不重建Label／Target／模型的前提下，把score eligibility從舊outer-policy OOS尾端延伸至目前canonical OHLCV source tail；未完成future 40-bar target的最新stock-day仍可推論，future target只屬training/evaluation而非score eligibility。production runtime manifest固定保存frozen model checkpoint SHA／settings／model spec／information cutoff及`same_day_and_earlier_ohlcv_only` contract。
- `workflow_runtime_score_store.py`改為continuous-ranker model identity validator與daily`ticker/date` key，不再要求binary event的`high_len` score表。全域date coverage不足仍fail-fast；coverage內個別ticker/date缺score則回`available=False / missing_ticker_date_score / score_source=canonical_runtime`，與C44 exact ranking既有candidate-preserving語意一致。每次promotion refresh後先clear runtime score bundle cache，避免同一process讀到舊score artifact。
- Formal bundle的consistency唯一FAIL為synthetic fixed-signal re-entry案例ticker=`3706`。該案例本意是驗證歷史non-daily fixed-signal continuation不得在re-entry trigger date重新查score，但promotion後它意外繼承global daily-universal workflow context，因沒有information date而被新runtime contract正確拒絕。修正方式不是放寬production guard，而是在該historical synthetic case內明確使用non-daily canonical ranking context；直接targeted contract已恢復`45/45 PASS`。meta-quality的`coverage_synthetic_suite_runs_successfully / line / branch / key targets`四個FAIL皆由同一synthetic提前中止連鎖造成，無獨立coverage defect證據。
- 新增runtime-promotion regression覆蓋：promoted MR-13E profile必須被score-export CLI接受、continuous profile必須dispatch至daily exporter、exporter source必須使用daily continuous contract，以及runtime daily score lookup必須保留C44 unavailable semantics並對全域coverage超界fail-fast。targeted Runtime Integration contract=`13/13 PASS`。本輪不新增MR/DL/SR/PARAM/AUD scientific identity、不重訓模型、不改C42/C44研究結果或Gate=`GO`決策。

## 2026-08-15 — P1 frozen MR-13E manifest semantic schema compatibility closure

- 最新程式基準=`test-branch-1_20260815_185412_cd54670.zip`，SHA256=`0f4b1b90bd87d924d7ef19fd2fa254d771207fb9c8dce72bdc4101fc4d14ee95`；使用者由正式互動選單`Research → 策略組合比較 → Runtime 整合 Gate → 套用／更新正式 Runtime`再次執行promotion時，production score refresh進入continuous ranker loader後被`ValueError: Continuous pairwise ranker manifest training semantics不一致`阻擋。同輪formal failure bundle=`to_chatgpt_bundle_20260815_185516_376fb4a9.zip`，SHA256=`edc02ad278c9558245dc161dad2a635f01250db3593bb40b2c854d60493bf5aa`。
- 此錯誤不是MR-13E模型／checkpoint／pairwise objective真的漂移。Frozen MR-13E manifest建立於Direct-R MR-13F/G研究之前；後續共用`ranker_training_contract.training_semantics()`新增了對pairwise MR-13E**不適用**的`raw_r_regression_contract=None` metadata key。runtime loader原先用整個dict exact equality，因舊manifest沒有這個後來新增的null key而產生false incompatibility。MR-13E真正的`pair_weighting=full_list_delta_ndcg_weighted`、whole-date batching、model margin、runtime score等語意未改。
- 修正為objective-relevant strict semantic compatibility：預期值為非null的contract仍要求key存在且完整exact equality；後來新增、對該objective canonical value=`None`的metadata key允許舊frozen manifest缺席，但若該key存在則只能是`None/{}`；任何unexpected key、任何不適用欄位的非null值、或真正pair/list/raw-R contract差異仍fail-fast。pairwise/listwise runtime loader共用此判斷，避免同類frozen metadata schema growth在continuous ranking objective重現。
- Formal consistency唯一FAIL實際來自`validate_breakout_quality_mr13e_strategy_source_gate_contract_case`的`NameError: name 'workflow' is not defined`；該synthetic已改由`get_breakout_quality_workflow_settings()`解析current workflow SSOT。MR-13E model synthetic中pre-promotion「workflow必須仍是MR-12B」硬編assertion同時移除，避免promotion後validator與config SSOT衝突。
- Meta-quality的`coverage_synthetic_suite_runs_successfully / coverage_line_percent_within_minimum / coverage_branch_percent_within_minimum / coverage_key_targets_hit`四項FAIL均是上述synthetic NameError使coverage run提前中止的連鎖結果，沒有證據支持降低coverage threshold；本輪未調任何coverage門檻。`checklist_t_rows_use_single_test_entry`則是B234的T332同列放入兩個validator entry造成，已拆為T332/T333，並新增T334/T335釘住manifest schema compatibility與current-workflow source resolution。
- Scientific identity/status不變：不新增MR/DL/SR/PARAM/AUD、不重訓MR-13E、不改C42/C44、8-seed robustness或Runtime Integration Gate run=`20260815_175208_210428b4f953`的`GO`。本輪僅修production promotion的frozen-artifact schema compatibility與formal validator/checklist engineering defects。
- 本輪獨立targeted驗證：`validate_breakout_quality_daily_full_list_ndcg_pairwise_contract_case=7/7 PASS`、`validate_breakout_quality_mr13e_strategy_source_gate_contract_case=4/4 PASS`、`validate_breakout_quality_runtime_integration_gate_contract_case=13/13 PASS`、`validate_breakout_quality_daily_pit_strategy_runtime_contract_case=9/9 PASS`、`validate_strategy_compare_config_driven_app_contract_case=114/114 PASS`、`validate_breakout_quality_strategy_comparison_contract_case=45/45 PASS`。全專案獨立靜態檢查=`340/340 Python compile`、`9/9 top-level config import`、bare `except`=0、local import cycle=0、lower-layer→`apps` reverse import=0；Checklist T=`286` rows全部single entry且T/G DONE集合一致，G=`1795` rows無排序／transition／chain／no-op問題，Markdown table欄位結構=0 mismatch。依PROJECT_SETTINGS未執行`apps/run_bundle.py`、`apps/test_suite.py`或其完整formal steps；需由使用者套用patch後以正式入口重跑local double-check。


## 2026-08-15 — P1 post-promotion continuous-ranker validator boundary closure

- 最新程式基準=`test-branch-1_20260815_191311_d1da767.zip`，SHA256=`192eb3ad84bc4257ff3e68842f6a3720be8206a73caee8dbe5e79c7fd6be38a4`；使用者已由正式互動選單完成MR-13E Exact production promotion，console=`Status=PROMOTED`、workflow=`daily_universal_no_time_full_list_ndcg_pairwise`、ranking=`resource-aware-continuous-score-constrained-optimal`，production score artifact coverage=`2020-12-31~2026-03-02`。同輪formal failure bundle=`to_chatgpt_bundle_20260815_191510_8d7a4050.zip`，SHA256=`7a9878c7b704d1f70c6b16cfafacb0ea13fd1d25b19a4e4d6ffed317ab7333e8`。
- Formal consistency唯一FAIL=`BREAKOUT_QUALITY_CONTINUOUS_RANKER / continuous_ranker_profile_is_blocked_from_binary_workflow_and_runtime_loader`，expected第四項仍要求`export_scores.py`引用classification-only profile集合；但P1 promotion後，共用`export_scores`必須合法dispatch已promotion的**daily-eligible continuous ranker**至獨立`workflow_runtime` production score branch。因此正式production code正確，舊synthetic把歷史11B的research-only限制錯誤外推成「所有continuous profile均不得經export入口」。
- B172的真正historical contract仍保留：`strategy_aligned_daily_percentile_mse / MR-11B`屬event-scope research-only profile，不得被binary training／binary runtime loader或Forward-OOS export載入。Synthetic改成behavioral boundary：直接呼叫共用continuous workflow exporter，要求11B因`training_sample_scope != daily_eligible_stock_days`在任何artifact/model讀取前fail-fast；同時保留binary train/profile集合、binary artifact loader與互動model workflow隔離檢查。已promotion的daily-universal continuous profile則由P1 runtime-promotion contract獨立覆蓋，不再要求classification-only exporter source形狀。
- Meta-quality唯一FAIL=`coverage_synthetic_suite_runs_successfully`只是上述1個synthetic failure的連鎖；同一formal run的target line=`79.04%`、branch=`61.65%`、key targets全部命中，故沒有coverage缺口，也不調低任何coverage threshold。
- Checklist B172文字同步釐清：shared `export_scores`可服務正式promotion的daily-eligible continuous workflow runtime，但11B event-scope仍必須fail-fast；T269 test identity不變，不新增事故型dedicated test，也不改scientific identity/status。Registry的`MR-13E=RUNTIME_PROMOTED`、Gate run=`20260815_175208_210428b4f953 GO`、SR-C42/C44結果與8-seed evidence全部不變。

## 2026-08-15 — SR-C46/C47 MR-13E No-R0 exact dual-arm implementation

- 最新程式基準=`test-branch-1_20260815_205941_47c8f4d.zip`，SHA256=`21568019289ce3d897c2098fb153e9735d0c1d77aabc3220541a347e8a1349f9`。本輪依使用者要求同時建立兩個Selection PIT controlled strategy arms，永久identity=`SR-C46 / SR-C47`；不新增MR/DL/PARAM/AUD identity，不重訓模型，不改Forward-OOS active matrix、multi-seed robustness或production runtime。
- `SR-C46 = Frozen MR-13E Score Exact No-R0`：完全重用C42的`DL-CONT13E-PIT` frozen score、historical Min ROOS params、all-off rules、同日baseline K、canonical 1% risk sizing、true-cash reservation、orderability與execution；唯一controlled change為移除baseline-derived R0 floor（`preserve_r0=False / r0_minimum_repair=False`）。共用generic deterministic exact branch-and-bound直接在完整candidate universe最大化`Σ Score_i`，只受exact K與canonical cash feasibility限制；不再呼叫R0 minimum-repair或feasible-ascent。
- `SR-C47 = Frozen MR-13E Score×Capital Exact No-R0`：與C46完全相同，唯一controlled change為objective=`Σ(Score_i × canonical reserved capital_i)`。`capital_i`定義為該candidate在final basket、canonical execution order與cash-capped sizing下的實際`reserved_cost_milli`；本輪刻意不做score normalization/shift、capital normalization、lambda、R0 floor或其他新數值門檻。若raw score為負，較大的capital會放大負貢獻，屬最簡單乘法objective本身的語意，不隱藏修正。
- Selection active matrix schema由30升為31：`C32 / C23 / C42 / C46 / C47`。既有contrasts保留`C32-C23 / C42-C23 / C42-C32`；新增primary contrasts=`C46-C42`（純R0 ablation）、`C47-C46`（No-R0下objective-only）、`C47-C42`（score×capital No-R0對current exact constrained）。`display_alignment_arm_ids`仍只保留core cross-stage references `C32/C23/C42`，避免Selection-only C46/C47被誤當成Forward對齊arm。
- Exact solver只保留一個共用owner：新增objective selector支援`score / score_capital / excess_alpha`與`preserve_reserve_floor`參數；C42/C44既有K/R0 constrained語意仍走`preserve_reserve_floor=True`，C46/C47走`False`。No-R0 seed只作incumbent lower bound，不限制完整include/exclude search space；R0 certificate/assertion只在preserve模式成立。production ranking仍是`resource-aware-continuous-score-constrained-optimal`與`preserve_k_r0=True`，不因研究arm implementation自動切換。
- 新增exhaustive synthetic oracle直接枚舉K=2全部canonical cash-feasible組合：C46 exact winner必須等於`ΣScore`全域最優；C47 exact winner必須等於`Σ(Score×reserved capital)`全域最優；並驗證兩者`resource_preservation_required=False`、`r0 repair/ascent=0`、exact optimality certified、同一frozen source與config contrasts。`validate_strategy_compare_config_driven_app_contract_case=117/117 PASS`。
- 本輪只完成implementation與contract validation，**尚未執行正式Selection PIT replay**，因此C46/C47狀態固定為`IMPLEMENTED / SELECTION_PIT_PENDING`，不得先寫RESULT_AVAILABLE、GO或REJECT。Dataset/Label/Target/model rebuild均不需要；正式比較應重用既有MR-13E PIT artifacts與Min parameter artifacts。是否進Forward、multi-seed、Pareto frontier或其他objective，必須等待C46/C47 Selection結果後再決策。
- 全專案回歸檢查另外抓到既有Runtime Integration Gate原先以「active DL candidate必須恰好一個」推導production candidate；Selection新增C46/C47後會因active DL arms=3而阻擋Gate。修正為在`STRATEGY_RUNTIME_INTEGRATION`顯式設定`selection_candidate_arm_id / forward_candidate_arm_id`，Gate依config解析candidate與同param/rule baseline；目前仍指向既有C42/C44，不把研究matrix membership當production ownership。這是engineering config-resolution修正，不新增scientific identity，也不改既有GO evidence。修正後runtime integration targeted contract=`13/13 PASS`。

## 2026-08-16 — SR-C46/C47 Selection result closure → SR-C48 Basket-level Pareto Exact No-R0 implementation

- 最新程式基準=`test-branch-1_20260816_152710_e7e45c1.zip`，SHA256=`6eff4eea4e4de0907c99dfa766710d76c8039d3d464c581e096054fe9548fca1` + 本輪patch。先補齊前一輪已取得但Registry仍標PENDING的正式Selection結果：run=`20260815_221854_C32-C23-C42-C46-C47_93d965ca2379`。
- `SR-C46 / Frozen MR-13E Score Exact No-R0`：Return=`95.89%`、MDD=`16.97%`、RoMD=`5.65`、Annual=`10.09%`、EV=`0.33R`、Exposure=`56.76%`、DL selection R=`-51.12R`。相對C42 Return=`-103.42pp`、RoMD=`-4.37`、Exposure=`-31.40pp`；但Target mean=`1.20R`、Target %ile=`0.648`、Top-K=`21.75%`、Opp gap=`3.27R`反而較強。Decision=`REJECTED_DEPLOYMENT_COLLAPSE / NO_FORWARD / HISTORICAL_ONLY`：純ΣScore No-R0會偏向低reserved-capital basket，模型selection quality本身不是主要失敗點。
- `SR-C47 / Frozen MR-13E Score×Capital Exact No-R0`：Return=`168.86%`、MDD=`20.00%`、RoMD=`8.44`、Annual=`15.18%`、EV=`0.69R`、Exposure=`88.05%`、DL selection R=`+105.30R`。相對C46 Exposure=`+31.28pp`、Return=`+72.97pp`，證明capital deployment確實是No-R0缺少的第二個目標；但相對C42 Return=`-30.46pp`、RoMD=`-1.58`、EV=`-0.06R`且Exposure只差`-0.11pp`。Decision=`REJECTED_SIMPLE_PRODUCT / NO_FORWARD / HISTORICAL_ONLY`：candidate-level `Σ(score_i×capital_i)`會過度把capital重寫進股票排序，不能取代R0。
- 新受控實驗=`SR-C48 / Frozen MR-13E Basket-level Pareto Exact No-R0`。固定：`DL-CONT13E-PIT`、historical Min ROOS params、all-off rules、baseline K、canonical 1% risk sizing、true-cash reservation、orderability、execution；不重訓模型、不改Target/Dataset/Label、不用Expected-R calibration。唯一portfolio研究變數：移除baseline-derived R0，改在basket level同時處理`Q=ΣScore`與`C=canonical reserved_cost_milli`。
- C48 exact定義：所有search pass先lexicographic最大化Score coverage，避免missing Score因視為0而打敗負Score；在最大coverage的K/cash-feasible universe中，pass-1 exact求score endpoint `(Q_max, C_min)`（Q優先、同Q取較高C），pass-2 exact求capital endpoint `(Q_min, C_max)`（C優先、同C取較高Q）。以兩端定義`Q_norm=(Q-Q_min)/(Q_max-Q_min)`與`C_norm=(C-C_min)/(C_max-C_min)`；若某軸沒有range，該軸固定為1。pass-3 exact最大化`Q_norm×C_norm`，同product tie依Q、C、baseline deterministic order決定。因scalar對Q/C單調且tie不偏好被支配點，final basket位於Pareto-efficient set。無lambda、無score shift、無candidate-level Score×Capital、無R0/minimum-repair/feasible-ascent。
- Exact implementation使用三個完整candidate include/exclude branch-and-bound pass；每個included partial state都重跑canonical cash-capped reservation。Pruning只使用admissible orderable-count、Score coverage、standalone optimistic Score upper與reserved-capital upper；沒有candidate truncation、beam width、time cutoff。Diagnostics保存score/capital/product三pass states、兩個normalization端點、final normalized Q/C/product與optimality certificate。
- Selection current matrix由schema `31 → 32`，並依研究生命週期把已REJECT的C46/C47及其contrasts移到`config/compatibility/strategy_compare_history.py`。Current active=`C32 / C23 / C42 / C48`；contrasts=`C32-C23 / C42-C23 / C42-C32 / C48-C42`。Forward仍=`C1 / C3 / C44`，robustness與production candidate仍固定C42/C44，不因C48 implementation改動。
- `validate_strategy_compare_config_driven_app_contract_case`新增C48 exhaustive oracle：小型K=2案例完整枚舉所有canonical cash-feasible baskets，獨立求score endpoint、capital endpoint與normalized-product global optimum，要求runtime C48選解、normalization端點與certificate完全一致；本輪implementation階段狀態只能是`IMPLEMENTED / SELECTION_PIT_PENDING / NO_FORWARD_YET / ROBUSTNESS_OFF`。
- C48 decision gate：先只跑Selection `C48-C42`。只有C48能在Exposure不再崩塌的前提下，至少接近／超過C42的Return、RoMD與EV，才另輪建立Forward；若仍明顯輸C42，停止No-R0 allocator路線，不繼續試lambda或其他Score×Capital變形。
- Implementation validation（非正式策略結果）：config-driven contract=`118/118 PASS`、Strategy Compare=`45/45 PASS`、readable report=`6/6 PASS`、Daily PIT runtime=`9/9 PASS`、MR-13E source gate=`4/4 PASS`、Runtime Integration=`13/13 PASS`；另以48組隨機小型universe逐組完整枚舉全部合法basket，三pass C48 runtime與獨立exhaustive oracle=`48/48`一致。依`PROJECT_SETTINGS`未執行`apps/run_bundle.py`或`apps/test_suite.py`。

## 2026-08-16 — SR-C48 Selection result closure / No-R0 allocator line stopped

- 結果來源：使用者由正式`apps/research.py → 策略組合比較 → Selection PIT`取得aggregate，period=`2014-01-01～2020-12-31`、config fingerprint=`27354349956f`。C42與C48使用同一frozen `DL-CONT13E-PIT` model-prediction metrics：Daily rho=`0.163`、Global rho=`0.053`、Pair一致=`55.75%`、Top-R=`0.95R`、Bottom-R=`0.67R`、Top-BottomR=`0.28R`，因此差異歸因於allocator，不是模型預測力改變。
- `SR-C48`結果：Return=`135.64%`、MDD=`17.52%`、RoMD=`7.74`、Annual=`13.03%`、Worst full year=`-9.66%`、Log R²=`0.8946`、Monthly win=`61.90%`、Win rate=`37.74%`、Payoff=`3.16`、EV=`0.51R`、Trades=`461`、Exposure=`84.97%`。相對C42：Return=`-63.68pp`、MDD=`-2.36pp`（較低）、RoMD=`-2.29`、Annual=`-3.93pp`、Worst full year=`-11.46pp`、Log R²=`-0.0512`、Monthly win=`-8.34pp`、EV=`-0.24R`、Exposure=`-3.19pp`。年度Return只有2018/2019高於C42，其餘5/7完整年度較低。
- R轉化：C48平均R=`0.51R` vs C42=`0.75R`，中位R=`-0.36R` vs `-0.28R`，DL選擇R=`+38.19R` vs `+117.16R`，RCE=`15.96%` vs `138.63%`。另一方面C48 Target mean=`0.87R` vs `0.71R`、Target percentile=`0.566` vs `0.546`、Opp gap=`3.61R` vs `3.97R`，顯示basket-level Pareto確實稍改善future Target selection quality，但這個改善沒有轉成realized trade R與portfolio profit。
- Execution：C48 fill rate=`97.46%`、daily orderable candidates=`57.92`、candidate shortage=`118 days`、underfilled end=`835 days`、position gap=`1499 slot-days`；Exposure仍為`84.97%`，不像C46崩到`56.76%`，所以C48失敗已不是單純deployment collapse。
- Decision=`REJECTED_NO_R0_PARETO / NO_FORWARD / HISTORICAL_ONLY`。C46證明純Score No-R0會deployment collapse；C47證明candidate-level Score×Capital能恢復deployment但仍輸C42；C48進一步證明basket-level normalized Pareto雖能兼顧capital與Target quality，仍無法恢復C42的realized-R conversion與portfolio economics。因此No-R0 allocator研究線在C48後停止，不建立Forward、不跑multi-seed、不再試lambda或其他Score×Capital/Pareto變形。Current strategy research anchor維持Min/all-off + frozen MR-13E + exact K/R0 constrained C42/C44。
- 研究生命週期同步：Strategy Compare config schema=`33`，current Selection matrix回到`C32/C23/C42`，contrasts=`C32-C23 / C42-C23 / C42-C32`；C48及`C48-C42`移入`config/compatibility/strategy_compare_history.py`供歷史重現。Production/runtime仍為C42/C44，不受本次closure影響。


## 2026-08-16 — AUD-min-roos-planned-risk-40d-alignment implementation

- 待決策問題：C46/C47/C48已證明把MR-13E Score與capital在Selector端以No-R0、candidate-level乘積或basket Pareto結合都不能取代C42；下一個關鍵不確定性是「若40D opportunity R直接用Min ROOS當日planned entry/initial-stop risk geometry定義，是否比現有fixed-risk target更接近實際completed-trade realized net R」。在此答案前不得建立新MR模型。
- Audit identity=`AUD-min-roos-planned-risk-40d-alignment`，config ID=`min-roos-planned-risk-40d-alignment`。狀態=`IMPLEMENTED / NOT_EXECUTED`；不新增MR/DL/SR/PARAM identity、不改MR-13E/C42/C44、不重跑策略、不訓練、不建立canonical Label。
- Read-only來源固定為current runtime-integration Selection candidate（目前C42）已完成Strategy Compare pair的`score_ranking_execution.csv`、`score_ranking_trades.csv`、`score_ranking_selected_target_diagnostics.csv`與canonical full OHLCV。`execution.csv`提供實際盤前planned `limit_px/init_sl`；realized R由canonical closed-trade reconstruction取得。
...
### 2026-08-16 — MR-13I / MR-13J Universal Risk-normalized 40D 模型實作

- 本段明確**取代**前一段「先做candidate-universe existing-target control」的下一步敘述。使用者重新確認長期目標是讓同一daily-universal DL可供breakout以外策略共用，因此training/sample universe不得改成Min ROOS candidate universe；MR-13H未建立正式identity，也不占Registry ID。
- A0結果只保留為mechanism evidence：在同一completed-trade actual entry date下，planned-risk 40D相對fixed-risk control的Spearman=`0.5267 vs 0.3689`、Pair=`70.03% vs 64.45%`、Top-Bottom realized R=`5.00R vs 4.43R`。它支持risk normalization值得進model experiment，但不等於新的universal target已勝MR-13E。
- `MR-13I`正式identity=`daily_universal_risk_normalized_net_full_list_ndcg_pairwise`。與MR-13E固定相同：daily feature-eligible stock-day universe、300×10 price/0050 sequence、固定40 trading bars、`inception_time_v1`、all-label daily full-list Delta-NDCG-weighted pairwise RankNet、seed與Selection/Forward split semantics。唯一scientific change是Target改為`daily_risk_normalized_net_opportunity_r_v1`。
- 13I的universal risk calibration只讀historical-effective Min ROOS的`atr_len / atr_times_init`。`high_len`屬breakout trigger、`atr_buy_tol`屬entry-order geometry、`atr_times_trail`屬strategy-exit geometry，全部排除；reference固定decision-date close，ATR由當日以前資料計算，initial stop直接重用`calc_initial_stop_from_reference`與canonical tick rule。不存在risk-param coverage的日期不得用未來param回填，也不得進gradient/validation/epoch selection。
- 13I Target固定看`t+1…t+40`；planned-stop touch採adverse-first，stop-touch bar high不計。安全區間選最早maximum high與到該peak的running low；opportunity numerator=`favorable PnL - adverse PnL - canonical round-trip costs`，分母=`canonical planned initial risk`。Qty／1% fixed-risk／max-position-cap／board-lot、買賣fee、minimum fee、sell tax與milli/tick accounting全部重用`core/price_utils.py`、`core/exact_accounting.py`、`core/order_lot_policy.py`；不模擬trailing/strategy exit，因此仍是fixed-40D universal target，不是realized-trade R。
- `MR-13J`正式identity=`daily_universal_risk_context_net_full_list_ndcg_pairwise`；Target/universe/horizon/loss與13I**完全相同**，唯一scientific change是Input增加5維decision-time context：`risk_distance_pct / risk_distance_atr / capital_per_risk / cost_per_risk / risk_capacity`。新architecture=`inception_time_risk_context_v1`，保留13E/I原InceptionTime sequence encoder，在encoder後增加`LayerNorm(5)→16→16` MLP再concat classifier，避免污染`inception_time_v1` checkpoint identity。
- 研究順序：13I=`IMPLEMENTED / FORWARD_MODEL_GATE_PENDING`並成為`apps/research.py → 模型訓練`的active model-research profile；13J=`IMPLEMENTED / BLOCKED_BY_MR13I_RESULT`，只完成程式與identity，不自動訓練。Production workflow仍固定MR-13E；C42/C44、K/R0 selector與runtime promotion identity完全不變。13I model gate若沒有足夠edge，直接停止，不做13J。
- Engineering：daily loader在13I/J啟動前先依Strategy Compare config解析`selection_min_roos / min_roos`正式risk-param sources；缺工件時model app在GPU訓練前BLOCKED，且不得由模型流程自行訓練或補值。13I可在沒有geometry coverage的日期維持sequence-only inference eligibility，但那些日期Target為invalid；13J因Input本身需要geometry，只在有合法risk calibration的日期產生score/context。


### 2026-08-16 — MR-13I Forward model result / MR-13J authorized

- 使用者由正式`apps/research.py → 模型訓練 → 訓練目前模型`完成`MR-13I / daily_universal_risk_normalized_net_full_list_ndcg_pairwise` Seed42 Forward model Gate。Dataset/index：`score_eligible=1,600,734`、`target_valid=1,176,931`、`tickers=563`，historical-effective Min ROOS risk-param coverage start=`2014-01-01`；selected epoch=`1/200`，epoch-selection Validation daily rho=`0.0495`，完整Selection重訓後原Validation daily/global rho=`0.0850/0.1436`、pair=`52.99%`。
- Forward all-stock OOS：daily/global rho=`0.0645/0.0150`、pair=`52.30%`；Top10% Target=`0.8042R`、Bottom10%=`0.8430R`，spread=`-0.0388R`；NDCG@10=`0.5162`、Top-K Target=`0.8782R`、Lift=`+0.0137R`、Oracle overlap=`3.29%`、Boundary=`47.50%`、Boundary gap=`-0.1468R`。Breakout-candidate OOS：daily/global rho=`0.0179/-0.0722`、pair=`51.29%`；Top10%=`0.9185R`、Bottom10%=`1.1579R`，spread=`-0.2394R`；NDCG@10=`0.6819`、Top-K Lift=`+0.0405R`、Oracle overlap=`55.82%`、Boundary gap=`-0.0034R`。
- 與frozen MR-13E Forward model reference比較：13E all-stock daily/global rho=`0.2151/0.1042`、pair=`57.42%`、Top10%-Bottom10% spread=`+0.8234R`、Top-K Lift=`+0.9951R`；breakout daily/global rho=`0.1721/0.1535`、pair=`57.43%`、spread=`+1.4294R`、Top-K Lift=`+0.4489R`。13I相對13E all-stock daily rho=`-0.1506`、pair=`-5.12pp`、spread=`-0.8622R`、Top-K Lift=`-0.9814R`；breakout daily rho=`-0.1542`、pair=`-6.14pp`、spread=`-1.6688R`。因此`MR-13I = FORWARD_OOS_MODEL_GATE_FAIL`，不得進Selection PIT、runtime source或Strategy Compare。
- 13I失敗不推翻A0 target-mechanism GO：A0在同entry completed trades上已證明planned-risk normalization相對entry-fixed control改善Spearman／Pair／Top-Bottom realized-R；13I目前唯一scientific change是把這個risk-normalized target交給**沒有顯式risk-param/geometry context**的sequence-only MR-13E backbone。尤其historical `atr_len/atr_times_init`會跨period改變，而13I input不直接提供這些risk/economic geometry，因此目前關鍵不確定性已縮成「顯式decision-time geometry能否讓模型學到同一Target」。
- 決策：`MR-13I = RESULT_AVAILABLE / REJECTED_AS_SEQUENCE_ONLY / NO_PIT / NO_RUNTIME_DL_SOURCE`；`MR-13J = NEXT_EXPERIMENT_AUTHORIZED / ACTIVE_MODEL_RESEARCH / FORWARD_MODEL_GATE_PENDING`。13J Target/universe/horizon/loss必須與13I完全相同，只新增既定5維context；若13J仍無法在Forward all-stock與breakout slice形成有意義的正向ranking/separation，則停止此risk-normalized DL路線，不再以Selector或Target微調救援。Production workflow與C42/C44仍固定MR-13E。

### 2026-08-16 — MR-13J continuous-ranker context gate closure

- 使用者首次由正式`apps/research.py → 模型訓練 → 訓練目前模型`啟動MR-13J時，在GPU與daily target/index之前被shared `train_continuous_ranker.validate_args()`以`continuous ranker只允許sequence-only architecture`阻擋。根因是MR-13J architecture/data/model均已支援5維context，但共用CLI validator仍保留MR-13E以前的sequence-only假設；這是engineering capability-gate漏接，不是新的scientific result。
- 修正後shared validator依trainer family判斷：market-set／derived-context仍一律拒絕；event continuous ranker若要求dataset context仍fail-fast；daily-universal trainer則可交由其profile-aware sample provider驗證合法context architecture。`train_daily_ranker` manifest同步寫入canonical target manifest的實際context欄位，避免13J checkpoint使用5維context但manifest仍顯示空欄位。
- Regression contract補入active MR-13J shared-CLI validation正例與event-ranker context反例；另以隔離的300×10 sequence＋5維geometry context完成1-epoch pairwise training/validation smoke，確認validator→model→pairwise loss→validation inference路徑可執行。MR-13J identity、Target、universe、loss與Forward gate狀態均不變；Production MR-13E/C42/C44不變。


### 2026-08-16 — MR-13J Forward model result / C49-C50 Selection strategy gate authorized

- 使用者由正式`apps/research.py → 模型訓練 → 訓練目前模型`完成`MR-13J / daily_universal_risk_context_net_full_list_ndcg_pairwise` Seed42 Forward model Gate。Dataset/index：`score_eligible=1,198,243`、`target_valid=1,176,931`、`tickers=563`，historical-effective Min ROOS risk-param coverage start=`2014-01-01`；context=`risk_distance_pct / risk_distance_atr / capital_per_risk / cost_per_risk / risk_capacity`；selected epoch=`1/200`，epoch-selection Validation daily rho=`0.0265`，完整Selection重訓後Validation daily/global rho=`0.1162/0.0202`、pair=`54.01%`。
- Forward all-stock OOS：daily/global rho=`0.1086/0.1280`、pair=`53.89%`；Top10% Target=`1.1727R`、Bottom10%=`0.6363R`，spread=`+0.5364R`；NDCG@10=`0.5600`、Top-K Target=`1.1348R`、Lift=`+0.2702R`、Oracle overlap=`4.04%`、Boundary=`48.76%`、Boundary gap=`-0.0673R`。Breakout-candidate OOS：daily/global rho=`0.0825/0.1179`、pair=`54.22%`；Top10%=`1.1875R`、Bottom10%=`0.6842R`，spread=`+0.5033R`；NDCG@10=`0.7077`、Top-K Lift=`+0.1217R`、Oracle overlap=`58.00%`、Boundary gap=`+0.0290R`。
- 相對13I，13J all-stock daily rho=`+0.0441`、global rho=`+0.1130`、pair=`+1.59pp`、Top-Bottom spread=`+0.5752R`、Top-K Lift=`+0.2565R`；breakout daily rho=`+0.0646`、global rho=`+0.1901`、pair=`+2.93pp`、spread=`+0.7427R`。因Target/universe/horizon/loss完全相同，改善可歸因於顯式risk/economic geometry input；`MR-13J = FORWARD_MODEL_GATE_PASS`。但其all-stock daily rho仍低於frozen MR-13E=`0.2151`，所以不直接promotion，下一關必須看策略轉化。
- 下一個最小必要實驗採三個核心arm：C42=`MR-13E + K/R0 exact` production research base；C49=`MR-13J + 完全相同K/R0 exact`，只換DL source以隔離model effect；C50=`MR-13J + K/exact/canonical cash + No-R0`，只相對C49移除R0以檢驗13J是否已把capital/risk/cost economics upstream到Score。既有`selection_pit` profile仍固定2014～2020、C32/C23/C42，不因新target改寫歷史Gate；C49/C50放在獨立`selection_risk_context` profile；C23只作同Min/all-off DL-off共同基準，核心scientific comparison仍是C42/C49/C50三臂。
- 新identity：`DL-CONT13J-PIT / CONT13J_PIT`、`SR-C49`、`SR-C50`。由於13J risk-normalized target/context的historical-effective risk-param coverage從2014-01-01才開始，而Selection PIT fold本身還需要更早的train/validation history，因此不得宣稱13J PIT可從2014-01-01合法出分。`selection_risk_context`不設定固定起訖，執行時由CONT13E/CONT13J PIT runtime共同coverage自動取交集；禁止current/future Min params backfill到更早日期。C49/C50只允許Selection PIT；在結果前不建立Forward、不開multi-seed、不改production runtime candidate C42/C44。C46/C47/C48的MR-13E No-R0 allocator結論仍維持REJECT。
- C49 primary attribution=`C49-C42`；C50 primary attribution=`C50-C49`，並保留`C50-C42`作最終整體架構比較。判斷R0是否可退休不能只看Return：至少同看Average Exposure、underfilled/position-gap、reserved-capital diagnostics、realized average R、EV、RCE、Return與RoMD。若C50再次出現明顯deployment collapse，即判定13J仍未取代R0；若C49本身已無策略增量，亦不再為13J調context/selector救援。

### 2026-08-16 — MR-13J Selection PIT result / Strategy Compare PIT ownership boundary closure

- 使用者在`[3] 策略組合比較 → Selection PIT 策略比較`執行C42/C49/C50前置時，舊planner因`CONT13J_PIT` top-level工件缺少而自行呼叫checkpoint-only PIT builder；該builder除了重建score／manifest，也直接執行PIT Audit／Model Gate，造成模型驗證報表從Strategy Compare入口輸出。這是工作類型ownership錯誤，不是報表renderer差異。
- 該次實際產出的MR-13J Selection PIT工件合法完成：score period=`2016-04-01～2020-12-31`、folds=`5`、coverage=`455,971/455,971 (100%)`。All eligible stock-days Global Spearman=`-0.0321`、Daily rho=`+0.0490`、Top=`0.7730R`、Bottom=`0.8277R`、spread=`-0.0547R`；Breakout candidates Global/Daily rho=`-0.0250/+0.0770`、spread=`+0.0050R`。年度Global Spearman正向=`3/5`，年度Top-Bottom spread正向=`4/5`。
...
### 2026-08-17 — MR-13J C49/C50 strategy-conversion closure

- 使用者由正式`apps/research.py → [3] 策略組合比較 → Selection PIT 策略比較`完成`selection_risk_context`，共同合法期間=`2016-04-01～2020-12-31`、config fingerprint=`66b78fa6407a`。C23=`Min ROOS`、C42=`MR-13E + K/R0 exact`、C49=`MR-13J + 完全相同K/R0 exact`、C50=`MR-13J + K/exact/canonical cash + No-R0`；因此C49-C42只隔離模型source，C50-C49只隔離R0。
- 核心結果：C42 Return/MDD/RoMD=`104.29% / 12.43% / 8.39`、EV=`0.83R`、平均曝險=`90.42%`；C49=`97.21% / 12.33% / 7.88`、EV=`0.87R`、平均曝險=`88.97%`。C49相對C42 Return=`-7.08pp`、RoMD=`-0.51`、年化=`-0.86pp`，雖MDD改善`0.10pp`、EV `+0.04R`、Log R²=`0.9208 vs 0.8622`、最差完整年度=`+0.32% vs -0.42%`，整體未形成可取代MR-13E的策略增量。資金端C49期末未滿倉日=`750 vs 643`、持股缺口=`1899 vs 1214`，亦顯示13J選股轉化帶來較弱部署。
- R診斷同方向：C49實際平均R=`0.87R`略高於C42=`0.83R`，exclusive DL選擇R=`42.16R vs 23.20R`、RCE=`59.58% vs 33.03%`；但13J PIT本身Daily/Global rho=`0.049/-0.032`、Top-Bottom=`-0.05R`，且較大的exclusive edge沒有轉成較高portfolio Return/RoMD。這足以判定「局部交易品質改善 ≠ 整體資金配置改善」，不需要再新增Audit。
- C50 No-R0明確失敗：Return/MDD/RoMD=`55.61% / 12.43% / 4.47`、EV=`0.47R`、平均曝險=`74.31%`、DL選擇R=`-49.10R`、RCE=`-262.26%`。相對C49 Return=`-41.60pp`、RoMD=`-3.41`、EV=`-0.40R`、平均曝險=`-14.66pp`。雖期末未滿倉日由`750`降至`331`、持股缺口由`1899`降至`646`，代表slot數更容易填滿，但每格實際部署的資金顯著縮小；這直接證明R0的功能不是單純「避免沒股票可買」，而是維持同參數DL-off baseline的reserved-capital floor。13J的`capital_per_risk / cost_per_risk / risk_capacity` context沒有把這個portfolio-level hard constraint可靠地學進Score。
- 決策：`MR-13J = STRATEGY_CONVERSION_REJECTED / NO_RUNTIME_PROMOTION`；`SR-C49 = REJECTED / NO_FORWARD`；`SR-C50 = REJECTED_NO_R0 / NO_FORWARD`。依事前停止條件，13J若C49本身無策略增量就不再以context/selector救援；C50又重現No-R0 deployment collapse，因此不建Forward C49/C50、不開multi-seed、不新增Audit。risk-normalized/context路線結案，current model research回到已promotion的MR-13E，C42/C44與R0維持。
- 工程收斂：`selection_risk_context`退出current Strategy Compare menu；`CONT13J_PIT / C49 / C50 / C49-C42 / C50-C49 / C50-C42`移至`config/compatibility/strategy_compare_history.py`只供歷史工件解讀／重現；schema升至`37`，current Selection恢復`selection_pit: C32/C23/C42`。不刪generic risk-context architecture、risk-normalized target與No-R0 solver implementation，因仍需歷史artifact reconstruction／compatibility；只退役一次性active registration與其experiment-specific synthetic expectation。


### 2026-08-17 — MR-13H Full-horizon No-breach Label simplification implementation

- 程式基準：`test-branch-1_20260817_003828_4bca445.zip`，SHA256=`fd4f399225f0362b5cd7729ee4e5086de01a04d526d014252b9e86cde82d7eb9`；再覆蓋上一輪`mr13j_closure_patch_20260817.zip`，SHA256=`eec8ee4f75cd1932853669ea88e07b95c4cd05052e25dc18f14a412333140554`。本輪在全新解壓基準實作，未取得MR-13H任何模型結果，狀態只能記為IMPLEMENTED。
- 待決策問題：MR-13E的`daily_opportunity_no_time_r_v1`是否因first -10% risk-breach造成離散path truncation，使近似future paths產生大幅label/rank跳變並降低可預測性；在增加cross-sectional architecture以前，先用單一Target變更隔離此不確定性。
- 新identity：`MR-13H / PROFILE-daily_universal_full_horizon_no_breach_full_list_ndcg_pairwise / TARGET-daily_full_horizon_opportunity_r_v1`。Reference固定MR-13E `daily_universal_no_time_full_list_ndcg_pairwise / daily_opportunity_no_time_r_v1`。
- 唯一scientific change：40D內Low首次觸及原risk barrier後，future path**不再截斷**；breach bar只保留diagnostic。Candidate target改為完整40D內earliest maximum High，adverse仍取horizon起點至selected peak之worst Low；target仍為`favorable / 10% R scale - adverse / 10% R scale`，不加time penalty。無breach path的新舊Target必須完全一致。
- 固定條件：`daily_eligible_stock_days` universe、300×10 lazy stock/0050 sequence、`inception_time_v1`、40-bar horizon、同一10% R scale、同一adverse-to-peak、`daily_pairwise_ranking`、`pairwise_logistic`、`full_list_delta_ndcg_weighted` reduction、`mean_daily_spearman` epoch selection、all-label scope、Seed42與既有Train/Validation/OOS policy全部沿用MR-13E。不得加入high_len、candidate membership、K、R0、cash、holdings或selector state。
- Dataset／Label rebuild需求：不建立expanded daily feature artifact，也不需要重建event Dataset；daily target仍由canonical sanitized OHLCV lazy materialize，既有future source已覆蓋40D。新Target有獨立identity/contract，不覆寫MR-13E Target工件。
- 新增正式只讀Label Audit：模型訓練選單在active research profile有reference時顯示「比較目前 Target 與 reference Target」。報表固定比較共同stock-day的Global/Daily rank correlation、Top-10%/Top-K overlap、40D breach率、breach後candidate peak較晚比例、target changed rate、delta distribution與no-breach invariant；並用config-driven `1%` band直接對照canonical `-10%` barrier上下兩側（預設`-9%～-10%` vs `-10%～-11%`）的target均值、ΔR與changed rate，檢查label cliff。不設自動GO/REJECT magic threshold、不訓練模型。
- Forward模型報表另新增checkpoint後的common-target reference evaluation：同一MR-13H scores同時對candidate target與MR-13E reference target算OOS/breakout ranking diagnostics；reference target不得參與loss、gradient、epoch selection或final refit。
- 階段治理：MR-13H尚未通過Forward Model Gate，因此未加入`BREAKOUT_QUALITY_CONTINUOUS_RANKER_PIT_GATE_PROFILES`，active Model Research選單不提供Selection PIT入口。Production/runtime workflow維持MR-13E + exact K/R0 constrained；Strategy Compare不新增MR-13H arm。
- 下一步：先執行Label Audit確認新舊label是否有實質rank差異與breach cliff；若證據支持繼續，再以同一MR-13E training contract訓MR-13H Forward。只有Forward模型證據成立才授權Selection PIT；cross-sectional architecture繼續後排。
### 2026-08-17 — MR-13H Label Audit + Seed42 Forward GO → Selection PIT authorized

- 程式基準：`test-branch-1_20260817_015333_b309e28(2).zip`，SHA256=`a89ae89d18fdae8d01321423cf7cfae7b6511cd1fb1e31d91a946050dad32eb8`。本輪不改MR-13H Target、architecture、loss、Seed或production workflow，只依已取得證據推進研究stage。
- Label Audit正式結果：common stock-days=`1,578,349`、40D risk breach=`31.31%`、Target changed=`6.65%`、candidate-vs-reference Daily rank correlation=`0.9435`、Top-10 overlap=`94.53%`、Top-K overlap=`94.53%`、Mean abs delta=`0.0714R`。這表示first-breach truncation只改變少量樣本，但仍保留足夠受控差異供實際訓練驗證。
- Seed42 Forward正式結果：selected epoch=`1`；validation refit Daily rho=`0.2603`；all-stock OOS Daily/Global rho=`0.2332/0.1369`、Pair=`58.04%`、Top10%-Bottom10% Target=`+1.0442R`、Top-K Lift=`+1.3400R`；breakout slice Daily/Global rho=`0.2056/0.1775`、Pair=`58.86%`、Top10%-Bottom10% Target=`+1.6044R`、Top-K Lift=`+0.5061R`。
- Frozen MR-13E Forward reference為all-stock Daily/Global rho=`0.2151/0.1042`、Pair=`57.42%`、Top10%-Bottom10%=`+0.8234R`、Top-K Lift=`+0.9951R`；MR-13H在self-target主要ranking指標一致改善。更重要的是同一MR-13H score對MR-13E reference Target仍有Daily rho=`0.2140`、Pair=`57.41%`，與MR-13E本身`0.2151/57.42%`幾乎相同，因此目前沒有看到以較容易Target換取舊ranking能力流失。
- Decision：`MR-13H = FORWARD_OOS_MODEL_GATE_PASS / SELECTION_PIT_AUTHORIZED`。PIT不加入13C/D/E batch，因其Target identity不同；由`ContinuousRankerResearchSpec.selection_pit_authorized=True`走單profile `[1] 模型訓練 → [2] 建立／更新 Selection PIT Scores → PIT模型驗證`。
...
### 2026-08-17 — MR-13K Selection PIT Model Gate PASS → C53 strategy translation ready

- 程式基準：`test-branch-1_20260817_174846_9cffdd4.zip`，SHA256=`dd2ecaef67481284ed64cd0f0b183739d2810569404fb756390f7a19b38f59d9`。本輪不改MR-13K Target、architecture、loss、Seed、PIT fold contract、K/R0、exact solver或production C42/C44；只記錄正式PIT結果並修正App簡易報表在audit尚未產生前的coverage顯示。
- Selection PIT正式結果：自動解析最早合法score date=`2013-04-01`；period=`2013-04-01～2020-12-31`、folds=`8`、score groups=`778,532`、coverage=`100.00%`。首fold best epoch=`3`，其餘7 folds best epoch=`1`；各fold Validation rho依序=`0.1221/0.1861/0.2909/0.2836/0.2467/0.2187/0.2946/0.3368`。
- PIT Model Gate：All eligible stock-days Global/Daily rho=`0.1869/0.2877`、Pair=`60.13%`、Top-Bottom spread=`+0.8284R`；年度rho>0=`8/8`、spread>0=`8/8`。Breakout slice Global/Daily rho=`0.1970/0.2081`、Pair=`59.55%`、Top-K Lift=`+0.2519R`、Boundary=`54.92%`。四項事前model Gate方向條件全部PASS。
- `drift=True`只代表至少一組相鄰fold score平均值位移達`>=1.0 pooled score SD`；current Gate刻意只判斷global/daily ranking正向與多數年度rho/spread正向，因此drift是calibration/distribution diagnostic，不是strategy replay blocker。若C53後續策略結果異常，才有理由回頭讀fold drift明細；目前不新增Audit。
...
### 2026-08-17 — MR-13K C54 Forward single-seed result

- Forward-OOS controlled replay期間=`2021-01-01～2026-03-02`；C54與C44固定同一Min params、all-off rules、K/R0、canonical sizing/cash/orderability/execution與`exact_branch_and_bound_v1`，唯一變因是frozen DL source `MR-13K` vs `MR-13E`。
- C54結果：Return=`168.67%`、MDD=`19.95%`、RoMD=`8.45`、Annual=`21.12%`、worst full year=`-12.07%`、Log R²=`0.8384`、monthly win=`60.32%`、win rate=`42.55%`、Payoff=`3.41`、EV=`0.80R`、trades=`329`、average exposure=`92.96%`。
- C44 reference：Return=`214.29%`、MDD=`15.13%`、RoMD=`14.17`、Annual=`24.86%`、worst full year=`-4.07%`、Log R²=`0.9090`、monthly win=`69.84%`、win rate=`46.13%`、Payoff=`3.40`、EV=`1.03R`、trades=`310`、average exposure=`92.80%`。
- 模型ranking與portfolio economics呈反向：C54 Daily/Global rho=`0.383/0.266`、Pair=`63.19%`、Top-Bottom=`1.74R`，高於C44的`0.215/0.104`、`57.42%`、`0.82R`；但C54 Return/RoMD/EV/MDD均劣於C44。Payoff幾乎相同（`3.41 vs 3.40`）而win rate下降（`42.55% vs 46.13%`），支持Pure-MFE能抓upside magnitude但缺少path realizability資訊的機制解釋。
- 年度C54-C44：2021=`8.85%-19.53%`、2022=`-12.07%-(-4.07%)`、2023=`110.58%-85.87%`、2024=`10.69%-22.51%`、2025=`10.30%-11.64%`、2026 YTD=`9.19%-7.82%`；13K具有regime-dependent upside capture，但不是跨期穩定優勢。
- Decision：單次Selection與Forward均不支持MR-13K promotion，但依使用者明確要求不在single-seed提前結案；current Selection `C42/C53`與Forward `C44/C54` 16-seed paired robustness仍照既定設定完成，再做13K最終判讀。Production/runtime仍固定C42/C44。

### 2026-08-17 — MR-13K mechanism diagnosis → MR-13L decomposed MFE/adverse learning formulation implemented

- 程式基準：`test-branch-1_20260817_192304_690164e.zip`，SHA256=`a4bbc11ff7451400736314a37c9d3453a686d71797c1d02986ca17849f765da1`；fresh extraction=`/tmp/mqp13k_next`（僅GPT工作環境）。依`doc/PROJECT_SETTINGS.md`先讀Registry/Log後才新增identity；本輪不執行`apps/run_bundle.py`或`apps/test_suite.py`。
- 研究動機：MR-13K Seed42已形成清楚mechanism split：Pure-MFE self-target learnability大幅提高（Forward Daily rho=`0.3833`、Pair=`63.19%`；Selection PIT Daily rho=`0.2877`、Pair=`60.13%`），但C53 Selection與C54 Forward的portfolio translation均低於同條件MR-13E exact reference。這支持「upside signal可學，但path realizability資訊不能直接刪除」；下一個最小問題是MR-13H難學是否來自把性質不同的MFE與adverse直接壓成單一scalar ranking target，而不是adverse component本身完全不可學。
- 新identity：`MR-13L / PROFILE-daily_universal_full_horizon_mfe_adverse_dual_mse / ARCH-inception_time_v1`。Reference固定MR-13H；Target仍是`daily_full_horizon_opportunity_r_v1`。MR-13L不是新的economic Target，也不是architecture變更。
- 唯一scientific change：原MR-13H直接以pairwise ranking學`MFE R - adverse-to-peak R`；MR-13L改用`daily_dual_component_r_regression / dual_mse_raw_r`，將InceptionTime既有兩個output neurons分別定義為`predicted adverse-to-peak R`與`predicted favorable MFE R`。兩者都是primary supervised outputs，loss固定為兩分量MSE的同一mean reduction；沒有auxiliary loss、沒有auxiliary weight、沒有lambda／penalty coefficient掃描。正式model score固定為`Predicted favorable MFE R - Predicted adverse R`，epoch selection仍只依recombined score對同一MR-13H Target的Validation mean daily Spearman。
- Dataset contract：canonical daily feature-eligible stock-day universe、300×10 lazy feature window、40D full horizon、fixed canonical risk-budget R scale、Seed policy與`inception_time_v1`全部固定。`daily_ranker_data.py`只把已存在的`target_favorable_return / target_adverse_return_to_peak`按同一fixed risk-budget轉成`target_favorable_r / target_adverse_r`供primary regression；兩分量逐筆重組必須等於同一MR-13H raw Target。Future component targets只進training/evaluation，不進runtime feature或decision-time input。
- 與舊v9 auxiliary-head實驗的差異：v9是binary classifier主目標外掛`mfe_progress / mae_risk_fraction / reward_risk_progress` auxiliary loss，且有額外loss weight；MR-13L則直接把兩個物理R分量本身設為primary objective，沒有binary主loss、沒有auxiliary branch、沒有調權重。因此不是重做已REJECT的v9 auxiliary路線。
- 報表／artifact contract：Forward OOS score額外寫出`predicted_favorable_r / predicted_adverse_r / predicted_composite_r`；report新增Validation、Forward OOS與breakout-candidate slice的兩分量RMSE/MAE/bias/global rho/daily rho。Frozen model manifest/report透過`dual_component_r_regression_contract`固定output mapping、equal-mean loss與runtime score semantics。
- Stage governance：`selection_pit_authorized=False`。MR-13L目前只授權Seed42 Forward model Gate，不建立PIT DL source、不建立Strategy Compare arm、不改production C42/C44。只有當adverse component呈現可重現正向signal，且recombined same-target ranking對MR-13H形成可信增量時，才討論Selection PIT；否則直接在模型層結案。MR-13K既有C42/C53與C44/C54 16-seed full-flow robustness是獨立current strategy evaluation，仍照使用者既定決策完成，不因MR-13L implementation提前停止。

### 2026-08-17 — MR-13L Seed42 Forward result → rejected at model Gate

- 程式基準：`test-branch-1_20260817_213957_d6bddc0.zip`，SHA256=`c5c96c26ad643ba6c75e3f4ab630986974c454c14a137090c630b05026881c37`。使用者完成`daily_universal_full_horizon_mfe_adverse_dual_mse / MR-13L` Seed42 Forward training；Target、40D horizon、fixed R scale、daily universe與`inception_time_v1`均維持MR-13H不變。
- Training：epoch1 Train Loss=`0.614691`、選模Validation Daily rho=`0.0227`為最佳；epoch2 Train Loss降至`0.322042`但Validation Daily rho降至`0.0218`，patience=1後停止，selected epoch=`1`。完整Selection refit固定1 epoch。Final refit後原Validation Daily rho=`0.3069`因該rows已被納入final refit，只是描述性in-sample metric，不是選模或泛化證據。
- Frozen Forward all-stock：Daily rho=`0.1052`、Global rho=`0.0254`、Pair=`53.51%`；Top 10% Target=`0.9142R`、Bottom 10%=`0.9292R`，故Top-Bottom=`-0.0150R`；NDCG@10=`0.5243`、Top-K Target=`1.2386R`、Lift=`+0.2274R`、Oracle overlap=`4.02%`、Boundary=`48.62%`、Boundary gap=`-0.0386R`。
- Breakout-candidate OOS：groups=`17,346`、Daily/Global rho=`0.0647/-0.0301`、Pair=`52.82%`；Top 10%=`1.1016R`、Bottom 10%=`1.3576R`，Top-Bottom=`-0.2560R`；NDCG@10=`0.6779`、Top-K Lift=`+0.1196R`。
- Controlled comparison：MR-13L與MR-13H使用完全相同economic Target。MR-13H Seed42 Forward all-stock Daily rho=`0.2332`、Pair=`58.04%`、Top-K Lift=`+1.3400R`，因此13L的dual raw-R MSE formulation在same-target ranking明顯退化。更強的反證是MR-13K雖只訓練Pure-MFE Target，其frozen score對MR-13H reference Target仍有Daily rho=`0.2381`、Pair=`58.24%`，亦明顯高於13L直接分解同一MR-13H Target後的`0.1052/53.51%`。
- Interpretation：這次結果否決的是「以兩個primary raw-R MSE heads等權回歸，再直接相減即可改善MR-13H learnability」這個learning formulation；不能僅由composite失敗推論adverse-to-peak本身不可預測。component-level MFE/adverse individual rho/RMSE已由詳細`continuous_ranker_report.md`保存，但使用者本次貼出的console沒有列出精確component值，故本Log不自行補值。
- Decision：`REJECTED_AT_FORWARD_MODEL_GATE / NO_PIT / NO_RUNTIME_DL_SOURCE`。不建立MR-13L PIT、不做Selection Strategy Compare、不做MR-13L multi-seed。Production/runtime仍固定MR-13E + Min/all-off + exact K/R0；MR-13K既定C42/C53與C44/C54 robustness仍獨立完成。
- Current config reconciliation：本輪最新基準`config/strategy_compare.py`的正式SSOT目前為`STRATEGY_COMPARE_ROBUSTNESS_SEED_COUNT=8`、`STRATEGY_COMPARE_ROBUSTNESS_SEED_GENERATOR_SEED=20260810`。因此後續由`[5] 一次執行全部 Multi-seed robustness`啟動的新run應以8 seeds為準；先前Log中「16-seed authorized」保留為當時研究計畫歷史，不得反向覆寫目前config。
- 下一個最小研究問題：先隔離「adverse-to-peak是否具有可重現cross-sectional ranking signal」，而不是再調dual-MSE權重或回歸loss。若下一個實驗啟動，優先使用單一adverse-only、same-day pairwise ranking的受控模型，固定現有features/architecture/horizon/risk scale；只有adverse-only Forward signal成立，才有理由研究與已證實可學的Pure-MFE ranking如何組合。

### 2026-08-17 — MR-13L closure → MR-13M adverse-only pairwise Forward experiment implemented

- 程式基準：`test-branch-1_20260817_220752_14e6bc7.zip`，SHA256=`1a012981c38b60b1e6dfe50794518a56fcd9bbb5815a6a16452207525490b402`；fresh extraction=`/tmp/mqp13m`（僅GPT工作環境）。依`doc/PROJECT_SETTINGS.md`先讀Registry/Log後才新增identity；本輪不執行`apps/run_bundle.py`或`apps/test_suite.py`。
- 待決策問題：MR-13L只否決dual raw-R MSE decomposition，仍不能判斷adverse-to-peak本身是否可預測。MR-13K已證明Pure-MFE具高learnability但portfolio path realizability不足，因此下一個最小必要實驗只隔離path-risk訊號，不再調dual-MSE loss／component weight／lambda。
- 新identity：`MR-13M / PROFILE-daily_universal_full_horizon_low_adverse_full_list_ndcg_pairwise / TARGET-daily_full_horizon_low_adverse_r_v1 / ARCH-inception_time_v1`。Target固定=`- adverse-to-peak R`，越高代表到達40D selected max-MFE peak前的逆行越小。
- 唯一scientific change：相對MR-13H固定40D full horizon、earliest maximum-high peak、risk-breach diagnostics、fixed canonical R scale、daily feature-eligible stock-day universe、300×10 lazy sequence、InceptionTime、Full-list Delta-NDCG weighted RankNet、mean-daily-Spearman epoch selection與Seed42；只把排序Target改為negative adverse R。MFE只用來選定future peak，不進reward；不加入breakout qualification、K、R0、cash、holdings或selector state。
- Stage governance：`selection_pit_authorized=False`。目前只允許Seed42 Forward model Gate；不建立PIT、DL source或Strategy Compare arm。若Forward all-stock與breakout slice的Daily rho／Pair／Top-K separation沒有明確正向signal，直接REJECT adverse-only learnability方向；若成立，再另開新的受控組合實驗，不在MR-13M內調MFE/adverse權重。Production/runtime固定MR-13E C42/C44；MR-13K current 8-seed robustness獨立繼續。


### 2026-08-17 — MR-13M Seed42 Forward result → path-risk learnability confirmed

- 程式基準：`test-branch-1_20260817_221751_79dc1d2.zip`，SHA256=`8e800f703d825bdbfcf165f9b3f6e8023be9570db347bbee6cbb533d5074a3f3`；fresh extraction=`/tmp/mqp13m_result`（僅GPT工作環境）。MR-13M維持MR-13H/13K相同40D full horizon、earliest max-MFE peak、fixed canonical R scale、daily feature-eligible stock-day universe、300×10 lazy sequence、InceptionTime與Full-list Delta-NDCG RankNet；唯一Target=`-adverse-to-peak R`。
- Training：epoch1 Validation Daily rho=`0.2861`；epoch2=`0.3443`為最佳；epoch3=`0.3270`，selected epoch=`2`。完整Selection refit固定2 epochs。
- Frozen Forward all-stock：Daily/Global rho=`0.3558/0.2999`、Pair=`62.75%`；Top 10% Target=`-0.2069R`、Bottom 10%=`-0.5961R`，故Top-Bottom=`+0.3892R`；NDCG@10=`0.7616`、Top-K Target=`-0.1190R`、Lift=`+0.2300R`、Oracle overlap=`6.79%`、Boundary=`48.44%`、Boundary gap=`-0.0066R`。
- Breakout-candidate OOS：groups=`17,346`、Daily/Global rho=`0.3388/0.3737`、Pair=`63.70%`；Top 10%=`-0.1413R`、Bottom 10%=`-0.6269R`，Top-Bottom=`+0.4856R`；NDCG@10=`0.7859`、Top-K Lift=`+0.1115R`、Oracle overlap=`64.25%`、Boundary=`54.90%`、Boundary gap=`+0.0782R`。
- Controlled interpretation：MR-13K Pure-MFE Forward Daily rho=`0.3833`、Pair=`63.19%`；MR-13M low-adverse=`0.3558/62.75%`，兩個分量都具有很強且相近的cross-sectional learnability。MR-13H直接排序`MFE R - adverse R`只有Daily rho=`0.2332`、Pair=`58.04%`，因此13H較難學不能再歸因於adverse component沒有signal；更合理的機制是假說為「兩個各自可學、但在raw-R差值所誘導的composite ordering中形成較難預測的trade-off/residual」。此機制仍屬推論，尚未由component joint-distribution Audit直接證實。
- Decision：`PATH_RISK_LEARNABILITY_CONFIRMED / COMPONENT_FORWARD_GO / NO_STANDALONE_PIT`。MR-13M不建立standalone PIT或strategy arm，因其只獎勵path safety而沒有upside magnitude，單獨portfolio結果不能回答最終選股目標。下一個最小必要實驗應保留MR-13K與MR-13M已證實的個別rank signal，將MFE與low-adverse轉成同日共同rank scale後作固定等權組合；不掃lambda、不改K/R0/cash/execution。若該組合仍無法改善同一MR-13H economic Target的Forward ranking，才考慮更複雜的multi-head／score ensemble；不要先做MR-13M standalone PIT。
- MR-13K既有Selection `C42/C53`與Forward `C44/C54` current 8-seed paired robustness仍是獨立研究線，照current config完成；production/runtime仍固定MR-13E C42/C44。

### 2026-08-17 — MR-13M result → MR-13N fixed equal-rank MFE + low-adverse experiment implemented

- 程式基準：`test-branch-1_20260817_224751_faf8b5e.zip`，SHA256=`2d7cde7a9f8675168ae6eef92749eabb9cbdadd86b66b965a6134fc6e26c1820`；fresh extraction=`/tmp/mqp13n`（僅GPT工作環境）。依`doc/PROJECT_SETTINGS.md`先讀Registry/Log並先在Registry占用`MR-13N`後才修改程式；本輪不執行`apps/run_bundle.py`或`apps/test_suite.py`。
- 待決策問題：MR-13K Pure-MFE Forward Daily rho=`0.3833`、Pair=`63.19%`；MR-13M low-adverse Forward Daily rho=`0.3558`、Pair=`62.75%`，證明兩個component個別都具強cross-sectional learnability。MR-13H直接排序raw `MFE R - adverse R`只有Daily rho=`0.2332`、Pair=`58.04%`，MR-13L raw-R dual MSE再相減更降至`0.1052/53.51%`。下一個最小問題是把兩個可學component放到共同rank scale後固定等權組合，是否能避免raw-R magnitude/calibration trade-off。
- 新identity：`MR-13N / PROFILE-daily_universal_full_horizon_equal_rank_mfe_low_adverse_full_list_ndcg_pairwise / TARGET-daily_full_horizon_equal_rank_mfe_low_adverse_v1 / ARCH-inception_time_v1`。
- Target contract：每個target-valid stock-day保留MR-13K/MR-13M相同40D complete horizon、earliest maximum-MFE peak、risk diagnostics與fixed canonical R scale。每天先各自建立`MFE R`與`-adverse-to-peak R`的average-rank percentile `[0,1]`，再固定`0.5 × MFE percentile + 0.5 × low-adverse percentile`；single-stock day兩component percentile固定`0.5`。權重是實驗定義，不由Train/Validation/OOS調整，不掃lambda。
...
### 2026-08-18 — MR-13O Pareto-dominance pairwise experiment implemented

- 程式基準：`test-branch-1_20260818_001037_841d25c.zip`，SHA256=`ae6c2327c0bc84bf2c757a3794cfeecb737ab4e6c74f09862c671ec0955dd1aa`；先疊加已完成的`mr13km_frozen_fusion_closure_patch_20260818.zip`（SHA256=`251a4a42c82908d2e65a5513c8c283a516912303f1543611ead188b040e5bffe`）並依該結案指令移除一次性`tools/audit/breakout_quality/frozen_rank_fusion.py`，fresh extraction=`/tmp/mqp_pareto`（僅GPT工作環境）。依`doc/PROJECT_SETTINGS.md`先讀Registry／Log並確認`MR-13O`未占用後才建立identity；本輪不執行`apps/run_bundle.py`或`apps/test_suite.py`。
- 待決策問題：MR-13K Pure-MFE與MR-13M Low-Adverse都能各自學出強Forward ranking signal，但MR-13N single-model fixed equal-rank composite與後續50/50 frozen expert fusion都近似隨機。Fusion Audit又顯示true component relation僅輕度反向（Daily rho=`-0.1261`），frozen scores卻高度鏡像（`-0.8649`），而true equal-rank Target對MR-13H economic Target高度一致（`0.8537`）。因此下一個最小實驗不再建立另一個scalar mix或掃權重，而只教模型「兩component都明確同方向勝出」的joint dominance。
- 新identity：`MR-13O / PROFILE-daily_universal_full_horizon_pareto_mfe_low_adverse_pairwise / OBJECTIVE-daily_pareto_pairwise_ranking / ARCH-inception_time_v1`。Economic Target identity直接沿用MR-13H `daily_full_horizon_opportunity_r_v1`供結果解讀；不新增新的scalar Label/Target ID。
- Supervision contract：每個target-valid stock-day從canonical `target_favorable_r / target_adverse_r`建立同日`MFE percentile`與`low-adverse percentile=(-adverse R)`。對同一天任兩支股票A/B，只有A兩component都嚴格高於B或兩component都嚴格低於B時才形成RankNet pair；component互有優劣或任一component tie的pair完全排除、沒有gradient。所有comparable pairs等權，沒有MFE/adverse mix coefficient、lambda、K、R0、strategy state或future portfolio information。
- Epoch-selection contract：新增`mean_daily_pareto_pair_concordance`，只在Validation strict Pareto-comparable pairs上量score方向一致率；global Pareto concordance只作tie-break。MR-13H economic Daily rho／Pair／Top-K仍由同一model score對`daily_full_horizon_opportunity_r_v1`描述性計算，但不得參與early stopping／best-epoch selection。Final refit仍依selected epoch使用完整Selection重訓。
- Reporting／artifact contract：MR-13O沿用既有scalar `model_score` artifact，另外在Forward report／simple report輸出Validation、OOS、breakout slice的comparable-pair count/rate與Daily/Global Pareto concordance；economic Target metrics仍保留，便於判斷「Pareto supervision可學」與「economic ordering有效」是否同時成立。
- Stage governance：`selection_pit_authorized=False`。目前只允許Seed42 Forward model Gate，不建立PIT、DL source或Strategy Compare arm。MR-13K Selection/Forward current 8-seed robustness（generator seed=`20260810`）保持獨立進行；production/runtime固定MR-13E C42/C44。若Pareto concordance本身接近隨機，直接停止此formulation；若Pareto可學但economic ordering無增量，也不進PIT。只有兩層Forward證據都成立，才另輪正式授權Selection PIT。


### 2026-08-18 — MR-13O formal bundle stale retired Audit cleanup
...
### 2026-08-18 — MR-13O Seed42 Forward result → weak Pareto edge, rejected at model Gate

- 程式基準：`test-branch-1_20260818_100207_a009b17.zip`，SHA256=`82a44233325d97281f4a62ccfc59b8f7a777a2ad3d2a84f349357de5e88f3d74`。MR-13O固定MR-13H economic Target作描述性評估，training只使用strict Pareto-comparable MFE/low-adverse pairs，epoch selection只看Validation Pareto concordance。
- 結果：selected epoch=`1`；Validation Pareto=`51.14%`、selected-epoch economic rho=`0.0896`；Forward economic Daily/Global rho=`0.0782/0.1008`、Pair=`52.59%`、Top10−Bottom10=`+0.6042R`、Top-K Lift=`+0.1120R`；Forward Pareto=`51.81%`。Breakout economic Daily rho=`0.0376`、Pareto=`50.30%`。
- 判讀：Validation→Forward Pareto沒有明顯崩壞，代表不是典型overfit；但edge穩定地只有約`+1.8pp`，breakout幾乎隨機，且economic ordering遠低於MR-13H/K。`REJECTED_AT_FORWARD_MODEL_GATE / NO_PIT / NO_RUNTIME_DL_SOURCE`。至此停止繼續修改MFE/adverse scalar Target、equal-rank、Pareto pair mask或loss小改。
- 下一研究分流：方案A＝固定MR-13H Target，只改parameter-free path-dynamics input representation；方案B＝保留13K/13M specialist models，在portfolio layer分工。使用者決定先進行方案B。

### 2026-08-18 — Plan B `SR-C55` implemented: MR-13K objective + MR-13M baseline-relative safety floor

- 待決策問題：C54的MR-13K可預測Pure-MFE很強，但Seed42 Forward win rate／EV／RoMD與Return低於C44；MR-13M low-adverse又具強獨立learnability。要測的是「不把兩個score相加」的情況下，能否讓13M只作guardrail，保留13K大winner upside同時減少path-risk差的選股。
- 新identity：`DL-CONT13M`＝既有MR-13M frozen Forward score的research-only secondary source；`SR-C55`＝Forward-only dual-model safety constrained arm。未新增MR、未重訓模型。
- 唯一controlled change：C55完全沿用C54的`min_roos / all_off / CONT13K primary score / K / R0 / canonical sizing / cash / orderability / execution / exact_branch_and_bound_v1`。新增hard contract `baseline_coverage_and_score_sum_floor_v1`：同日C3 DL-off Min ROOS baseline已選basket的MR-13M score coverage與score-sum形成safety floor；C55 exact search仍最大化MR-13K score-sum，但合法basket不得低於該floor。13M分數不加入objective、不調lambda、不設absolute score threshold。
- Solver／provenance：secondary `DL-CONT13M`納入required artifact source、comparison-period readiness與pair-cache fingerprint；selector diagnostics記錄baseline/selected safety coverage、score sum/mean、binding/violation與safety-pruned states。若baseline safety coverage不足，只要求至少同等coverage；不得以missing score規避已存在的baseline evidence。
- Stage：schema=`39`；Forward current=`C1/C3/C44/C54/C55`，primary contrast=`C55-C54`。MR-13K 8-seed robustness仍只跑C42/C53、C44/C54，不把C55混入。Production仍C42/C44。C55先跑Seed42 Forward；只有C55對C54形成可信portfolio translation改善才建立MR-13M Selection PIT與Selection dual-model arm，否則Plan B直接結案。

### 2026-08-18 — Research Queue governance introduced; `doc/ToDo.md` excluded from GPT research planning
...
## 2026-08-18 — C56 full-flow validation activated; C54/C55 removed from current test workload

- 使用者明確決策：在進Plan C-M前先把`SR-C56`完整測完，且`C54/C55`不用同行重訓／replay。工作基準=`test-branch-1_20260818_165751_ec32ec7.zip`，SHA256=`eb4d9267429fc582a1e7e4f03e69fa404093bc2d05eca14f9b4c956838ed751c`，fresh extraction=`/tmp/mqp_c56_full`；本輪先讀`doc/PROJECT_SETTINGS.md`、Registry、Experiment Log、Research Queue，未讀`doc/ToDo.md`。
- Seed42 Forward既有證據不變：C56 Return/MDD/RoMD=`148.34%/12.86%/11.53`、Win=`47.90%`、EV=`1.14R`，B2 concept已確認但未promotion。新的待決策問題是**C56 architecture本身是否跨Selection/Forward與跨seed穩定**，不是再比較C54/C55。
- 新增`SR-C57`作C56的Selection PIT counterpart。C57與C56固定相同Min/all-off、K/R0、canonical sizing/cash/orderability/execution、exact branch-and-bound與`same_day_rank_ols_v1` residual-safety contract；唯一stage差異是C57使用`CONT13K_PIT + CONT13M_PIT`且`selection_only=True`，C56使用`CONT13K + CONT13M`且`selection_only=False`。Current Selection=`C32/C23/C42/C57`；Current Forward=`C1/C3/C44/C56`。C53/C54/C55與只服務其舊比較的contrasts移至historical compatibility，production仍C42/C44。
- `MR-13M selection_pit_authorized=True`只為C56 full-flow secondary use授權；新增`DL-CONT13M-PIT / CONT13M_PIT`，不得解讀成13M standalone strategy/promotion。模型選單的「準備策略比較所需模型工件」改由所有current arms解析primary與secondary artifact dependencies，因此C57會同時要求合法`CONT13K_PIT`與`CONT13M_PIT`，Strategy Compare本身仍不得checkpoint-only重建PIT。
- 8-seed robustness scientific contract升級為dual-model same-seed：Selection fixed=`C32/C23`、stochastic=`C57`；Forward fixed=`C1/C3`、stochastic=`C56`；`seed_count=8`、`seed_generator_seed=20260810`、paired contrasts皆空。每個generated seed必須分別建立兩個training units（13K與13M），只有同一seed的primary+secondary都READY後才queue該C57/C56唯一一個strategy replay。故每stage為16 model-training units + 8 stochastic replay units，而不是把13M固定Seed42。Scientific fingerprint/observation保存完整runtime-source identities，避免不完整robustness。
- Runtime dual-source contract同步支援`selection_point_in_time`與`continuous_ranker_oos`，secondary PIT source各自pin score/manifest path；secondary filter/profile/architecture/source identity完整驗證。`Strategy Compare` schema=`41`。
- GPT targeted verification：`validate_strategy_compare_config_driven_app_contract_case`使用canonical `V16StrategyParams()`得到`136/136 PASS`，包含C57雙PIT pinned lookup、current/historical catalog隔離、兩stage每seed雙source training group與single-replay readiness gate。尚未執行`apps/run_bundle.py`或`apps/test_suite.py`；正式整合測試仍由使用者套用ZIP後執行。
- 執行順序固定：先`模型訓練 → 準備策略比較所需模型工件`補齊C57所需PIT sources，再跑Selection PIT C57 single-seed，最後跑Selection與Forward 8-seed robustness（可用「一次執行全部 Multi-seed robustness」）。Plan C-M移到C56 full-flow完成之後；不得依途中seed結果改residual formula、threshold、seed或objective。

## 2026-08-18 — Evaluation Framework Migration: Operational Rolling + Fixed-Window Stability replaces Selection/Frozen split

- 待決策問題：舊框架把2014～2020的annual expanding PIT與2021～2026的one-time Frozen Forward當成兩個頂層stage；前者每12M重新訓練且早期training history較少，後者用完整Selection history訓練一次後多年不更新。這使跨stage差異同時混入training-data maturity、refit policy與market regime，且Frozen多年不更新並非實際交易會採用的deployment policy。
- 現有證據：MR-13K historical PIT Daily rho=`0.2877`、Pair=`60.13%`，Legacy Frozen Forward Daily rho=`0.3833`、Pair=`63.19%`；MR-13M PIT Daily rho=`0.3034`、Pair=`60.96%`。但strategy conversion亦有明顯period effect：DL-off Min ROOS historical C23 EV=`0.42R`，Legacy Frozen Forward C3 EV=`1.02R`。因此不能把PIT/Frozen差距直接解讀為「模型無法學習早期市場」。
- C57 historical expanding PIT Seed42結果已取得：Return/MDD/RoMD=`114.09%/20.21%/5.64`、Annual=`11.49%`、Worst Full Year=`-3.35%`、Win=`35.09%`、EV=`0.53R`；同時C56 Legacy Frozen Forward=`148.34%/12.86%/11.53`、Win=`47.90%`、EV=`1.14R`。這兩組數值永久保留，但不再把其直接差值當成B2 cross-period stability Gate。
- 使用者決策：不再維持active `Selection PIT vs Frozen Forward-OOS`二分。正式時間驗證改為：1) **Operational Rolling**＝expanding history + PIT-safe periodic/annual refit，作為接近實際操作的主要strategy truth；2) **Fixed-Window Stability**＝固定calendar training window + 相同refit cadence，作為控制history length後的historical stability/learnability diagnostic。Frozen Forward只保留Legacy historical evidence，不再為新模型跑strategy Gate或robustness。
- OOS重新定義：以每一個score fold的information cutoff為準，而不是以某個人為年份切割。Train features、Target completion、inner validation/epoch selection、normalization/calibration、strategy parameter fitting及其他自動決策均不得使用score date之後資訊；保留inner validation。OOS可被研究者反覆查看形成下一實驗假說，但不得自動把future結果回填到當下fold。
- Artifact migration：2014～2020既有expanding annual PIT folds在contract相容時直接REUSE，不重訓；Operational以相同語意向2021+延伸。首次延伸canonical aggregate前保存`legacy_selection_snapshot/`。Fixed Stability預設train window=`120M`、fold=`12M`、inner validation=`24M`，使用獨立artifact path/fingerprint，不覆寫Operational。
- Strategy parameter migration：新增`P2_OPERATIONAL` stitch builder，只合併既有且各自合法的historical/current Min ROOS rolling schedules；要求train-window/horizon/search contract一致與transition無缺口，不重新最佳化歷史段，避免後段資料反向污染前段。
...
### 2026-08-19 — Model-training focus corrected + Rolling menu visibility policy
...
## 2026-08-19 — Rolling execution correction: complete-host prefetch與2-fold process parallel均退役
...
- 第一個Decision：`COMPLETE_HOST_PREFETCH_REJECTED_BY_RUNTIME_OBSERVATION / ROLLED_BACK`。`services/breakout_quality/train_continuous_ranker.py`恢復原本feature-only ordered prefetch；`train_prefetch_batches=8 / train_prefetch_workers=4`、pinned feature、context/target主執行緒staging、dedicated CUDA copy stream與non-blocking H2D維持。沒有改batch membership/order、same-date grouping、seed、optimizer step、loss、target、architecture、deterministic algorithms或TF32。
- 接著曾以`test-branch-1_20260819_011641_b24c3a5.zip`為基準新增不同Rolling folds的2-process平行執行：父程序先判定REUSE／checkpoint-rescore／legacy migration，只把缺少fold交給`spawn`獨立process，各process只寫自己的fold目錄，完成後再由父程序以原fingerprint/hash validator讀回並排序aggregate；Multi-seed外層已有GPU trainer parallelism時另強制nested fold worker=1。
- 使用者實機套用2-fold版本後回報：**總體變得更慢、VRAM用滿、GPU使用率約40%**。這組觀察表示單張RTX 5080上兩個獨立CUDA training process沒有填補compute idle，反而把顯存與CUDA execution資源推到飽和；由於沒有逐kernel profiler證據，根因只記為「與VRAM pressure／CUDA context與allocator contention相符」，不把單一機制寫成已證實。
...
### 2026-08-20 — OOS Test strategy contract audit：C60 secondary PIT跨mode誤接 + OOS params未固定2020 cutoff

- 程式基準：使用者提供`test-branch-1_20260820_015839_7a95a13(1).zip`，SHA256=`f659f51640736a677b765e90d9e3f96aa16609e6d835c07620f64abab135a85a`；current OOS Strategy Compare期間=`2021-01-01～2026-03-02`。
- 使用者回報 current OOS 結果：C61 Full=`129.08%/17.41%/RoMD 7.42`、C58 Min=`101.60%/13.12%/7.74`、C59 MR-13E=`201.35%/15.92%/12.64`、C60 MR-13K+MR-13M Residual Safety=`49.53%/14.77%/3.35`。C60相對歷史Frozen Forward C56=`148.34%/12.86%/11.53`出現異常崩落，因此啟動lineage／runtime wiring audit；本輪不把該49.53%視為有效研究證據。
- **根因A（直接解釋C60異常）**：`strategy_comparison._resolved_ranking_options()`對dual-model safety source只套用`continuous_score_overrides`，沒有對`selection_point_in_time` secondary source解析mode-specific PIT dirname。OOS C60 primary `CONT13K_ROLL`由pair runner正確override到`point_in_time_oos_2021_forward`，secondary `CONT13M_ROLL`卻因override為None fallback到canonical `point_in_time` Rolling 12M。結果實際比較是「fixed-2020 K + annual-refit M」再做same-day rank OLS residual safety，scientific contract失真且只會重創C60；C59單模型不受此bug影響。
- **根因B（影響全部C61/C58/C59/C60 OOS arms）**：`extending_window_oos`雖把模型固定為2020年底information cutoff，arms仍直接綁`extending_full_roos/extending_min_roos` annual rolling parameter schedule，2021後會讀取後續年度重新fit的策略參數。這違反PROJECT_SETTINGS E12「OOS整段所有自動決策共用固定2020 cutoff」；因此本輪C61/C58/C59/C60舊OOS報表全部只能視為`INVALID_MIXED_PARAMETER_POLICY`，即使C61/C58數值恰與後來C1/C3 baseline一致亦不能當作正確fixed-cutoff OOS evidence。
- **非bug差異**：current Daily Universal已解除舊optimizer/2011 selection-start對training universe的歷史截斷，因此Seed42與2020 cutoff相同也不保證current OOS權重等於legacy Frozen Forward權重；MR-13M selected epoch亦由legacy Forward 2變current OOS 3。此模型權重差異屬current training-universe contract變更，不能回退成舊權重以追求舊數字。
- 修正A：`_selection_pit_mode_paths()`改支援caller project root；`_resolved_ranking_options()`遇secondary source=`selection_point_in_time`時，直接解析該source的mode-specific score/manifest path。OOS C60 K/M現在都綁`point_in_time_oos_2021_forward`；Rolling兩者都維持canonical `point_in_time`。新增synthetic釘死primary/secondary不得跨evaluation namespace。
- 修正B：新增deterministic parameter builder=`oos_param_freeze`。OOS profile仍沿用C61/C58/C59/C60 scientific IDs，但以profile-specific binding把C61→`oos_full_roos`、C58/C59/C60→`oos_min_roos`；兩者分別由`extending_full_roos/extending_min_roos`只取`2021-01-01`當下合法effective member，移除所有parallel post-cutoff schedules，單一frozen member覆蓋2021→source latest。manifest固定pin source SHA、`freeze_cutoff_date=2020-12-31`、`freeze_effective_date=2021-01-01`、source effective member、coverage與output SHA；source一變即STALE／REBUILD。Rolling profile不套override，仍使用原Extending annual schedules。
- Auto-prerequisite：`source_param_source_id`正式進Strategy Compare dependency graph；若Extending source需先BUILD/REBUILD，OOS freeze會作下一個dependency wave，不要求人工準備。OOS Multi-seed fixed C58與RoMD reference同樣改綁`oos_min_roos`，因此single-seed與robustness不再使用不同策略參數語意。
- Strategy Compare config schema=`48→49`；不新增MR／SR／Cxx identity，不改模型Target／architecture／loss／seed、optimizer search space、portfolio execution或production C42/C44。
- Decision：`HARD_WIRING_BUG_FOUND / OOS_PARAMETER_CONTRACT_BUG_FOUND / PREVIOUS_OOS_STRATEGY_RESULT_INVALIDATED / MODEL_RETRAIN_NOT_REQUIRED / OOS_STRATEGY_RERUN_REQUIRED`。既有OOS MR-13E/K/M PIT model artifacts可REUSE；修正版首次Strategy Compare OOS執行只需deterministically BUILD兩個frozen param artifacts後重新replay C61/C58/C59/C60。

### 2026-08-20 — Current Strategy Compare收斂為Compare Suite單一矩陣；OOS／Rolling人讀名稱正名
- 使用者決策：OOS Test、Rolling Test、OOS Multi-seed Robustness、Rolling Multi-seed Robustness不得各自維護比較對象；否則每個mode都需額外解釋baseline，且容易再次漏掉arm。
- 架構修正：新增current `extending_current` Compare Suite，唯一持有arms=`C61/C58/C59/C60`與contrasts=`C61-C58/C59-C58/C59-C61/C60-C58/C60-C61/C60-C59`。`extending_window_oos`與`extending_window_rolling` profile只保存evaluation policy／param binding／artifact namespace並引用同一`suite_id`，不再保存current `arm_ids/contrast_ids`副本。Strategy Compare schema `49→50`。
- Robustness current profiles只保存`suite_id + seed/execution policy`；不再保存`fixed_arm_ids/stochastic_arm_ids/paired_contrasts`。Runtime由suite arm model dependency自動推導：C61/C58沒有model dependency→fixed references，只回放一次；C59有MR-13E dependency、C60有MR-13K+MR-13M dependencies→stochastic，每個generated seed重訓所需source後replay。兩個stochastic之間的suite contrast自動解析為`C60-C59` same-seed paired contrast。
- 因此OOS/ Rolling robustness aggregate與single-seed都固定顯示同四arm；Full/Min不因fixed而從比較表消失，只是不重複做每seed模型工作。OOS robustness模型訓練來源由2個增為3個（13E/K/M）；Rolling亦同理，這是補足C59 seed sensitivity evidence所需成本，不改模型科學設定。
- 人讀正名：suite保存base names=`Full / Min / Min MR-13E Constrained / Min MR-13K + MR-13M Residual Safety`。OOS renderer自動加`OOS`，Rolling renderer自動加`Rolling`；current report不再顯示`Full ROOS/Min ROOS`。Legacy C1/C3等歷史名稱與artifact identity不改。
...
### 2026-08-21 — Strategy Parameter policy-search dedup + Min policy-binding closure
- 基準：`test-branch-1_20260821_173252_f6f1604.zip`，SHA256=`8ff8eb3c8e6a470b076ae70bb533942dbe861f3bd23444927d6b282552fb6687`。使用者實機log顯示Full `base_finalist_best`與`base_finalists_agree`兩個work root各自重跑完全相同的2021 initial與2022+ rolling-tail Optimizer search；同一run內`FINALIST BEST`與`SEED ENSEMBLE`亦逐fold相同，而footer仍顯示`seeds=8 / min_agree=5`。
- 診斷一：production random-seed ensemble目前`enabled=False`，因此實際只有canonical optimizer seed單member，`base` ensemble必然退化成`base_finalist_best`；原footer錯把configured capacity N=8/min_agree=5當成effective runtime。修正後disabled ensemble的人讀表不再顯示重複`SEED ENSEMBLE RESULTS`，效能摘要顯示effective `seeds=1 / min_agree=1 / seed_ensemble=off`，但保留requested policy metadata供稽核。
- 診斷二：Outer Rolling單次search本來就同時輸出`roos_base_best.json`、`roos_base_finalists_agree.json`等policy artifacts；canonical service卻以policy-specific work root各跑一次同一搜尋。修正為同family／同build-contract共用`shared_policy_search` work domain：2021 initial與2022+ tail各只搜尋一次，第二個policy直接reuse同一search outputs並materialize自己的canonical selector artifact。兩個policy的科學結果並未被綁成相同；使用者log同一run已顯示Full OOS_CHAIN `base-finalist-best=6.25`、`base-finalists-agree=5.06`。
...
### 2026-08-23 — Strategy Compare single/multi execution SSOT closure + C60 same-seed safety correction

- End-to-end call-graph差分確認：current Robustness雖已共用Compare Suite、Optimizer、trainer底層helper、GPU policy與portfolio replay，但仍各自持有training lifecycle wrapper、PIT完成條件／artifact validation與per-arm replay argument assembly；這不符合`end_to_end_v1`「single-seed Compare Suite的純multi-seed版本」契約。
- 架構收斂：`services/research/strategy_compare_training.py`成為single/multi唯一training-unit lifecycle owner，統一trainer command、logged subprocess、PIT audit與consumer-contract validation；Robustness移除私有`_training_command()`／`_validate_training_artifacts()`與直接low-level lifecycle。`filters/breakout_quality/strategy_compare_execution.py`新增為single/multi共用per-arm replay owner，兩條orchestration不再直接組`run_comparison()`／`run_standalone_baseline()`參數。Robustness只保留seed binding、benchmark namespace、跨seed scheduling／retention／aggregate等真正multi-seed責任。
- Correctness修正：dual-model safety resolution過去在Selection-PIT source會以canonical production safety path覆寫caller已提供的isolated benchmark-seed override，使C60有可能形成`K=benchmark seed / M=production seed`混合replay；現在明確caller override優先於production fallback，並以隔離probe驗證K/M均解析至同一benchmark seed。
- Config治理：`config/training_policy.py`屬使用者可調的唯一benchmark seed題庫SSOT；本輪依C8保留基準中的當前`seed_count`／generator／resolved sequence，不為文件快照或validator還原特定數值。config-driven synthetic只驗證Strategy Compare／Robustness忠實採用`config/training_policy.py`當前值，並把該config加入impact mapping；固定測例需要seed時只可用隔離override，不把seed count／sequence硬編成runtime唯一合法答案。
- 研究治理：此修正不新增MR/SR/Cxx identity、不改suite／contrasts／Optimizer search、benchmark題庫設定或模型training science。**任何以受影響舊runtime完成的C60 multi-seed robustness結果不得作current evidence，必須以修正版重跑C60後再判讀**。C61/C62/C58/C63/C59不因此特定secondary-safety override bug自動失效。
...
### 2026-08-23 — MR-13P formal gate closure：Upside diagnostics owner 不再以 Pure-MFE target uniqueness 推定

- 使用者本機正式 `apps/run_bundle.py` 結果：quick gate／chain checks／ml smoke PASS；consistency FAIL 1 項，meta-quality coverage 4 項連鎖 FAIL。`to_chatgpt_bundle` 的唯一根因為 Strategy Compare Upside/First-Passage diagnostics 仍以 `daily_full_horizon_pure_mfe_r_v1` target-id 掃描並要求 profile 唯一；MR-13P 合法重用同一 Pure-MFE primary target 後，MR-13K 與 MR-13P 同時符合，synthetic suite 因 `full-horizon pure-MFE diagnostic profile必須唯一` 中止。Coverage line／branch／key-target FAIL 皆為 synthetic suite 未執行完成的衍生結果，不是四個獨立 regression。
- Engineering fix：Strategy Compare config 現在明確持有 Upside Realization path diagnostics 的 canonical profile owner，diagnostics 直接消費此 owner 並驗證其 target identity；不再把「共享同一 target」誤解為 producer ownership 衝突。Diagnostics contract fingerprint 同步包含 path profile，因此 owner 變更會只使 diagnostics stale，不影響 portfolio replay scientific identity。
...
### 2026-08-25 — Strategy Compare OOS/Rolling/Robustness reuse state SSOT 修正

- 使用者確認 current Compare Suite 除 C67 外既有 single-seed OOS／Rolling 已實際執行，但 OOS execution plan 對 C61/C62/C63 顯示 `RUN`，Rolling 對相同 baseline 顯示 `REUSE`；C58 又可由 shared paired artifact 正常 REUSE，顯示狀態受 artifact topology 影響。
- 根因為 parameter identity 層次混用：OOS replay 實際先對 canonical 跨年 schedule 做 in-memory 2021 fixed-cutoff freeze，completed baseline/pair metadata 保存的是 freeze 後 replay-effective SHA；reuse resolver 卻用完整 physical schedule scientific SHA 比對 standalone baseline。Rolling 不 freeze，因此兩個 SHA 恰好一致，造成 OOS/ Rolling 分叉。Robustness strategy-only baseline context 亦使用完整 schedule SHA 驗證 OOS replay context，存在同類誤判。
- 工程修正：新增 canonical evaluation-view parameter identity owner；preparation 一次解析 per-arm evaluation identity，pair cache 與 standalone baseline reuse 只消費該 identity。physical/runtime full-schedule scientific SHA 繼續保留給 Optimizer lineage 與 robustness benchmark scientific contract，不改 scientific parameter truth。
- 同時新增 canonical strategy result-state mapper。Single-seed plan 與 executor 現在共用 resolved per-arm state；Robustness plan/status/executor 共用 per-seed durable scientific-result state，並把缺少 transient baseline replay context 明確區分為 `REBUILD_CONTEXT`，不因 context 被清除而重跑已完成 seed scientific result。
- 本修正不改 C58～C67 arm 定義、Compare Suite、OOS/Rolling evaluation semantics、seed specification、model/target/selector、strategy accounting 或既有 research decision；C67 仍為 RESULT_PENDING，Robustness 研究優先順序仍依 Research Queue 延後。
...
### 2026-08-27 — MR-13T Raw-data Direct HM/HS Tri-head實作；先測direct joint supervision再改representation

- MR-13S與Actual Truth Geometry已把問題收斂：Forward OOS Raw Safety/Raw-MFE Dailyρ=`0.3556/0.3948`，但predicted Safety↔Raw-MFE=`-0.9373`、S5×M5=`0`；actual Daily S↔MFE僅=`-0.1261`且S5×M5=`23,242 / 3.82% / 0.92×`，Breakout actual=`-0.0547`且S5×M5=`986 / 5.68% / 1.09×`。因此真實joint support存在，但MR-13S兩個marginal objective沒有任何loss直接獎勵HM/HS。
- 使用者要求先重新評估「只改representation／人工path input」是否真為最優。Decision改為先做更小的supervision contrast：**保持raw data與MR-13S representation不變，只新增Direct HM/HS supervision**。舊binary-filter architecture REJECT只視為不同objective下negative prior，不作今日joint task永久禁止；MR-13N equal-rank、MR-13O Pareto、MR-13K/M frozen fusion仍是與joint scalar/fusion較直接相關的negative evidence。
- 新identity=`MR-13T`，profile=`daily_universal_safety_raw_mfe_hmhs_tri_head_full_list_ndcg_pairwise`，architecture identity=`ARCH-inception_time_safety_raw_mfe_hmhs_v1`。Canonical input仍`300×10` normalized stock/0050 OHLCV，未增加handcrafted features/context。Shared InceptionTime trunk超參數與MR-13S相同；原Raw Safety head與Safety-conditioned Raw-MFE head完整保留，第三個Direct HM/HS head只讀shared latent。
- Supervision固定：`S=same-date Low-Adverse Safety percentile`、`U=same-date Pure-MFE percentile`、`H=1[S>=0.5 AND U>=0.5]`。三head各使用同一full-list Delta-NDCG pairwise logistic，總loss固定=`(Ls+Lu+Lh)/3`，不掃lambda。Raw-MFE的Safety context仍stop-gradient；Joint head不讀Safety/MFE predicted score、不作product/sum/fusion。
- 為維持strict controlled contrast，**epoch selection仍只依Raw-MFE Validation mean Daily Spearman**，不以Joint Validation結果選epoch；Forward OOS／Breakout只在checkpoint後作frozen model Gate。Selection PIT/current-time validation/runtime strategy均未授權。
...
- MR-13U epoch selection事前固定Validation HM/HS Pair concordance；只有Pair完全同值才用Validation global PR-AUC tie-break。Forward/Breakout完全frozen，不回流selection/fitting。HM/HS population、Pair、Global/Mean-Daily PR-AUC、Top10/20 HM/HS rate/enrichment屬MR-13U Model-specific Extension evidence；不得改寫Standard Model SOP 1～6 schema。
...
### 2026-08-27 — Persistent Research report contract freeze；隔離MR-13T/U model-specific presentation（無scientific change）

- 依使用者明確授權，正式建立`core/research_report_contract.py`作為常駐Research報表machine-readable SSOT，freeze Standard Model SOP、Standard Strategy SOP、OOS↔Rolling Consistency與三個Reusable Audit的report identity、section/order、columns、label、precision/unit、metric direction與applicability；每份persistent contract保存user-approved fingerprint。
- Standard Model SOP恢復並固定為`1 Learnability / 2 Generalization / 3 Multi-head Learnability / 4 Truth-Prediction Geometry / 5 Ranking-Boundary / 6 Evidence Coverage`。MR-13T Direct HM/HS Joint Retrieval與MR-13U H-only Learnability/Generalization完全保留scientific payload，但改為`Model-specific Extension` namespace；extension不得取得Standard section number、取代Standard Generalization columns或注入Standard Evidence schema。
- Console與Markdown改消費同一canonical Model SOP contract；MR-13S/T/U cross-profile synthetic必須驗Standard schema invariance。三個Reusable Audit renderer不再自行持有persistent欄位precision/unit/direction；Trade Outcome/Path的`+1R/+2R/+3R` First-Passage columns正式freeze，若config企圖改變常駐threshold schema即BLOCK而非靜默改版。
- 所有persistent contract fingerprint drift在未同步取得使用者授權時必須以`UNAUTHORIZED_PERSISTENT_REPORT_CONTRACT_CHANGE` fail-fast。此工程只治理presentation/schema ownership，不改MR-13T/U targets、architecture、loss、training、checkpoint、OOS evidence、Strategy replay或promotion decision。

### 2026-08-27 — MR-13U H-only Model Gate FAIL；MR-13V nonlinear Direct-H readout實作

- MR-13U Seed42正式結果：selected epoch=`1`；Validation HM/HS Pair/PR-AUC=`53.28%/0.2336`。Frozen Forward OOS Pair=`52.34%`、PR-AUC=`0.2306`、Top10 HM/HS=`23.41% / 1.0495×`、Top20=`23.24% / 1.0418×`；Breakout Pair=`50.17%`、PR-AUC=`0.2465`、Top10=`23.69% / 0.9913×`、Top20=`24.70% / 1.0335×`。
- 相對MR-13T Direct H，Forward `53.42% / 0.2366 / 1.1159×`在Pair/PR-AUC/Top10均較強；Breakout MR-13T Pair/Top10=`50.25%/1.0299×`也未被H-only改善，只有MR-13U Breakout PR-AUC小幅高於`0.2441`。因此不支持Safety/MFE auxiliary gradients是HM/HS主要blocker；H-only路線停止，不做loss-weight或single-head延伸。Decision=`MR13U_MODEL_GATE_FAIL / H_ONLY_NO_GAIN / MULTITASK_INTERFERENCE_NOT_SUPPORTED_AS_PRIMARY_BLOCKER / NO_PIT / NO_STRATEGY_CONVERSION`。
- 下一個最小mechanism experiment=`MR-13V`。回到MR-13T相同shared tri-head、raw `300×10`、InceptionTime trunk、S/U/H targets、Seed42、split、optimizer、full-list Delta-NDCG、三head固定等權loss與Raw-MFE Validation epoch selection；唯一scientific change為Direct H readout由`Linear(latent,2)`改為固定`Linear(latent,latent)→ReLU→Linear(latent,2)`。目前latent width由既有filters/kernels決定為`128`，故architecture=`ARCH-inception_time_safety_raw_mfe_hmhs_mlp_v1`。不增加dropout，不掃width/depth/activation/lambda，不讀marginal predicted score。
- Scientific question：MR-13T shared latent已可各自支撐Safety/MFE OOS Dailyρ約`0.35/0.39`，但HM/HS是upper-right conjunction；Direct H weak edge是否主要來自single linear readout capacity不足，而非temporal representation本身缺失。
- Predeclared Gate：沿用既有Direct-vs-product model-specific evidence。Forward Direct Pair/PR-AUC/Top10 enrichment需相對MR-13T明顯改善，Breakout必須同方向，Raw Safety/Raw-MFE marginal learnability不得實質崩潰；若FAIL，不再做另一個head arithmetic/width/activation小改，下一步才進learned temporal pooling或raw-data joint architecture。
- Research report governance：本輪**不得修改**Standard Model SOP 1～6、欄位／順序、Generalization/Evidence schema、menu role、`core/research_report_contract.py` approved persistent fingerprint。MR-13V因輸出payload與MR-13T同型，直接重用既有`Model-specific Extension｜MR-13V｜Direct HM/HS Joint Retrieval` schema；不新增常駐report contract。
- Status：**IMPLEMENTED / RESULT_PENDING / NONLINEAR_JOINT_READOUT_ONLY / RAW_300x10_UNCHANGED / STANDARD_REPORT_FROZEN / NO_OOS_FIT / NO_PIT / NO_STRATEGY_CONVERSION**。

### 2026-08-27 — MR-13V Global-rank gain但Top-tail Gate FAIL；MR-13W Continuous Joint-Min target實作

- MR-13V Seed42正式結果：Validation Direct HM/HS Pair/PR-AUC/Top10=`57.73% / 0.2642 / 28.44% (1.2518×)`；Forward=`54.30% / 0.2388 / 24.53% (1.0997×)`，優於same-model marginal product=`53.06% / 0.2325 / 23.07% (1.0343×)`；Breakout=`51.90% / 0.2484 / 24.15% (1.0106×)`，亦高於product=`51.00% / 0.2435 / 23.40% (0.9793×)`。
- 相對MR-13T Linear-H，MR-13V確實改善Forward Pair `53.42→54.30%`、PR-AUC `0.2366→0.2388`與Breakout Pair `50.25→51.90%`、PR-AUC `0.2441→0.2484`；但真正predeclared top-tail conversion未改善：Forward Top10 enrichment `1.1159×→1.0997×`，Breakout `1.0299×→1.0106×`。Marginal geometry仍Daily predicted Safety↔MFE=`-0.9425`、S5×M5=`0`，Breakout=`-0.9005`、S5×M5=`7`。Decision=`MR13V_MODEL_GATE_FAIL / NONLINEAR_READOUT_GLOBAL_RANK_GAIN / TOP_TAIL_GAIN_NOT_CONFIRMED / MARGINAL_MIRROR_UNCHANGED / STOP_HEAD_CAPACITY_TUNING`。
- 使用者指出binary HM/HS target本身存在information-loss：`0.51/0.51`與`0.90/0.90`都被標成positive，pairwise training無法要求後者排序更高。研究順序因此在learned temporal pooling前插入一個更小的target-only contrast，先確認learning problem是否定義過粗。
- 新identity=`MR-13W`，profile=`daily_universal_safety_raw_mfe_joint_min_mlp_head_full_list_ndcg_pairwise`；**architecture不變**，沿用MR-13V `ARCH-inception_time_safety_raw_mfe_hmhs_mlp_v1`，因模型結構/input representation完全相同。第三head target唯一改為`Jmin=min(S,U)`，S/U分別為canonical same-date Low-Adverse Safety percentile與Pure-MFE percentile。`min`不再做subset/date rerank、不乘0.5 indicator、不使用soft-min/product/harmonic mean，也不以OOS挑aggregation。
- MR-13W strict controls：raw `300×10`、shared InceptionTime trunk、Raw Safety head、stop-gradient Safety-conditioned Raw-MFE head、128→128→2 ReLU joint MLP、Seed42、daily-universal split、optimizer、full-list Delta-NDCG、三head固定等權loss與**Raw-MFE Validation Dailyρ epoch selection全部維持MR-13V**。因此本輪只測target information content，不同時改readout、pooling或selection rule。
- Model-specific Extension固定顯示Joint-Min Dailyρ/Pair、population mean min、Top10/20 actual min、Top10 mean Safety/MFE、binary HM/HS rate/enrichment。Binary HM/HS從training target降為interpretation truth，用來回答continuous maximin ranking是否自然把真正joint-good樣本推到top tail。
- Predeclared decision：Forward與Breakout的Joint-Min ordering與Top10/20 floor需同方向成立，且Top10 Safety/MFE兩邊不能只靠單邊極端、marginal S/U learnability不得實質崩潰。若GO，支持binary HM/HS target information loss是主要blocker；若FAIL，不再用OOS嘗試其他aggregation formula，下一步才進learned temporal pooling／raw-data joint representation。
- Research report governance：依B280，Standard Model SOP 1～6、Generalization/Evidence schema與approved persistent fingerprint不變；新增evidence只註冊`Model-specific Extension / joint_min_retrieval`，不得取得Standard section number。
- Status：**IMPLEMENTED / RESULT_PENDING / TARGET_ONLY_CONTINUOUS_JOINT_MIN / SAME_ARCH_AS_MR13V / STANDARD_REPORT_FROZEN / NO_OOS_FIT / NO_PIT / NO_STRATEGY_CONVERSION**。

### 2026-08-28 — MR-13W Forward generalization FAIL；MR-13X Joint learned temporal pooling實作

- MR-13W Seed42正式結果：Validation Joint-Min Dailyρ/Pair=`0.1417/54.78%`，population mean min=`0.3224`、Top10 Min=`0.3612`、Top10 S/MFE=`0.5031/0.5370`、HM/HS enrichment=`1.2070×`；Forward OOS=`0.0431/51.38%`，Pop=`0.3199`、Top10 Min=`0.3332`、S/MFE=`0.4611/0.5521`、HM/HS=`1.0702×`；Breakout=`0.0176/49.58%`，Pop=`0.3267`、Top10 Min=`0.3261`、S/MFE=`0.4164/0.5936`、HM/HS=`0.9986×`。
- Marginal heads未崩：Forward Raw Safety/Raw-MFE Dailyρ=`0.3548/0.3855`、Breakout=`0.3109/0.3610`；但predicted Safety↔Raw-MFE仍Daily=`-0.9393`、Breakout=`-0.8959`，Daily predicted S5×M5=`0`。因此`min(S,U)`修正binary target depth後只在Validation出現弱joint learnability，沒有形成usable Forward/Breakout ranking。Decision=`MR13W_MODEL_GATE_FAIL / VALIDATION_JOINT_MIN_SIGNAL / FORWARD_GENERALIZATION_COLLAPSE / BREAKOUT_RANDOM / TARGET_INFORMATION_LOSS_NOT_PRIMARY_BLOCKER / STOP_AGGREGATION_TUNING`。不再以OOS嘗試soft-min/product/harmonic mean。
- 下一個最小representation experiment=`MR-13X`，profile=`daily_universal_safety_raw_mfe_joint_min_attn_pool_mlp_head_full_list_ndcg_pairwise`；new architecture identity=`ARCH-inception_time_safety_raw_mfe_joint_attn_mlp_v1`。Scientific question：InceptionTime temporal feature map是否含joint path information，但MR-13W對300 bars做global mean後才送Joint MLP而提前丟失。
- MR-13X strict controls：raw `300×10`、InceptionTime trunk全部模組/filters/kernels/residual/BatchNorm、Safety/Raw-MFE heads、S/U/Jmin targets、128→128→2 ReLU Joint MLP、Seed42、split、optimizer、full-list Delta-NDCG、三head固定等權loss與Raw-MFE Validation Dailyρ epoch selection全部不變。唯一scientific change為Joint branch從global mean改成shared feature map上的`Conv1d(128,1,k=1)` scalar score、time softmax、weighted temporal sum。Marginal heads仍讀原global mean。
- 不加入positional encoding、多頭QKV attention、temperature、dropout、attention hidden width、window mask或任何sweep。Same-seed contract要求MR-13W/X所有既有parameter tensors初始化byte-identical，X只多`joint_attention_scorer.weight/bias`，使observed差異能歸因Joint pooling mechanism。
- Predeclared Gate：MR-13X需相對MR-13W在Forward Joint-Min rho=`0.0431`/Pair=`51.38%`/Top10 Min=`0.3332`與Breakout rho=`0.0176`/Pair=`49.58%`/Top10 Min=`0.3261`有共同且實質改善；Breakout Top10 Min必須真正高於population `0.3267`，Top10 Safety/MFE不能靠單邊極端，marginal S/U不可實質退化。若FAIL，不再做另一個InceptionTime attention/pooling小變體，下一步直接進ModernTCN/Patch Transformer以同一Jmin objective比較raw-data temporal representation。
- Research report governance：Standard Model SOP 1～6、Generalization/Evidence schema與approved persistent fingerprint完全不改；MR-13X直接重用既有`Model-specific Extension / joint_min_retrieval`，不新增report contract。PIT/current Rolling/runtime strategy維持未授權。
- Status：**IMPLEMENTED / RESULT_PENDING / JOINT_TEMPORAL_POOLING_ONLY / SAME_JMIN_TARGET_AS_MR13W / SAME_SEED_EXISTING_PARAMS_IDENTICAL / STANDARD_REPORT_FROZEN / NO_OOS_FIT / NO_PIT / NO_STRATEGY_CONVERSION**。

### 2026-08-28 — MR-13X small positive pooling signal但Gate FAIL；MR-13Y ModernTCN architecture-family comparison實作

- MR-13X Seed42正式結果：Validation Joint-Min rho/Pair/Top10 Min=`0.1466 / 54.99% / 0.3708` vs Pop=`0.3224`；Forward=`0.0528 / 51.73% / 0.3358` vs Pop=`0.3199`，Top10 Safety/MFE=`0.5188/0.4840`、HM/HS enrichment=`1.0477×`；Breakout=`0.0349 / 49.81% / 0.3364` vs Pop=`0.3267`，Top10 Safety/MFE=`0.5033/0.5026`、HM/HS=`1.0179×`。
- 相對MR-13W，learned temporal attention確實帶來小幅mechanism gain：Forward rho `0.0431→0.0528`、Pair `51.38→51.73%`、Top10 Min `0.3332→0.3358`；Breakout rho `0.0176→0.0349`、Pair `49.58→49.81%`、Top10 Min `0.3261→0.3364`。更重要的是Breakout Top10 S/MFE由失衡`0.4164/0.5936`收斂為`0.5033/0.5026`，predicted Safety↔MFE由Daily `-0.9393→-0.9190`、Breakout `-0.8959→-0.8663`。但Forward/Breakout ordering仍太弱，Breakout Pair仍未離開50%，不得進Strategy conversion。Decision=`MR13X_MODEL_GATE_FAIL / LEARNED_POOLING_SMALL_POSITIVE_SIGNAL / JOINT_FLOOR_BALANCE_IMPROVED / FORWARD_STILL_WEAK / BREAKOUT_NEAR_RANDOM / INCEPTIONTIME_REPRESENTATION_GATE_FAIL / STOP_LOCAL_INCEPTIONTIME_PATCHING`。
- 依MR-13X predeclared stop rule，後續不再試第二種InceptionTime attention、temperature、window mask、positional encoding或pooling小變體；正式進raw-data architecture-family comparison。第一個新identity=`MR-13Y`。
- MR-13Y profile=`daily_universal_safety_raw_mfe_joint_min_modern_tcn_attn_pool_mlp_head_full_list_ndcg_pairwise`；new architecture=`ARCH-modern_tcn_safety_raw_mfe_joint_attn_mlp_v1`。不能直接復活historical `modern_tcn_v1`，因9B已REJECTED/legacy read-only，且MR-13Y結構另含Safety/Raw-MFE tri-head、Joint temporal attention與Joint-Min MLP。
- Strict controls：canonical raw `300×10`、S/U/Jmin targets、Safety與stop-gradient Safety-conditioned Raw-MFE global-average semantics、Joint scalar `1×1` temporal attention softmax pooling、latent-width→latent-width→2 ReLU joint MLP、Seed42、daily-universal split、optimizer、full-list Delta-NDCG、三head固定等權loss與Raw-MFE Validation Dailyρ epoch selection全部沿用MR-13X。唯一research dimension是temporal trunk family。
- ModernTCN trunk不重新調參，固定沿用historical 9B容量近似recipe：depth=`6`、channels=`96`、kernel=`51`、pointwise expansion=`4×`、BatchNorm、dropout=`0.10`、receptive field=`301`。Head與attention width只依trunk latent自然為96，不加96→128 adapter、不掃head width。Synthetic以same Seed42重建legacy 9B與MR-13Y，要求`stem.*`/`blocks.*`所有shared trunk tensors exact equal，確保不是偷偷改ModernTCN recipe。
- Predeclared Gate：相對MR-13X Forward Joint-Min rho/Pair/Top10 Min=`0.0528/51.73%/0.3358`、Breakout=`0.0349/49.81%/0.3364`，MR-13Y需在Forward與Breakout對Joint-Min ordering/top-tail floor都有共同且實質改善；Breakout Pair需離開random、Top10 Min穩定高於Pop=`0.3267`，marginal Safety/Raw-MFE不得實質崩潰。若FAIL，不做ModernTCN depth/kernel/channels/expansion/dropout tuning，下一步直接Patch Transformer同一Jmin/head contract。
- Research report governance：Standard Model SOP 1～6與approved persistent fingerprint完全不變；MR-13Y重用既有`Model-specific Extension｜MR-13Y｜Continuous Joint-Min Retrieval`，不新增persistent report schema。
- Status：**IMPLEMENTED / RESULT_PENDING / RAW_DATA_ARCHITECTURE_FAMILY_COMPARISON / HISTORICAL_9B_TRUNK_RECIPE_FROZEN / STANDARD_REPORT_FROZEN / NO_OOS_FIT / NO_PIT / NO_STRATEGY_CONVERSION**。

### 2026-08-28 — MR-13Y ModernTCN Gate FAIL；MR-13Z Patch Transformer architecture comparison實作

- MR-13Y Seed42正式結果：Validation Joint-Min rho/Pair/Top10 Min=`0.1825 / 56.15% / 0.3701`；Forward=`0.0482 / 51.54% / 0.3284`、HM/HS enrichment=`1.0332×`；Breakout=`0.0299 / 50.32% / 0.3396`、Top20 Min=`0.3417`。相對MR-13X，Forward Joint-Min `0.0528/51.73%/0.3358`全面較弱，Breakout只有Top10/20 floor局部小升。
- Primary blocker是marginal generalization collapse：MR-13Y Forward Raw Safety/Raw-MFE rho=`0.3001/0.2601`，明顯低於MR-13X=`0.3541/0.3764`；Breakout Raw-MFE=`0.2347`。Predicted Safety↔MFE雖Daily/Breakout由MR-13X `-0.9190/-0.8663`變為`-0.8294/-0.7549`，S5×M5增至`9/36`，但這與truth-correlation下降同時發生，不能解讀為joint geometry改善。Decision=`MR13Y_MODEL_GATE_FAIL / MODERNTCN_MARGINAL_GENERALIZATION_COLLAPSE / JOINT_MIN_NO_FORWARD_GAIN / BREAKOUT_LOCAL_FLOOR_GAIN_ONLY / GEOMETRY_DECORRELATION_NOT_USEFUL / NO_MODERNTCN_TUNING`。
- 依事前stop rule不調ModernTCN kernel/channels/depth/dropout，直接進第二個raw-data family。新identity=`MR-13Z`，profile=`daily_universal_safety_raw_mfe_joint_min_patch_transformer_attn_pool_mlp_head_full_list_ndcg_pairwise`，new architecture=`ARCH-patch_token_transformer_safety_raw_mfe_joint_attn_mlp_v1`。historical `patch_transformer_v1`仍legacy read-only，不復活。
- MR-13Z strict controls：raw `300×10`、S/U/Jmin truth、Raw Safety head、stop-gradient Safety-conditioned Raw-MFE、marginal mean pooling語意、Joint scalar softmax attention、latent→latent→2 ReLU joint MLP、Seed42、split、optimizer、full-list Delta-NDCG、三head固定等權loss與Raw-MFE Validation epoch selection全部沿用MR-13X。唯一research dimension為temporal trunk family。
- Patch Transformer固定historical 9F未調參recipe：10-bar non-overlap patch（300 bars→30 tokens）、embedding=128、3 TransformerEncoder layers、4 heads、FFN=256、GELU、sinusoidal positional encoding、dropout=0.10、LayerNorm。Marginal heads對normalized encoded tokens取mean；Joint head對同一token map用單一`1×1` scalar scorer + softmax(token) weighted sum。不得調patch size/stride、embedding、depth、heads、FFN、position、dropout或另加CLS/token pooling變體。
- Same Seed42 reconstruction contract：新MR-13Z的`patch_projection.* / patch_normalization.* / encoder.* / output_normalization.*`必須與legacy 9F逐tensor exact equal；active builder不得import legacy `models.patch_transformer`。
- Predeclared Gate仍以MR-13X為主要baseline（MR-13Y已因marginal collapse淘汰）：Forward Joint-Min rho/Pair/Top10 Min=`0.0528/51.73%/0.3358`，Breakout=`0.0349/49.81%/0.3364`。MR-13Z需在Forward與Breakout joint ordering/top-tail floor共同且實質改善、Breakout Pair離開random且marginal S/U不崩。若FAIL，不再逐一搜尋更多backbone family，下一步重開input representation / target observability問題。
- Research report governance：Standard Model SOP 1～6、Generalization/Evidence schema與approved fingerprint完全不變；MR-13Z直接重用`Model-specific Extension / joint_min_retrieval`。
- Status：**IMPLEMENTED / RESULT_PENDING / PATCH_TRANSFORMER_FAMILY_ONLY / SAME_JMIN_CONTRACT / STANDARD_REPORT_FROZEN / NO_OOS_FIT / NO_PIT / NO_STRATEGY_CONVERSION**。

### 2026-08-28 — MR-13Z Model Gate PASS；授權PIT-safe Direct Joint-Min conversion並建立SR-C75

- MR-13Z Seed42正式Model SOP完成。Forward Joint-Min Population/ρ/Pair/Top10 Min/HMHS×=`0.3199 / 0.1148 / 53.84% / 0.3495 / 1.1693×`；Breakout=`0.3267 / 0.0677 / 53.75% / 0.3639 / 1.1698×`。相對predeclared MR-13X control Forward=`0.0528/51.73%/0.3358/1.0477×`、Breakout=`0.0349/49.81%/0.3364/1.0179×`，ordering與top-tail joint floor均同方向明顯改善。
- Marginal learnability未觸發collapse：MR-13Z Forward Raw Safety/Raw-MFE Dailyρ=`0.3350/0.3857`、Breakout=`0.3300/0.3519`；不同於MR-13Y ModernTCN的Forward Raw-MFE=`0.2601`。Decision=`PATCH_TRANSFORMER_JOINT_SIGNAL_CONFIRMED / FORWARD_AND_BREAKOUT_GATE_GO / TOP_TAIL_JOINT_FLOOR_IMPROVED / MARGINAL_LEARNABILITY_PRESERVED / MODEL_GATE_PASS`。Patch Transformer architecture/recipe就此freeze，不做patch/head/depth/embedding/dropout tuning，也不繼續backbone shopping。
- Model Gate PASS只授權下一階段PIT-safe research evaluation，不等於production promotion。MR-13Z research spec因此正式開啟`selection_pit_authorized=True`與`current_time_validation_authorized=True`，production runtime仍未promotion。
- PIT score contract保持既有primary語意相容：`breakout_quality_score`仍是Raw-MFE legacy primary；MR-13Z fold artifact另保存`raw_safety_score`、`raw_mfe_score`、`joint_min_score`。這避免為C75偷偷重定義舊欄位。
- 新DL source=`CONT13Z_ROLL`，evaluation stage=`selection_point_in_time`。Model Research canonical Rolling/PIT producer負責建立合法fold checkpoints/scores；Strategy Compare的`selection_pit_from_existing_folds`只允許重組/重評既有fold，不得因缺fold自行訓練模型。
- 新strategy identity=`SR-C75`：Min MR-13Z Direct Joint-Min No-K/No-R0。與C74固定共用Min `base-finalist-best`、all-off、No-K/No-R0、physical max positions=10、canonical sizing/cash/orderability/execution；唯一selector treatment為formal `primary_score_column=joint_min_score` descending + canonical tie-break。無Safety gate、threshold、product、fitted weight或calibration。
- `extending_current` Compare Suite schema=`62`，arms新增C75；primary contrast=`C75-C74`（learned Direct Joint-Min vs parameter-free Safety×Conditional-MFE percentile product），secondary=`C75-C71`。C75 score-projection cache identity只綁實際消費的`joint_min_score`，不以Raw-MFE `breakout_quality_score`冒充。
- Predeclared conversion decision：先做single-seed OOS+Rolling。若沒有形成具決策價值的Return/MDD/RoMD與Joint/Trade-quality改善，關閉C75且不投入Multi-seed；只有成為真正candidate才進Extending Multi-seed，再依必要性做Fixed。OOS/strategy結果不得回頭修改MR-13Z fitting、target、architecture或threshold。
- Status：**MR13Z_MODEL_GATE_PASS / PIT_AUTHORIZED / C75_IMPLEMENTED_RESULT_PENDING / SINGLE_SEED_CONVERSION_NEXT / NO_PRODUCTION_PROMOTION / NO_ROBUSTNESS_YET**。

### 2026-08-28 — MR-13Z PIT fold execution optimization；降低CPU feature/diagnostic bottleneck（無scientific change）

- 使用者執行MR-13Z Extending-Window Rolling時，fold 1長時間停留於「訓練並評分」，GPU utilization約20%，但VRAM僅約`3.2 / 16 GB`。因此可排除顯存不足為主要限制；也不能直接以放大training batch追求VRAM utilization，因current full-list ranker的optimizer batch採date-coherent whole-day semantics，跨日合併會改optimizer update contract。
- 根因之一是daily-universal lazy feature provider原先對每個stock-day逐筆重算300-bar OHLCV normalization（含volume log/median/IQR），即使已有prefetch=8/workers=4，仍產生大量Python小工作。已把canonical numerical definition擴成batch-vectorized `normalize_ohlcv_array_windows`，scalar helper直接重用同一SSOT；`LazyDailyFeatureBank.__getitem__`仍取完全相同ticker/date/history rows，但一次對整個batch做normalization並依unique benchmark position填入0050 feature。Synthetic 300-bar fixture與舊baseline輸出byte-for-byte一致；本機4-worker/18-batch feeding benchmark由約`4,042 rows/s`提升至約`57,894 rows/s`，僅作engineering throughput evidence，不屬scientific result。
...
### 2026-08-28 — MR-13AA Model Gate：Patch Transformer未形成MR-13H target的通用architecture gain；MR-13AB first-passage Pure-MFE control實作

- MR-13AA Seed42正式Standard Model SOP完成：selected epoch=`2`；Validation post-refit Daily rho=`0.2410`。Forward OOS Daily rho/Pair/Top-K Lift=`0.1974 / 56.79% / +1.1405R`；Breakout candidate slice=`0.2044 / 58.69% / +0.4672R`。
- Primary historical MR-13H InceptionTime reference為Forward=`0.2332 / 58.04% / +1.3400R`、Breakout=`0.2056 / 58.86% / +0.5061R`。MR-13AA相對reference：Forward Δrho=`-0.0358`、ΔPair=`-1.25pp`、ΔTop-K=`-0.1995R`；Breakout Δrho=`-0.0012`、ΔPair=`-0.17pp`、ΔTop-K=`-0.0389R`。因此Patch Transformer在成熟MR-13H economic scalar target上沒有形成一致增量。
- Decision：**LEARNABILITY_CONFIRMED / NO_GENERAL_ARCHITECTURE_GAIN / PATCH_GAIN_JOINT_MIN_SPECIFIC_OR_INTERACTION / MODEL_GATE_FAIL_FOR_GENERAL_ARCHITECTURE_PROMOTION / NO_PIT / NO_STRATEGY_CONVERSION / NOT_PROMOTED**。MR-13Z的Patch Transformer仍保留其Joint-Min Model Gate PASS，但不得外推為所有target的通用最佳backbone；不做MR-13AA PIT、strategy conversion、Patch tuning或backbone sweep。
- 下一個最小缺失cell分配合法identity=`MR-13AB`，profile=`daily_universal_first_risk_breach_pure_mfe_full_list_ndcg_pairwise`，architecture回到MR-13K相同`inception_time_v1`。其唯一scientific change為把MR-13K full-horizon Pure-MFE future path改成MR-13E既有的first-risk-breach truncation；target identity=`daily_first_risk_breach_pure_mfe_r_v1`。
- MR-13AB scalar固定=`favorable_return_before_first_risk_breach / risk_budget_return`。first-risk-breach path規則直接重用MR-13E canonical semantics：same-bar adverse-first、breach-day high不計、breach後upside不計；若第一根future bar即breach則target=`0R`。若40D完全無breach，target與MR-13K `daily_full_horizon_pure_mfe_r_v1`完全同義。Adverse-to-peak仍可作diagnostic component，但**不從target扣除**。
- Strict control固定MR-13K：daily-universal universe、300×10 input、InceptionTime、40D horizon、fixed canonical R scale、full-list ΔNDCG pairwise objective/reduction、Seed42、split、Adam optimizer、`mean_daily_spearman` epoch selection、training/sample scope全部不變。禁止Safety head、threshold、lambda、time penalty、product/fusion、portfolio state、PIT、strategy conversion、Multi-seed與Fixed Window。
