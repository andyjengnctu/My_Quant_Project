...
| 項目 | 目前狀態 |
|---|---|
| 基準 ZIP | `test-branch-1_20260817_220752_14e6bc7.zip`，SHA256 `1a012981c38b60b1e6dfe50794518a56fcd9bbb5815a6a16452207525490b402`。此基準已含MR-13L結案；本輪新增MR-13M adverse-only pairwise Forward model experiment，尚無結果。 |
| SHA256／最新結果 | MR-13E仍是production anchor。MR-13L Seed42 Forward：selected epoch=`1`、選模Validation Daily rho=`0.0227`；frozen OOS Daily/Global rho=`0.1052/0.0254`、Pair=`53.51%`、Top10−Bottom10=`-0.0150R`、Top-K Lift=`+0.2274R`；breakout Daily/Global rho=`0.0647/-0.0301`、Pair=`52.82%`、Top10−Bottom10=`-0.2560R`。同Target MR-13H為Daily rho=`0.2332`、Pair=`58.04%`、Top-K Lift=`+1.3400R`，故MR-13L在Forward model Gate REJECT，不建PIT。MR-13K的current 8-seed Selection/Forward strategy robustness仍獨立繼續。 |
| 程式版本範圍 | Active sequence architecture仍為`inception_time_v1`；目前config選定的Model Research profile=`daily_universal_full_horizon_low_adverse_full_list_ndcg_pairwise / MR-13M`，`selection_pit_authorized=False`，只授權Seed42 Forward model Gate；production workflow profile仍為`daily_universal_no_time_full_list_ndcg_pairwise / MR-13E`。MR-13K C42/C53與C44/C54 current 8-seed robustness是獨立strategy evaluation。 |
| Policy 預設 | filter=`breakout_quality_v1`、architecture=`inception_time_v1`、Seed=`42`；Model Research profile=`daily_universal_full_horizon_low_adverse_full_list_ndcg_pairwise`；Production workflow profile=`daily_universal_no_time_full_list_ndcg_pairwise`。PIT score start=`auto`、fold=`12` months、inner validation=`24` months；MR-13M research-spec固定`selection_pit_authorized=False`。 |
| 當前最佳實證模型 | Production anchor仍為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise` + Min/all-off + exact K/R0 constrained。MR-13H已完成16-seed full-flow且未形成Forward promotion；MR-13K single-seed Selection/Forward皆低於MR-13E，但依使用者決策仍完成current 8-seed robustness；MR-13L已Forward REJECT。MR-13M只做adverse-only learnability Forward probe，尚不影響策略/runtime。 |
