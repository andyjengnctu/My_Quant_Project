...
| 項目 | 目前狀態 |
|---|---|
| 基準 ZIP | `test-branch-1_20260817_015333_b309e28(2).zip`，SHA256 `a89ae89d18fdae8d01321423cf7cfae7b6511cd1fb1e31d91a946050dad32eb8`。此基準已包含MR-13H no-breach實作、formal gatefix與使用者完成的Label Audit／Seed42 Forward模型工件；本輪只推進PIT/Selection strategy授權，不改Target或模型。 |
| SHA256／最新結果 | MR-13E仍是production anchor。MR-13H Label Audit：common=`1,578,349`、breach=`31.31%`、changed=`6.65%`、Daily rho=`0.9435`、Top-K overlap=`94.53%`；Seed42 Forward all-stock OOS Daily/Global rho=`0.2332/0.1369`、Pair=`58.04%`、Top-K Lift=`+1.3400R`，breakout Daily rho=`0.2056`。同score對MR-13E reference Target Daily rho=`0.2140`、Pair=`57.41%`。判定Forward GO並授權Selection PIT。 |
| 程式版本範圍 | Active sequence architecture仍為`inception_time_v1`；Model Research active profile=`daily_universal_full_horizon_no_breach_full_list_ndcg_pairwise / MR-13H`，production workflow profile=`daily_universal_no_time_full_list_ndcg_pairwise / MR-13E`。MR-13H已通過Forward Gate，現授權單profile Selection PIT與`DL-CONT13H-PIT / SR-C51`；Forward strategy source尚未建立。其他淘汰architecture/profile只保留歷史重現。 |
| Policy 預設 | filter=`breakout_quality_v1`、architecture=`inception_time_v1`、Seed=`42`；Model Research profile=`daily_universal_full_horizon_no_breach_full_list_ndcg_pairwise`；Production workflow profile=`daily_universal_no_time_full_list_ndcg_pairwise`。PIT score start=`auto`、fold=`12` months、inner validation=`24` months；MR-13H以research-spec `selection_pit_authorized=True`走單profile PIT，不加入13C/D/E同Target batch。 |
| 當前最佳實證模型 | Production anchor仍為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise` + Min/all-off + exact K/R0 constrained；MR-13H已取得Forward GO，但尚未完成Selection PIT/strategy translation，不得預先取代MR-13E。 |
