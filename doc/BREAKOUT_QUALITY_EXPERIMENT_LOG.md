...
| 項目 | 目前狀態 |
|---|---|
| 基準 ZIP | `test-branch-1_20260817_213957_d6bddc0.zip`，SHA256 `c5c96c26ad643ba6c75e3f4ab630986974c454c14a137090c630b05026881c37`。此基準包含MR-13L decomposed MFE/adverse dual-MSE實作；使用者已完成Seed42 Forward model run，本輪依實際結果更新Registry/Log並保持PIT staged off。 |
...
| 程式版本範圍 | Active sequence architecture仍為`inception_time_v1`；目前config選定的Model Research profile=`daily_universal_full_horizon_mfe_adverse_dual_mse / MR-13L`，但其Seed42 Forward model Gate已REJECT，`selection_pit_authorized=False`，不得建立PIT/runtime source；production workflow profile仍為`daily_universal_no_time_full_list_ndcg_pairwise / MR-13E`。MR-13K C42/C53與C44/C54 current 8-seed robustness是獨立strategy evaluation，不受13L結案影響。 |
| Policy 預設 | filter=`breakout_quality_v1`、architecture=`inception_time_v1`、Seed=`42`；Model Research profile=`daily_universal_full_horizon_mfe_adverse_dual_mse`；Production workflow profile=`daily_universal_no_time_full_list_ndcg_pairwise`。PIT score start=`auto`、fold=`12` months、inner validation=`24` months；MR-13L research-spec固定`selection_pit_authorized=False`，Forward結果已不足以授權PIT。 |
| 當前最佳實證模型 | Production anchor仍為`MR-13E / daily_universal_no_time_full_list_ndcg_pairwise` + Min/all-off + exact K/R0 constrained。MR-13H已完成16-seed full-flow且未形成Forward promotion；MR-13K single-seed Selection/Forward皆低於MR-13E，但依使用者決策仍完成current 8-seed robustness；MR-13L則已在Forward model Gate結案，不進PIT。 |
