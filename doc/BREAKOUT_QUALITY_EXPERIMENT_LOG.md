...
- Scanner仍只負責candidate discovery／參考資金，不假裝具有portfolio K/R0/cash state；正式basket membership與exact constrained ownership仍在`core/portfolio_entry_selection.py`。production score過期或缺失時fail-fast並明示重新執行「套用／更新正式 Runtime」，不使用stale score fallback。
