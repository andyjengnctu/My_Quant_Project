...
| B171 | P1 | Breakout quality / 11A連續目標可學性稽核契約 | `strategy_aligned_opportunity_r_v1`必須只由canonical event anchor與固定future high／low path cache建立group-level target；公式、risk budget、horizon與time penalty不得由Selection／Validation／OOS、模型score或actual trade R估計。High／Low同日歧義必須採adverse-first，風險觸發日High排除，首日觸及−10%輸出−1R；未滿horizon或無效K線標記invalid。第一階段只允許輸出version-scoped arrays、manifest、分布／同日rankability與optional actual-R方向診斷，不得訓練、選epoch、normalization、clipping或runtime部署 | DONE | direct synthetic contract驗證固定versioned target、目前40-bar／10% policy推導、快速20%機會扣2% adverse與時間成本、晚到5%機會被0.5R完整時間懲罰抵銷、同棒30% High與−10% Low採保守−1R、future不足invalid、group arrays deterministic、strict JSON、同日非tie排序與PASS／REJECT concordance、CLI註冊（以static source解析，禁止`tools/`反向import `apps/`）、target-version輸出路徑及audit payload `training_performed=false`；完整本機Dataset結果仍須另行執行，不得由synthetic結果預判可學性 | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/audit_continuous_target.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
