...
| B15 | P1 | 錯誤處理 | 壞 JSON、缺參數、缺檔、匯入失敗、API 失敗時訊息可定位 | DONE | 已補 module 與 downloader fatal error path；breakout quality 對有效期後未覆蓋 candidate、缺少 candidate score、research-only artifact、過期 source CSV inventory、high_len coverage、hash/schema/path 不一致一律 fail-fast 並帶可定位訊息，不得靜默拒絕或自動改用其他檔案 | `core/params_io.py`, `tools/validate/synthetic_error_cases.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
