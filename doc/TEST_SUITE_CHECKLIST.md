...
| B183 | P1 | Breakout quality / Selection PIT Score排序與策略比較契約 | Selection策略比較必須明確載入已通過模型audit的point-in-time Score來源，嚴格核對filter、architecture、profile、Target、Seed、期間、coverage及來源工件SHA256；Baseline與Score Sort只能切換Quality Score排序，不得改候選生成、歷史active params、資金、持倉、rotation、成交、停損停利或帳務。有效Score由高到低，同分回退原buy-sort，缺分不得排除或填0，須排在有效Score後並沿用原排序，最後維持deterministic ticker。Future Target只能在兩組replay完成後離線join，用於orderable coverage、selected percentile、top-k retention與opportunity gap，不得進runtime排序或optimizer | DONE | direct synthetic驗證PIT score store與manifest／audit來源hash、模型gate、日期coverage、scoped runtime source還原、Score同分／缺分fallback、Baseline與Score Sort單一差異、Selection historical active-param來源、post-replay Target診斷及策略輸出隔離；正式Selection績效仍須由本機完整replay產生 | `filters/breakout_quality/ranking_score_store.py`, `filters/breakout_quality/runtime.py`, `core/buy_sort.py`, `core/portfolio_candidates.py`, `core/portfolio_engine.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
