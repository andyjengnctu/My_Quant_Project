...
| B159 | P1 | Meta / PROJECT_SETTINGS sort-guard 與一次找齊原則契約 | `doc/PROJECT_SETTINGS.md` 只保留 TEST_SUITE_CHECKLIST sort-guard / exhaustive-check / 穩定主題摘要原則；以 `validate_project_settings_checklist_guard_and_exhaustive_inspection_case` 驗證索引式摘要、一次找齊、穩定主題摘要與 GPT checklist 不列入本地 formal 驗證 | DONE | 已以單一 static meta contract 收斂 PROJECT_SETTINGS 高階原則；主表與 `DONE` 摘要只保留索引式摘要、正式 validator 入口與穩定主題摘要，不驗 GPT checklist 內容 | `doc/PROJECT_SETTINGS.md`, `doc/TEST_SUITE_CHECKLIST.md`, `tools/validate/synthetic_meta_cases.py`, `tools/validate/synthetic_cases.py` |
