...
| B57 | P1 | 契約 | 系統評分分子必須可在總報酬率與年化報酬率之間切換，且 score 公式不得把分子 / 分母 / MDD 懲罰次方切換混成不同 mode | DONE | 已補 direct synthetic contract，直接釘死 `SCORE_NUMERATOR_METHOD` 可在 `ANNUAL_RETURN` / `TOTAL_RETURN` 間切換，兩者都必須共用同一個 `abs(MDD) ** SCORE_MDD_POWER + SCORE_MDD_DENOMINATOR_EPSILON` 分母，且 `LOG_R2` 品質單調性不得被新分子與 MDD 次方選項破壞；另補 `SCORE_WIN_RATE_TARGET` 目標式勝率倍率 contract，避免勝率加權退化成只改絕對尺度、無法有效影響候選排序 | `tools/validate/synthetic_strategy_cases.py`, `config/training_policy.py`, `core/portfolio_stats.py` |
