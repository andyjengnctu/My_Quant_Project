...
| B179 | P1 | Breakout quality / 11I Selection nested-OOS策略實現覆蓋契約 | 11I不得使用正式2021～2026 rolling params倒灌Selection；必須以2014-01-01～2020-12-31、120個月fixed train window與12個月OOS建立lookahead-safe nested params，並透過`V16_MODELS_DIR`隔離到research models目錄。Audit replay只允許2014-01-01～11F `final_refit_date_range.end=2020-11-05`，超出即fail-fast，避免Target跨入2021；必須重用canonical no-filter active-param replay、candidate capture與round-trip reconstruction，將qualified／orderable／actual trades對齊11F No-time Target；未交易候選不得標為0R。只可輸出coverage、Target match、Target↔strategy R及PASS／REJECT條件診斷；不得建立Target arrays、訓練、選epoch、調optimizer／loss／threshold、覆蓋正式`roos_*.json`、產生runtime score或加入互動選單 | DONE | direct synthetic contract驗證signal-date對齊與candidate去重、Target／R方向、research models path、2014～2020 nested window、2020-11-05 Selection Target邊界與2020-12-31拒絕、outer rolling writer實際採用`V16_MODELS_DIR`且不建立正式models目錄、CLI-only registry、未交易候選保持unlabeled及無訓練授權；正式coverage結果須由本機nested optimizer與Selection replay產生 | `core/model_paths.py`, `tools/optimizer/outer_rolling_oos.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/audit_selection_strategy_realization.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
