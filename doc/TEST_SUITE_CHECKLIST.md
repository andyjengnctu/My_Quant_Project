...
| B49 | P1 | 排序輸出 | ranking / scoring 輸出可排序、可比較、無 NaN | DONE | 已新增 ranking / scoring sanity case，直接驗 EV / PROJ_COST / HIST_WIN_X_TRADES / ASSET_GROWTH / BUY_LIMIT_OVERAGE_THEN_PROJ_COST 排序值可用、方向一致與型別正確；買入限價超出幅度排序必須以前收未超出限價者優先，並以預估投入資金作第二順位 | `tools/validate/synthetic_strategy_cases.py`, `core/buy_sort.py`, `tools/scanner/` |
