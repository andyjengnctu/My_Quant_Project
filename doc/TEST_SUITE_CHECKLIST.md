...
| B157 | P1 | Meta / 正式入口 help 穩定主題摘要契約 | `apps/test_suite.py --help` 只保留穩定、跨模組、對使用者仍有意義的主題摘要；若需描述已實作測試覆蓋範圍，必須用穩定 theme token，而不得逐項列舉高波動 exact contract 名稱，避免正式入口 help 再度變成高維護同步面 | DONE | 已補 static meta contract，直接掃描 `apps/test_suite.py` 的 help 說明列，釘死其必須保留單股公開盈虧/權益一致性、保守可執行出場解讀、架構文件檔案樹同步、`summary-section-heading-uniqueness` 與 治理原則 / TEST_SUITE_CHECKLIST guard 等穩定主題摘要；避免新增單一 validator 後反覆回寫 help 長列舉 | `tools/validate/synthetic_meta_cases.py`, `apps/test_suite.py` |
| B158 | P1 | Meta / 正式入口 help 舊 wording / 裸用詞排除契約 | `apps/test_suite.py --help` 在文件瘦身或 canonical wording 調整後，不得殘留 legacy 契約 wording、過時 rename wording 或未指名的 bare `checklist` 用詞；help 摘要必須與穩定主題策略一致，避免新舊契約混排造成半套同步 | DONE | 已補 static meta contract，直接掃描 `apps/test_suite.py` 的 help 說明列，釘死其必須保留 `debug-backtest` 現金路徑穩定主題，且不得再殘留 `debug-backtest entry cash deduction static contract`、`public profit/equity consistency contract`、`conservative-executable-exit interpretation contract` 等同鏈已退役 exact-contract wording，亦不得出現未指名 bare `checklist`；檢查 bare term 時必須先白名單 canonical `TEST_SUITE_CHECKLIST`，再檢查其餘殘留 `checklist`；對同一 help 摘要鏈的 stale-wording 排除，不得只拿單一 exemplar 代替整體舊 contract-listing 排除；排除詞與對應 sub-check 名稱都必須收斂為中性、最小必要表述，不得在同一 validator 內重複保留已被較短片段覆蓋的長舊短語，也不得讓 active sub-check 直接掛被排除的完整 legacy fragment 名稱 | `tools/validate/synthetic_meta_cases.py`, `apps/test_suite.py` |
