...
| B182 | P1 | Breakout quality / 泛用profile workflow與Selection point-in-time continuous-ranker Score契約 | 正式互動入口只提供模型研究與驗證、策略績效驗證及設定／工件狀態，低階build／audit與strategy compare維持統一入口下的CLI子命令。Experiment profile的training objective必須是模型流程派送單一來源：binary classification沿用既有classification workflow與hard-filter策略比較；continuous ranker使用expanding rolling folds產生Selection PIT Scores。每個fold的Inner Train只可使用Validation起點前且Label已完成的groups，Validation只可使用Score起點前且Label已完成的groups，完整歷史refit後只能評分下一段未見期間。串接工件必須逐group唯一、完整覆蓋指定Selection期間、保存fold日期、selected epoch、checkpoint與Score SHA256及information cutoff；Score表不得包含Future Target、Label或OOS統計。狀態頁與策略route必須依當前objective顯示／執行，不得把PIT Score冒充binary runtime artifact或反向回退 | DONE | direct synthetic contract覆蓋binary／continuous profile自動解析、主選單模型與策略route、hard-filter與score-ranking參數政策隔離、config-driven CLI、月曆fold邊界、`label_eval_end_date` embargo、PASS-only train／validation／refit集合、score日期與cutoff、重複／缺失拒絕、Future Target排除及orderable `target_date` coverage；完整PIT模型結果仍須由本機完整Dataset執行，不得由synthetic預判有效性 | `config/breakout_quality.py`, `filters/breakout_quality/paths.py`, `tools/filters/breakout_quality/continuous_ranker_pipeline.py`, `tools/filters/breakout_quality/build_point_in_time_scores.py`, `tools/filters/breakout_quality/audit_point_in_time_scores.py`, `tools/filters/breakout_quality/train_continuous_ranker.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py`, `tools/validate/synthetic_cli_cases.py` |
