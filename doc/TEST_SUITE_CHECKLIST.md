...
|---|---|---|---|---|---|---|
| B11 | P1 | 契約 | 跨工具 schema / 欄位語意一致 | DONE | 已補 missed sell / trade log / stats 與 formal summaries 的 CSV / XLSX / JSON contract；breakout quality 另釘死策略 high_len、optimizer range、dataset coverage、source CSV inventory freshness、walk-forward outer policy、architecture-scoped canonical model/runtime path、research output 隔離、manifest/hash/schema、active threshold、indexed feature bank、event group index、future path cache 與純 K 線風險調整 Label 契約使用同一來源；相同 ticker/date 的 sequence feature 只保存一次，Label policy 變更只允許沿用 future path cache 快速 relabel，不得重算 feature bank；Label 以突破日收盤為基準，於觀察期內逐日計算截至當日 MFE 與 MAE，只有在尚未觸及最大不利跌幅前，MFE 嚴格大於最低漲幅且 MFE／MAE 嚴格大於最低比率才是 PASS；零 MAE 視為比率條件成立，同日同時符合 PASS 與風險上限採保守 adverse-first REJECT，未達風險調整機會或先觸及風險上限均為 REJECT，資料不足或 K 線無效只作內部 INVALID 並排除；`tiny_cnn_v1`、`multiscale_cnn_v1`、`multiscale_cnn_v2`、`multiscale_cnn_v3`、`multiscale_cnn_v4`、`multiscale_cnn_v5`、`multiscale_cnn_v6`、`multiscale_cnn_v7`、`multiscale_cnn_v8`、`multiscale_cnn_v10` 與 `residual_tcn_v1` 由單一 factory 與版本化 model spec 管理；中型多尺度 v1／v2／v3 固定使用相同 16/16/16 三個時間尺度 branch、參數量、GroupNorm、window-average/last pooling 與不含 global max 的 244-bar receptive field；v4／v5 均回到 v1 的 Level input，並只將 Long Branch channels 分別降為 8／12，形成 16/16/8 與 16/16/12 容量及 14,530／18,530 個參數；v6 維持 v1 的 Level input、16/16/16 容量與 23,522 個參數，只將 Long Branch dropout 由 0.25 提高為 0.40；v7 維持 v1 容量與 dropout，只將 Short Branch 改為 Return／Delta、Medium／Long 保留 Level；v8 維持 v1 容量與 dropout，只將 Medium Branch 改為 Return／Delta、Short／Long 保留 Level；v10 為 6A AdamW-only 實驗的 v1-equivalent 工件版本，模型圖、初始權重與 23,522 個參數均與 v1 相同，只用獨立 architecture path 隔離結果；v2 唯一差異為短／中期 branch 使用 canonical OHLCV Return／Delta、v3 再只把個股 O/H/L/C 變化改為相對同欄 0050 變化，三者長期 branch 均保留 level，且 v1／v2／v3 舊 manifest payload 不得因新增版本失效；Dataset 不因模型切換重建，checkpoint／manifest／path architecture 必須完全一致；training manifest 另釘死 optimizer name、final refit selected/target/actual optimizer steps、至少一個完整 Selection pass、class weight mode、time weight mode 與 sample-weight summary；Adam／AdamW 支援名稱必須由 policy 單一來源供 optimizer factory、CLI、manifest、報表與 artifact stale guard 共用；舊 manifest 缺少 optimizer_name 時只可依歷史事實視為 Adam，optimizer 切換不得觸發 Dataset 重建；預設不做 inverse-frequency class balancing，`year_balanced_sqrt` 只以事件年度作溫和平衡且不得改變總 group weight；禁止 runtime 使用 stale model、平行 `dl_pass`、自建 OOS 日期或 legacy score path | `tools/validate/synthetic_contract_cases.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
...
| 2026-07-22 | B11 | breakout quality 6A 新增 AdamW-only 訓練實驗、`multiscale_cnn_v10` v1-equivalent 工件版本與 optimizer manifest 契約，主表改回 PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/train.py` |
| 2026-07-22 | B11 | 驗證 v10 模型圖與 v1 相同、AdamW factory、CLI／manifest／報表單一來源與 Dataset 不重建後重新收斂為 DONE | PARTIAL -> DONE | `validate_breakout_quality_policy_single_source_case` |
| 2026-07-22 | T266 | 擴充 runtime artifact contract，納入 v10 architecture-scoped path 與 optimizer-name stale guard | DONE -> PARTIAL | `validate_breakout_quality_runtime_artifact_contract_case` |
| 2026-07-22 | T266 | 驗證十一種模型工件隔離、v10 checkpoint reconstruction 與 Adam／AdamW config 不一致 fail-fast 後重新收斂為 DONE | PARTIAL -> DONE | `validate_breakout_quality_runtime_artifact_contract_case` |
