...
| B57 | P1 | 契約 | 系統評分必須可在 RoMD 與總 R 模式間切換，RoMD 分子可在總報酬率與年化報酬率間切換，且 score 公式不得把分子 / 分母 / MDD 懲罰次方 / R 中位數校正混成不同 mode | DONE | 已補 direct synthetic contract，釘死 `SCORE_CALC_METHOD` 可在 `RoMD` / `LOG_R2` / `TOTAL_R` 間切換，`SCORE_NUMERATOR_METHOD` 可在 `ANNUAL_RETURN` / `TOTAL_RETURN` 間切換；RoMD / LOG_R2 必須共用 `abs(MDD) ** SCORE_MDD_POWER + SCORE_MDD_DENOMINATOR_EPSILON` 分母，TOTAL_R 必須直接使用投組完成交易總 R；並補 `SCORE_WIN_RATE_TARGET`、`SCORE_MIN_FULL_YEAR_RETURN_TARGET` 與 `SCORE_MEDIAN_R_TARGET` 目標式倍率 contract，避免校正器退化成只改絕對尺度、無法有效影響候選排序 | `tools/validate/synthetic_strategy_cases.py`, `config/training_policy.py`, `core/portfolio_stats.py` |
