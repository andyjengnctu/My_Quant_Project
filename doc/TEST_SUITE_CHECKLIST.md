...
| B182 | P1 | Breakout quality / 泛用profile workflow與Selection point-in-time continuous-ranker Score契約 | 正式互動入口固定為`[1/Enter] 模型研究與驗證`、`[2] 策略績效驗證`、`[3] 查看目前設定與工件狀態`與`[0] 離開`；低階build／audit與strategy compare維持統一入口下的CLI子命令。Experiment profile的training objective必須是模型流程派送單一來源：binary classification沿用既有classification workflow與hard-filter策略比較；continuous ranker必須先自動準備Dataset與profile指定的Continuous Target，再使用expanding rolling folds產生Selection PIT Scores。Target manifest必須驗證group count、Dataset policy及來源Dataset artifact SHA256；衍生Target必須另驗證來源Target manifest SHA256，Dataset重新掃描後不得沿用stale Target。每個fold的Inner Train只可使用Validation起點前且Label已完成的groups，Validation只可使用Score起點前且Label已完成的groups，完整歷史refit後只能評分下一段未見期間。串接工件必須逐group唯一、完整覆蓋指定Selection期間、保存fold日期、selected epoch、checkpoint與Score SHA256及information cutoff；Score表不得包含Future Target、Label或OOS統計。PIT audit必須以同一payload輸出表格化終端摘要、Markdown易讀報表與完整JSON，涵蓋設定、coverage、核心排序、年度穩定性、fold drift、binary重疊、orderable coverage與研究邊界；直接CLI可顯示正式工件路徑，但互動式模型研究流程必須啟用compact console，不顯示內部工件路徑、重覆storage資訊或逐ticker Dataset skip明細；Dataset skip只可在完成列依原因彙總，technical rebuild reasons只可收斂成單行使用者語意，已符合的Continuous Target不得重覆宣告，PIT reused folds只可於完成摘要聚合。狀態頁只顯示Dataset、Continuous Target、PIT Scores與PIT模型驗證的聚合完整度，不逐檔列路徑；不得把PIT Score冒充binary runtime artifact或反向回退 | DONE | direct synthetic contract覆蓋binary／continuous profile自動解析、主選單Dataset→Target→PIT→audit命令順序、Target缺少／stale重建與重建後再驗證、Dataset artifact identity拒絕、active No-time固定公式重建、hard-filter與score-ranking參數政策隔離、月曆fold邊界、`label_eval_end_date` embargo、PASS-only train／validation／refit集合、score日期與cutoff、重複／缺失拒絕、Future Target排除、orderable `target_date` coverage，以及console／Markdown報表固定章節、互動式compact路徑抑制、Dataset跳過原因彙總、rebuild reason單行化、current target靜默、reused fold聚合、狀態頁聚合顯示與未執行策略optimizer邊界；完整PIT模型結果仍須由本機完整Dataset執行，不得由synthetic預判有效性 | `config/breakout_quality.py`, `filters/breakout_quality/continuous_target.py`, `filters/breakout_quality/paths.py`, `tools/filters/breakout_quality/prepare_continuous_target.py`, `tools/filters/breakout_quality/audit_no_time_continuous_target.py`, `tools/filters/breakout_quality/continuous_ranker_pipeline.py`, `tools/filters/breakout_quality/build_point_in_time_scores.py`, `tools/filters/breakout_quality/audit_point_in_time_scores.py`, `tools/filters/breakout_quality/train_continuous_ranker.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py`, `tools/validate/synthetic_cli_cases.py` |
