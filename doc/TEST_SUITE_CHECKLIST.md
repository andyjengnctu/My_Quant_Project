...
| B185 | P1 | Breakout quality / Score-ranking capture attribution與彩色易讀報表契約 | `[2] 策略績效驗證`的score-ranking比較必須同時輸出原策略比較與read-only capture audit；兩者都要有Markdown、JSON及真正帶色HTML，終端亦須以綠／紅／黃／灰呈現改善、惡化、注意與中性，Markdown以🟢／🔴／🟡／⚪保留相同語意。Audit只可讀既有兩組replay transaction history、strategy summary及post-replay selected-target diagnostics，逐筆重建投入／預留、stop distance、fill、holding、partial日曆天數、依daily-capacity交易日曆計算的尾倉slot-days、entry-date／月份集中度、可用時的產業集中度、exit reason、Realized R、Target R、capture ratio、realization gap及年度差異；Future Target不得進runtime或optimizer。只有Target選擇改善、經濟效果失敗且至少一項可觀測資本／capture瓶頸成立時，才可標記為參數適應候選。`--capture-audit-only`只能重用既有完整工件，不得重跑portfolio或回退其他Score來源 | DONE | direct synthetic驗證trade lifecycle、平均投入、stop distance、partial日曆天數與daily-capacity交易slot-days、entry-date／月份集中度、產業欄位缺失N/A、Realized R／Target capture、decision gate、Future Target runtime隔離、主報表與audit的Markdown色彩標記、HTML色彩class、JSON／CSV工件完整性，以及`--capture-audit-only`只讀既有工件且不得重跑portfolio；正式3.78 replay的capture結果仍須由本機既有工件執行，不得由synthetic預判 | `tools/filters/breakout_quality/strategy_report_style.py`, `tools/filters/breakout_quality/audit_score_ranking_capture.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
