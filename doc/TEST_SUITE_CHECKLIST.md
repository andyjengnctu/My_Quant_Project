...
文件分工：`PROJECT_SETTINGS.md` 管原則；`TEST_SUITE_CHECKLIST.md` 管主表、狀態、測試入口與收斂索引；`GPT_DELIVERY_CHECKLIST.md` 管 GPT 交付前操作檢查。
...
| B147 | P1 | Meta / 正式入口摘要同步契約 | `apps/test_suite.py` 若保留頂部 coverage / contract 摘要註解並列舉 `Txx`，則該列舉必須與實際已掛入 synthetic registry 的同主題 contract ID 同步；不得讓正式入口摘要註解漏列最新 `Txx`，避免維護者誤判 formal 覆蓋範圍 | DONE | 已補 static meta contract，直接切出 `apps/test_suite.py` 頂部 exact-contract 摘要註解 block，釘死該摘要註解必須包含 `T225/T226/T229/T230/T231/T232/T233/T234/T235/T236/T237/T238/T239/T240/T241/T242/T243/T244/T245/T246/T247`，且必須明確提到 `T234`、`T235`、`T236`、`T237`、`T238`、`T239`、`T240`、`T241`、`T242`、`T243`、`T244`、`T245`、`T246` 與 `T247`，並同步提到 `checklist-summary-heading-uniqueness` 主題；不得保留漏列新 ID 的舊註解；對應 validator 也必須同步掛入 `tools/validate/synthetic_cases.py` registry，避免 checklist 已列 DONE 但 formal suite 未實際執行 | `tools/validate/synthetic_meta_cases.py`, `tools/validate/synthetic_cases.py`, `apps/test_suite.py` |
...
| B160 | P1 | Meta / GPT_DELIVERY_CHECKLIST 文件分工與交付前自檢契約 | `doc/GPT_DELIVERY_CHECKLIST.md` 必須存在並作為 assistant 交付前操作檢查表，自身也必須以顯式檔名明確區分 `PROJECT_SETTINGS.md` / `TEST_SUITE_CHECKLIST.md` / `GPT_DELIVERY_CHECKLIST.md` 文件分工；`doc/TEST_SUITE_CHECKLIST.md` 也必須以顯式檔名維持相同三份文件分工宣告，且兩者不得彼此漂移、不得重複宣告或保留多版本並存。並承接開始前基準與已讀文件回報細項、無新增 formal gap 回報、formal test chain target 逐項自檢、bundle 原始失敗逐條消失確認、`doc/TEST_SUITE_CHECKLIST.md` 機械排序區塊重排核對、同日 `G` 區塊依 formal tracking ID sort key 抽出重排、禁止尾端追加、全表 `G` guard 檢查、雙 checklist 分工文件與 `apps/test_suite.py` 摘要/說明的裸 `checklist` 用詞掃描、未清問題揭露、交付前最嚴格自檢，以及前一輪修改仍錯時，將防再犯要求上提為可泛化、可操作的交付前檢查，且不得只補單一案例、單一字串或局部實作特例；若本輪新增或調整 validator / Txx / Bxx，而 `apps/test_suite.py` 仍保留人工維護摘要註解或 `--help` 長說明，還必須全文搜尋並同步更新相關 Txx / contract 主題；若失敗來自 exact-string contract，新增或修改條款時還必須逐字核對 formal expected literal，不得只做 markdown 格式化、反引號包裝、全半形替換、標點微調或語意接近改寫；若本輪修改任何會被 formal contract / parser / meta guard 逐字比對的 literal，或其對應 canonical 名稱、追蹤 ID、正式入口摘要 / help 關鍵字，交付前必須從對應 validator、meta guard、parser 的 expected literal、禁止字串與比對條件反查，逐項核對所有正向與反向 literal，不得只憑語意相近、單一例句或局部全文搜尋視為完成；若本輪問題屬既有 Bxx / Txx / validator contract 鏈，交付前必須先建立同源 / 同鏈 / 同契約收斂清單，至少涵蓋主表項、對應 Txx、validator 內所有子檢查、registry impacted_modules、正式入口摘要 / `--help`、相關文件、負向 guard、唯一性 guard、排序 guard 與 bundle 原始失敗項，並至少列出鏈根、掃描範圍、逐項結果與未清阻塞；不得只以「已檢查」或「已同步」概括帶過；交付時還必須簡要回報同源 / 同鏈 / 同契約收斂清單結果，至少包含鏈根、掃描範圍、已清項與未清阻塞；未逐項核對並清空前不得交付，且對同一 validator function 或同一 impacted_modules 集合內的相鄰缺口必須一次掃完，不得以先修第一個命中項、單一 assertion 或單一字串命中就停止交付前檢查；`PROJECT_SETTINGS.md`、`doc/ARCHITECTURE.md`、`doc/CMD.md` 與 `apps/test_suite.py` 摘要/說明也必須同步承接 `PROJECT_SETTINGS / TEST_SUITE_CHECKLIST / GPT_DELIVERY_CHECKLIST` 文件分工，且在同時存在兩份 checklist 時，`PROJECT_SETTINGS.md`、`doc/ARCHITECTURE.md`、`doc/CMD.md` 與 `apps/test_suite.py` 摘要/說明不得再以裸 `checklist` 混稱 | DONE | 已補 static meta contract，直接掃描 `doc/GPT_DELIVERY_CHECKLIST.md`、`doc/PROJECT_SETTINGS.md`、`doc/TEST_SUITE_CHECKLIST.md`、`doc/ARCHITECTURE.md`、`doc/CMD.md` 與 `apps/test_suite.py`；釘死 GPT_DELIVERY_CHECKLIST 必須作為 assistant 每輪交付前操作檢查表，自身也必須以顯式檔名明確區分 `PROJECT_SETTINGS.md` / `TEST_SUITE_CHECKLIST.md` / `GPT_DELIVERY_CHECKLIST.md` 文件分工；同時 `doc/TEST_SUITE_CHECKLIST.md` 也必須以顯式檔名維持相同三份文件分工宣告，不得與 GPT checklist 漂移，且兩份 checklist 的文件分工宣告都必須維持單一真理來源，不得重複宣告或保留多版本並存。並包含開始前基準與已讀文件回報細項、無新增 formal gap 回報、formal test chain target 逐項自檢、bundle 原始失敗逐條消失確認、`doc/TEST_SUITE_CHECKLIST.md` 機械排序區塊整段重排、同日 `G` 區塊依 formal tracking ID sort key 抽出重排、禁止尾端追加、全表 `G` guard 檢查、雙 checklist 分工文件與 `apps/test_suite.py` 摘要/說明的裸 `checklist` 用詞掃描、未清問題揭露、交付前最嚴格自檢，以及前一輪修改仍錯時，將防再犯要求上提為可泛化、可操作的交付前檢查，且不得只補單一案例、單一字串或局部實作特例；若本輪新增或調整 validator / Txx / Bxx，而 `apps/test_suite.py` 仍保留人工維護摘要註解或 `--help` 長說明，則必須全文搜尋並同步更新相關 Txx / contract 主題，不得只更新 registry、checklist 或 meta guard；若本輪修改任何會被 formal contract / parser / meta guard 逐字比對的 literal，或其對應 canonical 名稱、追蹤 ID、正式入口摘要 / help 關鍵字，則必須從對應 validator、meta guard、parser 的 expected literal、禁止字串與比對條件反查，逐項核對所有正向與反向 literal，不得只憑語意相近、單一例句或局部全文搜尋視為完成；若本輪問題屬既有 Bxx / Txx / validator contract 鏈，則必須先建立同源 / 同鏈 / 同契約收斂清單，至少涵蓋主表項、對應 Txx、validator 內所有子檢查、registry impacted_modules、正式入口摘要 / `--help`、相關文件、負向 guard、唯一性 guard、排序 guard 與 bundle 原始失敗項，並至少列出鏈根、掃描範圍、逐項結果與未清阻塞；不得只以「已檢查」或「已同步」概括帶過；交付時還必須簡要回報同源 / 同鏈 / 同契約收斂清單結果，至少包含鏈根、掃描範圍、已清項與未清阻塞；未逐項核對並清空前不得交付，且對同一 validator function 或同一 impacted_modules 集合內的相鄰缺口必須一次掃完；且 `doc/PROJECT_SETTINGS.md` / `doc/ARCHITECTURE.md` / `doc/CMD.md` 也都必須明確承接 `PROJECT_SETTINGS / TEST_SUITE_CHECKLIST / GPT_DELIVERY_CHECKLIST` 三份文件角色分工；在同時存在兩份 checklist 時 `doc/PROJECT_SETTINGS.md` / `doc/ARCHITECTURE.md` / `doc/CMD.md` 與 `apps/test_suite.py` 摘要 / `--help` 長說明都不得再以裸 `checklist` 混稱，正式入口摘要註解與 `--help` 長說明除 `gpt-delivery-checklist governance contract` 主題外，也都必須同步提到 `PROJECT_SETTINGS / TEST_SUITE_CHECKLIST / GPT_DELIVERY_CHECKLIST` 文件分工 | `doc/GPT_DELIVERY_CHECKLIST.md`, `doc/PROJECT_SETTINGS.md`, `doc/TEST_SUITE_CHECKLIST.md`, `doc/ARCHITECTURE.md`, `doc/CMD.md`, `apps/test_suite.py`, `tools/validate/synthetic_meta_cases.py`, `tools/validate/synthetic_cases.py` |
