...
| B158 | P1 | Meta / 正式入口 help 舊 wording / 裸用詞排除契約 | `apps/test_suite.py --help` 在文件瘦身或 canonical wording 調整後，help 成品層不得回退成高波動舊列舉，且不得出現未指名的 bare `checklist` 用詞；正式範圍只驗 `--help` 輸出本體的穩定主題與最小必要排除，不延伸到 validator 命名、sub-check 文字或 GPT 自檢條款 | DONE | 已補 static meta contract，直接掃描 `apps/test_suite.py` 的 help 說明列，釘死其必須保留 `debug-backtest` 現金路徑穩定主題，且不得出現未指名 bare `checklist`；檢查 bare term 時先白名單 canonical `TEST_SUITE_CHECKLIST`，再檢查其餘殘留 `checklist`。formal scope 僅驗正式入口 help 成品層，不再把 validator 命名、sub-check wording、排除片段長短、單一 exemplar 選擇或 sibling 去重上升為正式契約 | `tools/validate/synthetic_meta_cases.py`, `apps/test_suite.py` |
