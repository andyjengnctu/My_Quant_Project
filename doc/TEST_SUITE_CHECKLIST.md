...
| B182 | P1 | Breakout quality / Selection point-in-time continuous-ranker Score契約 | Selection策略期間使用的continuous-ranker Score必須由expanding rolling folds產生；每個fold的Inner Train只可使用Validation起點前且Label已完成的groups，Validation只可使用Score起點前且Label已完成的groups，完整歷史refit後只能評分下一段未見期間。串接工件必須逐group唯一、完整覆蓋指定Selection期間、保存fold日期、selected epoch、checkpoint與Score SHA256及information cutoff；Score表不得包含Future Target、Label或OOS統計。正式互動入口只提供模型研究與驗證、策略績效驗證及設定／工件狀態，低階build／audit與舊strategy compare維持CLI相容 | DONE | direct synthetic contract覆蓋config-driven CLI、月曆fold邊界、`label_eval_end_date` embargo、PASS-only train／validation／refit集合、score日期與cutoff、重複／缺失拒絕、Future Target排除、orderable `target_date` coverage及單一入口路由；完整PIT模型結果仍須由本機完整Dataset執行，不得由synthetic預判有效性 | `config/breakout_quality_workflow.py`, `filters/breakout_quality/paths.py`, `tools/filters/breakout_quality/continuous_ranker_pipeline.py`, `tools/filters/breakout_quality/build_point_in_time_scores.py`, `tools/filters/breakout_quality/audit_point_in_time_scores.py`, `tools/filters/breakout_quality/train_continuous_ranker.py`, `apps/breakout_quality.py`, `apps/breakout_quality_strategy_compare.py`, `tools/validate/synthetic_breakout_quality_cases.py`, `tools/validate/synthetic_cli_cases.py` |
