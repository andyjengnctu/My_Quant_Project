...
| B15 | P1 | 錯誤處理 | 壞 JSON、缺參數、缺檔、匯入失敗、API 失敗時訊息可定位 | DONE | 已補 module 與 downloader fatal error path；breakout quality 對有效期後未覆蓋 candidate、缺少 candidate score、research-only artifact、過期 source CSV inventory、high_len coverage、外部Mantis來源revision／config／checkpoint／package版本、hash/schema/path 不一致一律 fail-fast 並帶可定位訊息，不得靜默拒絕或自動改用其他檔案 | `core/params_io.py`, `tools/validate/synthetic_error_cases.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B16 | P2 | CLI | 互斥參數、預設值、help 與 shipped 指令文件一致 | DONE | 已補 dataset wrapper、local regression / no-arg CLI 與剩餘直接入口 CLI 契約，覆蓋 help、預設 passthrough、`--only` / `--steps` 正規化、未知參數、缺值、空值、位置參數拒絕；`apps/workbench.py` 也已納入 no-arg CLI formal 邊界，不再只驗 help。另 `run_all.py` 參數錯誤 stderr usage 必須同步列出 `meta_quality`；breakout quality 易讀報表須在完整 OOS 下依固定 score 額外輸出逐年分類、固定 coverage 排序及校準診斷，部分 calendar year 明確標記且不得用於調參；`doc/CMD.md` 的 shipped Python 指令與主要參數也併入同一 formal 邊界 | `tools/validate/synthetic_cli_cases.py`, `tools/validate/synthetic_meta_cases.py`, `apps/*.py`, `core/runtime_utils.py`, `doc/CMD.md` |
...
| B27 | P1 | Meta | 禁止循環依賴（module dependency cycle） | DONE | 已補 project import graph cycle guard，直接阻擋 `apps/` / `core/` / `tools/` 間的循環依賴；絕對 import、相對 import 與函式內 lazy import 都必須納入 dependency graph。既有 core fast-data、validator fixture 與 optimizer orchestration 循環已拆除，不得以延遲載入規避架構 guard | `tools/validate/synthetic_meta_cases.py`, `tools/validate/meta_contracts.py`, `core/portfolio_fast_access.py`, `core/portfolio_fast_data.py`, `core/breakout_reentry.py`, `tools/validate/synthetic_cli_cases.py`, `tools/validate/synthetic_contract_cases.py`, `tools/optimizer/static_ensemble_dashboard.py`, `tools/optimizer/session_factory.py`, `tools/optimizer/callbacks.py`, `tools/optimizer/main.py`, `tools/optimizer/outer_rolling_oos.py` |
...
| B170 | P1 | Breakout quality固定策略經濟對照與交易歸因契約 | no-filter與active quality-filter策略回放必須使用同一歷史active-param來源、資金／持股／rotation、forward-OOS執行期間與0050 benchmark；Score artifact可另含首個執行日前的signal anchor，但不得使回放提前；正式OOS只接受完整覆蓋期間的Rolling OOS單一schedule或Rolling seed ensemble，逐日使用當日已生效參數。hard-filter模式兩組與每個member只能有`use_breakout_quality_filter`一項差異；score-ranking模式兩組hard filter皆為False，且只能有`use_breakout_quality_ranking`一項差異。Ranking在多member來源必須先依runtime member vote count降冪，再以同票候選Quality Score降冪，最後沿用既有buy-sort；`base-finalist-best`隔離模式必須驗證selector、每期1 member與min_agree=1，此時Quality Score為第一個有效排序欄位；不同param policy輸出必須隔離，避免覆蓋；hard-filter的不可評分事件維持保守REJECT，score-ranking的缺分事件不得排除或填0，須保存不可評分payload並回退原buy-sort；normal／continuation沿用原始breakout rank payload；Re-entry的確認日只作新交易訊號日，必須從成交持倉／逐member watch state繼承原始breakout rank payload，不得以確認日重新查score table。單一或static run_best只可顯式標記為非OOS診斷，不得把filter或ranking開關放回optimizer重新搜尋。報表必須保留報酬／回撤／穩定性、交易結構、每日候選供給缺口、執行後持股缺口與round-trip交易歸因；歸因必須重用正式成交紀錄與runtime score，精確區分共同交易、no-filter-only、quality-filter-only、直接門檻拒絕與投組路徑擠出，並使R歸因可對帳。年度完整性必須依實際比較區間首尾月份判定，截斷年度不得標成完整年度；research-only score不得冒充正式runtime score | DONE | 專用薄入口重用Portfolio Simulator canonical runner與core engine／stats；direct synthetic contract驗證single、static ensemble、rolling schedule與rolling ensemble只切filter、額外member差異fail-fast、active threshold／architecture／profile及runtime artifact一致、rolling期間完整覆蓋、兩組benchmark／交易日期相同、daily capacity、strict JSON、截斷年度標記、round-trip配對、直接filter拒絕分類與R歸因reconciliation；另驗證ranking pair只切ranking、單參數Score先於既有排序、ensemble vote仍第一且Score只重排同票候選；`--param-policy base-finalist-best/base-finalists-agree`解析canonical ROOS檔名、selector錯配fail-fast、finalist-best每期1 member/min_agree=1及輸出目錄隔離；hard-filter不可評分保守REJECT、score-ranking缺分保存`available=false`並回退原排序；normal／continuation state保存canonical rank，ensemble成交逐member保存原始rank mapping，STOP後Re-entry保留確認日作交易signal但沿用原始score_date，且不得觸發確認日runtime lookup。`--attribution-only`可從既有比較輸出重建歸因且不得重跑策略；當`--params`缺省且`--param-policy=auto`時正式模式fail-fast；static run_best須顯式`--allow-static-diagnostic`且報表標示非無前視 | `apps/breakout_quality.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/trade_attribution.py`, `filters/breakout_quality/score_store.py`, `filters/breakout_quality/runtime.py`, `core/portfolio_candidates.py`, `core/buy_sort.py`, `core/portfolio_entries.py`, `core/portfolio_exits.py`, `core/portfolio_engine.py`, `core/portfolio_stats.py`, `tools/validate/synthetic_breakout_quality_cases.py`, `tools/validate/synthetic_unit_cases.py` |
| B171 | P1 | Breakout quality / 11A連續目標可學性稽核契約 | `strategy_aligned_opportunity_r_v1`必須只由canonical event anchor與固定future high／low path cache建立group-level target；公式、risk budget、horizon與time penalty不得由Selection／Validation／OOS、模型score或actual trade R估計。High／Low同日歧義必須採adverse-first，風險觸發日High排除，首日觸及−10%輸出−1R；未滿horizon或無效K線標記invalid。第一階段只允許輸出version-scoped arrays、manifest、分布／同日rankability與optional actual-R方向診斷，不得訓練、選epoch、normalization、clipping或runtime部署。actual-R來源必須在active 9A正式模型輸出樹依語意優先序搜尋hard-filter與score-ranking的`strategy_compare*`工件；有metadata時只接受目前filter／architecture／profile及`historical_active_param_oos`結果。每個目錄優先使用canonical round-trip檔，缺少衍生檔時只可重用既有`reconstruct_round_trips`由`no_filter_trades.csv`重建，並支援顯式trade-history override，不得分叉Round-trip定義 | DONE | direct synthetic contract驗證固定versioned target、目前40-bar／10% policy推導、快速20%機會扣2% adverse與時間成本、晚到5%機會被0.5R完整時間懲罰抵銷、同棒30% High與−10% Low採保守−1R、future不足invalid、group arrays deterministic、strict JSON、同日非tie排序與PASS／REJECT concordance、CLI註冊（以static source解析，禁止`tools/`反向import `apps/`）、target-version輸出路徑、audit payload `training_performed=false`、hard-filter標準round-trip優先、`no_filter_trades.csv` canonical fallback、跨正式score-ranking目錄發現且跳過static diagnostic，以及顯式`--trade-history` override；完整本機Dataset結果不得由synthetic結果預判實際R方向 | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/audit_continuous_target.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/filters/breakout_quality/trade_attribution.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B172 | P1 | Breakout quality / 11B同日percentile研究訓練契約 | `strategy_aligned_daily_percentile_mse`必須是experiment profile而非architecture；保留active 9A `inception_time_v1`與2-logit checkpoint shape，以PASS softmax probability對同日11A target percentile做MSE。Percentile只能由同一日期有效groups的average rank建立，同值共享平均rank、singleton固定0.5，不得使用跨日期或OOS統計。Inner Train可更新gradient，Validation只可選epoch；checkpoint寫入前不得建立或讀取OOS percentile，OOS只可在完整Selection refit與模型凍結後研究評估。Profile必須research-only、無threshold、不得由binary classification training／pretraining／evaluate／report／regime／export入口、runtime artifact loader或forward-OOS export載入，且不得覆蓋9A正式工件；11B只保留`train-continuous-ranker` CLI子命令供研究重現，臨時research不得加入互動選單 | DONE | direct synthetic contract驗證profile／architecture分離、classification profile集合排除11B、同日percentile／ties／singleton精確結果、跨日期隔離、11A manifest與array hash strict loader、竄改fail-fast、CLI registry與選單排除11B、active 2-logit head與PASS probability、binary入口與runtime loader隔離、research-only輸出字樣與checkpoint-before-OOS source ordering；完整模型有效性仍須由本機正式訓練結果判定，不得由synthetic先標記ACCEPTED | `config/breakout_quality_experiments.py`, `filters/breakout_quality/continuous_target.py`, `filters/breakout_quality/artifacts.py`, `tools/filters/breakout_quality/train.py`, `tools/filters/breakout_quality/train_continuous_ranker.py`, `tools/filters/breakout_quality/pretrain.py`, `tools/filters/breakout_quality/export_scores.py`, `tools/filters/breakout_quality/evaluate.py`, `tools/filters/breakout_quality/report.py`, `tools/filters/breakout_quality/regime_audit.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B173 | P1 | Breakout quality / 11C qualified candidate-set只讀失敗歸因契約 | 11C必須重用hard-filter `historical_active_param_oos` no-filter的canonical portfolio replay取得qualified與orderable候選；未明確指定strategy compare目錄時，必須透過共用canonical discovery依序辨識policy-specific hard-filter輸出與舊`strategy_compare`相容目錄，不得另寫active-param、ensemble min-agree、history filter、資金／持股或buy-sort資格規則。Candidate diagnostic snapshot只可在明確傳入`replay_counts`時建立，需保存trade／candidate／原始signal date且不得洩漏params runtime object。Audit必須以ticker／signal_date對齊凍結11A target與11B research score，分離occurrence與unique group口徑，並比較all OOS、qualified、orderable、actual trades；metadata只接受hard-filter、lookahead-safe、`historical_active_param_oos`與threshold gate，candidate replay的報酬、MDD、RoMD、曝險、PnL與trade counts必須與原strategy comparison一致；每日unique signal必須以ticker／signal-date pair計數，actual signals須明確輸出qualified／orderable membership。全流程research-only、不得建立optimizer／loss／epoch／threshold／runtime scores；11C只保留`audit-qualified-candidate-set` CLI子命令，臨時research不得加入互動選單 | DONE | direct synthetic contract驗證candidate snapshot保留原始signal date且移除runtime object、重複candidate occurrence依ticker／signal date去重、每日ticker-signal pair計數、candidate capture不改正式no-filter績效、actual membership與Target／Score↔R方向不顛倒、invalid strategy metadata fail-fast、CLI-only registry／選單排除11C、policy-specific／legacy hard-filter目錄discovery與canonical replay單一來源、diagnostic-only無訓練授權；正式母體與交易結果仍須由本機完整OOS replay產生，不得由synthetic先標記11C結論 | `core/portfolio_engine.py`, `tools/portfolio_sim/simulation_runner.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/audit_qualified_candidate_set.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B174 | P1 | Breakout quality / 11D Target成分與Label條件只讀歸因契約 | 11D必須只讀11A manifest／component arrays、11B凍結OOS scores及11C qualified／actual-trade工件；component loader須驗證filename、size與SHA256，並以group index固定推導`favorable_r`、`adverse_r`與`time_penalty_r`，逐筆重建既有`target_raw_r`，不得重新定義、normalization或擬合權重。Audit須同時輸出qualified Score↔各Target成分、actual各成分↔realized R、Score／Target decile，以及PASS／REJECT條件下的Target↔R、Score↔Target、Score↔R與各成分↔R；全流程research-only、不得建立optimizer、checkpoint、sampling、epoch、threshold、runtime score或策略回測。11D只保留`audit-target-attribution` CLI子命令，臨時research不得加入互動選單 | DONE | direct synthetic contract驗證固定R單位成分可精確重建11A target、Score／Target／realized R方向不顛倒、PASS與REJECT條件指標均輸出、Markdown包含停止／conditional magnitude判定邊界、public strict component loader、CLI-only registry與無訓練授權；正式成分與Label條件結果仍須由本機既有11A／11B／11C工件產生 | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/audit_target_component_attribution.py`, `tools/filters/breakout_quality/audit_qualified_candidate_set.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B175 | P1 | Breakout quality / 11E固定Time-penalty消融只讀契約 | 11E必須strict讀取已完成11D報表與qualified／actual attribution CSV，驗證來源SHA256與research-only狀態；唯一允許的Target消融為`target_no_time_r=favorable_r-adverse_r`，並逐筆驗證原11A `target_raw_r=target_no_time_r-time_penalty_r`。Audit須比較qualified Score↔原Target／No-time Target、actual兩種Target↔realized R與top-bottom decile spread，以及PASS／REJECT條件結果；不得反向加分time penalty、搜尋係數、建立新target arrays、optimizer、checkpoint、sampling、epoch、threshold或runtime score。11E只保留`audit-target-time-ablation` CLI子命令，臨時research不得加入互動選單 | DONE | direct synthetic contract驗證固定favorable-minus-adverse推導、原Target精確重建、No-time與actual R方向及delta、decile spread、PASS／REJECT條件、Markdown單一變更與無訓練邊界、CLI-only registry、來源SHA256竄改fail-fast及無optimizer／checkpoint；正式結果仍須由本機既有11D工件產生 | `tools/filters/breakout_quality/audit_target_component_attribution.py`, `tools/filters/breakout_quality/audit_target_time_penalty_ablation.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B176 | P1 | Breakout quality / 11F No-time Target version與Selection-only可學性稽核契約 | 11F只可在11E overall、PASS與decile spread三個固定門檻均改善後，strict讀取11A component arrays及11E report／CSV SHA256，建立獨立`strategy_aligned_opportunity_no_time_r_v1` arrays。公式固定為`favorable_return/risk_budget-adverse_return/risk_budget`，必須沿用11A valid mask、peak、risk-breach與adverse-first語意，不得反向加分time或搜尋係數。Audit只能輸出Inner Train、Validation、Selection分布、Binary AUC、同日rankability、pair非tie、PASS／REJECT concordance及與11A Target關係；不得建立OOS指標、讀actual R／11B score、訓練、選epoch、normalization、profile、checkpoint、threshold或runtime score。公式假設須標記為prior iterative OOS informed但no OOS fitted coefficient；11F只保留`audit-no-time-target` CLI子命令，臨時research不得加入互動選單 | DONE | direct synthetic contract驗證新Target ID與固定R單位公式、invalid sentinel、Selection三split metrics與同日非tie、11E positive gate及artifact SHA256 tamper fail-fast、Markdown OOS誠實邊界、CLI-only registry、無optimizer／checkpoint與source code不計算OOS metrics；正式Selection結果仍須由本機11A／11E工件產生，不得由synthetic預判可學性 | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/audit_no_time_continuous_target.py`, `tools/filters/breakout_quality/audit_target_time_penalty_ablation.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B177 | P1 | Breakout quality / 11G PASS-conditional No-time magnitude研究訓練契約 | 11G必須以experiment profile `strategy_aligned_no_time_pass_magnitude_mse`管理，不得新增architecture；Target固定為11F `strategy_aligned_opportunity_no_time_r_v1`，training label scope固定為原始Label=PASS。Inner Train／Validation／完整Selection refit只可使用PASS groups，同日percentile只可由同日期PASS groups的average rank建立，REJECT target不得影響percentile、loss或epoch。Epoch只依Validation PASS-only mean daily Spearman選擇，同分才比較PASS-only MSE；checkpoint寫入前不得建立OOS percentile。模型凍結後主要判定只看OOS PASS-only排序與actual PASS trades Score↔R，overall／REJECT只作診斷。11G必須research-only、無threshold、無9A融合權重、不得匯出forward-OOS正式scores或加入互動選單 | DONE | direct synthetic contract驗證No-time target／PASS-only profile與classification集合隔離、PASS scope精確篩選、同日REJECT值不影響PASS percentile、既有CLI註冊且選單排除11G、checkpoint-before-OOS、PASS／REJECT actual trade分層、research-only及無runtime combination；正式有效性仍須由本機Selection訓練與凍結OOS結果判定 | `config/breakout_quality_experiments.py`, `tools/filters/breakout_quality/train_continuous_ranker.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B178 | P1 | Breakout quality / 11H PASS-only實現落差只讀歸因契約 | 11H必須只讀已完成11G PASS-only report／scores、11F No-time Target component arrays與11A canonical actual trade matches，逐一驗證artifact SHA256、research-only狀態、PASS scope、actual PASS配對數及`target_raw_r=favorable_r-adverse_r`。Audit只可聚焦OOS PASS與actual PASS，固定計算Score↔Target／Favorable／Adverse、`realization_gap_r=target_raw_r-r_multiple`、`favorable_capture_ratio=r_multiple/favorable_r`、控制Target或Favorable＋Adverse後的partial Spearman，以及依Score／Target top-bottom decile的成分、realized R、gap與capture ratio；不得修改Target、建立optimizer／profile／checkpoint、訓練、選epoch、調sampling／threshold、產生runtime score或策略回測。11H只保留`audit-pass-realization-gap` CLI子命令，臨時research不得加入互動選單 | DONE | direct synthetic contract驗證No-time Target逐筆重建、Score能學Target但與realized R反向、Score↔gap與partial rank歸因、decile未實現機會、Markdown gap／capture／判定邊界、CLI-only registry、11G／11A來源SHA256 fail-fast、actual PASS count一致及無訓練授權；正式結果仍須由本機11G／11F／11A凍結工件產生 | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/train_continuous_ranker.py`, `tools/filters/breakout_quality/audit_pass_realization_gap.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B179 | P1 | Breakout quality / 11I Selection nested-OOS策略實現覆蓋契約 | 11I不得使用正式2021～2026 rolling params倒灌Selection；必須以2014-01-01～2020-12-31、120個月fixed train window與12個月OOS建立lookahead-safe nested params，並透過`V16_MODELS_DIR`隔離到research models目錄；`--optimizer-trials`預設必須直接引用`config.training_policy.OPTIMIZER_OUTER_ROLLING_OOS_TRIALS_DEFAULT`，不得另設magic number，明確CLI值可單次覆蓋。Audit replay只允許2014-01-01～11F `final_refit_date_range.end=2020-11-05`，超出即fail-fast，避免Target跨入2021；必須重用canonical no-filter active-param replay、candidate capture與round-trip reconstruction，將qualified／orderable／actual trades對齊11F No-time Target；未交易候選不得標為0R。只可輸出coverage、Target match、Target↔strategy R及PASS／REJECT條件診斷；不得建立Target arrays、訓練、選epoch、調optimizer／loss／threshold、覆蓋正式`roos_*.json`、產生runtime score或加入互動選單 | DONE | direct synthetic contract驗證signal-date對齊與candidate去重、Target／R方向、research models path、2014～2020 nested window、2020-11-05 Selection Target邊界與2020-12-31拒絕、trial預設動態跟隨training policy且CLI override有效、不得殘留1000 magic number、outer rolling writer實際採用`V16_MODELS_DIR`且不建立正式models目錄、CLI-only registry、未交易候選保持unlabeled及無訓練授權；正式coverage結果須由本機nested optimizer與Selection replay產生 | `core/model_paths.py`, `tools/optimizer/outer_rolling_oos.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/audit_selection_strategy_realization.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B180 | P1 | Breakout quality / 11J canonical per-candidate counterfactual execution契約 | 11J必須以11I completed Selection nested-OOS結果為前置，且只在11I overall與PASS Target↔strategy R均為正、actual coverage未滿時執行。每個`ticker/signal_date`獨立移除portfolio capacity與cash competition，但candidate qualification／orderability仍由同一canonical replay產生；進場、shadow、每日出場、期末結算、費稅與R必須重用既有SSOT。候選發現必須以11I完全相同的2014-01-01～2020-11-05 replay完成，`replay_counts`必須是普通dict，不得傳入observer、自訂dict或counterfactual狀態機。Orderable執行資料只可透過獨立`replay_execution_rows` sidecar追加，sidecar不得recursive deepcopy含`_params_obj`的`signal_state`，只可保存entry SSOT必要欄位、params reference、cloned shadow與單一ticker market array。必須先以普通dict canonical rows精確通過2,003筆guard，才可離線執行；2020-11-06～2020-12-31依sidecar market calendars延伸，不得重跑portfolio。未成交訊號必須保持NaN，不得標0R。只可輸出qualified／orderable／filled／closed coverage與Target↔counterfactual R診斷；不得建立Target arrays、訓練、選epoch、調optimizer／loss／threshold、產生runtime score或加入互動選單 | DONE | direct synthetic驗證普通dict counts與sidecar分離、核心不存在research lifecycle callbacks、正式限價成交後由`execute_bar_step`停損、未成交維持NaN、coverage分母、canonical snapshot key、candidate／snapshot長度拒絕、params identity不複製、shadow獨立clone、sidecar market calendar、2,003 guard先於offline execution、CLI-only、11I report／artifact／params strict來源、entry／shadow／exit／accounting SSOT與無訓練授權；完整Selection結果須由本機nested replay產生 | `core/portfolio_engine.py`, `core/portfolio_entries.py`, `tools/portfolio_sim/simulation_runner.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/audit_candidate_counterfactual_execution.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B181 | P1 | Breakout quality / 11K portfolio selection-pressure只讀歸因契約 | 11K必須只讀11I completed JSON、orderable candidates與actual trade matches，逐一驗證artifact SHA256、filter、Target方向與actual coverage；不得重播portfolio、建立counterfactual、補未交易R或修改core。Orderable occurrence identity固定為ticker＋trade date，actual trade固定以ticker＋entry date對齊且必須是orderable完整子集。Audit須輸出同日Target percentile、selected top-half／top-quartile比例、按每日actual買入數k的Target top-k retention、Target opportunity gap、Target↔selected indicator、候選壓力分桶及actual selected percentile↔R；未選候選realized R保持NaN。11K只保留`audit-selection-pressure` CLI，禁止加入互動選單、訓練、Target arrays、optimizer、threshold或runtime ranking | DONE | direct synthetic驗證兩日競爭案例的selected percentile=0.75、top-k retention=0.5、Target gap=1.5R，未選候選R維持NaN、daily／bucket輸出完整、ticker＋trade-date duplicate fail-fast、Markdown非counterfactual邊界、CLI-only、11I artifact來源及`strategy_replay_performed=false`／`counterfactual_performed=false`／`training_performed=false` | `tools/filters/breakout_quality/audit_selection_strategy_realization.py`, `tools/filters/breakout_quality/audit_portfolio_selection_pressure.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B182 | P1 | Breakout quality / 泛用profile workflow與Selection point-in-time continuous-ranker Score契約 | 正式互動入口固定為`[1/Enter] 模型研究與驗證`、`[2] 策略績效驗證`、`[3] 查看目前設定與工件狀態`與`[0] 離開`；低階build／audit與strategy compare維持統一入口下的CLI子命令。Experiment profile的training objective必須是模型流程派送單一來源：binary classification沿用既有classification workflow與hard-filter策略比較；continuous ranker必須先自動準備Dataset與profile指定的Continuous Target，再使用expanding rolling folds產生Selection PIT Scores。Target manifest必須驗證group count、Dataset policy及來源Dataset artifact SHA256；衍生Target必須另驗證來源Target manifest SHA256，Dataset重新掃描後不得沿用stale Target。每個fold的Inner Train只可使用Validation起點前且Label已完成的groups，Validation只可使用Score起點前且Label已完成的groups，完整歷史refit後只能評分下一段未見期間。串接工件必須逐group唯一、完整覆蓋指定Selection期間、保存fold日期、selected epoch、checkpoint與Score SHA256及information cutoff；Score表不得包含Future Target、Label或OOS統計。PIT audit必須以同一payload輸出：直接CLI使用完整表格化終端報表，Markdown與JSON保留設定、coverage、核心排序、年度穩定性、fold drift、binary重疊、orderable coverage與研究邊界；互動式模型研究流程只顯示期間／coverage、PASS-only核心排序、年度穩定、分類重疊、orderable coverage、模型gate與下一步，不得重覆Workflow設定或完整fold表。直接CLI可顯示正式工件路徑，但互動流程必須啟用compact console，不顯示內部工件路徑、重覆storage資訊或逐ticker Dataset skip明細；Dataset skip只可在完成列依原因彙總，technical rebuild reasons只可收斂成單行使用者語意，基礎Target audit不得另行洗版，最終Continuous Target只可一行摘要，PIT epoch選擇與歷史refit不得逐epoch列印，新建fold只可一fold一行完成摘要，reused folds只可於完成摘要聚合。狀態頁只顯示Dataset、Continuous Target、PIT Scores與PIT模型驗證的聚合完整度，不逐檔列路徑；不得把PIT Score冒充binary runtime artifact或反向回退 | DONE | direct synthetic contract覆蓋binary／continuous profile自動解析、主選單Dataset→Target→PIT→audit命令順序、Target缺少／stale重建與重建後再驗證、Dataset artifact identity拒絕、active No-time固定公式重建、hard-filter與score-ranking參數政策隔離、月曆fold邊界、`label_eval_end_date` embargo、PASS-only train／validation／refit集合、score日期與cutoff、重複／缺失拒絕、Future Target排除、orderable `target_date` coverage，以及direct console／Markdown完整報表、互動compact核心摘要、compact路徑抑制、Dataset跳過原因彙總、rebuild reason單行化、current target靜默、基礎Target audit靜默、逐epoch輸出抑制、reused fold聚合、狀態頁聚合顯示與未執行策略optimizer邊界；完整PIT模型結果仍須由本機完整Dataset執行，不得由synthetic預判有效性 | `config/breakout_quality.py`, `filters/breakout_quality/continuous_target.py`, `filters/breakout_quality/paths.py`, `tools/filters/breakout_quality/prepare_continuous_target.py`, `tools/filters/breakout_quality/audit_no_time_continuous_target.py`, `tools/filters/breakout_quality/continuous_ranker_pipeline.py`, `tools/filters/breakout_quality/build_point_in_time_scores.py`, `tools/filters/breakout_quality/audit_point_in_time_scores.py`, `tools/filters/breakout_quality/train_continuous_ranker.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py`, `tools/validate/synthetic_cli_cases.py` |
| B183 | P1 | Breakout quality / Selection PIT Score排序與策略比較契約 | Selection策略比較必須明確載入已通過模型audit的point-in-time Score來源，嚴格核對filter、architecture、profile、Target、Seed、期間、coverage及來源工件SHA256；Baseline與Score Sort只能切換Quality Score排序，不得改候選生成、歷史active params、資金、持倉、rotation、成交、停損停利或帳務。有效Score由高到低，同分回退原buy-sort；缺分事件須保存`available=false`、原因、原始score date及score source，不得排除、填0、重新查分或在continuation／re-entry轉成可評分，須排在有效Score後並沿用原排序，最後維持deterministic ticker。Continuation／re-entry的交易signal date與原始Score事件日期必須分離；runtime Score identity、coverage及Future Target事後診斷一律使用保存的原始score date，交易與成交追蹤才使用當前signal date。Future Target只能在兩組replay完成後離線join，用於orderable coverage、selected percentile、top-k retention與opportunity gap，不得進runtime排序或optimizer | DONE | direct synthetic驗證PIT score store與manifest／audit來源hash、模型gate、日期coverage、scoped runtime source還原、Score同分／缺分fallback、不可評分payload可安全保存至candidate與continuation／re-entry且不發生`float(None)`、PIT source不遺失、Baseline與Score Sort單一差異、Selection historical active-param來源、原始score date對回PIT Score／Target、真正Score mismatch可追蹤fail-fast、post-replay Target診斷及策略輸出隔離；正式Selection績效仍須由本機完整replay產生 | `filters/breakout_quality/ranking_score_store.py`, `filters/breakout_quality/runtime.py`, `core/buy_sort.py`, `core/extended_signals.py`, `core/portfolio_candidates.py`, `core/breakout_reentry.py`, `core/portfolio_engine.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B184 | P1 | Breakout quality / 單一Seed與唯一正式入口契約 | breakout-quality所有正式模型流程只能由`config/breakout_quality.py`的單一`BREAKOUT_QUALITY_RANDOM_SEED`派送；binary、continuous、pretraining、PIT及strategy identity不得另設workflow專用Seed或依profile暗中切換預設值。正式Strategy Compare入口只能是`python apps/breakout_quality.py strategy-compare`，已刪除的`apps/breakout_quality_strategy_compare.py`不得重新出現；CLI `--seed`只可作單次重現覆寫，且不同Seed工件必須由fingerprint與strategy gate拒絕混接 | DONE | direct synthetic驗證canonical config只有一個可編輯Seed設定、所有profile忠實解析同一非負整數Seed、負值拒絕、workflow不引用第二個Seed symbol，以及舊strategy compare app實體不存在且唯一CLI由canonical app承接 | `config/breakout_quality.py`, `apps/breakout_quality.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
| B185 | P1 | Breakout quality / Score-ranking capture attribution與console易讀報表契約 | `[2] 策略績效驗證`的score-ranking比較必須同時在console完整顯示原策略比較與read-only capture audit；兩者共用統一標題、段落、表格、判讀與工件清單格式，TTY以綠／紅／黃／灰呈現改善、惡化、注意與中性，非TTY退回純文字，Markdown以🟢／🔴／🟡／⚪保留相同語意。正式檔案只輸出Markdown、JSON與必要CSV，不得建立HTML，且重建時須移除舊版HTML。所有console工件路徑只可顯示專案根目錄相對路徑。Audit只可讀既有兩組replay transaction history、strategy summary及post-replay selected-target diagnostics，逐筆重建投入／預留、stop distance、fill、holding、partial日曆天數、依daily-capacity交易日曆計算的尾倉slot-days、entry-date／月份集中度、可用時的產業集中度、exit reason、Realized R、Target R、capture ratio、realization gap及年度差異；Future Target不得進runtime或optimizer。只有Target選擇改善、經濟效果失敗且至少一項可觀測資本／capture瓶頸成立時，才可標記為參數適應候選。`--capture-audit-only`只能重用既有完整工件，不得重跑portfolio或回退其他Score來源 | DONE | direct synthetic驗證trade lifecycle、平均投入、stop distance、partial日曆天數與daily-capacity交易slot-days、entry-date／月份集中度、產業欄位缺失N/A、Realized R／Target capture、decision gate、Future Target runtime隔離、主報表與audit的console完整章節及強制色彩、Markdown色彩標記、JSON／CSV工件完整性、HTML不產生且舊檔清除、專案根目錄相對路徑，以及`--capture-audit-only`只讀既有工件且不得重跑portfolio；正式3.78 replay的capture結果仍須由本機既有工件執行，不得由synthetic預判 | `filters/breakout_quality/console_report.py`, `tools/filters/breakout_quality/strategy_report_style.py`, `tools/filters/breakout_quality/audit_score_ranking_capture.py`, `tools/filters/breakout_quality/strategy_compare.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
...
| T270 | `validate_breakout_quality_qualified_candidate_set_audit_contract_case` | B173 |
| T271 | `validate_breakout_quality_target_component_attribution_contract_case` | B174 |
| T272 | `validate_breakout_quality_target_time_penalty_ablation_contract_case` | B175 |
| T273 | `validate_breakout_quality_no_time_target_selection_audit_contract_case` | B176 |
| T274 | `validate_breakout_quality_pass_conditional_ranker_contract_case` | B177 |
| T275 | `validate_breakout_quality_pass_realization_gap_attribution_contract_case` | B178 |
| T276 | `validate_breakout_quality_selection_strategy_realization_contract_case` | B179 |
...
| T282 | `validate_breakout_quality_score_ranking_capture_audit_contract_case` | B185 |
...
| 2026-07-26 | B11 | sequence-only score export改為每個canonical ticker/date feature group只推論一次並精確廣播，dataset group/key與ranking score單一真理契約改回PARTIAL | DONE -> PARTIAL | `filters/breakout_quality/inference.py`, `tools/filters/breakout_quality/export_scores.py`, `tools/filters/breakout_quality/common.py`, `tools/filters/breakout_quality/evaluate.py` |
...
| 2026-07-26 | B170 | 新增固定參數no-filter vs active 9A策略經濟效果入口與每日capacity診斷，策略比較契約改為PARTIAL | NEW -> PARTIAL | `apps/breakout_quality.py`, `tools/filters/breakout_quality/strategy_compare.py`, `core/portfolio_engine.py` |
...
| 2026-07-26 | B170 | 9A策略OOS結果顯示報酬／回撤／EV惡化，新增round-trip交易歸因與截斷年度完整性修正，策略比較契約改回PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/trade_attribution.py`, `tools/filters/breakout_quality/strategy_compare.py`, `core/portfolio_stats.py` |
...
| 2026-07-26 | B170 | 新增Quality Score同票候選排序、原始signal date score沿用與探索性比較模式，策略比較契約改回PARTIAL | DONE -> PARTIAL | `core/portfolio_candidates.py`, `core/buy_sort.py`, `core/portfolio_engine.py`, `tools/filters/breakout_quality/strategy_compare.py` |
...
| 2026-07-26 | B170 | score-ranking首個OOS執行日需查前一交易日Score，發現score coverage與execution window混用，策略比較契約改回PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/export_scores.py` |
...
| 2026-07-29 | B171 | 新增11A固定連續target、group arrays與分布／同日rankability稽核CLI，研究契約由NEW進入PARTIAL | NEW -> PARTIAL | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/audit_continuous_target.py`, `apps/breakout_quality.py` |
...
| 2026-07-30 | B171 | 完整資料audit已完成但只尋找衍生`no_filter_round_trips.csv`，在保留原始`no_filter_trades.csv`時仍跳過actual-R診斷，optional診斷契約由DONE退回PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/audit_continuous_target.py` |
...
| 2026-07-30 | B173 | 新增11C candidate diagnostic replay capture、四層coverage audit CLI與互動選單`[11]`，契約由NEW進入PARTIAL | NEW -> PARTIAL | `core/portfolio_engine.py`, `tools/portfolio_sim/simulation_runner.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/audit_qualified_candidate_set.py`, `apps/breakout_quality.py` |
...
| 2026-07-30 | B173 | 獨立檢查發現每日unique signal只按日期計數、actual coverage未驗證集合membership且candidate capture只比trade count，契約由DONE退回PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/audit_qualified_candidate_set.py` |
| 2026-07-30 | B173 | 改以ticker／signal-date pair計數、actual集合交集coverage、qualified／orderable membership及完整no-filter績效摘要一致性後重新收斂為DONE | PARTIAL -> DONE | `tools/filters/breakout_quality/audit_qualified_candidate_set.py`, `tools/filters/breakout_quality/strategy_compare.py` |
| 2026-07-30 | B174 | 新增11D固定Target成分、qualified／actual與PASS／REJECT條件歸因CLI，研究契約由NEW進入PARTIAL | NEW -> PARTIAL | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/audit_target_component_attribution.py`, `apps/breakout_quality.py` |
...
| 2026-07-30 | B175 | 新增11E固定移除time penalty、qualified／actual與PASS／REJECT消融CLI，研究契約由NEW進入PARTIAL | NEW -> PARTIAL | `tools/filters/breakout_quality/audit_target_time_penalty_ablation.py`, `apps/breakout_quality.py` |
...
| 2026-07-30 | B176 | 新增11F獨立No-time Target arrays、11E fixed gate與Selection-only audit CLI，研究契約由NEW進入PARTIAL | NEW -> PARTIAL | `filters/breakout_quality/continuous_target.py`, `tools/filters/breakout_quality/audit_no_time_continuous_target.py`, `apps/breakout_quality.py` |
...
| 2026-07-30 | B178 | 11G結果淘汰後新增PASS-only realization-gap、capture ratio與partial rank只讀歸因CLI，研究契約由NEW進入PARTIAL | NEW -> PARTIAL | `tools/filters/breakout_quality/audit_pass_realization_gap.py`, `apps/breakout_quality.py` |
...
| 2026-07-30 | B179 | 11H確認策略實現落差後新增Selection nested-OOS參數隔離、canonical replay與coverage audit CLI，研究契約由NEW進入PARTIAL | NEW -> PARTIAL | `tools/optimizer/outer_rolling_oos.py`, `tools/filters/breakout_quality/audit_selection_strategy_realization.py`, `apps/breakout_quality.py` |
...
| 2026-07-30 | B179 | 發現11I `--optimizer-trials`另行硬編碼1000，未忠實採用training policy設定，契約由DONE退回PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/audit_selection_strategy_realization.py` |
...
| 2026-07-30 | B180 | 11I確認Target方向成立但actual coverage只有21.77%，新增per-candidate counterfactual execution契約 | NEW -> PARTIAL | `core/portfolio_engine.py`, `core/portfolio_entries.py`, `tools/filters/breakout_quality/audit_candidate_counterfactual_execution.py` |
...
| 2026-07-30 | T277 | 本機首次11J以單一延伸replay只重現1,978／2,003 qualified signals，兩階段日期語意缺口使契約由DONE退回PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/audit_candidate_counterfactual_execution.py` |
...
| 2026-07-31 | B180 | 兩階段replay仍只建立1,978／2,003 observer states，證明raw-candidate與candidate-row identity分叉，契約由DONE退回PARTIAL | DONE -> PARTIAL | `core/portfolio_engine.py`, `tools/filters/breakout_quality/audit_candidate_counterfactual_execution.py` |
...
| 2026-08-01 | B180 | 第五次本機11J在低記憶體單次replay下仍只重現1,969／2,003，證明自訂observer dict本身仍未忠實重現11I普通dict counts路徑，契約退回PARTIAL | DONE -> PARTIAL | `core/portfolio_engine.py`, `tools/filters/breakout_quality/audit_candidate_counterfactual_execution.py` |
| 2026-08-01 | B180 | 恢復普通dict canonical replay，移除research lifecycle callbacks，另以`replay_execution_rows` sidecar追加compact orderable執行快照後重新收斂為DONE | PARTIAL -> DONE | `core/portfolio_engine.py`, `tools/portfolio_sim/simulation_runner.py`, `tools/filters/breakout_quality/strategy_compare.py`, `validate_breakout_quality_candidate_counterfactual_execution_contract_case` |
...
| 2026-08-02 | B183 | 本機Score Sort在post-replay診斷以continuation／re-entry的新交易signal date查PIT table，將沿用原始score date的合法Score誤判為不一致，契約退回PARTIAL | DONE -> PARTIAL | `tools/filters/breakout_quality/strategy_compare.py` |
| 2026-08-02 | B183 | Score identity、coverage與Future Target診斷改用保存的原始score event date，交易signal date維持交易追蹤用途後重新收斂為DONE | PARTIAL -> DONE | `tools/filters/breakout_quality/strategy_compare.py`, `validate_breakout_quality_selection_point_in_time_score_sort_contract_case` |
...
| 2026-08-02 | B185 | 新增read-only Score-ranking capture attribution、主報表與audit的Markdown／JSON／彩色HTML、終端色彩及既有工件only模式並收斂為DONE | NEW -> DONE | `tools/filters/breakout_quality/strategy_report_style.py`, `tools/filters/breakout_quality/audit_score_ranking_capture.py`, `tools/filters/breakout_quality/strategy_compare.py`, `validate_breakout_quality_score_ranking_capture_audit_contract_case` |
| 2026-08-02 | B185 | 使用者要求易讀報表改為console直接顯示、停止HTML並統一相對路徑與簡易報表格式，契約退回PARTIAL | DONE -> PARTIAL | `filters/breakout_quality/console_report.py`, `tools/filters/breakout_quality/strategy_compare.py`, `tools/filters/breakout_quality/audit_score_ranking_capture.py` |
