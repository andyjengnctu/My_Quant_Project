...
| B180 | P1 | Breakout quality / 11J canonical per-candidate counterfactual execution契約 | 11J必須以11I completed Selection nested-OOS結果為前置，且只在11I overall與PASS Target↔strategy R均為正、actual coverage未滿時執行。每個`ticker/signal_date`獨立移除portfolio capacity與cash competition，但candidate qualification／orderability仍由同一canonical replay產生；進場、shadow、每日出場、期末結算、費稅與R必須重用既有SSOT。候選發現必須以11I完全相同的2014-01-01～2020-11-05 replay完成，identity直接使用portfolio engine既有canonical rows；capture不得recursive deepcopy含`_params_obj`的`signal_state`，不得重複保存qualified raw candidate。Orderable只可保存entry SSOT必要欄位、params reference、cloned shadow與單一ticker market array。通過2,003筆guard後才可離線執行，每日只掃描open positions；2020-11-06～2020-12-31必須直接依open positions的market calendars延伸，不得再建立第二次portfolio replay。未成交訊號必須保持NaN，不得標0R。只可輸出qualified／orderable／filled／closed coverage與Target↔counterfactual R診斷；不得建立Target arrays、訓練、選epoch、調optimizer／loss／threshold、產生runtime score或加入互動選單 | DONE | direct synthetic驗證正式限價成交後由`execute_bar_step`停損、未成交維持NaN、coverage分母、canonical snapshot key、candidate／snapshot長度拒絕、params identity不複製、shadow獨立clone、compact orderable capture、open-key-only管理、單次canonical replay、CLI-only、11I report／artifact／params strict來源、entry／shadow／exit／accounting SSOT與無訓練授權；完整Selection結果須由本機nested replay產生 | `core/portfolio_engine.py`, `core/portfolio_entries.py`, `tools/filters/breakout_quality/audit_candidate_counterfactual_execution.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
