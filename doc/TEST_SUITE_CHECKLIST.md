...
| B180 | P1 | Breakout quality / 11J canonical per-candidate counterfactual execution契約 | 11J必須以11I completed Selection nested-OOS結果為前置，且只在11I overall與PASS Target↔strategy R均為正、actual coverage未滿時執行。每個`ticker/signal_date`獨立移除portfolio capacity與cash competition，但candidate qualification／orderability仍由同一canonical replay產生；進場、shadow、每日出場、期末結算、費稅與R必須重用既有SSOT。候選只收至2020-11-05，持倉可管理至2020-12-31。未成交訊號必須保持NaN，不得標0R。只可輸出qualified／orderable／filled／closed coverage與Target↔counterfactual R診斷；不得建立Target arrays、訓練、選epoch、調optimizer／loss／threshold、產生runtime score或加入互動選單 | DONE | direct synthetic驗證正式限價成交後由`execute_bar_step`停損、未成交維持NaN、coverage分母、CLI-only、optional observer hooks、11I report／artifact／params strict來源、重用entry／shadow／exit／accounting SSOT與無訓練授權；完整Selection結果須由本機nested replay產生 | `core/portfolio_engine.py`, `core/portfolio_entries.py`, `tools/filters/breakout_quality/audit_candidate_counterfactual_execution.py`, `apps/breakout_quality.py`, `tools/validate/synthetic_breakout_quality_cases.py` |
