...
| B151 | P2 | 文件 / GUI workbench 細部分頁文件契約 | GUI workbench 的分頁細節改由程式與局部說明承接；`doc/CMD.md` / `doc/ARCHITECTURE.md` 不再以頁籤細節作主要承載面，避免架構與指令文件被 GUI 局部 UI 細節綁死 | N/A | 已自 long-term formal scope 移除；GUI workbench 的具體頁籤與局部 UI 細節不再由 `doc/CMD.md` / `doc/ARCHITECTURE.md` 承載，也不再由 formal validator 逐條釘死 | `doc/CMD.md`, `doc/ARCHITECTURE.md` |
| B152 | P2 | 文件 / trade_analysis rename 文件細節契約 | trade_analysis 的 legacy alias、legacy 輸出目錄命名與 rename 過程細節，不再要求由 `doc/CMD.md` / `doc/ARCHITECTURE.md` 長期承載；正式文件只保留子系統角色、正式入口與輸出位置等穩定資訊 | N/A | 已自 long-term formal scope 移除；trade_analysis rename 的 legacy alias / output-dir 細節不再由 `doc/CMD.md` / `doc/ARCHITECTURE.md` 逐條承接，也不再由文件同步 validator 釘死 | `doc/CMD.md`, `doc/ARCHITECTURE.md` |
| B153 | P2 | 契約 / trade_analysis canonical alias export 契約 | `tools/trade_analysis/` rename 後，package `tools.trade_analysis` 與 `tools/trade_analysis/trade_log.py` 必須同步提供 canonical `run_trade_analysis` / `run_trade_backtest` / `run_prepared_trade_backtest` / `run_ticker_analysis` aliases，並保留 legacy `run_debug_*` 相容入口；不得只剩 legacy debug 命名，避免模組名稱已切換但公開 API 仍被舊語意綁死 | DONE | 已補 static export contract，直接比對 `tools/trade_analysis/__init__.py` 與 `tools/trade_analysis/trade_log.py`；釘死 package 與 trade_log 都必須同時暴露 canonical aliases 與 legacy `run_debug_*` 相容 aliases；文件瘦身後不再要求 `doc/CMD.md` / `doc/ARCHITECTURE.md` 逐條承接 alias 細節 | `tools/validate/synthetic_meta_cases.py`, `tools/trade_analysis/__init__.py`, `tools/trade_analysis/trade_log.py` |
...
| B158 | P1 | Meta / 正式入口 help 舊 wording / 裸用詞排除契約 | `apps/test_suite.py --help` 在文件瘦身或 canonical wording 調整後，help 成品層不得回退成高波動舊列舉，且不得出現未指名的 bare `checklist` 用詞；正式範圍只驗 `--help` 輸出本體的穩定主題與最小必要排除，不延伸到 validator 命名、sub-check 文字或 GPT 自檢條款 | DONE | 已補 static meta contract，直接擷取 `apps/test_suite.py --help` 輸出並檢查說明列，釘死其必須保留 `debug-backtest` 現金路徑穩定主題，且不得回退到 `contract` 長列舉或殘留 bare `checklist`；formal scope 僅驗正式入口 help 成品層，不再把 validator 命名、sub-check wording、排除片段長短、單一 exemplar 選擇或 sibling 去重上升為正式契約 | `tools/validate/synthetic_meta_cases.py`, `apps/test_suite.py` |
...
| B166 | P2 | 文件 / ARCHITECTURE local_regression exact file-tree 契約 | `doc/ARCHITECTURE.md` 不再以 local_regression exact file-tree 承載 `run_meta_quality.py` 等局部檔名；架構文件只保留子系統角色與正式入口，不再逐條維護 local_regression 檔案樹 | N/A | 已自 long-term formal scope 移除；local_regression 的局部 shipped 檔名不再由 `doc/ARCHITECTURE.md` exact file-tree 承接 | `doc/ARCHITECTURE.md` |
...
| 2026-04-13 | B151 | 依文件瘦身方向將 GUI workbench 分頁細節退出 `doc/CMD.md` / `doc/ARCHITECTURE.md` 的 long-term formal scope，改列 N/A | DONE -> N/A | `doc/CMD.md` |
...
| 2026-04-13 | B152 | 依文件瘦身方向將 trade_analysis rename legacy alias / output-dir 細節退出 `doc/CMD.md` / `doc/ARCHITECTURE.md` 的 long-term formal scope，改列 N/A | DONE -> N/A | `doc/ARCHITECTURE.md` |
...
| 2026-04-13 | B162 | 依文件瘦身方向將 ARCHITECTURE apps exact file-tree 同步契約退出 long-term formal scope，改列 N/A | DONE -> N/A | `doc/ARCHITECTURE.md` |
| 2026-04-13 | B163 | 依文件瘦身方向將 ARCHITECTURE models exact file-tree 同步契約退出 long-term formal scope，改列 N/A | DONE -> N/A | `doc/ARCHITECTURE.md` |
...
| 2026-04-13 | T237 | 依文件瘦身方向將 GUI workbench 文件同步 validator 退出 formal registry，改列 N/A | DONE -> N/A | `validate_gui_workbench_documentation_sync_case` |
...
| 2026-04-13 | T238 | 依文件瘦身方向將 trade_analysis legacy naming 文件同步 validator 退出 formal registry，改列 N/A | DONE -> N/A | `validate_trade_analysis_legacy_naming_documentation_contract_case` |
...
| 2026-04-13 | T249 | 依文件瘦身方向將 ARCHITECTURE apps exact file-tree validator 退出 formal registry，改列 N/A | DONE -> N/A | `validate_architecture_workbench_entry_file_tree_sync_case` |
| 2026-04-13 | T250 | 依文件瘦身方向將 ARCHITECTURE models exact file-tree validator 退出 formal registry，改列 N/A | DONE -> N/A | `validate_architecture_models_best_params_file_tree_sync_case` |
...
| 2026-04-13 | T253 | 依文件瘦身方向將 ARCHITECTURE local_regression exact file-tree validator 退出 formal registry，改列 N/A | DONE -> N/A | `validate_architecture_local_regression_meta_quality_file_tree_sync_case` |
