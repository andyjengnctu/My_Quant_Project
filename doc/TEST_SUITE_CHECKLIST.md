...
| B57 | P1 | 契約 | 系統評分模型必須維持為模型選項，總 R 必須屬於評分分子選項，且 score 公式不得把分子 / 分母 / MDD 懲罰次方 / R 中位數校正混成不同 mode；`TOTAL_R` 與 `SCORE_MEDIAN_R_AMP` 的 R 來源必須固定為單股回測統計 | DONE | 已補 direct synthetic contract，釘死 `SCORE_CALC_METHOD` 可在 `RoMD` / `LOG_R2` 間切換，`SCORE_NUMERATOR_METHOD` 可在 `ANNUAL_RETURN` / `TOTAL_RETURN` / `TOTAL_R` 間切換；RoMD / LOG_R2 必須共用 `abs(MDD) ** SCORE_MDD_POWER + SCORE_MDD_DENOMINATOR_EPSILON` 分母，TOTAL_R 只作為分子並仍需通過同一個 MDD 分母；`TOTAL_R` 分子與 R 中位數校正一律使用單股回測總 R / R 中位數，其它 MDD、勝率、年度最差報酬等 score 欄位仍維持投組口徑；並補 `SCORE_WIN_RATE_TARGET`、`SCORE_MIN_FULL_YEAR_RETURN_TARGET` 與 `SCORE_MEDIAN_R_TARGET` 目標式倍率 contract，避免校正器退化成只改絕對尺度、無法有效影響候選排序 | `tools/validate/synthetic_strategy_cases.py`, `config/training_policy.py`, `core/portfolio_stats.py`, `core/portfolio_fast_data.py` |
