...
3. 若新舊 formal 規則只是同一 shipped 契約下的子面向擴充，優先併入既有 `Bxx` / `Txx`；不要把已確認可合併的同族 CLI、文件、coverage completeness 或 per-file minimum 子項再拆成平行主表 ID。若 repo 內既有獨立主表尚未實際退役，不得先在 GPT checklist 宣稱已一律合併。
