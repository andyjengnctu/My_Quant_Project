...
14. 若本輪修改 formal-facing 文件、formal validator、required fragment、排除詞或 stale-wording 檢查，交付前必須確認其中不再直掛 `project-settings`、`PROJECT_SETTINGS.md`、`GPT_DELIVERY_CHECKLIST.md` 或其他 GPT 控制面來源名稱；若僅需表達 guardrail / governance 語意，一律改用中性主題名稱。
15. 除使用者明確要求註解清理，或該註解／docstring 會被 parser、`--help`、UI、report、export、bundle 或 formal contract 直接讀取／輸出外，一般註解不納入 GPT 交付前最嚴格檢查與交付阻塞。
16. 若本輪將既有 formal contract 改列 `N/A`、compatibility stub 或其他非正式長期路徑，交付前必須同步檢查 registry completeness、defined/imported validator set、`doc/TEST_SUITE_CHECKLIST.md` parser 與 `done/unfinished` 摘要。
