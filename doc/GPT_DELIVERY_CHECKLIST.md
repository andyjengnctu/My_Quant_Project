...
9. 若本輪維護 `apps/test_suite.py` 頂部 exact-contract 摘要註解中的 `Txx` 清單，交付前必須自 `doc/TEST_SUITE_CHECKLIST.md` 的同主題 `T` 映射反查首尾區間是否仍有被跳過的中間 ID，並同步核對對應主題字串；不得只補尾端新增 `Txx`、跳過同主題既有 ID，或只因 `--help` 已有相近主題就視為完成。
...
11. 若本輪問題屬既有 Bxx / Txx / validator contract 鏈，交付前必須先建立同源 / 同鏈 / 同契約收斂清單，至少涵蓋主表項、對應 Txx、validator 內所有子檢查、registry impacted_modules、正式入口摘要註解 / `--help` 長說明、相關文件、負向 guard、唯一性 guard、排序 guard 與 bundle 原始失敗項；並至少列出鏈根、掃描範圍、逐項結果與未清阻塞；不得只以「已檢查」或「已同步」概括帶過；未逐項核對並清空前，不得交付。
