...
- `outputs/trading/account/`：Workbench 帳戶儀表板與績效統計的**唯讀衍生快照**；`account_snapshot.json` 含最新估值/風控/read model，`performance_summary.json` 含「庫存股／平倉股／加總」統計與 account event 重建的完整平倉紀錄。此目錄不是 mutable account truth；現金、open positions 與 revision/event chain 仍只存在 `state/trading/account.json`。
