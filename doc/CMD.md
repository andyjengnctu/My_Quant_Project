...
- 「實際交易」主頁以 **資訊儀表板 → 每日 Trading 流程 → 今日 Scanner Pool → 帳戶/持股 → 績效統計** 為主線。頂部「Trading 操作總覽」仍只消費 canonical Operations Status；帳戶儀表板則由 `services/trading/account_dashboard.py` 讀取 canonical account state、Trading V2 最新市價與 canonical Trading Params，顯示現金、持股市值、帳戶淨值、未實現PnL、單筆風險預算與已管理持倉 Stop 風險。`state/trading/account.json` 仍是唯一可寫 account truth；`outputs/trading/account/account_snapshot.json` 與 `performance_summary.json` 只是唯讀衍生快照，不得被交易 mutation 反向讀取。
- 實際交易頁的「今日 Scanner Pool」直接攤開 canonical candidate 順序與類型、買入限價、初始 Stop、Target/完成線、EV、歷史勝率、交易次數、資產成長、參考投入與 Scanner 摘要；使用者依此自行到券商下單。雙擊 Scanner 股票或持有股會切到「單股回測檢視」的 Trading 模式。原本建議掛單、券商 ORDERED/PARTIAL/FILLED/CANCELLED、Stop/TP 與 Indicator SELL 操作仍保留，但收在頁面底部「進階」分頁，不再主導每日檢視。頁面仍提供 read-only「實盤就緒檢查」，輸出 `outputs/trading/operational_audit/prelive_audit.json/.md`。
