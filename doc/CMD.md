...
- `[1] Daily Update` 互動模式現在先選 `Enter`（正常 DUE ONLY）、`R`（重新下載／重新驗證 current Trading target）或 `0` 返回。`R` 只建立新的 Trading overlay force-refresh batch：unique refresh identity 使舊 batch/ledger artifact不能被當成本輪完成證據，provider data request也會 bypass process cache；不重跑 `[2]/[3]`、不改 Provider Snapshot、Research或歷史 bootstrap。下載過程會以同一行即時顯示 request進度、目前 dataset/data_id、RUN/DONE/REUSE、本輪 data/usage request與executor已知 quota；這些數字直接來自 executor/shared client，不為顯示另打一套 provider usage call。完成摘要分開列 `Archive datasets READY x/51` 與 active strategy `Trading required READY x/N`，並在 force-refresh 顯示 schema-observed 與 StockInfo/delisting current-universe reference差異。instrument reference屬 non-blocking diagnostic：它可指出 observed/reference/missing/extra IDs，但在未有能處理停牌／dataset-specific合法缺列的 authoritative active-universe contract前，不得把 MATCH/DIFF 冒充「所有股票絕對完整」的正式 gate。
