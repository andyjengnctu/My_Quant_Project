...
- 「交易中心」專注 **資訊儀表板 → 每日 Trading 流程 → 今日 Scanner Pool → 成交登錄**。頂部「Trading 操作總覽」只消費 canonical Operations Status；Scanner 直接攤開 canonical candidate 順序與類型、買入限價、初始 Stop、Target/完成線、EV、歷史勝率、交易次數、資產成長、參考投入與 Scanner 摘要，使用者依此自行到券商下單。成交登錄只輸入股票、數量、成交價、成交日；若能唯一對應既有 active broker order，系統自動走 canonical order/account reconciliation，否則以實際帳戶成交事件入帳。價金、買入/賣出手續費、交易稅、成本與損益全部由 SSOT 自動計算。原本建議掛單、券商 ORDERED/PARTIAL/FILLED/CANCELLED、Stop/TP 與 Indicator SELL 操作仍保留在頁面底部「進階」分頁。頁面仍提供 read-only「實盤就緒檢查」，輸出 `outputs/trading/operational_audit/prelive_audit.json/.md`。
- 「帳務中心」獨立管理帳戶現金、庫存股、逐筆買入明細、逐筆賣出明細／沖抵明細與績效統計。庫存欄位採券商口徑顯示「股票、均價、股數、持有成本、市價、市值、損益、報酬率」；均價以實際成交價加權，不含買入手續費，持有成本則為剩餘買入價金 + 剩餘買入手續費。實際帳戶成交的 broker fee 使用未折扣 `0.001425`，不得回頭改變 Research／回測 execution policy；交易稅仍由 canonical security-profile 稅制解析。賣出損益為「賣出價金 − 賣出手續費 − 交易稅 − 沖抵持有成本」。沖抵依 canonical 平均成本比例分攤，不臆測券商未提供的 FIFO/lot 配對規則。`state/trading/account.json` 仍是唯一可寫 account truth；`outputs/trading/account/account_snapshot.json` 與 `performance_summary.json` 只是唯讀衍生快照，不得被交易 mutation 反向讀取。
