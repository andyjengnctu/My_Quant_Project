...
- 「單股回測檢視」預設為 `Trading` 模式，仍可切換 `Research / Trading` 並提供「持有股」下拉。Research 模式保留既有 Research dataset/Params/fixed-risk 與歷史績效股控制；Trading 模式鎖定 canonical Trading V2 + canonical Trading Params，表單不顯示 Research-only 的「參數／固定風險／計算歷史績效股」控制，也不顯示 `Scanner：...` 說明列，只保留 Trading 實際需要的候選、持股、檢視與圖表控制。若股票來自最新 Trading Scanner Pool，回測視圖優先使用該 candidate 已凍結的 representative Params。Trading 模式按「計算候選股」會直接執行同一 `run_trading_candidate_scan()`，並在獨立「Scanner Pool」分頁列出與實際交易頁相同的最新 pool；持有股下拉或 Trading 主頁導覽都可直接載入單股 K 線。
