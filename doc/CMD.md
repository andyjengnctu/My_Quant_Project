...
Workbench「完整更新 Trading 資料」與 Smart Downloader `[1]` 在同一 `services/trading/market_data_update.py` process 中共用一個 FinMind request client/cache。`TaiwanStockPriceAdj` 仍由 FinMind 原生資料決定，不能因為 V2 只修最近數日就把 execution-critical legacy CSV 改成 append-only；每個新 Trading day仍保留 current-vintage full-history refresh semantic。當大量 ticker stale 時，canonical refresher以六個月 full-market date ranges 抓完整歷史並本地拆成既有 `data/trading/tw_stock_data_vip/<ticker>.csv`；只有少數 ticker stale 時則自動選較便宜的 per-ticker full-history path。Full-market capability/coverage不成立時 fail-safe 回到舊 per-ticker exact path，不發布部分歷史；bulk結果若讓任何可讀既有CSV的歷史日期消失，也視為coverage失敗並fallback。
