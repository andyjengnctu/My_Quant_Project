...
`tools/trade_analysis/trade_log.py` 為 debug 正式入口；`tools/trade_analysis/backtest.py` 只保留主控 façade，進場流程在 `tools/trade_analysis/entry_flow.py`，出場 / 錯失賣出 / 期末結算在 `tools/trade_analysis/exit_flow.py`，明細列建構 helper 在 `tools/trade_analysis/log_rows.py`，K 線 payload / HTML 輸出 / GUI 內嵌 matplotlib figure 在 `tools/trade_analysis/charting.py`，輸出摘要在 `tools/trade_analysis/reporting.py`。
