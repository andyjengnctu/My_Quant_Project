...
未提供 `--target-date` 時，worker 由 READY Provider Snapshot + V2 archive/dataset/discovery state 解析 current target，再讀 `state/trading/market_data_v2/market_date_discovery_state.json` 判斷是否到新 completed Trading day 的 probe 時點；預設 17:45 前、weekend 與已排定 next probe 前一律 0 provider call。只有 discovery due 才用 `TaiwanStockPriceAdj` current full-market probe；發現新 completed Trading day 時只推進 V2 discovery target，不再呼叫 Legacy Trading update。51-dataset V2 仍依 publication/due planner 分時更新；`--target-date` 明確指定時跳過 discovery，只做該 target deterministic due/recovery。
...
### Legacy Trading Canonical Fetch 去重（過渡相容）

Workbench「完整更新 Trading 資料」目前仍可在 Legacy transition 的 `services/trading/market_data_update.py` process 使用 FinMind request client/cache；Smart Downloader `[1]` 已於 Round 5 切至 V2-only daily updater，不再進入此路徑。`TaiwanStockPriceAdj` 仍由 FinMind 原生資料決定，不能因為 V2 只修最近數日就把 execution-critical legacy CSV 改成 append-only；每個新 Trading day仍保留 current-vintage full-history refresh semantic。當大量 ticker stale 且每個 target ticker 都已有可讀 legacy history baseline 時，canonical refresher才可用六個月 full-market date ranges 抓完整歷史；每個可由 `TaiwanStockTradingDate` 覆蓋的 range 都必須包含 calendar 已發布的所有交易日，且不得讓任何既有 CSV 歷史日期消失；當日 completed market date 仍由較早發布的 PriceAdj probe 決定，不等待 calendar 同日更新，全部驗證通過後才原子發布。只要出現新 ticker、缺檔或不可讀 CSV，就直接改走 canonical per-ticker PriceAdj full-history producer；沒有可讀 PriceAdj baseline 時以 PriceAdj provider response 本身作 current-vintage canonical history，不再用 raw `TaiwanStockPrice` 強迫逐日全歷史一致。Actionable ticker 仍必須有 confirmed market-date PriceAdj row；Full-market capability/calendar coverage 不成立時亦 fail-safe 回到 per-ticker 路徑，不發布部分歷史。

目前 config `1994-10-01` 起算、六個月一段，到 2026-09-07 共 64 個 PriceAdj range identities；bulk completeness 另外查一次 no-date `TaiwanStockTradingDate`，但完整 V2 planner 若包含相同 request identity 可由 process-local cache 重用。新／缺／不可讀 ticker 會依 stale ticker 數改走 per-ticker PriceAdj full-history，而且每個 stale ticker只計一個 PriceAdj request，不再額外打一個 raw full-history date-evidence request；provider不支援 bulk 或觸發 fallback 時實際 request 仍可能高於正常 bulk 路徑。`provider_request_session` 會回報 total/legacy/V2 provider request counts、cache hit/miss與 uncached historical bulk fetch；歷史 bulk raw frame用完即釋放，不會把64段全市場原始資料全部留在shared cache。這些只是 operational metrics，不改 neutral Provider Snapshot、registry fingerprint、V2 logical manifest或 Trading execution truth。
