...
- `BREAKOUT_QUALITY_MODEL_ARCHITECTURE` 只描述網路與輸入結構。目前 active且已接受的排序／高品質基準為9A `inception_time_v1`；8F `multiscale_cnn_sequence_only_v1` 保留為高覆蓋實證基準。8P `multiscale_cnn_sequence_only_dual_path_v1` 已由完整OOS淘汰，僅供舊checkpoint／manifest重建與歷史重現。
- `BREAKOUT_QUALITY_EXPERIMENT_PROFILE` 描述 optimizer、LR schedule、augmentation 與 training sampling unit。profile 定義集中在 `config/breakout_quality_experiments.py`：active 9A與高覆蓋基準8F均沿用 `unique_group_sampling / time_weight=none`；9A完整OOS已確認固定coverage排序能力優於8F。8K `unique_group_date_balanced` 已由完整 OOS 淘汰，只保留歷史重現；8J direct best inner checkpoint 同樣只供歷史重現。`baseline`、`adamw_only`、`adam_warmup_cosine`、`history_masking_only` 保留為歷史 profile。
...
- AdamW、scheduler、augmentation 與 sampling 等訓練方法不再建立假模型版本。結構或輸入表示真正改變時才新增具描述性的 architecture；目前active architecture與排序／高品質基準為9A `inception_time_v1`，仍沿用8F `unique_group_sampling / batch_size=128 / time_weight=none / patience=1 / final_refit=selected_epochs`；8F `multiscale_cnn_sequence_only_v1`保留為高覆蓋實證基準。下一項只測InceptionTime的BatchNorm→GroupNorm。8P dual-path與8G～8O均已淘汰。
...
- 若易讀報表曾出現「同一 ticker/date 的 ranking metrics 出現不一致 score」，且尚保留首次9A成功報表所用的原始`research_scores.csv`，套用本版後只需重跑report，不要重跑export；固定threshold與ranking／calibration數值會沿用原第一row代表Score語義。若該CSV已被任何後續export覆寫，重新推論即使契約相同仍可能有GPU／BF16末位差，無法保證逐位元還原舊檔；要完全一致只能還原原CSV備份。
