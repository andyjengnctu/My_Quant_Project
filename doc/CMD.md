...
- 主頁籤為「單股回測檢視／投組回測檢視／實際交易／資料中心」；「資料中心」只消費 `services/trading/market_data_ops.py` local-first read model，集中顯示 Trading target、V2 readiness、51-dataset freshness/schema/coverage、publication schedule、next check、最近活動與最後一次實際觀察到的 provider quota；按「刷新狀態」不得呼叫 FinMind。資料中心只提供「立即檢查 Due」與「完整更新 Trading 資料」兩個 canonical action，分別委派 Round 4 one-shot updater 與 Round 1 canonical full update，不提供逐 dataset 手動下載第二條路徑。OS Task Scheduler 目前仍是外部排程，不由 Workbench 假裝已註冊或自動切換。
...
Windows Task Scheduler 應每 15 分鐘喚醒一次此 one-shot app；排程頻率不是 provider query 頻率。沒有 dataset due 且 market-date discovery 未 due 時 provider quota 消耗必須為 0。Round 5/6 Workbench「資料中心」會把 dataset next-check 與 Trading Market Date Discovery next-probe 一起納入 Next Check / Schedule；本頁仍只讀 local state，不為刷新 UI 額外打 `/user_info`。Windows Task Scheduler registration 目前仍是外部 deployment concern，尚未由程式自動安裝。
