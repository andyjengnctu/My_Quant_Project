...
- 「帳務中心」獨立管理帳戶現金、庫存股、逐筆買入明細、逐筆賣出明細／沖抵明細與績效統計。庫存欄位採券商口徑顯示「股票、均價、股數、持有成本、市價、市值、損益、報酬率」；均價以實際成交價加權，不含買入手續費，持有成本則為剩餘買入價金 + 剩餘買入手續費。點選庫存後可直接登錄賣出，只需輸入數量、成交價、成交日，股票由庫存自動帶入；系統直接以實際成交更新 account truth，**不要求也不維護券商 SELL order 對應**。庫存與買／賣成交明細不分 manual／strategy，皆可由帳務中心修正或刪除；庫存修正是 broker-truth reconciliation，不偽造成交易，成交明細則採 append-only void/correction 保留原 event。歷史成交可直接修改/刪除；系統保留原 event，以 void/correction 後重播有效帳務事件並重新計算後續成本沖抵，只有修正後形成超賣、同日買賣或負現金等不合法歷史才拒絕。買入明細未選庫存時顯示全部；選取庫存時只顯示該**目前持倉生命週期**對應的買入紀錄，取消選取即恢復全部。所有股票表格共用排序、同列再點取消、12 列分頁與左側單股檢視入口；日期欄位本身直接開日曆。只有具方向性的績效數字（如損益、報酬率、未實現損益）採台股色彩：正值紅、負值綠、0 白，其餘價格、成本、股數、市值等一般數字維持一般文字色。實際帳戶成交 broker fee 使用未折扣 `0.001425`，買入/賣出手續費與交易稅均依券商帳務語意**無條件捨去至整數元**後入帳；不得回頭改變 Research／回測 execution policy，交易稅稅率仍由 canonical security-profile 解析。帳務中心帳戶儀表只顯示「現金餘額、持股市值、可清算淨值、帳戶淨值」；其下績效統計只保留「庫存股／平倉股／加總 × 股票市值、持有成本、未實現損益、報酬率、勝率、期望值、風報比」。賣出損益為「賣出價金 − 賣出手續費 − 交易稅 − 沖抵持有成本」。沖抵依 canonical 平均成本比例分攤，不臆測券商未提供的 FIFO/lot 配對規則。`state/trading/account.json` 仍是唯一可寫 account truth；`outputs/trading/account/account_snapshot.json` 與 `performance_summary.json` 只是唯讀衍生快照，不得被交易 mutation 反向讀取。
