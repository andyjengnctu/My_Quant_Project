...
Windows Task Scheduler 應每 15 分鐘喚醒一次此 one-shot app；排程頻率不是 provider query 頻率，沒有 due dataset 時 provider quota 消耗必須為 0。
