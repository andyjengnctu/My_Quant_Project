...
- 「帳務中心」獨立管理帳戶現金、庫存股、逐筆買入明細、逐筆賣出明細／沖抵明細與績效統計。庫存欄位採券商口徑顯示「股票、均價、股數、持有成本、市價、市值、損益、報酬率」；均價以實際成交價加權，不含買入手續費，持有成本則為剩餘買入價金 + 剩餘買入手續費。點選庫存後可直接登錄賣出，只需輸入數量、成交價、成交日，股票由庫存自動帶入；系統直接以實際成交更新 account truth，**不要求也不維護券商 SELL order 對應**。庫存與買／賣成交明細不分 manual／strategy，皆可由帳務中心修正或刪除；庫存修正是 broker-truth reconciliation，不偽造成交易，成交明細採 append-only void/correction 保留原 event。歷史成交修正/刪除後以完整 effective-event replay 重建現金、庫存、後續平均成本、沖抵成本與 realized PnL；若移除早期 BUY 後仍有有效 SELL 且需要外部／舊券商庫存，系統必須追加明確 `historical_inventory_reconciliation` audit event，保存補足股數、成本 evidence、effective-before revision 與 dependent SELL lineage，禁止只在 replay 內隱性猜出不存在的庫存；dependent SELL 後續被 void 時該 reconciliation 亦不再具經濟效力。`set_cash_balance` 在 replay 中以原事件的 cash delta 重放，不再當成第二個 opening-cash anchor；因此刪除 BUY 會正確恢復現金。Workbench 使用者操作不攜帶畫面舊 revision，mutation lock 內讀取最新 account truth；低階 caller 若明確 pin `expected_revision` 仍維持 conflict fail-fast。買入明細未選庫存時顯示全部；選取庫存時只顯示該**目前持倉生命週期**對應的買入紀錄，取消選取即恢復全部。股票表格共用排序、同列再點取消、12 列分頁與左側單股檢視入口；日期欄位本身直接開日曆。頁面所有靜態控制與動態表格 cell 都直接綁定整頁 MouseWheel，因此滑鼠停在表格、輸入框、按鈕或空白區都可上下捲動。只有具方向性的績效數字採台股色彩：正值紅、負值綠、0 白，其餘一般數字維持一般文字色。實際帳戶成交 broker fee 使用未折扣 `0.001425`，買入/賣出手續費與交易稅均依券商帳務語意**無條件捨去至整數元**後入帳；legacy event 在 read/replay 時亦依目前 actual-account policy 重新投影，原 event 不改寫。帳務中心把 revision、市價日與重新整理併入帳戶儀表板標頭；市價日、現金餘額、帳戶淨值以淡藍色顯示。儀表板下方績效統計固定順序「庫存股／平倉股／加總」，不可排序，只顯示「股票檔數、價值、成本、損益、報酬率、勝率、期望值(EV R)、風報比」：庫存股即使缺市價也保留檔數與持有成本，但價值／未實現損益／報酬率顯示不可完整估值，且未平倉 row 的勝率／EV／風報比為 N/A；平倉股使用 closed lifecycle 的賣出淨收入、已沖抵成本、realized PnL、canonical closed-trade win rate／payoff 與 `core/report_metrics.py` 同口徑的 EV(R)，加總只以 closed trades 計算勝率／EV／風報比，股票檔數採 unique ticker union。賣出損益為「賣出價金 − 賣出手續費 − 交易稅 − 沖抵持有成本」。操作說明不常駐各區塊，而由頁面底部「操作提示」依滑鼠所在操作區情境顯示。`state/trading/account.json` 仍是唯一可寫 account truth；`outputs/trading/account/account_snapshot.json` 與 `performance_summary.json` 只是唯讀衍生快照，不得被交易 mutation 反向讀取。
