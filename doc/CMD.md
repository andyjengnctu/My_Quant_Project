...
- Market Data V2 的 51 個 included datasets 現由 `core/market_data_freshness_contract.py` 各自解析 canonical operational contract，至少固定 cadence、expected-date semantics、row expectation、completeness mode、schema validation requirement 與 publication first-check policy；Trading sync manifest 建立前會驗證 contract 必須完整覆蓋 current included registry。Publication timing 由 `config/market_data.py::MARKET_DATA_V2_PUBLICATION_POLICY` 單一持有：有 provider documentation evidence 的 dataset 使用具 grace 的 verified override，尚無明確 provider 時間者使用隔日 01:45 conservative fallback，且 read model 會分開呈現 verified/fallback counts。此輪只建立 contract truth；`last_success_at/latest_data_date/next_check_at/status` 等 persistent dataset state 與 due planner 留待後續 round，不能把目前 `SYNCED` 誤解成 51/51 dataset publication-complete。
