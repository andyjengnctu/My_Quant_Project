...
5. `apps/research.py → [3] 策略組合比較 → [1] Extending-Window Test`：進入後選OOS Test或Rolling Test | 2021→最新 | 12M，兩者都引用同一`extending_current` Compare Suite＝`C61/C62/C58/C63/C59/C60`。OOS人讀DL-off名稱為`Full Base-Finalist-Best OOS / Full Base-Finalists-Agree OOS / Min Base-Finalist-Best OOS / Min Base-Finalists-Agree OOS`，另列MR-13E與MR-13K+MR-13M arms，並由Optimizer canonical `full/oos`或`min/oos`解析2020-cutoff fixed artifacts；C62/C63只改parameter policy為`base-finalists-agree`，整段2021→最新不再換參數；Rolling人讀名稱對應`Full Rolling / Min Rolling / ... Rolling`，直接使用Optimizer canonical `full/rolling`或`min/rolling` schedule。C60 primary K與secondary M必須解析到同一mode的PIT namespace。
6. 若seed robustness仍可能改變決策，執行`apps/research.py → [3] 策略組合比較 → [2] Extending-Window Multi-seed Robustness Test`。Current入口會顯示`end_to_end_v1`固定4題seed、`300 trials/fold`與benchmark artifact BUILD／REUSE計畫；一次確認後由canonical Optimizer建立／續跑C61/C58/C59/C60 per-seed strategy params，再為C59/C60建立same-seed E/K/M模型並replay，**不會fallback用production Seed42參數**。先跑OOS；只有結果仍可能改變決策才跑Rolling 2021→latest。
