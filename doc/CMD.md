...
- AdamW、scheduler、augmentation 與 sampling 等訓練方法不再建立假模型版本。結構或輸入表示真正改變時才新增具描述性的 architecture；目前程式執行 8F `multiscale_cnn_sequence_only_v1 / unique_group_sampling / time_weight=none`，實證比較基準仍為 8A `sequence_only_v1 / baseline`。
