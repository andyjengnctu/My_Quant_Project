...
- `BREAKOUT_QUALITY_EXPERIMENT_PROFILE` 描述 optimizer、LR schedule、augmentation 與 training sampling unit。profile 定義集中在 `config/breakout_quality_experiments.py`：目前 8F 使用 `unique_group_sampling`，沿用 baseline 的 Adam 固定 LR與無 augmentation，只把 training batch unit 改為 unique `ticker/date` groups；`baseline`、`adamw_only`、`adam_warmup_cosine`、`history_masking_only` 保留為歷史 profile。
...
- AdamW、scheduler、augmentation 與 sampling 等訓練方法不再建立假模型版本。結構或輸入表示真正改變時才新增具描述性的 architecture；目前正式研究基準為 8F `multiscale_cnn_sequence_only_v1 / unique_group_sampling / batch_size=128 / time_weight=none / patience=1`；8G patience 5 與 8H batch size 64 均因完整 OOS 退步而淘汰。
