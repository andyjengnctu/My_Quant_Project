...
- Research V2 Round 9 目前只有**candidate producer/service**，尚未新增日常互動指令，也不會自動 promotion。candidate 只 pin neutral READY Provider Snapshot，輸出至 `data/research/market_data_v2/candidates/<provider_snapshot_fingerprint>/`；`daily_universe.sqlite3` 由 raw `TaiwanStockPrice` presence 建 historical daily universe，`research_v2_candidate_manifest.json` 保存 exact-candidate completeness ceiling、PIT assessment 與 blockers。`research_v1` 仍是 active frozen `2026-03-02`；Research V2 的 `frozen_cutoff` 仍必須為 `null`。
