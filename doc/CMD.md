...
- 主頁籤為「單股回測檢視／投組回測檢視／實際交易」；實際交易頁頂部「Trading 操作總覽」會由 canonical operations-status read model 彙整 Data/Params、account、BUY/SELL broker orders、Scanner/Proposed/Protection freshness、fill transaction 與 active-Stop 缺口，明確顯示下一個合法動作並控制 1～4 workflow 按鈕；「全狀態刷新」只委派既有 state/recovery/read services，不在 UI 重算交易規則。實際交易頁另提供「實盤就緒檢查」，會執行 read-only pre-live operational audit 並寫出 `outputs/trading/operational_audit/prelive_audit.json/.md`；目前版本預期為 `LIVE_BLOCKED`，因 overnight strategy position 尚未完成 completed-bar daily roll-forward/trailing-stop 推進，且 Full rule-based `ind_sell_signal` 尚未接入實際 Trading SELL lifecycle。這個 gate 與一般 setup/workflow availability 分開：可以繼續建立/測試 Trading state，但不得據此宣稱系統已 live-ready。實際交易頁可分步執行「1 更新資料／2 更新 Params／3 Scanner 候選」，也可用「每日流程 1→2→3」依序完成資料、參數與候選更新。Scanner只在Trading params與目前Trading data同一最新交易日且selector解析為單一runtime member時執行。完成Scanner後可按「4 建議掛單」：系統會以目前`state/trading/account.json`的真實cash／持股、最新Trading收盤價、canonical max positions／position sizing／reserved-capital語意產生account-aware proposed orders，輸出至`outputs/trading/proposed_orders/`；proposal本身不代表已送單或成交。使用者實際在券商送出某筆買單後，必須選取該proposal並按「確認選取已送單」，才會寫入`state/trading/orders.json`的`ORDERED` broker truth，且同時凍結當次盤前決策實際使用的strategy params。券商有實際成交時，使用者再輸入「本次成交股數／本次成交價／成交日」並按「確認選取成交」；系統只接受使用者明確輸入的實際fill，不依high/low自動猜成交，部分成交轉`PARTIAL`並保留剩餘掛單，全部成交轉`FILLED`，cash／持股只在此時透過canonical exact accounting更新。每次confirmed strategy fill後，Workbench會依該持股的canonical position state與ORDERED時凍結的params機械刷新「成交後 Stop / TP 保護單計畫」：Stop保護目前實際持股全量，TP股數直接沿用canonical半倉停利規則，partial fill只保護已成交持股；計畫不讀成交後行情、不接受新的人工判斷，輸出至`outputs/trading/protection_orders/`，本身仍不是券商賣單。使用者若已在券商實際送出某個Stop或TP，可選取持股後按「確認 Stop 已送單」／「確認 TP 已送單」寫入`state/trading/orders.json`的SELL ORDERED；系統會檢查所有非OCO protection SELL有效委託股數不得超過當下實際持股。若券商確實支援native OCO/互斥群組，使用者必須明確輸入實際「券商 OCO/互斥群組 ID」並按「確認 Stop+TP 已以券商 OCO 送單」，系統才允許Stop full與TP共享同一持股；沒有明確確認時絕不自行假設OCO能力。Protection SELL只有使用者輸入券商實際成交股數／成交價／成交日後才可進入PARTIAL/FILLED並透過canonical exact accounting更新account；系統仍不得由行情high/low或trigger touch自動推定成交。`ORDERED/PARTIAL`的未成交餘額可用「確認選取剩餘委託已取消」轉為`CANCELLED`，已成交部分不回滾。有未解決`ORDERED/PARTIAL`時禁止修改account或重建新的盤前allocation，而且同一information date只要曾有實際送單，即使之後全部FILLED/CANCELLED也不得重新allocation。頁面同時可初始化 Trading account、更新現金、登記既有持股，以及修正／移除尚未有賣出歷史且尚未由策略接管的 manual-adopted 持股。所有帳戶、掛單與成交變更都必須由Trading services寫入各自state；confirmed fill使用可恢復write-ahead journal避免account/order跨檔半完成，禁止手動編輯JSON。
...
- `outputs/trading/operational_audit/`：實盤前 operational safety audit；`prelive_audit.json` 為 machine-readable checks/blockers，`prelive_audit.md` 為人讀摘要。此目錄只存 audit output，不是 account/order/position current truth；目前 required-live capability 尚缺時必須明確為 `LIVE_BLOCKED`。
