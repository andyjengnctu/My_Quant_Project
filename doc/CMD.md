...
5. `apps/research.py → [3] 策略組合比較 → [1] Extending-Window Test`：進入後選OOS Test或Rolling Test | 12M，均比較`C61 Full ROOS / C58 Min ROOS / C59 MR-13E / C60 MR-13K+MR-13M`。OOS會自動由`P4_EXTENDING/P2_EXTENDING`取`2021-01-01`當下合法member建立`oos_full_roos/oos_min_roos` frozen artifacts，整段2021→最新不再換參數；Rolling才直接使用P4/P2 Extending stitched schedule。C60 primary K與secondary M必須解析到同一mode的PIT namespace。
