...
未提供 `--target-date` 時，Round 6 起 worker 會先讀本機 `state/trading/market_data_v2/market_date_discovery_state.json` 判斷是否到達新 completed Trading day 的 probe 時點；預設 17:45 前、weekend 與已排定 next probe 前一律 0 provider call。只有 discovery due 才用 `TaiwanStockPriceAdj` current full-market range 做一次 canonical completed-day probe；沒有新日採 15/30/60 分鐘 bounded retry，仍沒有則排到下一 weekday 17:45。發現新 completed Trading day 時，以同一 shared provider client 呼叫 `services/trading/market_data_update.py` 更新 execution-critical legacy CSV + canonical Trading snapshot，但指定 V2 full sync defer，讓 51-dataset V2 仍依 publication/due planner 分時更新，避免 17:45 一次提前查詢尚未發布的 dataset。`--target-date` 明確指定時跳過 discovery，只做該 target 的 deterministic due/recovery。
