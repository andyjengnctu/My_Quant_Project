...
- 「交易中心」專注 **資訊儀表板 → 每日 Trading 流程 → 今日 Scanner Pool → 買入成交登錄**。Scanner 直接攤開 canonical candidate 順序與類型、買入限價、初始 Stop、Target/完成線、EV、歷史勝率、交易次數、資產成長、參考投入與 Scanner 摘要，使用者依此自行到券商下單；Workbench **不管理券商 OMS**，不在主介面維護 ORDERED/PARTIAL/FILLED/CANCELLED、券商委託號、Stop/TP 掛單或 Indicator SELL 掛單。Stop、Target 與 SELL signal 仍由 canonical strategy semantics 計算，但只作決策資訊。買入成交只輸入股票、數量、成交價、成交日；Scanner 選取股保留 strategy-management lineage，自行買入則記為 manual account truth。買入成功後自動切換至「帳務中心」並刷新。**賣出成交不在交易中心輸入**，改由帳務中心選取庫存後登錄。各股票表格最左側提供單股回測檢視入口；欄位第一次點擊升冪、第二次降冪，交易日期預設由舊到新；同列再點一次取消選取。表格每頁最多 12 列，超過以「上一頁／下一頁」換頁，不設表內 scrollbar，頁面本身保留整頁捲動。日期欄位本身可直接點開日曆，也可手打 `YYYY-MM-DD`。
- 「帳務中心」獨立管理帳戶現金、庫存股、逐筆買入明細、逐筆賣出明細／沖抵明細與績效統計。庫存欄位採券商口徑顯示「股票、均價、股數、持有成本、市價、市值、損益、報酬率」；均價以實際成交價加權，不含買入手續費，持有成本則為剩餘買入價金 + 剩餘買入手續費。點選庫存後可直接登錄賣出，只需輸入數量、成交價、成交日，股票由庫存自動帶入；系統直接以實際成交更新 account truth，**不要求也不維護券商 SELL order 對應**。庫存與買／賣成交明細不分 manual／strategy，皆可由帳務中心修正或刪除；庫存修正是 broker-truth reconciliation，不偽造成交易，成交明細採 append-only void/correction 保留原 event。歷史成交修正/刪除後以完整 effective-event replay 重建現金、庫存、後續平均成本、沖抵成本與 realized PnL；若移除早期 BUY 後仍有後續 SELL，優先使用 immutable SELL 的 `position_before`／allocated-cost evidence 重建當時 broker inventory，舊資料缺 snapshot 時才以保存的平均成本 allocation 作 compatibility reconstruction。`set_cash_balance` 在 replay 中以原事件的 cash delta 重放，不再當成第二個 opening-cash anchor；因此刪除 BUY 會正確恢復現金。Workbench 使用者操作不攜帶畫面舊 revision，mutation lock 內讀取最新 account truth；低階 caller 若明確 pin `expected_revision` 仍維持 conflict fail-fast。買入明細未選庫存時顯示全部；選取庫存時只顯示該**目前持倉生命週期**對應的買入紀錄，取消選取即恢復全部。股票表格共用排序、同列再點取消、12 列分頁與左側單股檢視入口；日期欄位本身直接開日曆。頁面所有靜態控制與動態表格 cell 都直接綁定整頁 MouseWheel，因此滑鼠停在表格、輸入框、按鈕或空白區都可上下捲動。只有具方向性的績效數字採台股色彩：正值紅、負值綠、0 白，其餘一般數字維持一般文字色。實際帳戶成交 broker fee 使用未折扣 `0.001425`，買入/賣出手續費與交易稅均依券商帳務語意**無條件捨去至整數元**後入帳；legacy event 在 read/replay 時亦依目前 actual-account policy 重新投影，原 event 不改寫。帳務中心把 revision、市價日與重新整理併入帳戶儀表板標頭；市價日、現金餘額、帳戶淨值以淡藍色顯示。儀表板下方績效統計固定順序「庫存股／平倉股／加總」，不可排序，只顯示「股票檔數、價值、成本、損益、報酬率、勝率、期望值、風報比」；庫存股的價值/損益為目前可清算淨值/未實現損益，平倉股為賣出淨收入/已實現損益，加總為兩者合計，股票檔數採 unique ticker union。賣出損益為「賣出價金 − 賣出手續費 − 交易稅 − 沖抵持有成本」。操作說明不常駐各區塊，而由頁面底部「操作提示」依滑鼠所在操作區情境顯示。`state/trading/account.json` 仍是唯一可寫 account truth；`outputs/trading/account/account_snapshot.json` 與 `performance_summary.json` 只是唯讀衍生快照，不得被交易 mutation 反向讀取。
- 使用者可分步操作「1 更新資料／2 套用 Params／3 Scanner 候選」，而「每日流程 1→2→3」的 canonical 內部順序仍是 **Data → position roll-forward → Indicator SELL plan → Params → Scanner**。Scanner 只接受 selected params 與 Trading data 同一 latest date；每檔 candidate 自身 trade_date 與 execution-plan seed date 必須等於該 information date，舊訊號只會被 skip。「4 建議掛單」已移到進階掛單分頁；proposal 仍只屬盤前衍生資料，不代表已送單或成交。實際 BUY/SELL submission、fill 與 cancel 必須由使用者依券商真實狀態確認；market high/low 不會自動推定成交。所有 account/order/journal current state 實體路徑由 canonical path owner 解析、ticker 由 canonical identity owner 正規化、cash/fee/tax/PnL 與 milli scale 只走 exact-accounting SSOT；禁止手動編輯 state JSON。
