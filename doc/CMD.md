...
6. 若seed robustness仍可能改變決策，執行`apps/research.py → [3] 策略組合比較 → [2] Extending-Window Multi-seed Robustness Test`。Current Round1只會顯示`end_to_end_v1`固定4題seed、`300 trials/fold`與benchmark artifact缺件並BLOCKED，**不會fallback用production Seed42參數執行舊model-only robustness**；Round2建立C61/C58/C59/C60 per-seed strategy params及C59/C60 same-seed E/K/M模型後，才先跑OOS、必要時再Rolling 2021→latest。
