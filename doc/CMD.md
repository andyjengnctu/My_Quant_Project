...
`apps/gui.py` 是 GUI 單一啟用入口；GUI 工作台與頁籤 registry 在 `tools/gui/workbench.py`，目前內建 `單股回測檢視` 與 `投組回測檢視` 兩個上層分頁。單股頁：K 線圖分頁預設佔主要版面、GUI 開啟即預設最大化，內嵌圖預設顯示最近 18 個月且可平移/縮放到完整歷史，色系採台股紅漲綠跌與深色背景；左上只顯示滑鼠所在 K 棒的 OHLCV，停利 / 限價 / 成交 / 停損值改由右側彩色 sidebar 顯示，避免與左上 legend 重複；買訊/賣訊以半透明註記顯示掛單或歷績資訊，右側 sidebar 顯示買入訊號 / 符合歷史績效 / 全歷史摘要 / 選取日線值與「回到最新 K 線」操作；成交量預設隱藏且可切換為同圖 overlay，拖曳時價量時間軸同步；交易明細與 Console 為獨立分頁，控制列固定使用完整資料集，提供候選股掃描下拉選單，股票代號輸入後按 Enter 即執行回測、候選股下拉選取即直接回測。投組頁：以 GUI 方式顯示執行進度條、內嵌淨值曲線、摘要 sidebar、交易明細 / 年度報酬 / Console 分頁，並保留開啟 Excel / dashboard / 輸出資料夾操作。GUI 預設不輸出單股 HTML。
...
# apps/gui.py 為單一 GUI 啟用入口，從 tools.gui 套件 façade 匯入 main；workbench / panel registry 在 tools/gui/workbench.py，單股回測檢視頁籤在 tools/gui/single_stock_inspector.py
...
正式測試入口已收斂為 `apps/test_suite.py`；日常 GUI 單一啟用入口為 `apps/gui.py`；`apps/` 不再保留舊的 regression / consistency 測試入口。
...
- `outputs/debug_trade_log/`：單檔 debug trade log、Excel 明細與 HTML K 線檢視。
...
- `python apps/gui.py`：啟動股票工具工作台；目前提供 `單股回測檢視` 與 `投組回測檢視` 兩個上層分頁。
