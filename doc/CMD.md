...
- 「單股回測檢視」新增 `Research / Trading` 切換與「持有股」下拉。Research 模式沿用既有 Research dataset/params/risk 控制；Trading 模式鎖定 canonical Trading V2 + canonical Trading Params，若股票來自最新 Trading Scanner Pool，回測視圖優先使用該 candidate 已凍結的 representative Params。Trading 模式按「計算候選股」會直接執行同一 `run_trading_candidate_scan()`，並在獨立「Scanner Pool」分頁列出與實際交易頁相同的最新 pool；持有股下拉或 Trading 主頁導覽都可直接載入單股 K 線。
