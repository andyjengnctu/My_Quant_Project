...
- 「交易中心」專注 **資訊儀表板 → 每日 Trading 流程 → 今日 Scanner Pool → 買入成交登錄**。頂部「Trading 操作總覽」只消費 canonical Operations Status；Scanner 直接攤開 canonical candidate 順序與類型、買入限價、初始 Stop、Target/完成線、EV、歷史勝率、交易次數、資產成長、參考投入與 Scanner 摘要，使用者依此自行到券商下單。買入成交只輸入股票、數量、成交價、成交日，所有日期輸入同時支援 `YYYY-MM-DD` 手打與日曆點選；若能唯一對應既有 active BUY broker order，系統自動走 canonical order/account reconciliation，否則以實際帳戶買入事件入帳。**賣出成交不在交易中心主流程輸入**，改由帳務中心選取庫存後登錄。價金、手續費、交易稅、成本與損益全部由 SSOT 自動計算。原本建議掛單、券商 ORDERED/PARTIAL/FILLED/CANCELLED、Stop/TP 與 Indicator SELL 操作仍保留在頁面底部「進階」分頁。各表格依實際列數調整高度，不設表內垂直 scrollbar；頁面本身保留整頁捲動。頁面仍提供 read-only「實盤就緒檢查」，輸出 `outputs/trading/operational_audit/prelive_audit.json/.md`。
- 「帳務中心」獨立管理帳戶現金、庫存股、逐筆買入明細、逐筆賣出明細／沖抵明細與績效統計。庫存欄位採券商口徑顯示「股票、均價、股數、持有成本、市價、市值、損益、報酬率」；均價以實際成交價加權，不含買入手續費，持有成本則為剩餘買入價金 + 剩餘買入手續費。點選庫存後可直接登錄賣出，只需輸入數量、成交價、成交日，股票由庫存自動帶入；若存在 active SELL broker order，必須在帳務中心選取/對應該實際券商單後走 canonical order/account reconciliation。庫存、手動買入明細與手動賣出明細支援修改/刪除；交易明細採 append-only correction/void audit trail，原 event 不覆寫，且為避免重寫已被後續交易消費的成本分攤，只能由同股票最新一筆有效手動交易往回修正，策略 order/fill lineage 不允許在帳務中心直接改寫。所有日期輸入同時支援手打與日曆點選。實際帳戶成交的 broker fee 使用未折扣 `0.001425`，不得回頭改變 Research／回測 execution policy；交易稅仍由 canonical security-profile 稅制解析。賣出損益為「賣出價金 − 賣出手續費 − 交易稅 − 沖抵持有成本」。沖抵依 canonical 平均成本比例分攤，不臆測券商未提供的 FIFO/lot 配對規則。績效與庫存損益採台股色彩：正值紅、負值綠、0 白。帳務表格依實際列數調整高度，無表內 scrollbar、無固定空白列；整頁可捲動。`state/trading/account.json` 仍是唯一可寫 account truth；`outputs/trading/account/account_snapshot.json` 與 `performance_summary.json` 只是唯讀衍生快照，不得被交易 mutation 反向讀取。
