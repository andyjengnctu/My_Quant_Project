...
Dataset `READY` 不是「日期到了」的同義詞：current validation contract、freshness status、target date、schema 與 request-scope coverage 必須同時成立；舊 schema state 只保留作 bounded resume hint並要求新版重新驗證。Trading batch fingerprint 也包含 validation-contract version，dated provider payload 必須嚴格落在 request date window，所以修正前同日期 artifact 不會被新版直接視為完整。
