...
未提供 `--target-date` 時，worker 只使用目前 canonical Trading market-data snapshot 的 `market_date` 作 target；它不額外呼叫 provider 探測「今天是不是新交易日」。因此 Round 4 自動化的是 V2 dataset publication/freshness lifecycle；execution-critical legacy Trading target 的全自動 discovery / canonical fetch 去重留在後續 Round 6。

Windows Task Scheduler 應每 15 分鐘喚醒一次此 one-shot app；排程頻率不是 provider query 頻率，沒有 due dataset 時 provider quota 消耗必須為 0。Round 5 Workbench「資料中心」可檢視 auto policy / next check / dataset state，但目前不直接管理 Windows Task Scheduler 註冊；面板顯示的 quota 是最近一次真正 sync 時 executor 已取得的 observation，不會為了刷新 UI 額外打 `/user_info`。
