...
- `BREAKOUT_QUALITY_MODEL_ARCHITECTURE` 只描述網路與輸入結構。目前policy預設暫時切換為尚待完整Selection／OOS驗證的active research architecture `inception_time_market_set_v1`，並使用獨立filter id；9A-BN `inception_time_v1`仍是已接受排序／高品質比較基準，8F `multiscale_cnn_sequence_only_v1`保留高覆蓋基準。Stage 0／1只允許研究鏈，不具正式forward runtime資格。9F `patch_transformer_v1`完整OOS排序低於9A，已轉為legacy read-only；其10-bar非重疊patch、128維embedding、3層／4-head Transformer、256維MLP、sinusoidal position與patch-mean pooling規格只供舊工件重建。9E MOMENT、9D MantisV2、9C TS2Vec、9B ModernTCN、9A-GN及8P亦為legacy read-only。
