...
6. 若seed robustness仍可能改變決策，執行`apps/research.py → [3] 策略組合比較 → [2] Extending-Window Multi-seed Robustness Test`。Current入口會顯示`end_to_end_v1`固定4題seed、由canonical Optimizer依`OPTIMIZER_OUTER_ROLLING_OOS_TRIALS_DEFAULT`即時解析的trial budget與benchmark artifact BUILD／REUSE計畫；一次確認後由canonical Optimizer一次建立／續跑各seed/family的2021→latest完整schedule，再為C59/C60建立same-seed E/K/M模型並replay，**不會fallback用production Seed42參數**。先跑OOS；只有結果仍可能改變決策才跑Rolling 2021→latest。
