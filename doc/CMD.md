...
- `BREAKOUT_QUALITY_MODEL_ARCHITECTURE` 只描述網路與輸入結構。目前policy已退回9A-BN `inception_time_v1`，作為排序／高品質正式基準；8F `multiscale_cnn_sequence_only_v1`保留為高覆蓋實證基準。9C `ts2vec_frozen_linear_v1`完整OOS排序明顯低於9A，與9B `modern_tcn_v1`、9A-GN及8P一樣轉為legacy read-only。
...
- 既有supervised Dataset、feature bank、future-path cache、4維event context arrays與labels可直接沿用，不需重建或relabel。9C獨立Selection-only rolling-window dataset與pretrained encoder工件只保留歷史重建；active 9A workflow不再執行pretraining stages。TS2Vec frozen probe與sequence-only models在forward時明確不讀取event context。8F 在 Inner Train／Final Refit 只保留每個 `ticker/date` 的最小原始 row index，batch size 128 因而代表 128 個 unique groups；early-stopping patience 固定為 1。Validation、Selection、OOS 與報表仍使用完整 rows及既有 `1/group_size` 口徑。每個 architecture/profile 使用獨立工件路徑。
