...
HELP_DESCRIPTION = "說明: 預設資料集為完整、模型模式預設為 split；可用 --model split|legacy 或環境變數 V16_OPTIMIZER_MODEL 切換；split 模式採 train/test 完全分離：訓練只看 train RoMD、測試只看單一連續 test holdout RoMD，Champion 永遠保留目前測試 RoMD 最好的策略+參數，不會把測試分數自動回灌到同一次訓練；互動式 Optimizer 選單預設訓練 50000 次，輸入 0 可寫入 best run 並重新測試目前記憶庫最佳；非互動模式預設訓練次數為 0；可用環境變數 V16_OPTIMIZER_TRIALS 指定 trial 數、V16_OPTIMIZER_SEED 指定固定 seed；train/test 切分設定來自 config/training_policy.py；正式現役版固定使用 champion_params.json。"
