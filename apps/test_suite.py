...
def main(argv=None) -> int:
    ...
    if has_help_flag(argv):
        ...
        print("說明: reduced 一鍵測試正式入口；會串接所有已實作測試（含 meta quality、dataset fingerprint、artifact integrity、single-stock compounding-capital contract、single-ticker compounding parity contract、unsupported use_compounding guardrail 與 memory tracker lifecycle contract），若失敗再依主控台建議用 run_all.py --only 重跑失敗步驟。")
        return 0
