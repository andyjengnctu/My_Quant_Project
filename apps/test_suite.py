...
# consistency step 透過 synthetic registry 覆蓋最新 exact-ledger / summary-comment / GUI / trade_analysis 文件同步 / canonical-alias / synthetic-contract-path / checklist-transition / checklist-summary-heading-uniqueness / public-stats-profit-equity / help-theme-rename / project-settings exhaustive-check / checklist-sort-guard contract / PROJECT_SETTINGS-TEST_SUITE_CHECKLIST-GPT_DELIVERY_CHECKLIST file-division sync 與 gpt-delivery-checklist governance contract（含 T225/T226/T229/T230/T231/T232/T233/T234/T235/T236/T237/T238/T239/T240/T241/T242/T243/T244/T245/T246/T247；T235 同時約束 process_debug_position_step 與 append_debug_forced_closeout）。
...
def main(argv=None) -> int:
    ...
    if has_help_flag(argv):
        ...
        print("說明: reduced 一鍵測試正式入口；會串接所有已實作測試（含 meta quality、dataset fingerprint、artifact integrity、single-stock compounding-capital contract、single-ticker compounding parity contract、exact-accounting ledger / cost-basis allocation / integer tick-limit / cash-risk boundary / single-vs-portfolio parity / display-derived contract、single-backtest legacy stats schema contract、single-backtest public profit/equity consistency contract、single-backtest exact cash/equity path static contract、debug-backtest entry cash-path static contract、shared display-money rounding helper static contract、real-case completed-trade rounding oracle contract、trade-rebuild shared rounding helper contract、debug forced-closeout exact total-pnl contract、synthetic-meta source-path binding contract、unit-display rounding-helper contract、debug-exit display-capital exact-ledger contract、debug-entry display-capital exact-total contract、debug-exit entry-capital fallback exact-ledger contract、debug-half-exit leg-return exact-allocated-cost contract、debug-sell-signal exact mark-to-market profit-pct contract、average-price total helper contract、portfolio-rotation exact mark-to-market return contract、validator/oracle exact-ledger total contract、checklist DONE test-summary markdown structure contract、same-bar stop-priority oracle snapshot contract、critical-helper single-source-of-truth guard、policy/config coverage-target contract、unsupported use_compounding guardrail、GPT 端 dynamic-test / formal-step bypass boundary contract、checklist convergence first-nonempty-title / summary-table-ID-order / summary-section-heading-uniqueness / `NEW`-only-first-occurrence / stale-validator-reference guard、summarize_result flattened-summary accessor contract、specific pass-only exception traceability contract、broad-exception traceability contract、GUI TclError fallback traceability contract、optional-dependency fallback traceability contract、GUI trade-box capital / round-trip display contract、package_zip root-bundle-preserve / commit-zip-test_suite orchestration contract、core R-multiple exact-ledger contract、debug-exit total-return milli-binding forced-closeout contract、log_utils outputs-root create-path guard、memory tracker lifecycle contract、project-settings exhaustive-check / checklist-sort-guard contract、PROJECT_SETTINGS / TEST_SUITE_CHECKLIST / GPT_DELIVERY_CHECKLIST 文件分工同步與 gpt-delivery-checklist governance contract），若失敗再依主控台建議用 run_all.py --only 重跑失敗步驟。")
        return 0
